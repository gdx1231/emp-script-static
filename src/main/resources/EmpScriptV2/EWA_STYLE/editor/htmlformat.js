  ...
  function Parser() {
    ...
    this.get_content = function () { //function to capture regular content between tags
      ...
      while (this.input.charAt(this.pos) !== '<') {
        ...
        ch1ar = this.input.charAt(this.pos);
        this.pos++;
        this.line_char_count++;
        
        
        if (this.Utils.in_array(ch1ar, this.Utils.whitespace)) {
          if (content.length) {
            space = true;
          }
          this.line_char_count--;
        ...
        content.push(ch1ar); //letter at-a-time (or string) inserted to an array
      }
    ...
    this.get_tag = function () { //function to get a full tag and parse its type
      ...
      do {
        ...
        if (this.Utils.in_array(ch1ar, this.Utils.whitespace)) { //don't want to insert unnecessary space
          space = true;
          this.line_char_count--;
          continue;
        }
        ...
        if (ch1ar === "'" || ch1ar === '"') {
          if (!content[1] || content[1] !== '!') { //if we're in a comment strings don't get treated specially
            ch1ar += this.get_unformatted(ch1ar);
            space = true;
          }
        }
        
        if (ch1ar === '=') { //no space before =
          space = false;
        }
        
        if (content.length && content[content.length-1] !== '=' && ch1ar !== '>'
        ...
        content.push(ch1ar); //inserts character at-a-time (or string)
      } while (ch1ar !== '>');
