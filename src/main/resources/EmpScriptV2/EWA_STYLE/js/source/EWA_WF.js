...
function EWAC_WfUnit(id) {
	...
	this.Log = function(adm, date, text) {
		...
		o.innerHTML += '<div style="font-size:9px;color:darkblue">' + adm + '<br><nobr>' + date.split(' ')[0] + '</nobr></div>';
	...
	if (pid && window.parent && window.parent.$X(pid)) {
		...
		if (!canOpr) {
			if (ss.length == 0) {
				...
				window.parent.$X(pid).parentNode.innerHTML = '<b>等待:<span style="color:blue">'
					+ ss.join('</span> 或 <span style="color:blue">') + '</span></b>';
			}
			...
			back.onclick = function() {
				...
				s.parentNode.parentNode.cells[0].innerHTML = '<div style="color:red">返回步骤</div>';
			}
