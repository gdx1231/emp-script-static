...
EWA.UI.findScrollParent = function(element) {
...
function EWA_UI_DialogClass() {
    ...
    this.ResizeByContent = function() {
        ...
        if (obj && obj.tagName && obj.tagName.toUpperCase() == "IFRAME") {
...
__Dialog._DialogMointer = function() {
    ...
    if (oFrame.document.readyState && oFrame.document.readyState == "complete") {
        window.clearInterval(wnd._TimerHandle);
        setTimeout(function() {
...
__Dialog._SetTitle = function(wnd) {
    ...
    // 原用 body.onunload，但浏览器对 unload 有 Permissions-Policy 限制，
    // 会打 "unload is not allowed" 告警；改用不受限制的 pagehide 事件监听 iframe 离开/刷新。
    if (wnd._PageHideHandler) {
        // 同一 wnd 重复加载时先移除旧监听，避免重复绑定
        try {
            oFrame.removeEventListener('pagehide', wnd._PageHideHandler);
        } catch (e) {
        }
    }
    wnd._PageHideHandler = function() {
        try {
            wnd._Url = "";
        } catch (e) {
        }
    ...
    if (oFrame.addEventListener) {
        oFrame.addEventListener('pagehide', wnd._PageHideHandler);
    }
    ...
    // 按内容自适应尺寸。内容页首次测量可能发生在字体/EWA 行折叠等尚未稳定时
    // （如 CSS grid 版式 .ewa-frame），会暂时量出比实际更高的高度而把弹窗顶成“全高”。
    // 处理策略：测量阶段先把弹窗隐藏，首测未触发“超高封顶”说明尺寸稳定 → 立即显示；
    // 一旦首测触发了全高封顶（很可能是临时撑高），继续隐藏，等字体就绪/延时补测得到
    // 真实高度后再一次性显示，避免“先撑成全高、随后又突然缩小”的闪变。
    var _frameWnd = oFrame.contentWindow;
    var _revealed = false;
    var _autoSize = function() {
        if (wnd == null || wnd._Dialog == null) {
            return;
        }
        try {
            wnd._Dialog.ResizeByContent();
            if (wnd._Dialog._IsHeightCapped
                && oFrame.document.body
                ...
                $(oFrame.document.body).css('overflow', 'hidden auto');
            }
            wnd.MoveCenter();
        } catch (e) {
        }
    ...
    var _show = function() {
        if (_revealed || wnd == null || wnd._Dialog == null) {
            return;
        }
        _revealed = true;
        try {
            var _f = wnd._Dialog.GetFrame();
            if (_f) {
                _f.style.visibility = '';
            }
        } catch (e) {
    ...
    try {
        var _fr0 = wnd._Dialog.GetFrame();
        if (_fr0) {
            _fr0.style.visibility = 'hidden';
        }
    } catch (e) {
    }
    setTimeout(function() {
        _autoSize();
        if (wnd == null || wnd._Dialog == null
            || !wnd._Dialog._IsHeightCapped) {
            // 首测未超高封顶（尺寸已稳定），立即显示
            _show();
        }
        // 首测触发了全高封顶：暂不显示，等下面 fonts.ready/兜底补测后再显示
    }, 111);
    if (_frameWnd && _frameWnd.document && _frameWnd.document.fonts
        && _frameWnd.document.fonts.ready) {
        _frameWnd.document.fonts.ready.then(function() {
            setTimeout(function() {
                _autoSize();
                _show();
            }, 60);
    ...
    // 兜底：fonts.ready 不可用或内容在更晚时才稳定时，仍补测并显示
    setTimeout(function() {
        _autoSize();
        _show();
    }, 600);
...
function EWA_ListFrameClass() {
    ...
    this.mergeButtonsInRow = function(herderText) {
        ...
        for (var name in ewa.ItemList.Items) {
            ...
            var itemName = ewa._GetSubValue("Name", "Name", node);
            ...
            var text = ewa._GetSubValue("DescriptionSet", "Info", node);
            var title = ewa._GetSubValue("DescriptionSet", "Memo", node);

            var ewa_click = ewa._GetSubValue("EventSet", 'EventName', node);
        ...
        if (this.getObj('.ewa-col-funcs').length == 0) {
            let tb = this.getObj()[0];
            for (let i = 0;i < tb.rows.length;i++) {
                let $td = $(tb.rows[i].insertCell(-1));
                if (i == 0) {
                    $td.addClass('EWA_TD_H ewa-col-funcs').text(herderText);
                    lastCellIndex = $td[0].cellIndex;
                } else {
    ...
    this.stickyHeaders = function() {
        ...
        if (!scrollParent) {
            return; // 如果没有找到滚动父元素，则不执行sticky
        }
        const trFunc = mainTable.find('.ewa_lf_func').parent();
        if (trFunc.length == 1 && !trFunc.hasClass('ewa-lf-sticky-func')) {
            trFunc.css('position', 'sticky').css('top', trFunc.offset().top).addClass('ewa-lf-sticky-func');
        }

        const divSearch = mainTable.find('.ewa-lf-search');
        if (divSearch.length == 1 && !divSearch.hasClass('ewa-lf-sticky-search')) {
            // 如果没有设置sticky，则设置
            divSearch.css('position', 'sticky').css('top', divSearch.offset().top).addClass('ewa-lf-sticky-search');
        }

        const lfHeader = mainTable.find('.ewa-lf-header');
        if (lfHeader.length == 1 && !lfHeader.hasClass('ewa-lf-sticky-header')) {
            lfHeader.css('position', 'sticky').css('top', lfHeader.offset().top).addClass('ewa-lf-sticky-header');
        }

        const refId = 'scroll-ref-' + this._Id;
        //避免重复加载事件
        if (!$(scrollParent).attr(refId)) {
            const scrollDom = scrollParent.tagName == 'BODY' ? window : scrollParent;
            const scrollTopDom = scrollParent.tagName == 'BODY' ? document.documentElement : scrollParent;
            $(scrollParent).attr(refId, 1);
    ...
    this.stickyColumns = function(columns) {
        ...
        //		const scrollParent = EWA.UI.findScrollParent(tb);
        const scrollParent = tb.parentNode;
        // console.log(scrollParent);

        const refId = 'scroll-ref-hor-' + this._Id;
        if (!$(scrollParent).attr(refId)) {
            const scrollDom = scrollParent.tagName == 'BODY' ? window : scrollParent;
            const scrollTopDom = scrollParent.tagName == 'BODY' ? document.documentElement : scrollParent;
            $(scrollParent).attr(refId, 1);
            $(scrollParent).addClass('ewa-lf-sticky-scroll');
            ...
            // 滚动条滑动时，触发一次事件，显示或隐藏最后一个column的右侧阴影
            $(scrollDom).trigger('scroll');
        }
