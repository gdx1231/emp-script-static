...
x),r=r.replace("<input","<input disabled "),q.push(r)):q.push(this._SearchSingle(h,t)):"date"==u?q.push(this._SearchDate(h,t,w)):"number"==u?q.push(this._SearchNumber(h,t,w)):"fix"==u&&(r="<div >"+this._SearchFix(h,r,"span",t)+"</div>",q.push(r));q.push("</td></tr></table>");"fix"==u?d.push(q.join("")):e.push(q.join(""))}if(0!=e.length+d.length){b.push('<table class="ewa-lf-search-main" id="'+a+'" cellspacing="0" cellpadding="0" style="width:100%">');b.push("<tr><td class='ewa-lf-search-des'></td><td>");
0<e.length&&b.push(" "+e.join("")+" ");if(0<d.length)for(e=0;e<d.length;e++)b.push(" "+d[e]+" ");b.push("</td></tr></table>");var y=document.createElement("div");this._SEARCH_BOX=y;y.className="ewa-lf-search";y.innerHTML=b.join("");b=$X("EWA_LF_"+this.Id).parentNode;b.parentNode.insertBefore(y,b);a="EWA_SEARCH_ITEM_"+this.Id;var z=this;n&&this._SearchCreateDateRange();p&&this._SearchCreateTextType();z._SEARCH_ITEM_EXP=this.SearchGetExp(y);"{}"!=JSON.stringify(g)&&(z._SearchExp=z._SEARCH_ITEM_EXP);
$(y).find("input[type=text]").on("compositionstart",function(){z._is_search_composition=!0}).on("compositionend",function(){z._is_search_composition=!1});this._TIMER_SEARCH=window.setInterval(function(){if(!z._is_search_composition){try{if(!$X(a)){window.clearInterval(z._TIMER_SEARCH);return}}catch(B){window.clearInterval(z._TIMER_SEARCH);return}var A=z.SearchGetExp(y);A!=z._SEARCH_ITEM_EXP&&(z._SEARCH_ITEM_EXP=A,z.DoSearch($X(a)))}},700)}};this._SearchSingle=function(a,b){var d=[];d.push('<input t="text" type="text" autocomplete="off" class="EWA_INPUT" maxlength="40" name="');
