...
w=$(".ewa-slider-puzzle-swiper");var y=$(".ewa-slider-puzzle-verify");v=v.clientX-t-$(x).offset().left;x=$(x).width()+29;0>v?v=0:v>x&&(v=x);w.css("left",v);y.css("left",v+10)});l.bind(r,function(w){l.unbind(q);l.unbind(r);w=new EWA_UrlClass;console.log(n.position());console.log(n.offset());w.AddParameter("left",n.position().left);w.AddParameter("top",n.position().top);w.AddParameter("left1",n.offset().left);w.AddParameter("top1",n.offset().top);w.AddParameter("ewa_trigger_valid_mode","check");w.AddParameter("ewa_trigger_valid_name",
h.ewa_trigger_valid_name);w.AddParameter("ewa_ajax",h.ewa_trigger_valid);$J(w.GetUrl(),function(v){console.log(v);v.RST?(l.addClass("ewa-slider-puzzle-successful"),e&&e(v)):(l.addClass("ewa-slider-puzzle-fail"),setTimeout(function(){l.removeClass("ewa-slider-puzzle-fail");m.css("left","0px");n.css("left","10px");p.css("opacity","1");g&&g(v);v.newConfig&&(v.newConfig.ewa_trigger_valid_name=h.ewa_trigger_valid_name,v.newConfig.ewa_trigger_valid=h.ewa_trigger_valid,h=v.newConfig,b(h))},500))})})})}function b(l){var m=
$(".ewa-slider-puzzle-imgbox"),n=$(".ewa-slider-puzzle-imgbox img");m.css("width",l.imgBigWidth).css("height",l.imgBigHeight);n.length?n.attr("src",l.imgBig):m.prepend("<img src='"+l.imgBig+"' />");$(".ewa-slider-puzzle-verify").css("background-image","url('"+l.imgSmall+"')").css("top",l.imgTop).css("width",l.imgSmallWidth).css("height",l.imgSmallHeight)}var d=[];d.push('<div class="ewa-slider-puzzle-box">');d.push('<div class="ewa-slider-puzzle-imgbox">');d.push('<div class="ewa-slider-puzzle-verify"></div>');
d.push('<div class="ewa-slider-puzzle-refresh"></div>');d.push("</div>");d.push('<div class="ewa-slider-puzzle-handle">');d.push('<span class="ewa-slider-puzzle-swiper"></span>');d.push('<span class="ewa-slider-puzzle-text"></span>');d.push("</div>");d.push("</div>");var e,g,h,k=/iphone|android|ipad/ig.test(navigator.userAgent)||window.EWA_App;EWA.UI.SlidePuzzle=function(l,m,n,p){h=l;e=n;g=p;$(m||"body").append(d.join(""));m=$(".ewa-slider-puzzle-refresh");$(".ewa-slider-puzzle-box").css("width",
l.imgBigWidth);b(l);m.click(function(q){q.stopPropagation();q=new EWA_UrlClass;q.AddParameter("ewa_trigger_valid_mode","refresh");q.AddParameter("ewa_trigger_valid_name",h.ewa_trigger_valid_name);q.AddParameter("ewa_ajax",h.ewa_trigger_valid);q.AddParameter("ewa_trigger_valid_width",document.documentElement.clientWidth);$J(q,function(r){r.RST?(r.ewa_trigger_valid_name=h.ewa_trigger_valid_name,r.ewa_trigger_valid=h.ewa_trigger_valid,h=r,b(h)):alert(r.ERR)})});a()}})();
...
"Input character selection");this._InitTranslations();this._GetDropListValue();this._InitMustInputs();this._InitEncyptions();this._InitTriggerValids()};this._InitTriggerValids=function(){var a=$("#EWA_FRAME_"+this._Id),b="EWA.F.FOS['"+this._Id+"'].callTriggerValid(this)";this.triggerValids={};for(var d in this.ItemList.Items){var e=this.ItemList.Items[d],g=this.ItemList.GetItemValue(e,"Name","Name");if(e=$(e).find("DataItem Set[TriggerValid]").attr("TriggerValid")){this.triggerValids[g]=null;g=a.find("#"+
g);0==g.length&&contine;g.attr("ewa_trigger_valid",e);if("submit"==g[0].type)break;(e=g.attr("onclick"))&&g.attr("_onclick",e);g.attr("onclick",b)}}};this.callTriggerValid=function(a){var b=$(a).attr("id"),d=this.getUrlClass(),e=$(a).attr("ewa_trigger_valid");d.AddParameter("ewa_ajax",e);d.AddParameter("ewa_trigger_valid_name",b);d.AddParameter("ewa_trigger_valid_mode","create");d.AddParameter("ewa_trigger_valid_width",document.documentElement.clientWidth);var g=this,h=EWA_Utils.tempId();$(a).attr("_trigger_valid_id",
h);$J(d.GetUrl(),function(k){if(k.RST){var l=$DialogHtml("<div id='"+h+"'></div>","enus"==EWA.Lang?"Silde puzzle":"\u62fc\u56fe\u9a8c\u8bc1",k.bigImgWidth+20,200,!1);k.ewa_trigger_valid_name=b;k.ewa_trigger_valid=e;EWA.UI.SlidePuzzle(k,$("#"+h),function(m){$(a).removeAttr("onclick");var n=!1;$(a).attr("_onclick")&&($(a).attr("onclick",$(a).attr("_onclick")),n=!0);setTimeout(function(){$(l.getMain()).animate({opacity:0},500)},500);setTimeout(function(){g.triggerValids[b]=m.VALID;n&&a.click();l.Close()},
1E3)});setTimeout(function(){l.AutoSize()},100)}else alert(k.ERR)})};this._InitMustInputs=function(){var a=$("#EWA_FRAME_"+this._Id),b;for(b in this.ItemList.Items){var d=this.ItemList.Items[b],e=this.ItemList.GetItemValue(d,"Name","Name");0<$(d).find('IsMustInput Set[IsMustInput="1"]').length&&(d="#"+e,a.find(".ewa-row-"+e).addClass("ewa-row-must"),e=a.find(d),e.addClass("ewa-row-item-must"),0<e.length&&e.attr("type")&&"hidden"==e.attr("type").toLowerCase()&&e.prev().addClass("ewa-row-item-must"))}};
...
1<g.length&&b.AddParameter(g[1].id,g[1].value))}}if(this.PostAddData)for(n in this.PostAddData)b.AddParameter(n,this.PostAddData[n]);if(this.triggerValids)for(var p in this.triggerValids)b.AddParameter(p+"_TRIGGER_VALID_RESULT",this.triggerValids[p]);return b};this.CreatePostData=function(){var a=$("#f_"+this._Id)[0],b={},d;for(d in this.ItemList.Items){var e=this.ItemList.GetItemValue(this.ItemList.Items[d],"Name","Name"),g=this.GetObject(e,a);if(null!=g){var h=this._GetObjectValue(g);null!=h&&(b[e]=
h);"DIV"==g.tagName.toUpperCase()&&"1"==g.getAttribute("EWA_DHTML")&&(h=$X("UP_IMG_"+e).value,b[e+"_imgs_split"]=h);e=g.getAttribute("ewa_tag");if("swffile"==e||"IMG_UPLOAD"==e)if(h=g.getAttribute("UP_PARAS"),null!=h&&0<h.length){h=h.split(",");for(var k=0;k<h.length;k++){var l=h[k];b[l]=g.getAttribute(l)}}else b.UP_URL=g.value;"markdown"==e&&(g=g.parentNode.getElementsByTagName("textarea"),1<g.length&&(b[g[1].id]=g[1].value))}}return b};this.DoPostBefore=function(){return!0};this.isDoPostBeforeTimer=
