...
"")+1*h.css("padding-right").replace("px","");e.find(".ewa-row-"+a).hide();null==d&&(d=" - ");var q=l.width(),r=g[0].tagName,u=h[0].tagName;g.parent().hasClass("ewa-select-filter-target")&&(g=e.find(".ewa-row-"+a+" .EWA_TD_M table:eq(0)"),r="SELECT_WITH_FILTER");var t=0;b=null;d&&(l.append('<span class="mearge-span">'+d+"</span>"),k&&m.find("table:eq(0) td:eq(1)").html('<span class="mearge-span">'+d+"</span>").show(),b=l.find(".mearge-span"),t=b.width()+1+1*b.css("padding-left").replace("px","")+
1*b.css("padding-right").replace("px","")+1*b.css("margin-left").replace("px","")+1*b.css("margin-right").replace("px",""));r==u?(d=(q-t)/2-n-2,g.css("width",d),h.css("width",d),k&&m.find("table:eq(0) td:eq(2), table:eq(0) td:eq(0)").css("width",d)):"SELECT"==r?(g.css("width",100),h.css("width",q-t-n-p-2-2-g.width())):"SELECT_WITH_FILTER"==r?(g.css("width",200),h.css("width",q-t-n-p-2-2-g.width())):"SELECT"==u&&(h.css("width",100),g.css("width",q-t-n-p-2-2-g.width()));h.css("float","left");b&&b.css("float",
...
a||(a=_EWA_EVENT_MSG.ValidCodeError,$Tip(a),$(".ewa-valid-code-img").parent().parent().find("input").focus().select()))};this.checkIdempotenceError=function(){$Tip(_EWA_EVENT_MSG.IdempotanceError)};this.checkValidSildePuzzleError=function(){$Tip(_EWA_EVENT_MSG.ValidSildePuzzleError)};this.Init=function(a){this.Xml=new EWA.C.Xml;this.Xml.LoadXml(a);this.ItemList.F=this;this.ItemList.Init(this.Xml);this.Resources=new EWA_FrameResoures;this.Resources.Init(this.Xml);"enus"==EWA.LANG&&$(".ewa-ipt-droplist").attr("placeholder",
"Input character selection");this._InitTranslations();this._GetDropListValue();this._InitMustInputs();this._InitEncyptions();this._InitTriggerValids()};this._InitTriggerValids=function(){var a=$("#EWA_FRAME_"+this._Id),b="EWA.F.FOS['"+this._Id+"'].callTriggerValid(this)";this.triggerValids={};for(var d in this.ItemList.Items){var e=this.ItemList.Items[d],g=this.ItemList.GetItemValue(e,"Name","Name"),h=$(e).find("DataItem Set[TriggerValid]").attr("TriggerValid");if(h){this.triggerValids[g]=null;var k=
a.find("#"+g);0==k.length&&contine;k.attr("ewa_trigger_valid",h);e=$(e).find("Tag Set").attr("Tag");console.log(e);if("submit"==e)break;if("smsValid"==e){k=a.find(".ewa-row-"+g+" .ewa-sms-valid-code-button");e=k.attr("id");e||(e=EWA_Utils.tempId(),k.attr("id",e));var l=$("<a style='position:absolute;left:0;top:0;right:0;bottom:0'></a>");l.attr("ewa_trigger_valid",h);l.attr("ewa_trigger_valid_rid",g);l.attr("ewa_trigger_valid_click_id",e);k.parent().css("position","relative").append(l);k=l}(g=k.attr("onclick"))&&
k.attr("_onclick",g);k.attr("onclick",b)}}};this.callTriggerValidBefore=function(a){return!0};this.callTriggerValid=function(a){if(this.callTriggerValidBefore(a)){var b=$("#EWA_FRAME_"+this._Id),d=$(a).attr("id");d||(d=$(a).attr("ewa_trigger_valid_rid"));var e=this.getUrlClass(),g=$(a).attr("ewa_trigger_valid");e.AddParameter("ewa_ajax",g);e.AddParameter("ewa_trigger_valid_name",d);e.AddParameter("ewa_trigger_valid_mode","create");e.AddParameter("ewa_trigger_valid_width",document.documentElement.clientWidth);
var h=this,k=EWA_Utils.tempId();$(a).attr("_trigger_valid_id",k);$J(e.GetUrl(),function(l){if(l.RST){var m=$DialogHtml("<div id='"+k+"'></div>","enus"==EWA.Lang?"Silde puzzle":"\u62fc\u56fe\u9a8c\u8bc1",l.bigImgWidth+20,200,!1);l.ewa_trigger_valid_name=d;l.ewa_trigger_valid=g;l.ewa_url=e.GetUrl();EWA.UI.SlidePuzzle(l,$("#"+k),function(n){$(a).removeAttr("onclick");var p=!1;$(a).attr("_onclick")&&($(a).attr("onclick",$(a).attr("_onclick")),p=!0);setTimeout(function(){$(m.getMain()).animate({opacity:0},
500)},500);setTimeout(function(){h.triggerValids[d]=n.VALID;var q=$(a).attr("ewa_trigger_valid_click_id");q?($(a).remove(),b.find("#"+q).click()):p&&a.click();m.Close()},1E3)});setTimeout(function(){m.AutoSize()},100)}else alert(l.ERR)})}};this._InitMustInputs=function(){var a=$("#EWA_FRAME_"+this._Id),b;for(b in this.ItemList.Items){var d=this.ItemList.Items[b],e=this.ItemList.GetItemValue(d,"Name","Name");0<$(d).find('IsMustInput Set[IsMustInput="1"]').length&&(d="#"+e,a.find(".ewa-row-"+e).addClass("ewa-row-must"),
...
this.SubBottomsArray[d];b.$jscomp$loop$prop$total$34=0;var g="#EWA_LF_"+this._Id+' .ewa-lf-data-row [name="'+e+'"]';b.$jscomp$loop$prop$fm_length$35=0;$(g).each(function(h){return function(){var k=this.hasAttribute("value")?this.value:this.hasAttribute("title")?this.title:this.innerText;null!=k&&0!==k.length&&(k=k.replace(/,/ig,""),isNaN(k)||(h.$jscomp$loop$prop$total$34+=1*k,0<k.indexOf(".")&&(h.$jscomp$loop$prop$fm_length$35=2)))}}(b));g="<span id='"+e+"' name='"+e+"' class='ewa-lf-sub'>"+b.$jscomp$loop$prop$total$34.fm(b.$jscomp$loop$prop$fm_length$35)+
"</span>";a.find("td[id=ADD_ROW_"+e+"]").html(g);b={$jscomp$loop$prop$total$34:b.$jscomp$loop$prop$total$34,$jscomp$loop$prop$fm_length$35:b.$jscomp$loop$prop$fm_length$35}}};this.ChangeRowStyle=function(a,b,d){for(var e=$X("EWA_LF_"+this._Id),g=1;g<e.rows.length;g++){var h=e.rows[g];if(!(0==h.cells.length||h.cells.length<=a)){var k=h.cells[a].childNodes[0].getAttribute(b);if(null!=k&&d[k])for(var l=0;l<h.cells.length;l++)h.cells[l].style.cssText=h.cells[l].style.cssText+";"+d[k]}}if(null==this.ReloadAfter){var m=
