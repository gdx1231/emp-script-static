...
b=m;var n=[],p=!1,t=GetInnerText(e);t=t.trim();this._ReshowButs={};for(var r in this.ItemList.Items){m=this.ItemList.Items[r];var q=this._GetSubValue("Tag","Tag",m);if(null!=q&&("button"==q.trim().toLowerCase()||"butflow"==q.trim().toLowerCase()))for(q=0;q<h.rows[0].cells.length;q++)if(h.rows[0].cells[q].childNodes[0].id.toUpperCase()==r){n.push(q);p=!0;q=this._GetSubValue("DescriptionSet","Info",m);var v=this._GetSubValue("DescriptionSet","Memo",m),u=this._GetSubValue("EventSet","EventName",m),x=
"";null!=u&&"ewa_click"==u.toLowerCase().trim()&&(x=this._GetSubValue("EventSet","EventValue",m));t=t.replace(q,"");m=this._ReShowButton(q,v,null);this._ReshowButs[q]=m;m.setAttribute("f_id",this._Id);e.childNodes[1].appendChild(m);""==x?m.onclick=function(){var z=this.getAttribute("f_id");z=EWA.F.FOS[z].SelectCheckedRows();if(null==z||0==z.length)EWA.UI.Msg.Alert(_EWA_INFO_MSG["EWA.SYS.CHOOSE_ITEM"],_EWA_INFO_MSG["EWA.SYS.CHOOSE_ITEM_TITLE"]);else{z=z[0].getElementsByTagName("input");for(var C=0;C<
z.length;C++){var D=z[C];if("button"==D.type&&D.value==this.childNodes[0].innerHTML){D.click();break}}}}:(m.setAttribute("ewa_click",x),m.onclick=function(){eval(this.getAttribute("ewa_click"))});break}}p&&this._ReShowSplit(e.childNodes[1]);n=e.getElementsByTagName("input");h={};for(q=0;q<n.length;q++){var w=n[q];w.style.display="none";if(null==w.id||""==w.id)w.id=("_tmp_"+Math.random()).replace(".","_");if("button"==w.type){if(m=this._ReShowButton(w.value,w.title,w.id),p=w.nextSibling){var y="";
3==p.nodeType&&(y=p.nodeValue.trimEx());"|"==y&&this._ReShowSplit(e.childNodes[1])}}else(p=w.nextSibling)&&(y=3==p.nodeType?p.nodeValue.replace("|",""):p.innerHTML),m=this._ReShowButton(y.trimEx(),w.title,w.id),m.id=("_tmp_"+Math.random()).replace(".","_"),h[w.name]||(h[w.name]=[]),h[w.name].push(m.id);e.childNodes[1].appendChild(m)}t=t.replace(/ /ig,"").replace(/\|/ig,"").trimEx();""!=t?0<=t.indexOf("if")?c.style.display="none":d.innerHTML=t.replace(/ /,""):c.style.display="none";k.appendChild(a);
for(r in h)for(y=h[r],q=0;q<y.length;q++)w=$X(y[q]),w.setAttribute("_r_ids",y.join(",")),w.onclick=function(){var z=this.getAttribute("_ewa_event_id");$X(z).click();z=this.getAttribute("_r_ids").split(",");for(var C=0;C<z.length;C++)w.id!=z[C]&&($X(z[C]).style.fontWeight="",$X(z[C]).style.color="");this.style.fontWeight="bold";this.style.color="blue"};b.innerHTML='<b>[<i class="fa fa-star-o"></i>&nbsp;'+this.Description+"]</b>";if(0<this.Url.toUpperCase().indexOf("EWA_BTNS_IN_TITLE"))var A=this,B=
0,E=setInterval(function(){B++;1E3<B&&window.clearInterval(E);EWA.OW.Load();EWA.OW.Dia&&(window.clearInterval(E),A.ReShowButtonsInDailogTitle())},10)};this._ReShowSplit=function(a){EWA.UI.Utils.CreateObject(window,"div","",a).className="ewa_lf_func_split"};this._ReShowButton=function(a,b,c){var d=EWA.UI.Utils.CreateObject(window,"div","cursor:pointer",document.body);d.innerHTML="<nobr>"+a+"</nobr>";null!=c&&(d.setAttribute("_ewa_event_id",c),d.onclick=function(){var e=this.getAttribute("_ewa_event_id");
$X(e).click()});EWA.B.IE&&(d.onselectstart=function(){return!1});d.className="ewa_lf_func_dact";d.title=b;return d};this.CheckValid=function(a){var b=a.tagName.toLowerCase();if("span"==b||"img"==b||"div"==b)return!0;b=this.ItemList.GetObjectValue(a);return this.ItemList.CheckValid(a,b)};this.ShowEdit=function(a){a.parentNode.style.width=a.clientWidth+"px";a.style.display="none";__show_obj__=a.nextSibling;__show_obj__.style.display="";var b=__show_obj__.childNodes[0];3==b.nodeType&&(b=__show_obj__.childNodes[1]);
b.setAttribute("old",b.value);"SELECT"!=b.tagName||b.getAttribute("func")||(b.setAttribute("func",1),b.onchange=function(){EWA.F.FOS[this.getAttribute("__ewa_fid__")].EditAfter(this)});null==b.getAttribute("__ewa_fid__")&&(b.setAttribute("__ewa_fid__",this._Id),b.onblur=function(){EWA.F.FOS[this.getAttribute("__ewa_fid__")].EditAfter(this)},b.onkeydown=function(c){c=null==c?window.event:c;var d=c.target?c.target:c.srcElement;13==c.keyCode&&"INPUT"==d.tagName&&EWA.F.FOS[this.getAttribute("__ewa_fid__")].EditAfter(this);
...
c.para1=e.unURL()))))}return b};this.SearchFilterDate=function(a){var b=$(this._MENU_DATE_RANGE.SHOW_BY_OBJECT).parentsUntil(".ewa-lf-search-item").last(),c=b.find("input")[0];b=b.find("input")[1];if("Clear"==a)$(c).val(""),$(b).val("");else{var d=EWA_Utils.Date(),e="",g="";"Today"==a?g=e=d.GetDateString():"Week"==a?(a=d.GetBenWeek(),e=a[0],g=a[1]):"Today-Weekend"==a?(a=d.GetBenWeek(),e=a[d.GetDateString()],g=a[1]):"Month"==a?(a=d.GetBenMonth(),e=a[0],g=a[1]):"Today-EOM"==a?(a=d.GetBenMonth(),e=a[d.GetDateString()],
g=a[1]):"Quarter"==a?(a=d.GetBenQ(),e=a[0],g=a[1]):"Today-EOQ"==a?(a=d.GetBenQ(),e=a[d.GetDateString()],g=a[1]):"Year"==a?(a=d.GetBenYear(),e=a[0],g=a[1]):"Today-EOY"==a&&(a=d.GetBenYear(),e=a[d.GetDateString()],g=a[1]);a=EWA_Utils.Date(e).FormatDate();d=EWA_Utils.Date(g).FormatDate();d+=" 23:59:59";$(c).val(a);$(b).val(d)}};this._SearchCreateDateRange=function(){var a="_MENU_DATE_RANGE"+this._Id;$("."+a).remove();var b="enus"==EWA.LANG.toLowerCase()?"TxtEn":"Txt",c=JSON.parse(JSON.stringify(this.RESOURCES.search_date_items)),
...
return b};this._SearchCreateItm=function(){var a="EWA_SEARCH_ITEM_"+this.Id,b=[],c=[],d=[],e={},g;for(g in this._SearchJson)this._SearchJson[g].ORI_NAME=g,e[g.toUpperCase()]=this._SearchJson[g];this._SearchJson=e;var h="EWA.F.FOS[&quot;"+this._Id+"&quot;].ChangeSearchTextType(this)",k="EWA.F.FOS[&quot;"+this._Id+"&quot;].ChangeSearchDateType(this)",l=$J2MAP(this.RESOURCES.search_text_items,"Id");e=this._GetInitSearchMap();var m=!1,n=!1;for(g in this.ItemList.Items)if(null!=this._SearchJson[g]){var p=
[],t=this._SearchJson[g],r=t.T,q=this._GetSubValue("DescriptionSet","Info",this.ItemList.Items[g]);p.push("<table class='ewa-lf-search-item ewa-ref-"+g+" ewa-lf-search-type-"+r+" "+("fix"==r?"ewa-lf-search-fix-m"+(t.M||"0"):"")+"' cellspacing=0 cellpadding=0>");p.push("<tr><td class='ewa-lf-search-item-title' style='position:relative'><nobr><b>");p.push(q);var v=q="",u="lk";e[g.toUpperCase()]&&(v=e[g.toUpperCase()],u=v.tag,q=v.para1,v=v.para2);var x="";"text"==r?(n=l[u]||l.lk,x="enus"==EWA.LANG?n.TxtEn:
n.Txt,p.push("<a class='ewa-lf-search-type' tag='"+u+"' href='javascript:void(0)' onclick='"+h+"'>"),p.push(x),p.push("</a>"),n=!0):"date"==r&&(p.push("<a  href='javascript:void(0)' class='fa fa-calendar-check-o'  onclick='"+k+"'></a>"),m=!0);p.push("</b></nobr></td><td class='ewa-lf-search-item-ctl'>");"text"==r?"blk"==u||"nblk"==u?(t=this._SearchSingle(g,x),t=t.replace("<input","<input disabled "),p.push(t)):p.push(this._SearchSingle(g,q)):"date"==r?p.push(this._SearchDate(g,q,v)):"number"==r?p.push(this._SearchNumber(g,
q,v)):"fix"==r&&(t="<div >"+this._SearchFix(g,t,"span",q)+"</div>",p.push(t));p.push("</td></tr></table>");"fix"==r?c.push(p.join("")):d.push(p.join(""))}if(0!=d.length+c.length){b.push('<table class="ewa-lf-search-main" id="'+a+'" cellspacing="0" cellpadding="0" style="width:100%">');b.push("<tr><td class='ewa-lf-search-des'></td><td>");0<d.length&&b.push(" "+d.join("")+" ");if(0<c.length)for(d=0;d<c.length;d++)b.push(" "+c[d]+" ");b.push("</td></tr></table>");var w=document.createElement("div");
this._SEARCH_BOX=w;w.className="ewa-lf-search";w.innerHTML=b.join("");b=$X("EWA_LF_"+this.Id).parentNode;b.parentNode.insertBefore(w,b);a="EWA_SEARCH_ITEM_"+this.Id;var y=this;m&&this._SearchCreateDateRange();n&&this._SearchCreateTextType();y._SEARCH_ITEM_EXP=this.SearchGetExp(w);"{}"!=JSON.stringify(e)&&(y._SearchExp=y._SEARCH_ITEM_EXP);$(w).find("input[type=text]").on("compositionstart",function(){y._is_search_composition=!0}).on("compositionend",function(){y._is_search_composition=!1});this._TIMER_SEARCH=
window.setInterval(function(){if(!y._is_search_composition){try{if(!$X(a)){window.clearInterval(y._TIMER_SEARCH);return}}catch(B){window.clearInterval(y._TIMER_SEARCH);return}var A=y.SearchGetExp(w);A!=y._SEARCH_ITEM_EXP&&(y._SEARCH_ITEM_EXP=A,y.DoSearch($X(a)))}},700)}};this._SearchSingle=function(a,b){var c=[];c.push('<input t="text" type="text" autocomplete="off" class="EWA_INPUT" maxlength="40" name="');c.push(a.toInputValue());c.push('"');null!=b&&(c.push(' value="'),a=b.toInputValue(),c.push(a),
c.push('" '));c.push(" />");return c.join("")};this._SearchDate=function(a,b,c){var d=[];d.push("<table border=0 cellpadding=0 cellspacing=0>");d.push('<td  ><input type="text" autocomplete="off" class="EWA_INPUT" t="date" readonly maxlength="8" onclick="EWA.UI.Calendar.Pop(this)" name="');d.push(a);d.push('"');b&&(b=b.toInputValue(),d.push(' value="'),d.push(b),d.push('"'));d.push(' /></td><td><span class="ewa-lf-search-split"> - </span></td>');d.push('<td  ><input type="text" autocomplete="off" class="EWA_INPUT" t="date" readonly maxlength="8" onclick="EWA.UI.Calendar.Pop(this)" name="');
