...
!1;this.CheckedAllAfter=this.ReloadAfter=null;this.IsNotMDownAutoChecked=!1;this.composeSearchTexts=function(){var a=[],b=[],c=$("#EWA_RESHOW_"+this._Id+" .ewa-lf-search-type-text");if(0==c.length)return null;c.each(function(g){var h=$(this).find(".ewa-lf-search-item-title").text();h=h.replace(":","").replace("\uff1a","").replace("\u5305\u542b","").replace("Like","").trim();var k=$(this).find("input[type=text]").attr("name");a.push(h);b.push(k);0<g&&$(this).hide()});var d=$(c[0]);d.addClass("ewa-app-lf-search-compose");
d.find("input[type=text]").attr("name",b.join(",")).attr("placeholder",a.join(", "));var e=d.find("a").clone();d.find(".ewa-lf-search-item-title b").text("enus"==EWA.LANG?"Search":"\u7efc\u5408\u641c\u7d22").append(e);return c};this.switchButtonAction=function(a,b){if(b){var c=this.getUrlClass();c.AddParameter("ewa_switch_name",a.name);c.AddParameter("ewa_action",b);c.AddParameter("ewa_ajax","json");c.AddParameter("ewa_switch",a.checked?"on":"off");c.AddParameter("ewa_action_key",$(a).parentsUntil("tbody").last().attr("ewa_key"));
var d=a.parentNode,e=d.getAttributeNames();b={};for(var g in e){var h=e[g],k=d.getAttribute(h);0!=h.indexOf("on")&&"name"!=h&&"id"!=h&&"class"!=h&&(b[h]=k)}var l=this;c=c.GetUrl();$JP(c,b,function(m){l.extSwitchCallBack&&l.extSwitchCallBack(a,m)})}};this.replaceRowsData=function(a,b){var c=this.getUrlClass();c.AddParameter("EWA_AJAX","LF_RELOAD");c.AddParameter("EWA_IS_SPLIT_PAGE","no");c.AddParameter("EWA_IS_HIDDEN_CAPTION","yes");c=c.GetUrl();a&&(c+="&"+a);var d=$("#EWA_LF_"+this._Id);$J2(c,function(e){$(e).find("tr.ewa-lf-data-row").each(function(){var g=
$(this).attr("ewa_key"),h=d.find('tr[ewa_key="'+g+'"]');$(this).find("td").each(function(){var k=$(this).children(0).attr("name");k&&(k=h.find(".ewa-col-"+k),0!=k.length&&(b?b(this,k[0]):k.html($(this).html())))})})})};this.ShowRecycle=function(){var a=$X("EWA_LF_"+this._Id);if(null!=a&&(a=$(a).parentsUntil("table").last().find(".ewa_lf_func_caption"),0!=a.length)){var b="\u56de\u6536\u7ad9";window._EWA_INFO_MSG&&window._EWA_INFO_MSG.RECYCLE&&(b=window._EWA_INFO_MSG.RECYCLE);b="<div class='ewa_lf_func_dact ewa-lf-func-recycle' onclick='EWA.F.FOS[\""+
...
"$(this).parent().parent().find('[id=\""+b+"\"]')[0].click()");$(this).addClass("ewa-map-button")})};this.Merge=function(a,b,c,d){$("#EWA_LF_"+this._Id+" tr[ewa_key]").each(function(){var e=$(this).find('[id="'+a+'"]');e.parent().hide();var g=$(this).find('[id="'+b+'"]').parentsUntil("tr").last();c||(c="<br>");0<=c.indexOf("<")?g.append(c):g.append("<span class='ewa-mearge-str'>"+c+"</span>");g.append(e);d&&d(g,this)});$("#EWA_LF_"+this._Id+' tr[ewa_tag="HEADER"] [id="'+a+'"]').parent().hide()};this.Mearge=
function(a,b,c){console.log("\u62fc\u5199\u9519\u8bef\uff0c\u8bf7\u7528 Merge");this.Merge(a,b,c)};this.MergeExp=function(a,b,c,d){if(b){var e=b.match(/@@[a-zA-Z0-9\-\._:]*\b/ig),g=[],h=b,k={};for(b=0;b<e.length;b++){var l=e[b];g.push(l);var m=l.replace("@@","");h=h.replace(l,"<span class='ewa-lf-mearge ewa-lf-mearge-"+m+"' mid=\""+m+'"></span>');m!=a&&$("#EWA_LF_"+this._Id+' tr[ewa_tag="HEADER"] [id="'+m+'"]').parent().hide();k[m]=$("#EWA_LF_"+this._Id+' tr[ewa_tag="HEADER"] [id="'+m+'"]').html()}$("#EWA_LF_"+
this._Id+" tr[ewa_key]").each(function(){var n=document.createElement("div");n.style.display="none";n.innerHTML=h;document.body.appendChild(n);if(0==$(this).find('[id="'+a+'"]').length)console.log("not find "+a);else{var p=$(this).find('[id="'+a+'"]').parent(),t;for(t in g){var r=g[t].replace("@@",""),q=$(this).find('[id="'+r+'"]');if(0!=q.length){r!=a&&q.parent().hide();var v=$(n).find('span[mid="'+r+'"]');c&&(v.append("<span class='ewa-lf-mearge-memo'></span>"),v.find(".ewa-lf-mearge-memo").html(k[r]));
v.append(q)}}for(;0<n.childNodes.length;)p.append(n.childNodes[0]);$(n).remove();d&&d(p,this)}})}else console.log("mergeExp \u6ca1\u6709\u8bbe\u7f6e")};this.MeargeExp=function(a,b,c,d){console.log("\u62fc\u5199\u9519\u8bef\uff0c\u8bf7\u7528 MereExp");this.MergeExp(a,b,c,d)};this.SubBottoms=function(a){this.SubBottomsArray=a.split(",");this._SubBottoms()};this._SubBottoms=function(){var a=this.AddRow([]);$(a).find("td").addClass("ewa-lf-sub-td");var b=0,c;for(c in this.SubBottomsArray){var d=this.SubBottomsArray[c],
e=0;$("#EWA_LF_"+this._Id+" .ewa-col-"+d).each(function(){var g=GetInnerText(this).replace(/,/ig,"");isNaN(g)||(e+=1*g,0<g.indexOf(".")&&(b=2))});$(a).find("td[id=ADD_ROW_"+d+"]").html("<nobr class='ewa-lf-sub'>"+e.fm(b)+"</nobr>")}};this.ChangeRowStyle=function(a,b,c){for(var d=$X("EWA_LF_"+this._Id),e=1;e<d.rows.length;e++){var g=d.rows[e];if(!(0==g.cells.length||g.cells.length<=a)){var h=g.cells[a].childNodes[0].getAttribute(b);if(null!=h&&c[h])for(var k=0;k<g.cells.length;k++)g.cells[k].style.cssText=
g.cells[k].style.cssText+";"+c[h]}}if(null==this.ReloadAfter){var l=this;this.ReloadAfter=function(){l.ChangeRowStyle(a,b,c)}}};this.MDownEvent=function(a,b){};this.LoadJson=function(a,b){null!=a&&$J(this.Url+"&EWA_ACTION="+a+"&EWA_AJAX=JSON",b)};this.Workflow=function(a,b,c,d,e,g){EWA.F.CID=this._Id;this._Ajax=this.CreateAjax();this._Ajax.AddParameter("EWA_AJAX","WORKFLOW");this._Ajax.AddParameter("EWA_ID",this._Id);this._Ajax.AddParameter("EWA_WF_NAME",b);null!=c?(a=this.GetRowKey(c),this._Ajax.AddParameter("EWA_ACTION_KEY",
...
this.ItemList.Items[r];var q=this._GetSubValue("Tag","Tag",m);if(null!=q&&("button"==q.trim().toLowerCase()||"butflow"==q.trim().toLowerCase()))for(q=0;q<h.rows[0].cells.length;q++)if(h.rows[0].cells[q].childNodes[0].id.toUpperCase()==r){n.push(q);p=!0;q=this._GetSubValue("DescriptionSet","Info",m);var v=this._GetSubValue("DescriptionSet","Memo",m),u=this._GetSubValue("EventSet","EventName",m),y="";null!=u&&"ewa_click"==u.toLowerCase().trim()&&(y=this._GetSubValue("EventSet","EventValue",m));t=t.replace(q,
...
d=0;d<c.length;d++){var e=c[d];if("text"==e.type)e.value="";else if("radio"==e.type||"checkbox"==e.type)e.checked=!1}};this.SearchGetExp=function(a){var b=[];$(a).find(".ewa-lf-search-item").each(function(){var c=$(this).find("input");if(0==c.length&&(c=$(this).find("select"),0==c.length))return;var d=c[0],e=$(d).attr("t");if(1==c.length){var g=d.value;""!=d.value.trim()&&("text"==e&&(c=$(d.parentNode.parentNode).find("a").attr("tag"))&&(e=c),b.push("@!@"+d.name+"~!~"+e+"~!~"+g))}else if(("date"==
e||"number"==e)&&2==c.length)g=d.value,c=c[1].value,""==c&&""==g||b.push("@!@"+d.name+"~!~"+e+"~!~"+g+"~!~"+c);else if("fix"==e&&"INPUT"==d.tagName){var h=[""];$(this).find("input").each(function(){this.checked&&h.push(this.value)});1!=h.length&&b.push("@!@"+d.name+"~!~"+e+h.join("~!~"))}});return b.join("")};this.DoSearch=function(a){var b=!1;a.id=="EWA_SEARCH_ITEM_"+this.Id?a=$X("EWA_SEARCH_ITEM_"+this.Id).childNodes[0]:(a=a.parentNode.parentNode.parentNode.parentNode,b=!0);this._SearchExp=this.SearchGetExp(a);
...
[],t=this._SearchJson[g],r=t.T,q=this._GetSubValue("DescriptionSet","Info",this.ItemList.Items[g]);p.push("<table class='ewa-lf-search-item ewa-ref-"+g+" ewa-lf-search-type-"+r+" "+("fix"==r?"ewa-lf-search-fix-m"+(t.M||"0"):"")+"' cellspacing=0 cellpadding=0>");p.push("<tr><td class='ewa-lf-search-item-title' style='position:relative'><nobr><b>");p.push(q);var v=q="",u="lk";e[g.toUpperCase()]&&(v=e[g.toUpperCase()],u=v.tag,q=v.para1,v=v.para2);var y="";"text"==r?(n=l[u]||l.lk,y="enus"==EWA.LANG?n.TxtEn:
n.Txt,p.push("<a class='ewa-lf-search-type' tag='"+u+"' href='javascript:void(0)' onclick='"+h+"'>"),p.push(y),p.push("</a>"),n=!0):"date"==r&&(p.push("<a  href='javascript:void(0)' class='fa fa-calendar-check-o'  onclick='"+k+"'></a>"),m=!0);p.push("</b></nobr></td><td class='ewa-lf-search-item-ctl'>");"text"==r?"blk"==u||"nblk"==u?(t=this._SearchSingle(g,y),t=t.replace("<input","<input disabled "),p.push(t)):p.push(this._SearchSingle(g,q)):"date"==r?p.push(this._SearchDate(g,q,v)):"number"==r?p.push(this._SearchNumber(g,
...
this._SEARCH_BOX=w;w.className="ewa-lf-search";w.innerHTML=b.join("");b=$X("EWA_LF_"+this.Id).parentNode;b.parentNode.insertBefore(w,b);a="EWA_SEARCH_ITEM_"+this.Id;var x=this;m&&this._SearchCreateDateRange();n&&this._SearchCreateTextType();x._SEARCH_ITEM_EXP=this.SearchGetExp(w);"{}"!=JSON.stringify(e)&&(x._SearchExp=x._SEARCH_ITEM_EXP);$(w).find("input[type=text]").on("compositionstart",function(){x._is_search_composition=!0}).on("compositionend",function(){x._is_search_composition=!1});this._TIMER_SEARCH=
window.setInterval(function(){if(!x._is_search_composition){try{if(!$X(a)){window.clearInterval(x._TIMER_SEARCH);return}}catch(B){window.clearInterval(x._TIMER_SEARCH);return}var A=x.SearchGetExp(w);A!=x._SEARCH_ITEM_EXP&&(x._SEARCH_ITEM_EXP=A,x.DoSearch($X(a)))}},700)}};this._SearchSingle=function(a,b){var c=[];c.push('<input t="text" type="text" autocomplete="off" class="EWA_INPUT" maxlength="40" name="');c.push(a.toInputValue());c.push('"');null!=b&&(c.push(' value="'),a=b.toInputValue(),c.push(a),
