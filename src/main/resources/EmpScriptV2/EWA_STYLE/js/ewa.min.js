...
EWA_Utils.ShowBgPic=function(a){if(a=$(a).css("background-image"))if(a=a.slice(4,a.length-1)){if(a.startsWith('"')||a.startsWith("'"))a=a.slice(1,a.length-1);0<a.indexOf("pic_no.jpg")||0<a.indexOf("transparent.png")||(a=a.replace("_SMAILL.",".").split("$resized")[0],a=a.replace("small","a"),window.open(a))}};EWA_Utils.RunCmd=function(a,b){"string"==typeof a?(b=a,EWA.LastMenuMsg=""):(b=a.getAttribute("EWA_CMD"),EWA.LastMenuMsg=GetInnerText(a).trimEx());null!=b&&0<b.trim().length&&eval(b)};
...
new EWA_UrlClass(this.Url),b={},d=a.GetParameter("ewa_search");if(!d)return b;this.Url=a.RemoveParameter("ewa_search");d=d.unURL();a=d.split(",");for(var e in a){var g=a[e],h=g.indexOf("["),k=g.indexOf("]");0>h||k<h||(d=g.substring(0,h).trim(),0!=d.length&&(h=g.substring(h+1,k).trim().toLowerCase(),0!=h.length&&(g=g.substring(k+1).trim(),""!=g&&(b[d.toUpperCase()]||(b[d.toUpperCase()]={name:d,tag:h}),d=b[d.toUpperCase()],"lt"==h||"lte"==h?d.para2=g.unURL().replace(/\+/ig," "):d.para1=g.unURL().replace(/\+/ig,
" ")))))}return b};this.SearchFilterDate=function(a){var b=$(this._MENU_DATE_RANGE.SHOW_BY_OBJECT).parentsUntil(".ewa-lf-search-item").last(),d=b.find("input")[0];b=b.find("input")[1];if("Clear"==a)$(d).val(""),$(b).val("");else{var e=EWA_Utils.Date(),g="",h="";"Today"==a?h=g=e.GetDateString():"Week"==a?(a=e.GetBenWeek(),g=a[0],h=a[1]):"Today-Weekend"==a?(a=e.GetBenWeek(),g=a[e.GetDateString()],h=a[1]):"Month"==a?(a=e.GetBenMonth(),g=a[0],h=a[1]):"Today-EOM"==a?(a=e.GetBenMonth(),g=a[e.GetDateString()],
h=a[1]):"Quarter"==a?(a=e.GetBenQ(),g=a[0],h=a[1]):"Today-EOQ"==a?(a=e.GetBenQ(),g=a[e.GetDateString()],h=a[1]):"Year"==a?(a=e.GetBenYear(),g=a[0],h=a[1]):"Today-EOY"==a&&(a=e.GetBenYear(),g=a[e.GetDateString()],h=a[1]);a=EWA_Utils.Date(g).FormatDate();e=EWA_Utils.Date(h).FormatDate();e+=" 23:59:59";$(d).val(a);$(b).val(e)}};this._SearchCreateDateRange=function(){var a="_MENU_DATE_RANGE"+this._Id;$("."+a).remove();var b="enus"==EWA.LANG.toLowerCase()?"TxtEn":"Txt",d=JSON.parse(JSON.stringify(this.RESOURCES.search_date_items)),
