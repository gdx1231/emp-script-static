...
!1;this.CheckedAllAfter=this.ReloadAfter=null;this.REPLACE_HISTORY_STATE=this._isStickyHeaders=this._isMergeButtonsInRow=this.IsNotMDownAutoChecked=!1;this._IsSearchGroup=!0;this.getObj=function(a){return a?$("#EWA_LF_"+this._Id).find(a):$("#EWA_LF_"+this._Id)};this.mergeButtonsInRowInit=function(){this._isMergeButtonsInRow=!0;this.mergeButtonsInRow();this.ReloadAfter};this.mergeButtonsInRow=function(a){a=a||EWA.LANG=="enus"?"Options":"\u64cd\u4f5c";const b=[],c={};var d=this.getObj("tr:eq(0)"),e;
for(e in this.ItemList.Items){var g=this.ItemList.Items[e],h=this._GetSubValue("Tag","Tag",g);if(h&&(h.toLowerCase()=="button"||h.toLowerCase()=="butflow")&&(h=this._GetSubValue("Name","Name",g),d.find(".ewa-col-"+h).length!==0)){var k=this._GetSubValue("DescriptionSet","Info",g),l=this._GetSubValue("DescriptionSet","Memo",g),m=this._GetSubValue("EventSet","EventName",g);m!=null&&m.toLowerCase().trim()=="ewa_click"&&(g=this._GetSubValue("EventSet","EventValue",g),g=$("<input type=button>").attr("onclick",
g.replace("event","this")).addClass("IX_BUTTON").attr("value",k).attr("rid",e),l&&g.attr("title",l),b.push(g),c[h]=!0)}}if(this.getObj(".ewa-col-funcs").length==0){d=this.getObj()[0];for(e=0;e<d.rows.length;e++)h=$(d.rows[e].insertCell(-1)),e==0?(h.addClass("EWA_TD_H ewa-col-funcs").text(a),lastCellIndex=h[0].cellIndex):h.addClass("EWA_TD_M ewa-col-funcs");let n=0,p=this;this.getObj(".ewa-lf-data-row").each(function(){let r=$(this).find(".ewa-col-funcs");$(this).find("input[type=button]").each(function(){n===
0&&p.getObj(".ewa-col-"+this.id).hide();c[this.id]||($(this).parent().hasClass("ewa-with-icon")?r.append($(this).parent().parent()):r.append(this))});n++});this.getObj().parent().css("overflow-x","auto").css("width","100%")}a=$("#_G_"+this._Id+" .ewa-lf-search");if(!a.attr("set_new_buttons"))for(a.attr("set_new_buttons","yes"),d=0;d<b.length;d++)a.append(b[d])};this.changeTag=function(a){a=new EWA_UrlClass("fake?"+a);let b=new EWA_UrlClass(this.Url);for(let c in a._Paras)b.AddParameter(c,a._Paras[c]);
this.Url=b.GetUrl();this.Reload()};this._InitMneu=function(){var a=window["_EWA_MENU_"+this._Id];if(a&&a.length!=0){var b=[];for(let c=0;c<a.length;c++){let d=a[c],e=(c==0?"ewa-lf-menu-marked":"")+(c==a.length-1?"ewa-lf-menu-last":""),g=d.Cmd.replace(/"/g,"&quot;");b.push(`<a class="ewa-lf-menu ${e}" onclick="${"$(this).parent().find('.ewa-lf-menu-marked').removeClass('ewa-lf-menu-marked');$(this).addClass('ewa-lf-menu-marked');"} ${g}">${d.Txt}</a>`)}a=b.join('<b class="ewa-lf-menu-span"></b >');
$("#EWA_RESHOW_"+this._Id).find(".ewa_lf_func_caption").after(a)}};this.stickyHeaders=function(){if(this._isStickyColumns)console.warn("stickyColumns is enabled, stickyHeaders will not work properly.");else{this._isStickyHeaders=!0;var a=$("#EWA_RESHOW_"+this._Id),b=EWA.UI.findScrollParent(a[0]);if(b){var c=a.find(".ewa_lf_func").parent();c.length!=1||c.hasClass("ewa-lf-sticky-func")||c.css("position","sticky").css("top",c.offset().top).addClass("ewa-lf-sticky-func");c=a.find(".ewa-lf-search");c.length!=
1||c.hasClass("ewa-lf-sticky-search")||c.css("position","sticky").css("top",c.offset().top).addClass("ewa-lf-sticky-search");a=a.find(".ewa-lf-header");a.length!=1||a.hasClass("ewa-lf-sticky-header")||a.css("position","sticky").css("top",a.offset().top).addClass("ewa-lf-sticky-header");a="scroll-ref-"+this._Id;if(!$(b).attr(a)){c=b.tagName=="BODY"?window:b;const d=b.tagName=="BODY"?document.documentElement:b;$(b).attr(a,1);$(c).on("scroll",function(){d.scrollTop===0?$(b).removeClass("ewa-lf-sticky-scroll-vertical"):
$(b).hasClass("ewa-lf-sticky-scroll-vertical")||$(b).addClass("ewa-lf-sticky-scroll-vertical")});$(c).trigger("scroll")}}}};this.stickyColumns=function(a){if(this._isStickyHeaders)console.warn("stickyHeaders is enabled, stickyColumns will not work properly.");else{this._isStickyColumns=!0;var b=$X("EWA_LF_"+this._Id);if(b==null)console.log("not found table[#EWA_LF_"+this._Id+"]");else if(b.rows.length!=0){for(let d=0;d<a;d++)this.stickyColumn(d,d==a-1,b);var c=b.parentNode;a="scroll-ref-hor-"+this._Id;
if(!$(c).attr(a)){b=c.tagName=="BODY"?window:c;const d=c.tagName=="BODY"?document.documentElement:c;$(c).attr(a,1);$(c).addClass("ewa-lf-sticky-scroll");$(b).on("scroll",function(){d.scrollLeft===0?$(c).addClass("ewa-lf-sticky-last-hide"):$(c).hasClass("ewa-lf-sticky-last-hide")&&$(c).removeClass("ewa-lf-sticky-last-hide")});$(b).trigger("scroll")}}}};this.stickyColumn=function(a,b,c){c=c||$X("EWA_LF_"+this._Id);if(c==null)console.log("not found table[#EWA_LF_"+this._Id+"]");else if(c.rows.length!=
0){var d=c.rows[0].cells[a];if(d){var e=$(d).position().left;d=$(d).width();e={left:e,width:d};for(d=0;d<c.rows.length;d++){let g=$(c.rows[d].cells[a]);g.css(e).addClass("ewa-lf-sticky");b&&g.addClass("ewa-lf-sticky-last").css("overflow","initial")}}}};this.composeSearchTexts=function(){var a=[],b=[],c=$("#EWA_RESHOW_"+this._Id+" .ewa-lf-search-type-text");if(c.length==0)return null;c.each(function(g){var h=$(this).find(".ewa-lf-search-item-title").text();h=h.replace(":","").replace("\uff1a","").replace("\u5305\u542b",
"").replace("Like","").trim();var k=$(this).find("input[type=text]").attr("name");a.push(h);b.push(k);g>0&&$(this).hide()});let d=$(c[0]);d.addClass("ewa-app-lf-search-compose");d.find("input[type=text]").attr("name",b.join(",")).attr("placeholder",a.join(", "));let e=d.find("a").clone();d.find(".ewa-lf-search-item-title b").text(EWA.LANG=="enus"?"Search":"\u7efc\u5408\u641c\u7d22").append(e);return c};this.switchButtonAction=function(a,b){if(b){var c=this.getUrlClass();c.AddParameter("ewa_switch_name",
a.name);c.AddParameter("ewa_action",b);c.AddParameter("ewa_ajax","json");c.AddParameter("ewa_switch",a.checked?"on":"off");c.AddParameter("ewa_action_key",$(a).parentsUntil("tbody").last().attr("ewa_key"));var d=a.parentNode,e=d.getAttributeNames();b={};for(let h in e){let k=e[h],l=d.getAttribute(k);k.indexOf("on")!=0&&k!="name"&&k!="id"&&k!="class"&&(b[k]=l)}var g=this;c=c.GetUrl();$JP(c,b,function(h){g.extSwitchCallBack&&g.extSwitchCallBack(a,h)})}};this.ShowRecycle=function(){var a=$X("EWA_LF_"+
this._Id);if(a!=null&&(a=$(a).parentsUntil("table").last().find(".ewa_lf_func_caption"),a.length!=0)){var b="\u56de\u6536\u7ad9";window._EWA_INFO_MSG&&window._EWA_INFO_MSG.RECYCLE&&(b=window._EWA_INFO_MSG.RECYCLE);b="<div class='ewa_lf_func_dact ewa-lf-func-recycle' onclick='EWA.F.FOS[\""+this._Id+"\"].ShowRecycle1(this)' ><b class='fa fa-recycle'></b>"+b+"</div>";a.parent().append(b)}};this.ShowRecycle1=function(a){var b=new EWA_UrlClass(this.Url);$(a).attr("recycle")?(b=b.RemoveParameter("ewa_recycle"),
$(a).attr("recycle",null).removeClass("ewa-lf-func-recycle-yes")):(b=b.AddParameter("ewa_recycle","1"),$(a).attr("recycle",1).addClass("ewa-lf-func-recycle-yes"));this.Url=b;this.Goto(1)};this.BindButton=function(a,b){$("#EWA_LF_"+this._Id+' tr[ewa_key] [id="'+a+'"]').each(function(){$(this).attr("onclick","$(this).parent().parent().find('[id=\""+b+"\"]')[0].click()");$(this).addClass("ewa-map-button")})};this.Merges=function(a){if(a==null||a.length==0)console.log("cfg.from, cfg.to, cfg.str, cfg.func, cfg.header");
else for(let b=0;b<a.length;b++){let c=a[b];this.Merge(c.from,c.to,c.str,c.func,c.header)}};this.Merge=function(a,b,c,d,e){let g=$("#EWA_LF_"+this._Id);c==null?c="":c.indexOf("<")==-1&&(c="<span class='ewa-merge-str ewa-mearge-str'>"+c+"</span>");g.find("tr[ewa_key],tr.ewa-lf-sub-tr").each(function(){let h=$(this).find('[id="'+b+'"]').parentsUntil("tr").last();if(h.attr("ewa-merged")!="yes"){h.attr("ewa-merged","yes");var k=$(this).find('[id="'+a+'"]');k.parentsUntil("tr").last().hide().addClass("ewa-row-merge-hide");
h.append(c);h.append(k);d&&d(h,this)}});e&&(e="<span class='ewa-lf-merge-header-split'></span>"+g.find('tr[ewa_tag="HEADER"] [id="'+a+'"]').html(),g.find('tr[ewa_tag="HEADER"] [id="'+b+'"]').append(e));g.find('tr[ewa_tag="HEADER"] [id="'+a+'"]').parent().hide();g.find("td#ADD_ROW_"+a).hide()};this.Mearge=function(a,b,c){console.log("\u62fc\u5199\u9519\u8bef\uff0c\u8bf7\u7528 Merge");this.Merge(a,b,c)};this.createMergeExpParameters=function(a){const b=a.match(/@(@)?[a-zA-Z_][a-zA-Z0-9_]*/g)||[],c=
{};try{const d={};$(`<div>${a}</div>`).find("*").addBack().each(function(){$.each(this.attributes,function(e,g){d[g.name]=g.value})});b.forEach(e=>{const g=e.startsWith("@@")?e.slice(2):e.slice(1);let h=!1,k="";for(const [l,m]of Object.entries(d))if(m.includes(e)){h=!0;k=l;break}c[e]={id:g,isAttr:h,attrName:k}})}catch(d){b.forEach(e=>{const g=e.startsWith("@@")?e.slice(2):e.slice(1);c[e]={id:g,isAttr:!1,attrName:""}})}return c};this.MergeExp=function(a,b,c,d,e){if(b){var g=$("#EWA_LF_"+this._Id),
h=this.createMergeExpParameters(b),k=b,l={};b=[];for(const n in h){var m=h[n];const p=m.id;m.isAttr||(k=k.replace(n,"<span class='ewa-lf-merge ewa-lf-merge-"+p+" ewa-lf-mearge ewa-lf-mearge-"+p+"' mid=\""+p+'"></span>'));p!=a&&(g.find('tr[ewa_tag="HEADER"] [id="'+p+'"]').parent().hide(),g.find("td#ADD_ROW_"+p).hide());m=g.find('tr[ewa_tag="HEADER"] [id="'+p+'"]').html();l[p]=m;b.push(m)}e&&(e=b.join("<span class='ewa-lf-merge-header-split'></span>"),g.find('tr[ewa_tag="HEADER"] [id="'+a+'"]').html(e));
g.find("tr[ewa_key],tr.ewa-lf-sub-tr").each(function(){if($(this).find('[id="'+a+'"]').length==0)console.log("not find "+a);else{var n=$(this).hasClass("ewa-lf-sub-tr")?$(this).find('[id="ADD_ROW_'+a+'"]'):$(this).find(".ewa-col-"+a);if("yes"!=n.attr("ewa-merged")){var p=$('<div style="display:none"></div>');p.html(k);for(const t in h){var r=h[t];const v=r.isAttr,u=r.id;var q=$(this).find('[id="'+u+'"]');if(q.length!=0)if(u!=a&&q.parent().hide().addClass("ewa-row-merge-hide"),v){const w=r.attrName;
p.find("["+w+"='"+t+"']").each(function(){$(this).attr(w)===t&&$(this).attr(w,q.text().trim())})}else r=p.find('span[mid="'+u+'"]'),c&&(r.append("<span class='ewa-lf-mearge-memo'></span>"),r.find(".ewa-lf-mearge-memo").html(l[u])),r.append(q)}for(;p[0].childNodes.length>0;)n.append(p[0].childNodes[0]);$(p).remove();d&&d(n,this);n.attr("ewa-merged","yes")}}})}else console.log("mergeExp \u6ca1\u6709\u8bbe\u7f6e")};this.MeargeExp=function(a,b,c,d){console.log("\u62fc\u5199\u9519\u8bef\uff0c\u8bf7\u7528 MereExp");
...
r=this._GetSubValue("Tag","Tag",p);if(r&&(r.toLowerCase()=="button"||r.toLowerCase()=="butflow")&&(r=this._GetSubValue("Name","Name",p),b.find(".ewa-col-"+r).length!==0)){m=!0;var q=this._GetSubValue("DescriptionSet","Info",p),t=this._GetSubValue("DescriptionSet","Memo",p),v=this._GetSubValue("EventSet","EventName",p),u="";v!=null&&v.toLowerCase().trim()=="ewa_click"&&(u=this._GetSubValue("EventSet","EventValue",p));e=e.replace(q,"");p=(p=this._GetSubValue("DataItem","Icon",p))?this._ReShowButtonIcon(p,
q,r,null):this._ReShowButton(q,t,null);this._ReshowButs[q]=p;p.setAttribute("f_id",this._Id);p.setAttribute("t_id",r);k.childNodes[1].appendChild(p);u==""?p.setAttribute("onclick","EWA.F.FOS['"+this._Id+"'].reShowButtonClick(this)"):p.setAttribute("onclick",u)}}m&&this._ReShowSplit(k.childNodes[1]);r=k.getElementsByTagName("input");m={};for(b=0;b<r.length;b++){var w=r[b];w.style.display="none";if(w.id==null||w.id=="")w.id=("_tmp_"+Math.random()).replace(".","_");if(w.type=="button"){if(p=this._ReShowButton(w.value,
w.title,w.id),q=w.nextSibling){var x="";q.nodeType==3&&(x=q.nodeValue.trimEx());x=="|"&&this._ReShowSplit(k.childNodes[1])}}else(q=w.nextSibling)&&(x=q.nodeType==3?q.nodeValue.replace("|",""):q.innerHTML),p=this._ReShowButton(x.trimEx(),w.title,w.id),p.id=("_tmp_"+Math.random()).replace(".","_"),m[w.name]||(m[w.name]=[]),m[w.name].push(p.id);k.childNodes[1].appendChild(p)}e=e.replace(/ /ig,"").replace(/\|/ig,"").trimEx();e!=""?e.indexOf("if")>=0?g.style.display="none":h.innerHTML=e.replace(/ /,""):
g.style.display="none";a.appendChild(c);for(n in m)for(x=m[n],b=0;b<x.length;b++)w=$X(x[b]),w.setAttribute("_r_ids",x.join(",")),w.onclick=function(){var B=this.getAttribute("_ewa_event_id");$X(B).click();B=this.getAttribute("_r_ids").split(",");for(var C=0;C<B.length;C++)w.id!=B[C]&&($X(B[C]).style.fontWeight="",$X(B[C]).style.color="");this.style.fontWeight="bold";this.style.color="blue"};d.innerHTML='<b><i class="fa fa-star-o">&nbsp;</i>'+this.Description+"</b>";if(this.Url.toUpperCase().indexOf("EWA_BTNS_IN_TITLE")>
0)var y=this,z=0,A=setInterval(function(){z++;z>1E3&&window.clearInterval(A);EWA.OW.Load();EWA.OW.Dia&&(window.clearInterval(A),y.ReShowButtonsInDailogTitle())},10);this._InitMneu()}};this.reShowButtonClick=function(a){a=a.getAttribute("t_id");var b=this.SelectCheckedRows();if(b==null||b.length==0)EWA.UI.Msg.Alert(_EWA_INFO_MSG["EWA.SYS.CHOOSE_ITEM"],_EWA_INFO_MSG["EWA.SYS.CHOOSE_ITEM_TITLE"]);else for(var c=b[0].getElementsByTagName("input"),d=0;d<c.length;d++)if(b=c[d],b.name===a){a=function(e){e.stopPropagation()};
b.addEventListener("click",a);b.click();b.removeEventListener("click",a);break}};this._ReShowSplit=function(a){EWA.UI.Utils.CreateObject(window,"div","",a).className="ewa_lf_func_split"};this._ReShowButtonIcon=function(a,b,c,d){var e=EWA.UI.Utils.CreateObject(window,"div","cursor:pointer",document.body);e.innerHTML='<nobr class="ewa-button-with-icon"><label><i class="'+a+'">&nbsp;</i></label></nobr>';$(e).find("nobr").attr("title",b).attr("rid",c);d!=null&&(e.setAttribute("_ewa_event_id",d),e.onclick=
function(){var g=this.getAttribute("_ewa_event_id");$X(g).click()});EWA.B.IE&&(e.onselectstart=function(){return!1});e.className="ewa_lf_func_dact";e.title=b;return e};this._ReShowButton=function(a,b,c){var d=EWA.UI.Utils.CreateObject(window,"div","cursor:pointer",document.body);d.innerHTML="<nobr>"+a+"</nobr>";c!=null&&(d.setAttribute("_ewa_event_id",c),d.onclick=function(){var e=this.getAttribute("_ewa_event_id");$X(e).click()});EWA.B.IE&&(d.onselectstart=function(){return!1});d.className="ewa_lf_func_dact";
...
h;for(h in this._SearchJson)this._SearchJson[h].ORI_NAME=h,g[h.toUpperCase()]=this._SearchJson[h];this._SearchJson=g;var k="EWA.F.FOS[&quot;"+this._Id+"&quot;].ChangeSearchTextType(this)",l="EWA.F.FOS[&quot;"+this._Id+"&quot;].ChangeSearchDateType(this)",m=$J2MAP(this.RESOURCES.search_text_items,"Id");g=this._GetInitSearchMap();var n=!1,p=!1;for(h in this.ItemList.Items)if(this._SearchJson[h]!=null){var r=[],q=this._SearchJson[h],t=q.T,v=this._GetSubValue("DescriptionSet","Info",this.ItemList.Items[h]);
r.push("<table class='ewa-lf-search-item ewa-ref-"+h+" ewa-lf-search-type-"+t+" "+(t=="fix"?"ewa-lf-search-fix-m"+(q.M||"0"):"")+"' cellspacing=0 cellpadding=0>");r.push("<tr><td class='ewa-lf-search-item-title' style='position:relative'><nobr><b>");r.push(v);var u=v="",w="lk";g[h.toUpperCase()]&&(u=g[h.toUpperCase()],w=u.tag,v=u.para1,u=u.para2);var x="";t=="text"?(p=m[w]||m.lk,x=EWA.LANG=="enus"?p.TxtEn:p.Txt,r.push("<a class='ewa-lf-search-type' tag='"+w+"' href='javascript:void(0)' onclick='"+
k+"'>"),r.push(x),r.push("</a>"),p=!0):t=="date"&&(r.push("<a  href='javascript:void(0)' class='fa fa-calendar-check-o'  onclick='"+l+"'></a>"),n=!0);r.push("</b></nobr></td><td class='ewa-lf-search-item-ctl'>");t=="text"?w=="blk"||w=="nblk"?(v=this._SearchSingle(h,x),v=v.replace("<input","<input disabled "),r.push(v)):r.push(this._SearchSingle(h,v)):t=="date"?r.push(this._SearchDate(h,v,u)):t=="number"?r.push(this._SearchNumber(h,v,u)):t=="fix"&&(v="<div >"+this._SearchFix(h,q,"span",v)+"</div>",
r.push(v));r.push("</td></tr></table>");"3"==q.M?e.push(r.join("")):t=="fix"&&this._IsSearchGroup?c.push(r.join("")):d.push(r.join(""))}if(d.length+c.length+e.length!=0){b.push('<table class="ewa-lf-search-main" id="'+a+'" cellspacing="0" cellpadding="0">');b.push("<tr><td class='ewa-lf-search-des'></td><td>");d.length>0&&b.push(" "+d.join("")+" ");for(d=0;d<c.length;d++)b.push(" "+c[d]+" ");for(d=0;d<e.length;d++)b.push(" "+e[d]+" ");b.push("</td></tr></table>");var y=document.createElement("div");
this._SEARCH_BOX=y;y.className="ewa-lf-search";y.innerHTML=b.join("");b=$X("EWA_LF_"+this.Id).parentNode;b.parentNode.insertBefore(y,b);a="EWA_SEARCH_ITEM_"+this.Id;var z=this;n&&this._SearchCreateDateRange();p&&this._SearchCreateTextType();z._SEARCH_ITEM_EXP=this.SearchGetExp(y);JSON.stringify(g)!="{}"&&(z._SearchExp=z._SEARCH_ITEM_EXP);$(y).find("input[type=text]").on("compositionstart",function(){z._is_search_composition=!0}).on("compositionend",function(){z._is_search_composition=!1});this._TIMER_SEARCH=
window.setInterval(function(){if(!z._is_search_composition){try{if(!$X(a)){window.clearInterval(z._TIMER_SEARCH);return}}catch(B){window.clearInterval(z._TIMER_SEARCH);return}var A=z.SearchGetExp(y);A!=z._SEARCH_ITEM_EXP&&(z._SEARCH_ITEM_EXP=A,z.DoSearch($X(a)))}},700)}};this._SearchSingle=function(a,b){var c=[];c.push('<input t="text" type="text" autocomplete="off" class="EWA_INPUT" maxlength="40" name="');c.push(a.toInputValue());c.push('"');b!=null&&(c.push(' value="'),a=b.toInputValue(),c.push(a),
...
d.push(a);d.push('1"');c&&(b=c.toInputValue(),d.push(' value="'),d.push(b),d.push('"'));d.push("/></td></tr></table>");return d.join("")};this._SearchNumber=function(a,b,c){var d=[];d.push('<input type="text" t="number" size="10" name="');d.push(a);d.push('"');b&&(b*=1,d.push(' value="'),d.push(b),d.push('"'));d.push(' /><span class="ewa-lf-search-split"> - </span>');d.push('<input type="text" t="number" size="10" name="');d.push(a);d.push('1"');c&&(b=c*1,d.push(' value="'),d.push(b),d.push('"'));
d.push("/>");return d.join("")};this._SearchFix=function(a,b,c,d){var e=[],g="radio";if(b.M=="1")g="checkbox";else{if(b.M=="2")return this._SearchFixSelect(a,b,c,d);if(b.M=="3")return this._SearchFixButton(a,b,c,d)}c="<"+c+"><nobr><input t='fix' [@CHK] id='@ID' value=\"@V\" type='"+g+"' name='"+a+"' /><label for='@ID'>@T</label></nobr></"+c+"> ";g={};if(d){d=d.split(";");for(var h in d)g[d[h].trim().toUpperCase()]=1}for(h=0;h<b.D.length;h++){var k=b.D[h];d=k[0];var l=k[1];k[2]&&(l+="("+k[2]+")");
k=this.Id+"_"+a+"_SearchFix_"+h;var m=c.replace("@ID",k);m=m.replace("@ID",k);m=m.replace("@V",d);m=g[d.toUpperCase().trim()]?m.replace("[@CHK]","checked"):m.replace("[@CHK]","");m=m.replace("@T",l);e.push(m)}return e.join("")};this.searchFixButtonClick=function(a){$(a).parent().find("button").each(function(){this==a?($(this).addClass("ewa-lf-search-fix-button-checked"),$(this).find("input").prop("checked",!0)):($(this).removeClass("ewa-lf-search-fix-button-checked"),$(this).find("input").prop("checked",
!1))})};this.reShowStatistics=function(a){this._is_search_composition=!0;for(var b in a){const e=a[b];if(e.T=="fix"){var c=$(".ewa-lf-search-item.ewa-ref-"+b);c.length==0&&(console.log("NOT fund .ewa-lf-search-item.ewa-ref-"+b),contine);c="2"===e.M?c.find("option[id]"):"3"===e.M?c.find("button"):c.find("input");for(let g=0;g<e.D.length;g++){var d=e.D[g];if(d[2]==void 0)continue;const h=c[g];h?(d=d[1]+(d[2]?"("+d[2]+")":""),h.tagName=="INPUT"?h.value=d:h.tagName=="OPTION"?h.text=d:$(h).find("span").text(d)):
console.log("not fund input/option/button index="+g)}}}this._SEARCH_ITEM_EXP=this.SearchGetExp(this._SEARCH_BOX);this._is_search_composition=!1};this._SearchFixButton=function(a,b,c,d){c="<button  class=\"@CLS\" type='button' id='@ID' onclick=\"EWA.F.FOS['"+(this._Id+"'].searchFixButtonClick(this)\"><input t='fix' style='display:none' type='radio' name=\"")+a+'" value="@V" @CKD><span>@T</span></button>';for(var e=[],g=0,h=0;h<b.D.length;h++){var k=b.D[h],l=k[1];k[2]&&(l+="("+k[2]+")",g+=k[2]);var m=
"ewa-lf-search-fix-button",n=c.replace("@ID",this.Id+"_"+a+"_SearchFix_"+h);n=n.replace("@V",k[0]);n=n.replace("@T",l);d?k[0]==d?(n=n.replace("@CKD","checked"),m+=" ewa-lf-search-fix-button-checked"):n=n.replace("@CKD",""):0===h?(n=n.replace("@CKD","checked"),m+=" ewa-lf-search-fix-button-checked"):n=n.replace("@CKD","");n=n.replace("@CLS",m);e.push(n)}e[0]=e[0].replace("(-1)","("+g+")");return e.join("")};this._SearchFixSelect=function(a,b,c,d){c=[];c.push("<select t='fix' name='"+a+"'><option></option>");
for(var e=0;e<b.D.length;e++){var g=b.D[e],h=this.Id+"_"+a+"_SearchFix_"+e,k=g[1];g[2]&&(k+="("+g[2]+")");h="<option @CKD t='fix' id='@ID' value=\"@V\">@T</option> ".replace("@ID",h);h=h.replace("@V",g[0]);h=h.replace("@T",k);h=d&&g[0]==d?h.replace("@CKD","selected"):h.replace("@CKD","");c.push(h)}c.push("</select>");return c.join("")};this._SearchCreate=function(){var a=[];a.push("<div><table border=0 style='margin:0px;' class=EWA_TABLE cellspacing=1>");for(var b in this.ItemList.Items)if(this._SearchJson[b]!=
null){var c=this._SearchJson[b],d=c.T,e=this._GetSubValue("DescriptionSet","Info",this.ItemList.Items[b]);a.push("<tr><td class=EWA_TD_L width=140><nobr>"+e+"<nobr></td><td width=350 class=EWA_TD_M>");d=="text"?a.push(this._SearchSingle(b)):d=="date"?a.push(this._SearchDate(b)):d=="number"?a.push(this._SearchNumber(b)):d=="fix"&&a.push(this._SearchFix(b,c,"div"));a.push("</td></tr>")}a.push("<tr><td class=EWA_TD_B colspan=2 align=right><input onclick='EWA.F.FOS[\""+this._Id+"\"].SearchClear(this);' type=button value='"+
_EWA_INFO_MSG["EWA.LF.SEARCH.BUT.CLEAR"]+"'> <input type=submit value='"+_EWA_INFO_MSG["EWA.LF.SEARCH.BUT.SEARCH"]+"' onclick='EWA.F.FOS[\""+this._Id+"\"].DoSearch(this);'> <input type=button value='"+_EWA_INFO_MSG["EWA.LF.SEARCH.BUT.CLOSE"]+"' onclick='EWA.F.FOS[\""+this.Id+"\"]._SearchDialog.Show(false);'></td></tr>");a.push("</table></div>");this._SearchHtml=a.join("")};this.createRegExp=function(a){if(!a)return null;a=String(a).trim();if(a.length===0)return null;for(var b=[],c=0;c<a.length;c++){var d=
...
h&&(g=this.text)})}let l=this.createRegExp(g),m=d[0].split(",");for(let n in m){var k=m[n];this._SearchJson[k]&&(k=this._SearchJson[k].ORI_NAME);k=$(b).find(".ewa-lf-data-row .ewa-col-"+k).not("[ewa-search-marked=yes]");k.attr("ewa-search-marked","yes");try{k.find("*").each(function(){this.children.length==0&&(this.innerHTML=this.innerHTML.replace(l,"<font color=red><b>$1</b></font>"))})}catch(p){console.log(p)}}}}};this.DblClick=function(a){this._IsDblClick=a;for(var b=$X("EWA_LF_"+this._Id),c=1;c<
b.rows.length;c++)b.rows[c].ondblclick=function(){var d=this.getElementsByTagName("input");d.length>a&&d[a].disabled==0&&d[a].click()}};this.Init=function(a){this.Xml=new EWA.C.Xml;this.Xml.LoadXml(a);this.ItemList.Init(this.Xml);this.ItemList.F=this;this.Resources=new EWA_FrameResoures;this.Resources.Init(this.Xml);if((a=$("#EWA_LF_"+this._Id)[0])&&a.tagName=="table"&&!(a.rows.length<=1)){a=a.rows[1].getElementsByTagName("input");for(var b=0;b<a.length;b++)if(a[b].type.toUpperCase()=="RADIO"){this._TrSelectMulti=
!1;break}}};this.SetPageParameters=function(a,b,c,d,e){this._PageCurrent=a;this._PageSize=c;this._PageCount=b;this._RecordCount=d;this._Sort=e};this.SetPageParametersName=function(a,b,c,d,e){this._PageCurrentName=a;this._PageSizeName=c;this._PageCountName=b;this._RecordCountName=d;this._SortName=e};this.CheckedAll=function(){var a=document.getElementById("EWA_LF_"+this._Id);if(!(a.rows.length<=1)){for(var b=0;b<a.rows.length;b+=1){var c=a.rows[b].getElementsByTagName("input");c.length!=0&&(c[0].checked=
this._IsCheckedAll)}this._IsCheckedAll=this._IsCheckedAll?!1:!0;this.CheckedAllAfter&&this.CheckedAllAfter(a)}};this.SelectChecked=function(){var a=$X("EWA_LF_"+this._Id);if(!a||a.rows.length<1)return"";var b=[];let c=[];$("#EWA_LF_"+this._Id+">tbody>tr").find("input[type='radio'],input[type='checkbox']").each(function(){this.parentNode.className.indexOf("ewa-switch")==-1&&c.push(this)});if(c.length>0)for(var d=0;d<c.length;d++)c[d].checked&&b.push(c[d].value);else for(d=0;d<a.rows.length;d++){var e=
a.rows[d];e.getAttribute("__ewa_lf_mdown")==1&&b.push(e.getAttribute("EWA_KEY"))}return b.join(",")};this.SelectCheckedInputs=function(){var a=[],b=$X("EWA_LF_"+this._Id);if(b.rows.length<1)return a;b=b.getElementsByTagName("input");a=[];for(var c=0;c<b.length;c++)(b[c].type.toUpperCase()=="CHECKBOX"||b[c].type.toUpperCase()=="RADIO")&&b[c].checked&&a.push(b[c]);return a};this.SelectCheckedRows=function(){for(var a=[],b=$X("EWA_LF_"+this._Id),c=0;c<b.rows.length;c++){var d=b.rows[c],e=d.getElementsByTagName("input");
e.length!=0&&e[0].checked&&e[0].parentNode.className.indexOf("ewa-switch")==-1&&a.push(d)}if(a.length==0)for(c=0;c<b.rows.length;c++)d=b.rows[c],d.getAttribute("__ewa_lf_mdown")==1&&a.push(d);return a};this.Reload=function(a){this.StopAjaxAfterReload?this.StopAjaxAfterReload=!1:this.Goto(this._PageCurrent,a)};this.refreshPage=function(a,b,c){this.replaceRowsData(null,null,a,b,c)};this.replaceRowsData=function(a,b,c,d,e){var g=this.getUrlClass();g.AddParameter("EWA_AJAX","LF_RELOAD");g.AddParameter("EWA_IS_SPLIT_PAGE",
"no");g.AddParameter("EWA_IS_HIDDEN_CAPTION","yes");g=g.GetUrl();a&&(g+="&"+a);let h=a?new EWA_AjaxClass:this.CreateAjax(),k=this;h.PostNew(g,function(){if(h._Http.readyState==4){h.HiddenWaitting();var l=h._Http.responseText;h._Http.status!=200?(console.error(l),alert("ERROR:\r\n"+h._Http.statusText)):EWA.F.CheckCallBack(l)?k.replaceRowsWithDataHtml(l,b,c,d,e):console.error(l)}})};this.replaceRowsWithDataHtml=function(a,b,c,d,e){let g=$("<div></div>");g.html(a);let h=[],k=$("#EWA_LF_"+this._Id);g.find(".ewa-lf-data-row").each(function(){var l=
'tr[ewa_key="'+$(this).attr("ewa_key")+'"]';let m=k.find(l);if(m.length==0)console.warn(l+" not found");else{l=$(this).attr("ewa_row_sign");var n=m.attr("ewa_row_sign"),p=!0;(l||n)&&n===l&&(p=!1);if(p){m.attr("ewa_row_sign",l);l=m.clone()[0];for(n=0;n<this.cells.length;n++){p=m[0].cells[n];let r=this.cells[n];p.removeAttribute("ewa-merged");p.removeAttribute("ewa-search-marked");b?b(r,p):p.innerHTML=r.innerHTML}h.push({before:l,current:m[0]})}}});g.remove();h.length!=0&&(this.SearchMark(),this.IsReShow&&
this.ReShowWithNoButtons(),this._ReShowSearchQuick(),this._IsDblClick!=null&&this.DblClick(this._IsDblClick),this.SubBottomsArray&&this._SubBottoms(),!e&&this.ReloadAfter&&this.ReloadAfter(c),!e&&this.ReloadAfterApp&&this.ReloadAfterApp(c));d&&d(h)};this.DoAction=function(a,b,c,d,e,g){EWA.F.CID=this._Id;if(b){if(a&&$(a).parent().hasClass("scroll-delete")){var h=$(a).attr("do-action-time")||0,k=(new Date).getTime();if(k-h<1E3)return;$(a).attr("do-action-time",k)}this.lastAction=b;this._Ajax=new EWA_AjaxClass;
this._Ajax.LoadingType="image";this._Ajax.AddParameter("EWA_AJAX","1");this._Ajax.AddParameter("EWA_ACTION",b);this._Ajax.AddParameter("EWA_ID",this._Id);this._Ajax.AddParameter("EWA_NO_CONTENT","1");this._Ajax.AddParameter("EWA_ACTION_TIP",d);g!=null&&g.trim().length>0?(this._Ajax.AddParameter("EWA_AFTER_EVENT",g),this.StopAjaxAfterReload=!0,this._Ajax.AddParameter("EWA_ACTION_RELOAD","0")):this.StopAjaxAfterReload=!1;if(a&&a.getAttribute&&a.getAttribute("box_menu_item")=="yes"){if(this._Ajax.AddParameter("___CALL__FROM__",
