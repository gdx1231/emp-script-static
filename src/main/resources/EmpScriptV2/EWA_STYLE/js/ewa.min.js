...
function(){var a=$X("EWA_LF_"+this._Id);if(!a||1>a.rows.length)return"";var b=[],d=[];$("#EWA_LF_"+this._Id+">tbody>tr.ewa-lf-data-row").find("input[type='radio'],input[type='checkbox']").each(function(){-1==this.parentNode.className.indexOf("ewa-switch")&&d.push(this)});if(0<d.length)for(var e=0;e<d.length;e++)d[e].checked&&b.push(d[e].value);else for(e=0;e<a.rows.length;e++){var g=a.rows[e];1==g.getAttribute("__ewa_lf_mdown")&&b.push(g.getAttribute("EWA_KEY"))}return b.join(",")};this.SelectCheckedInputs=
