...
this._Id+"'>");b.push("<tr style='display:none'><td style='padding:0'></td></tr>");b.push("<tr><td class='ewa_lf_func' style='padding:0'>");b.push("<div style='display:none'></div>");b.push("<div><div class='ewa_lf_func_caption'></div></div>");b.push("</td></tr>");b.push("<tr><td style='width:100%;padding:0;vertical-align:top'>");b.push("<div id='"+a+"' style=''></div>");b.push("</td></tr>");b.push("</table>");var c=$(b.join("")),d=!(this.Url.toUpperCase().indexOf("EWA_AJAX=INSTALL")>0||this.Url.toUpperCase().indexOf("EWA_CALL_METHOD=INNER_CALL")>
0||this.Url.toUpperCase().indexOf(".JSP")>0);d?EWA.UI.Utils.SetStyle(c[0],"width:100%;height:100%;border-spacing:0"):EWA.UI.Utils.SetStyle(c[0],"width:100%;border-spacing:0");css="";var e=$X("EWA_LF_"+this._Id);a=e.parentNode.parentNode;b=e.parentNode;c.find("td:eq(2)>div").append(b);b=c[0];let g=c.find("tr:eq(0)")[0],h=c.find("td:eq(0)")[0],k=c.find("td:eq(1)")[0],l=c.find("td:eq(2)")[0];for(d&&window.setTimeout(function(){$X("__EWA_DEBUG")&&l.childNodes[0].appendChild($X("__EWA_DEBUG"))},100);a.childNodes.length>
0;)k.childNodes[0].appendChild(a.childNodes[0]);c=c.find(".ewa_lf_func_caption")[0];var m=!1;d=GetInnerText(k);d=d.trim();this._ReshowButs={};e=$(e.rows[0]);for(var n in this.ItemList.Items){var p=this.ItemList.Items[n],r=this._GetSubValue("Tag","Tag",p);if(r&&(r.toLowerCase()=="button"||r.toLowerCase()=="butflow")&&(r=this._GetSubValue("Name","Name",p),e.find(".ewa-col-"+r).length!==0)){m=!0;var q=this._GetSubValue("DescriptionSet","Info",p),t=this._GetSubValue("DescriptionSet","Memo",p),v=this._GetSubValue("EventSet",
"EventName",p),u="";v!=null&&v.toLowerCase().trim()=="ewa_click"&&(u=this._GetSubValue("EventSet","EventValue",p));d=d.replace(q,"");p=this._ReShowButton(q,t,null);this._ReshowButs[q]=p;p.setAttribute("f_id",this._Id);p.setAttribute("t_id",r);k.childNodes[1].appendChild(p);u==""?p.setAttribute("onclick","EWA.F.FOS['"+this._Id+"'].reShowButtonClick(this)"):p.setAttribute("onclick",u)}}m&&this._ReShowSplit(k.childNodes[1]);r=k.getElementsByTagName("input");m={};for(e=0;e<r.length;e++){var w=r[e];w.style.display=
"none";if(w.id==null||w.id=="")w.id=("_tmp_"+Math.random()).replace(".","_");if(w.type=="button"){if(p=this._ReShowButton(w.value,w.title,w.id),q=w.nextSibling){var x="";q.nodeType==3&&(x=q.nodeValue.trimEx());x=="|"&&this._ReShowSplit(k.childNodes[1])}}else(q=w.nextSibling)&&(x=q.nodeType==3?q.nodeValue.replace("|",""):q.innerHTML),p=this._ReShowButton(x.trimEx(),w.title,w.id),p.id=("_tmp_"+Math.random()).replace(".","_"),m[w.name]||(m[w.name]=[]),m[w.name].push(p.id);k.childNodes[1].appendChild(p)}d=
d.replace(/ /ig,"").replace(/\|/ig,"").trimEx();d!=""?d.indexOf("if")>=0?g.style.display="none":h.innerHTML=d.replace(/ /,""):g.style.display="none";a.appendChild(b);for(n in m)for(x=m[n],e=0;e<x.length;e++)w=$X(x[e]),w.setAttribute("_r_ids",x.join(",")),w.onclick=function(){var B=this.getAttribute("_ewa_event_id");$X(B).click();B=this.getAttribute("_r_ids").split(",");for(var C=0;C<B.length;C++)w.id!=B[C]&&($X(B[C]).style.fontWeight="",$X(B[C]).style.color="");this.style.fontWeight="bold";this.style.color=
"blue"};c.innerHTML='<b><i class="fa fa-star-o">&nbsp;</i>'+this.Description+"</b>";if(this.Url.toUpperCase().indexOf("EWA_BTNS_IN_TITLE")>0)var y=this,z=0,A=setInterval(function(){z++;z>1E3&&window.clearInterval(A);EWA.OW.Load();EWA.OW.Dia&&(window.clearInterval(A),y.ReShowButtonsInDailogTitle())},10);this._InitMneu()};this.reShowButtonClick=function(a){a=a.getAttribute("t_id");var b=this.SelectCheckedRows();if(b==null||b.length==0)EWA.UI.Msg.Alert(_EWA_INFO_MSG["EWA.SYS.CHOOSE_ITEM"],_EWA_INFO_MSG["EWA.SYS.CHOOSE_ITEM_TITLE"]);
