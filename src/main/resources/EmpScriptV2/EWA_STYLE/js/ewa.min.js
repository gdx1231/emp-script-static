...
!1;this.CheckedAllAfter=this.ReloadAfter=null;this.REPLACE_HISTORY_STATE=this._isStickyHeaders=this.IsNotMDownAutoChecked=!1;this._IsSearchGroup=!0;this.mergeButtonsInRow=function(a){var b=[],c=this.getObj("tr:eq(0)"),d;for(d in this.ItemList.Items){var e=this.ItemList.Items[d],g=this._GetSubValue("Tag","Tag",e);if(g&&(g.toLowerCase()=="button"||g.toLowerCase()=="butflow")&&(g=".ewa-col-"+this._GetSubValue("Name","Name",e),c.find(g).length!==0)){var h=this._GetSubValue("DescriptionSet","Info",e);
g=this._GetSubValue("DescriptionSet","Memo",e);var k=this._GetSubValue("EventSet","EventName",e);k!=null&&k.toLowerCase().trim()=="ewa_click"&&(e=this._GetSubValue("EventSet","EventValue",e),e=$("<input type=button>").attr("onclick",e.replace("event","this")).addClass("IX_BUTTON").attr("value",h).attr("rid",d),g&&e.attr("title",g),b.push(e))}}if(this.getObj(".ewa-col-funcs").length==0){c=this.getObj()[0];for(d=0;d<c.rows.length;d++)e=$(c.rows[d].insertCell(-1)),d==0?(e.addClass("EWA_TD_H ewa-col-funcs").text(a),
lastCellIndex=e[0].cellIndex):e.addClass("EWA_TD_M ewa-col-funcs");this.getObj(".ewa-lf-data-row").each(function(){let l=$(this).find(".ewa-col-funcs");$(this).find("input[type=button]:gt(0)").each(function(){l.append(this)})});this.getObj().parent().css("overflow-x","auto").css("width","100%")}a=$("#_G_"+this._Id+" .ewa-lf-search");if(!a.attr("set_new_buttons"))for(a.attr("set_new_buttons","yes"),c=0;c<b.length;c++)a.append(b[c])};this.changeTag=function(a){a=new EWA_UrlClass("fake?"+a);let b=new EWA_UrlClass(this.Url);
for(let c in a._Paras)b.AddParameter(c,a._Paras[c]);this.Url=b.GetUrl();this.Reload()};this._InitMneu=function(){var a=window["_EWA_MENU_"+this._Id];if(a&&a.length!=0){var b=[];for(let c=0;c<a.length;c++){let d=a[c],e=(c==0?"ewa-lf-menu-marked":"")+(c==a.length-1?"ewa-lf-menu-last":""),g=d.Cmd.replace(/"/g,"&quot;");b.push(`<a class="ewa-lf-menu ${e}" onclick="${"$(this).parent().find('.ewa-lf-menu-marked').removeClass('ewa-lf-menu-marked');$(this).addClass('ewa-lf-menu-marked');"} ${g}">${d.Txt}</a>`)}a=
