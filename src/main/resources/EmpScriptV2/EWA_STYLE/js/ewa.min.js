...
this.Mearge=function(a,b,c){console.log("\u62fc\u5199\u9519\u8bef\uff0c\u8bf7\u7528 Merge");this.Merge(a,b,c)};this.MergeExp=function(a,b,c,d,e){if(b){var g=$("#EWA_LF_"+this._Id),h=b.match(/@@[a-zA-Z0-9\-\._:]*\b/ig),k=[],l=b,m={};b=[];for(var n=0;n<h.length;n++){var p=h[n];k.push(p);var r=p.replace("@@","");l=l.replace(p,"<span class='ewa-lf-merge ewa-lf-merge-"+r+" ewa-lf-mearge ewa-lf-mearge-"+r+"' mid=\""+r+'"></span>');r!=a&&(g.find('tr[ewa_tag="HEADER"] [id="'+r+'"]').parent().hide(),g.find("td#ADD_ROW_"+
r).hide());p=g.find('tr[ewa_tag="HEADER"] [id="'+r+'"]').html();m[r]=p;b.push(p)}e&&(e=b.join("<span class='ewa-lf-merge-header-split'></span>"),g.find('tr[ewa_tag="HEADER"] [id="'+a+'"]').html(e));g.find("tr[ewa_key],tr.ewa-lf-sub-tr").each(function(){if($(this).find('[id="'+a+'"]').length==0)console.log("not find "+a);else{var q=$(this).hasClass("ewa-lf-sub-tr")?$(this).find('[id="ADD_ROW_'+a+'"]'):$(this).find(".ewa-col-"+a);if("yes"!=q.attr("ewa-merged")){var t=$('<div style="display:none"></div>');
t.html(l);for(var v in k){var u=k[v].replace("@@",""),w=$(this).find('[id="'+u+'"]');if(w.length!=0){u!=a&&w.parent().hide().addClass("ewa-row-merge-hide");var x=t.find('span[mid="'+u+'"]');c&&(x.append("<span class='ewa-lf-mearge-memo'></span>"),x.find(".ewa-lf-mearge-memo").html(m[u]));x.append(w)}}for(;t[0].childNodes.length>0;)q.append(t[0].childNodes[0]);$(t).remove();d&&d(q,this);q.attr("ewa-merged","yes")}}})}else console.log("mergeExp \u6ca1\u6709\u8bbe\u7f6e")};this.MeargeExp=function(a,
...
p.push(t);var v=t="",u="lk";e[g.toUpperCase()]&&(v=e[g.toUpperCase()],u=v.tag,t=v.para1,v=v.para2);var w="";q=="text"?(n=l[u]||l.lk,w=EWA.LANG=="enus"?n.TxtEn:n.Txt,p.push("<a class='ewa-lf-search-type' tag='"+u+"' href='javascript:void(0)' onclick='"+h+"'>"),p.push(w),p.push("</a>"),n=!0):q=="date"&&(p.push("<a  href='javascript:void(0)' class='fa fa-calendar-check-o'  onclick='"+k+"'></a>"),m=!0);p.push("</b></nobr></td><td class='ewa-lf-search-item-ctl'>");q=="text"?u=="blk"||u=="nblk"?(r=this._SearchSingle(g,
...
d.length>0&&b.push(" "+d.join("")+" ");for(d=0;d<c.length;d++)b.push(" "+c[d]+" ");b.push("</td></tr></table>");var x=document.createElement("div");this._SEARCH_BOX=x;x.className="ewa-lf-search";x.innerHTML=b.join("");b=$X("EWA_LF_"+this.Id).parentNode;b.parentNode.insertBefore(x,b);a="EWA_SEARCH_ITEM_"+this.Id;var y=this;m&&this._SearchCreateDateRange();n&&this._SearchCreateTextType();y._SEARCH_ITEM_EXP=this.SearchGetExp(x);JSON.stringify(e)!="{}"&&(y._SearchExp=y._SEARCH_ITEM_EXP);$(x).find("input[type=text]").on("compositionstart",
function(){y._is_search_composition=!0}).on("compositionend",function(){y._is_search_composition=!1});this._TIMER_SEARCH=window.setInterval(function(){if(!y._is_search_composition){try{if(!$X(a)){window.clearInterval(y._TIMER_SEARCH);return}}catch(A){window.clearInterval(y._TIMER_SEARCH);return}var z=y.SearchGetExp(x);z!=y._SEARCH_ITEM_EXP&&(y._SEARCH_ITEM_EXP=z,y.DoSearch($X(a)))}},700)}};this._SearchSingle=function(a,b){var c=[];c.push('<input t="text" type="text" autocomplete="off" class="EWA_INPUT" maxlength="40" name="');
...
this.ReShowWithNoButtons(),this._ReShowSearchQuick(),this._IsDblClick!=null&&this.DblClick(this._IsDblClick),this._IsAddPreRow&&this.AddPreRow(),this.SubBottomsArray&&this._SubBottoms(),this._isStickyHeaders&&this.stickyHeaders(),this.ReloadAfter&&this.ReloadAfter(a),this.ReloadAfterApp&&this.ReloadAfterApp(a),this._IsCheckedAll=!0))):alert("ERROR:\r\n"+b._Http.statusText);this.RewriteInfo()}};this._CallBackJs=function(){var a=this._Ajax;if(a._Http.readyState==4){if(top.EWA&&top.EWA.MQE){var b=new EWA_MqeClass;
b.Data=a.GetRst();b.FromId=EWA.F.CID;b.Type="POST";top.EWA.MQE.AddMqe(b)}a.HiddenWaitting();a._Http.status==200?(a=a._Http.responseText,EWA.F.CheckCallBack(a)&&eval(a)):alert("ERROR:\r\n"+a._Http.statusText);this.EditAfterEvent&&this.EditAfterEvent()}};this.EditAfterEvent=function(){this.getUrlClass().GetParameter("EWA_ROW_SIGN")&&this.refreshPage()};this._GetItem=function(a){for(var b=this.ItemList,c=0;c<b.length;c+=1){var d=b[c],e=this._GetSubItem("Name",d);if(this.Xml.GetElementAttribute(e,"Name")==
a)return d}};this._GetSubItem=function(a,b){b=this.Xml.GetElements(a+"/Set",b);if(b==null||b.length==0)return null;if(a=="DescriptionSet")for(a=0;a<b.length;a+=1){var c=b[a].getAttribute("Lang");if(c!=null&&c.trim().toUpperCase()==EWA.LANG.trim().toUpperCase())return b[a]}return b[0]};this._GetSubValue=function(a,b,c){a=this._GetSubItem(a,c);return a==null?null:this.Xml.GetElementAttribute(a,b)};this.AddColumns=function(a,b,c,d,e,g,h,k){var l=$X("EWA_LF_"+this._Id);k==null&&(k=l.rows[0].cells.length);
for(var m=0;m<a.length;m++)for(var n=a[m],p=k+m,r=0;r<l.rows.length;r++){var q=l.rows[r].insertCell(p);q.title=n[d];if(r==0)q.className="EWA_TD_H ewa-add-column ewa-col-"+n[b],q.innerHTML='<nobr cellIdx="'+p+'" id="'+n[b]+'">'+n[c]+"</nobr>",q.title=n[d],q.width=100;else{var t=l.rows[r].getAttribute("EWA_KEY");q.className="EWA_TD_M ewa-add-column ewa-col-"+n[b];t=t+"_"+n[b];var v=this._GetAddControl(n[g],n[c],n[d]);v==null&&(v=e);var u=h.replace("[ID]",n[b]);v=v.indexOf("<select")>=0?v.replace("!!",
u.replace("onblur=","onchange=")):v.replace("!!",u);q.innerHTML=v;q.childNodes[0].id=t}}};this._GetAddControl=function(a,b,c){if(a==null)return null;a=a.toUpperCase().trim();b=b?b.replace(/</ig,"&lt;").replace(/>/ig,"&gt;").replace(/"/ig,"&quot;"):"";c=c?c.replace(/</ig,"&lt;").replace(/>/ig,"&gt;").replace(/"/ig,"&quot;"):"";return a=="SELECT"?'<select des="'+b+'"  title="'+c+'" !!></select>':a=="DATE"?'<input type=text des="'+b+'" title="'+c+'" class=EWA_INPUT onclick="EWA.UI.Calendar.Pop(this,false);" readonly !!>':
a=="TIME"?'<input type=text des="'+b+'" title="'+c+'"  class=EWA_INPUT onclick="EWA.UI.Calendar.Pop(this,true);" readonly !!>':a=="STRING"||a=="NUMBER"?'<input type=text des="'+b+'"  title="'+c+'" class=EWA_INPUT !!>':null};this.AddedValues=function(a,b,c,d,e){for(var g=0;g<a.length;g++){var h=a[g],k=$X(h[b]+"_"+h[c]);if(!k)break;var l=k.tagName.toUpperCase();l=="INPUT"||l=="SELECT"||l=="TEXTAREA"?k.value=h[d]:k.innerHTML=h[d];e&&(this.GetRow(k).getElementsByTagName("input")[0].checked=!0)}};this.AddRow=
function(a){for(var b=$X("EWA_LF_"+this._Id),c=b.insertRow(-1),d=0;d<b.rows[0].cells.length;d++){var e=c.insertCell(-1),g=b.rows[0].cells[d];e.className="EWA_TD_M";a[d]!=null&&(e.innerHTML=a[d]);e.style.display=g.style.display;e.id="ADD_ROW_"+g.childNodes[0].id}return c};this.Calc=function(a,b,c,d){for(var e=$X("EWA_LF_"+this._Id),g={},h=0;h<a.length;h++)g[a[h]]=0;for(h=b;h<c;h++)for(a=e.rows[h],b=0;b<a.cells.length;b++)if(g[b]!=null){var k=a.cells[b],l=k.getElementsByTagName("input");l.length>0?
g[b]+=l[0].value*1:(k=GetInnerText(k),k==""&&(k=0),g[b]+=k*1)}c=e.rows[d];for(h=0;h<c.cells.length;h++)g[h]!=null&&(k=Math.round(g[h]*100)/100,d=GetInnerText(c.cells[h]),k!=d*1&&(this.IsCalcChanged=!0),c.cells[h].innerHTML=k.fm())};this.mergeHeaders=function(a,b,c){var d=$($X("EWA_LF_"+this._Id)),e=d.find('tr[ewa_tag="HEADER"]');if(e.length!=0&&(a=e.find('nobr[id="'+a+'"]').parent(),a.length!=0)){var g=d.find("tr[ewa_mearge]");g.length==0&&(g=$(d[0].insertRow(1)),g.attr("ewa_mearge","gdx"));d=-1;
for(var h=0;h<e[0].cells.length;h++){var k=e[0].cells[h];k==a[0]&&(d=h,k.colSpan=c);if(d==-1)$(k).attr("ewa_tdmearged")!="1"&&(k.rowSpan=2);else if(d+c==h)d=-1,k.rowSpan=2;else{k.rowSpan=1;var l=g[0].insertCell(-1);l.innerHTML=k.innerHTML;l.className=k.className;l.style.cssText=k.style.cssText;k.style.display="none";$(k).attr("ewa_tdmearged",1)}k==a[0]&&(k.innerHTML=b,k.style.display="",k.style.width="")}}};this.MeargeHeader=function(a,b,c){console.log("\u62fc\u5199\u9519\u8bef\uff0c\u8bf7\u7528 mergeHeaders");
