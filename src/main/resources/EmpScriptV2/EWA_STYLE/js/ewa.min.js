...
a||(a=_EWA_EVENT_MSG.ValidCodeError,$Tip(a),$(".ewa-valid-code-img").parent().parent().find("input").focus().select()))};this.checkIdempotenceError=function(){$Tip(_EWA_EVENT_MSG.IdempotanceError)};this.Init=function(a){this.Xml=new EWA.C.Xml;this.Xml.LoadXml(a);this.ItemList.F=this;this.ItemList.Init(this.Xml);this.Resources=new EWA_FrameResoures;this.Resources.Init(this.Xml);"enus"==EWA.LANG&&$(".ewa-ipt-droplist").attr("placeholder","Input character selection");this._InitTranslations();this._GetDropListValue();
...
b.value));e.PostNew(n,function(){if(e.IsRunEnd()){var q=e.GetRst(),t="__json"+Math.random();try{eval('window["'+t+'"]='+q)}catch(v){b.setAttribute("canopen",0);a.value=v;b.setAttribute("canopen",1);return}q=window[t];if(0!=q.length){t=q[0];var u=[];q={};for(var r in t)u.push(t[r]),q["@"+r.toUpperCase()]=t[r],t[r]&&b.setAttribute("para_"+r.toLowerCase(),t[r]);if(null==d||""==d)q=u.join("/");else{r=d.match(/@[a-zA-Z0-9\-\._:]*\b/ig);for(t=0;t<r.length;t++){u=r[t];var w=q[u.toUpperCase()];d=d.replace(u,
...
if("nofile"==a.UPLOAD_STATUS)return this.uploadProcess=!1,a.removeWaitBox(),!1;$X(a.WAIT_ID)||a.createWaitBox();return this.uploadProcess=!0};this.DoPost=function(a,b,d){if(this.posting)return!1;if(!d){d=this.DoPostBefore();if(d instanceof Array){var e=this;this._checkStatusInc=0;this._checkStatusFristResult=d;this._checkStatusTimer=window.setInterval(function(){var n=0==e._checkStatusInc?e._checkStatusFristResult:e.DoPostBefore();e._checkStatusInc++;var p=n[0];if(null!==p&&void 0!==p){window.clearInterval(e._checkStatusTimer);
e._checkStatusTimer=null;e._checkStatusInc=null;e._checkStatusFristResult=null;var q=n[1];n=n[2];p&&!n?e.DoPost(a,b,!0):p&&n?$Confirm(q,q,function(){e.DoPost(a,b,!0)},function(){}):q&&$Tip(q)}},232);return}if(!d)return}var g=$(a).find("input[ewa_tag=ht5upload]");if(!(0<g.length)||this.uploadProcess||this.CheckValidAll(a)){var h=0;e=this;for(d=0;d<g.length>0;d++){var k=g[d];this._uploadingFileCheck(k)&&h++}if(0<h)setTimeout(function(){e.DoPost(a,b)},700);else{g=$(a).find("input[ewa_tag=ht5takephoto]");
