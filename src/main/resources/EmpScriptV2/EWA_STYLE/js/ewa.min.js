...
(g[r]._children||(g[r]._children=[]),g[r]._children.push(t))}p("___ROOT___",0)}else for(w=0;w<a.length;w++)t=a[w],n(t);h&&(b.value=h);$(b).trigger("change");k&&k(b)}$Install=function(a,b,d,e){var g=new EWA_UrlClass;g.SetUrl(a);g.GetParameter("EWA_AJAX")||(a=g.AddParameter("EWA_AJAX","INSTALL"));(new EWA_AjaxClass).Install(a,"",b,d,e)};
...
EWA.LANG.trim().toUpperCase())return b[a]}return b[0]};this._GetSubValue=function(a,b,d){a=this._GetSubItem(a,d);return null==a?null:this.Xml.GetElementAttribute(a,b)};this.AddColumns=function(a,b,d,e,g,h,k,l){var m=$X("EWA_LF_"+this._Id);null==l&&(l=m.rows[0].cells.length);for(var n=0;n<a.length;n++)for(var p=a[n],q=l+n,r=0;r<m.rows.length;r++){var u=m.rows[r].insertCell(q);u.title=p[e];if(0==r)u.className="EWA_TD_H ewa-add-column ewa-col-"+p[b],u.innerHTML='<nobr cellIdx="'+q+'" id="'+p[b]+'">'+
p[d]+"</nobr>",u.title=p[e],u.width=100;else{var t=m.rows[r].getAttribute("EWA_KEY");u.className="EWA_TD_M ewa-add-column ewa-col-"+p[b];t=t+"_"+p[b];var w=this._GetAddControl(p[h],p[d],p[e]);null==w&&(w=g);var v=k.replace("[ID]",p[b]);w=0<=w.indexOf("<select")?w.replace("!!",v.replace("onblur=","onchange=")):w.replace("!!",v);u.innerHTML=w;u.childNodes[0].id=t}}};this._GetAddControl=function(a,b,d){if(null==a)return null;a=a.toUpperCase().trim();b=b?b.replace(/</ig,"&lt;").replace(/>/ig,"&gt;").replace(/"/ig,
"&quot;"):"";d=d?d.replace(/</ig,"&lt;").replace(/>/ig,"&gt;").replace(/"/ig,"&quot;"):"";return"SELECT"==a?'<select des="'+b+'"  title="'+d+'" !!></select>':"DATE"==a?'<input type=text des="'+b+'" title="'+d+'" class=EWA_INPUT onclick="EWA.UI.Calendar.Pop(this,false);" readonly !!>':"TIME"==a?'<input type=text des="'+b+'" title="'+d+'"  class=EWA_INPUT onclick="EWA.UI.Calendar.Pop(this,true);" readonly !!>':"STRING"==a||"NUMBER"==a?'<input type=text des="'+b+'"  title="'+d+'" class=EWA_INPUT !!>':
null};this.AddedValues=function(a,b,d,e,g){for(var h=0;h<a.length;h++){var k=a[h],l=$X(k[b]+"_"+k[d]);if(!l)break;var m=l.tagName.toUpperCase();"INPUT"==m||"SELECT"==m||"TEXTAREA"==m?l.value=k[e]:l.innerHTML=k[e];g&&(this.GetRow(l).getElementsByTagName("input")[0].checked=!0)}};this.AddRow=function(a){for(var b=$X("EWA_LF_"+this._Id),d=b.insertRow(-1),e=0;e<b.rows[0].cells.length;e++){var g=d.insertCell(-1),h=b.rows[0].cells[e];g.className="EWA_TD_M";null!=a[e]&&(g.innerHTML=a[e]);g.style.display=
h.style.display;g.id="ADD_ROW_"+h.childNodes[0].id}return d};this.Calc=function(a,b,d,e){for(var g=$X("EWA_LF_"+this._Id),h={},k=0;k<a.length;k++)h[a[k]]=0;for(k=b;k<d;k++)for(a=g.rows[k],b=0;b<a.cells.length;b++)if(null!=h[b]){var l=a.cells[b],m=l.getElementsByTagName("input");0<m.length?h[b]+=1*m[0].value:(l=GetInnerText(l),""==l&&(l=0),h[b]+=1*l)}d=g.rows[e];for(k=0;k<d.cells.length;k++)null!=h[k]&&(l=Math.round(100*h[k])/100,e=GetInnerText(d.cells[k]),l!=1*e&&(this.IsCalcChanged=!0),d.cells[k].innerHTML=
l.fm())};this.mergeHeaders=function(a,b,d){var e=$($X("EWA_LF_"+this._Id)),g=e.find('tr[ewa_tag="HEADER"]');if(0!=g.length&&(a=g.find('nobr[id="'+a+'"]').parent(),0!=a.length)){var h=e.find("tr[ewa_mearge]");0==h.length&&(h=$(e[0].insertRow(1)),h.attr("ewa_mearge","gdx"));e=-1;for(var k=0;k<g[0].cells.length;k++){var l=g[0].cells[k];l==a[0]&&(e=k,l.colSpan=d);if(-1==e)"1"!=$(l).attr("ewa_tdmearged")&&(l.rowSpan=2);else if(e+d==k)e=-1,l.rowSpan=2;else{l.rowSpan=1;var m=h[0].insertCell(-1);m.innerHTML=
l.innerHTML;m.className=l.className;m.style.cssText=l.style.cssText;l.style.display="none";$(l).attr("ewa_tdmearged",1)}l==a[0]&&(l.innerHTML=b,l.style.display="",l.style.width="")}}};this.MeargeHeader=function(a,b,d){console.log("\u62fc\u5199\u9519\u8bef\uff0c\u8bf7\u7528 mergeHeaders");this.mergeHeaders(a,b,d)};this.getUrlClass=function(){var a=new EWA_UrlClass;a.SetUrl(this.Url);return a}}
