...
g=[],h=b;b={};for(var k=this.getObj(),l=0;l<e.length;l++){var m=e[l];g.push(m);var n=m.replace("@@","");h=h.replace(m,"<span class='ewa-row-merge ewa-row-merge-"+n+"' mid=\""+n+'"></span>');n!=a&&k.find(".ewa-row-"+n).hide();b[n]=this.GetItemDescription(n).Info}e=document.createElement("div");e.style.display="none";e.innerHTML=h;document.body.appendChild(e);m=k.find('[id="'+a+'"]');if(0==m.length)console.log("not find "+a);else{a=m.parent();for(var p in g)m=g[p].replace("@@",""),h=k.find('[id="'+
m+'"]'),0==h.length?console.log("not found [id="+m+"]"):(l=$(e).find('span[mid="'+m+'"]'),"placeholder"==c?h.attr("placeholder",b[m]):c&&(l.append("<span class='ewa-row-merge-memo'></span>"),l.find(".ewa-row-merge-memo").text(b[m])),l.append(h));for(;0<e.childNodes.length;)a.append(e.childNodes[0]);$(e).remove();d&&d(a)}}else console.log("mergeExp \u6ca1\u6709\u8bbe\u7f6e")};this.MergeItems=function(){for(var a in this.MeargeMap){var b=this.MeargeMap[a],c=$X("_ewa_tr$"+a);if(c){for(var d=c.cells.length,
