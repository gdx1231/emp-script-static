...
this._Id+"'>");c.push("<tr style='display:none'><td style='padding:0'></td></tr>");c.push("<tr><td class='ewa_lf_func' style='padding:0'>");c.push("<div style='display:none'></div>");c.push("<div><div class='ewa_lf_func_caption'></div></div>");c.push("</td></tr>");c.push("<tr><td style='width:100%;padding:0;vertical-align:top'>");c.push("<div id='"+a+"' style=''></div>");c.push("</td></tr>");c.push("</table>");var d=$(c.join("")),e=!(this.Url.toUpperCase().indexOf("EWA_AJAX=INSTALL")>0||this.Url.toUpperCase().indexOf("EWA_CALL_METHOD=INNER_CALL")>
...
d=d.find(".ewa_lf_func_caption")[0];var m=!1;e=GetInnerText(k);e=e.trim();this._ReshowButs={};b=$(b.rows[0]);for(var n in this.ItemList.Items){var p=this.ItemList.Items[n],r=this._GetSubValue("Tag","Tag",p);if(r&&(r.toLowerCase()=="button"||r.toLowerCase()=="butflow")&&(r=this._GetSubValue("Name","Name",p),b.find(".ewa-col-"+r).length!==0)){m=!0;var q=this._GetSubValue("DescriptionSet","Info",p),t=this._GetSubValue("DescriptionSet","Memo",p),v=this._GetSubValue("EventSet","EventName",p),u="";v!=null&&
v.toLowerCase().trim()=="ewa_click"&&(u=this._GetSubValue("EventSet","EventValue",p));e=e.replace(q,"");p=this._ReShowButton(q,t,null);this._ReshowButs[q]=p;p.setAttribute("f_id",this._Id);p.setAttribute("t_id",r);k.childNodes[1].appendChild(p);u==""?p.setAttribute("onclick","EWA.F.FOS['"+this._Id+"'].reShowButtonClick(this)"):p.setAttribute("onclick",u)}}m&&this._ReShowSplit(k.childNodes[1]);r=k.getElementsByTagName("input");m={};for(b=0;b<r.length;b++){var w=r[b];w.style.display="none";if(w.id==
null||w.id=="")w.id=("_tmp_"+Math.random()).replace(".","_");if(w.type=="button"){if(p=this._ReShowButton(w.value,w.title,w.id),q=w.nextSibling){var x="";q.nodeType==3&&(x=q.nodeValue.trimEx());x=="|"&&this._ReShowSplit(k.childNodes[1])}}else(q=w.nextSibling)&&(x=q.nodeType==3?q.nodeValue.replace("|",""):q.innerHTML),p=this._ReShowButton(x.trimEx(),w.title,w.id),p.id=("_tmp_"+Math.random()).replace(".","_"),m[w.name]||(m[w.name]=[]),m[w.name].push(p.id);k.childNodes[1].appendChild(p)}e=e.replace(/ /ig,
"").replace(/\|/ig,"").trimEx();e!=""?e.indexOf("if")>=0?g.style.display="none":h.innerHTML=e.replace(/ /,""):g.style.display="none";a.appendChild(c);for(n in m)for(x=m[n],b=0;b<x.length;b++)w=$X(x[b]),w.setAttribute("_r_ids",x.join(",")),w.onclick=function(){var B=this.getAttribute("_ewa_event_id");$X(B).click();B=this.getAttribute("_r_ids").split(",");for(var C=0;C<B.length;C++)w.id!=B[C]&&($X(B[C]).style.fontWeight="",$X(B[C]).style.color="");this.style.fontWeight="bold";this.style.color="blue"};
d.innerHTML='<b><i class="fa fa-star-o">&nbsp;</i>'+this.Description+"</b>";if(this.Url.toUpperCase().indexOf("EWA_BTNS_IN_TITLE")>0)var y=this,z=0,A=setInterval(function(){z++;z>1E3&&window.clearInterval(A);EWA.OW.Load();EWA.OW.Dia&&(window.clearInterval(A),y.ReShowButtonsInDailogTitle())},10);this._InitMneu()}};this.reShowButtonClick=function(a){a=a.getAttribute("t_id");var b=this.SelectCheckedRows();if(b==null||b.length==0)EWA.UI.Msg.Alert(_EWA_INFO_MSG["EWA.SYS.CHOOSE_ITEM"],_EWA_INFO_MSG["EWA.SYS.CHOOSE_ITEM_TITLE"]);
else{b=b[0].getElementsByTagName("input");for(var c=0;c<b.length;c++){var d=b[c];if(d.name===a){d.click();break}}}};this._ReShowSplit=function(a){EWA.UI.Utils.CreateObject(window,"div","",a).className="ewa_lf_func_split"};this._ReShowButton=function(a,b,c){var d=EWA.UI.Utils.CreateObject(window,"div","cursor:pointer",document.body);d.innerHTML="<nobr>"+a+"</nobr>";c!=null&&(d.setAttribute("_ewa_event_id",c),d.onclick=function(){var e=this.getAttribute("_ewa_event_id");$X(e).click()});EWA.B.IE&&(d.onselectstart=
function(){return!1});d.className="ewa_lf_func_dact";d.title=b;return d};this.CheckValid=function(a){var b=a.tagName.toLowerCase();if(b!="input"&&b!="textarea"&&b!="select"||"button"==a.type||"submit"==a.type||"image"==a.type)return!0;b=this.ItemList.GetObjectValue(a);return this.ItemList.CheckValid(a,b)};this.ShowEdit=function(a){a.parentNode.style.width=a.clientWidth+"px";a.style.display="none";__show_obj__=a.nextSibling;__show_obj__.style.display="";var b=__show_obj__.childNodes[0];b.nodeType==
