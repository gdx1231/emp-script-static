...
z.setAttribute("json",JSON.stringify(v));return z}b.options.length=0;g&&(b.options[0]=new Option("",""),g instanceof Array&&(b.options[0].text=g[0],b.options[0].value=g[1]));if(l){var p=function(v,x){if((v=q[v])&&v._children&&0!=v._children.length)for(var y,z=0;z<v._children.length;z++)y=v._children[z],n(y,x),p(y[e],x+1)},q={___ROOT___:[]};g={___ROOT___:{}};for(var r,u,t,w=0;w<a.length;w++){t=a[w];r=t[l];u=t[e];if(null===r||""===r||"0"===r)r="___ROOT___";q[r]||(q[r]=[]);q[r].push(t);g[u]=t;g[r]&&
(g[r]._children||(g[r]._children=[]),g[r]._children.push(t))}p("___ROOT___",0)}else for(w=0;w<a.length;w++)t=a[w],n(t);h&&(b.value=h);$(b).trigger("change");k&&k(b)}$Install=function(a,b,d,e){a.GetUrl?a=a.GetUrl():a.getUrl?a=a.getUrl():a.href&&(a=a.href);var g=new EWA_UrlClass;g.SetUrl(a);g.GetParameter("EWA_AJAX")||g.AddParameter("EWA_AJAX","INSTALL");(new EWA_AjaxClass).Install(g.GetUrl(),"",b,d,e)};
...
g+=h;var q=[];if("number"===typeof n.length&&!n.propertyIsEnumerable("length")){var r=n.length;for(m=0;m<r;m+=1)q[m]=d(m,n)||"null";var u=0===q.length?"[]":g?"[\n"+g+q.join(",\n"+g)+"\n"+p+"]":"["+q.join(",")+"]";g=p;return u}if("object"===typeof l)for(r=l.length,m=0;m<r;m+=1){var t=l[m];"string"===typeof t&&(u=d(t,n,l))&&q.push(b(t)+(g?": ":":")+u)}else for(t in n)(u=d(t,n,l))&&q.push(b(t)+(g?": ":":")+u);u=0===q.length?"{}":g?"{\n"+g+q.join(",\n"+g)+"\n"+p+"}":"{"+q.join(",")+"}";g=p;return u}}
...
"number"===typeof n.length)l=n;else throw Error("JSON.stringify");else l=function(r,u){if(Object.hasOwnProperty.call(this,r))return u};return d("",{"":m})},parse:function(m,n){function p(q,r){var u,t=q[r];if(t&&"object"===typeof t)for(u in t)if(Object.hasOwnProperty.call(t,u)){var w=p(t,u);void 0!==w?t[u]=w:delete t[u]}return n.call(q,r,t)}if(/^[\],:{}\s]*$/.test(m.replace(/\\["\\\/bfnrtu]/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,"]").replace(/(?:^|:|,)(?:\s*\[)+/g,
...
a.childNodes.length;b+=1)EWA$UI$COMMON.Dispose(a.childNodes[b])}},Drop:function(a){EWA$UI$COMMON.Dispose(a);a.parentNode.removeChild(a)},AniExpand:function(a,b,d,e,g){if(null==e||0>=e)e=10;var h=0,k=b/(1*e),l=d/(1*e),m=a.clientWidth+b,n=a.clientHeight+d,p=window.setInterval(function(){var q=EWA$UI$COMMON.GetObjSize(a);if(h<e-1){var r=q.H+l,u=q.W+k;r>n&&(r=n);u>m&&(u=m);EWA$UI$COMMON.SetSize(a,u,r)}else EWA$UI$COMMON.SetSize(a,m,n);u=EWA$UI$COMMON.GetObjSize(a);r=u.W-q.W;u=u.H-q.H;q=EWA$UI$COMMON.GetPosition(a);
q.Y=q.Y-a.clientHeight-1*a.style.borderTopWidth.replace("px","")-1*a.style.borderBottomWidth.replace("px","");"LEFT_TOP"!=g&&("LEFT_BOTTOM"==g?(u=q.Y-u,r=q.X,EWA$UI$COMMON.MoveTo(a,r,u)):"RIGHT_TOP"==g?(u=q.Y,r=q.X-r,EWA$UI$COMMON.MoveTo(a,r,u)):"RIGHT_BOTTOM"==g?(u=q.Y-u,r=q.X-r,EWA$UI$COMMON.MoveTo(a,r,u)):(u=1*q.Y-u/2,r=1*q.X-r/2,EWA$UI$COMMON.MoveTo(a,r,u),EWA._TmpAniObj=a));h++;h>=e&&(window.clearInterval(p),EWA.UI.Utils.AniExpandCompleateFunction&&(EWA.UI.Utils.AniExpandCompleateFunction(),
...
(function(){function a(){var l=$(".ewa-slider-puzzle-box"),m=$(".ewa-slider-puzzle-swiper"),n=$(".ewa-slider-puzzle-verify"),p=$(".ewa-slider-puzzle-text"),q=k?"touchmove":"mousemove",r=k?"touchend":"mouseup";m.bind(k?"touchstart":"mousedown",function(u){u.stopPropagation();n.css({display:"block"});var t=(k?u.touches[0]:u).clientX-$(this).offset().left;$(this).offset();p.css("opacity","0");l.unbind(q);l.unbind(r);console.log(l[0]);l.bind(q,function(w){var v=k?w.touches[0]:w,x=$(".ewa-slider-puzzle-handle");
w=$(".ewa-slider-puzzle-swiper");var y=$(".ewa-slider-puzzle-verify");v=v.clientX-t-$(x).offset().left;x=$(x).width()+29;0>v?v=0:v>x&&(v=x);w.css("left",v);y.css("left",v+10)});l.bind(r,function(w){l.unbind(q);l.unbind(r);w=new EWA_UrlClass(h.ewa_url);console.log(n.position());console.log(n.offset());w.AddParameter("left",n.position().left);w.AddParameter("top",n.position().top);w.AddParameter("left1",n.offset().left);w.AddParameter("top1",n.offset().top);w.AddParameter("ewa_trigger_valid_mode","check");
...
function(a,b,d,e,g,h,k){var l=this;$J(e,function(m){l.createFilterCheckboxByData(a,b,d,m,g,h,k)})};this.createFilterCheckboxByData=function(a,b,d,e,g,h,k){var l={},m=m?"[]":d.split(",");d={};for(var n in m)d[m[n]]=1;m=[];var p=[],q=a.id;for(n in e){var r=e[n],u=r[g];u&&(u=u.replace(/</ig,"&lt;"));var t=r[h];t&&(t=t.replace(/</ig,"&lt;"));var w=r[k],v=d[u],x=[];x.push("<nobr class='ewa-filter-chk' style='float:left;margin-right:4px'>");x.push('<input class="ewa-filter-chkbox" type="checkbox" ');v&&
x.push(" checked ");r=q+"_"+n;x.push(' value="'+u+'" id="'+r+'" name="'+q+'"><label ');x.push(' for="'+r+'">'+t+"</label></nobr>");v?p.push(x.join("")):m.push(x.join(""));b&&(u=w?w.substring(0,1).toUpperCase():"",l[u]||(l[u]=[]),l[u].push(r))}a.innerHTML=p.join(" ")+" "+m.join(" ");if(b){e=['<a href="javascript:void(0)">ALL</a>'];for(n="A".charCodeAt();n<="Z".charCodeAt();n++)g=String.fromCharCode(n),l[g]&&e.push('<a href="javascript:void(0)">'+g+"</a>");b.innerHTML=e.join(" ");$(b).find("a").each(function(){var y=
...
m.ITEM.LST;h.options.length=0;var p=[],q;for(q in n){var r=n[q];if(!r.V||!k[r.V]){var u=new Option(r.T,r.V);r.json&&$(u).attr("json",r.json);h.options[h.options.length]=u;p.push(u);r._used=!0}}for(q in n)r=n[q],r._used||(u=new Option(r.T,r.V),r.json&&$(u).attr("json",r.json),h.options[h.options.length]=u);if(b)h.value=b;else for(var t in p)if(p[t].value){h.value=p[t].value;break}if(d)try{d(m,h)}catch(w){console.log(w)}}else $Tip(m.ERR)})}};this._checkSelectOptionsChange=function(){if(this._SelectFilters){var a=
...
m.find("td:eq(0)").html(b));if(0==l.length)alert("target parent not exists");else{var n="border-box"==g.css("box-sizing")?0:1*g.css("padding-left").replace("px","")+1*g.css("padding-right").replace("px",""),p="border-box"==g.css("box-sizing")?0:1*h.css("padding-left").replace("px","")+1*h.css("padding-right").replace("px","");e.find(".ewa-row-"+a).hide();null==d&&(d=" - ");var q=l.width(),r=g[0].tagName,u=h[0].tagName;g.parent().hasClass("ewa-select-filter-target")&&(g=e.find(".ewa-row-"+a+" .EWA_TD_M table:eq(0)"),
r="SELECT_WITH_FILTER");var t=0;b=null;d&&(l.append('<span class="mearge-span">'+d+"</span>"),k&&m.find("table:eq(0) td:eq(1)").html('<span class="mearge-span">'+d+"</span>").show(),b=l.find(".mearge-span"),t=b.width()+1+1*b.css("padding-left").replace("px","")+1*b.css("padding-right").replace("px","")+1*b.css("margin-left").replace("px","")+1*b.css("margin-right").replace("px",""));r==u?(d=(q-t)/2-n-2,g.css("width",d),h.css("width",d),k&&m.find("table:eq(0) td:eq(2), table:eq(0) td:eq(0)").css("width",
d)):"SELECT"==r?(g.css("width",100),h.css("width",q-t-n-p-2-2-g.width())):"SELECT_WITH_FILTER"==r?(g.css("width",200),h.css("width",q-t-n-p-2-2-g.width())):"SELECT"==u&&(h.css("width",100),g.css("width",q-t-n-p-2-2-g.width()));h.css("float","left");b&&b.css("float","left");g.css("float","right");l.append(g);l.addClass("ewa-frame-merge");k&&(a=e.find(".ewa-row-"+a+" .EWA_TD").html(),m.find("table:eq(0) td:eq(2)").html(a))}}}};this.MergeExp=function(a,b,d,e){if(b){var g=b.match(/@@[a-zA-Z0-9\-\._:]*\b/ig),
...
g.type.toUpperCase()&&(this._GetDropListValue1(e,g),this._bindCompostion(e,g))}}a=a.getElementsByTagName("select");for(d=0;d<a.length;d++)if(e=a[d],b=e.getAttribute("_ListFilterType"))g=e.getAttribute("_ListFilterField"),this.initSelectFilter(e.id,b,g);this._SelectFilters&&this._checkSelectOptionsChange()}};this._GetDropListValue1=function(a,b){if(""==b.value)b.setAttribute("setvalue",1);else{var d=b.getAttribute("DlsShow");if(null==d||""==d)b.setAttribute("canopen",0),a.value=b.value,b.setAttribute("canopen",
1);else{var e=new EWA_AjaxClass,g=b.getAttribute("xmlName"),h=b.getAttribute("itemName"),k=b.getAttribute("paraName"),l=b.getAttribute("DlsAction");e=new EWA.C.Ajax;var m=b.value;var n=this.EWA_CGI?this.EWA_CGI:window.EWA_CGI?window.EWA_CGI:EWA.CP+"/EWA_STYLE/cgi-bin/";var p=-1==n.indexOf("?")?"?":"&";n=(n+(p+"XMLNAME="+g+"&ITEMNAME="+h+"&EWA_AJAX=JSON")).replace("//","/");n=n.replace("//","/");n=n.replace("//","/");e.AddParameter(k,m);null!=l&&0<l.trim().length&&(e.AddParameter("EWA_ACTION",l),e.AddParameter(b.id,
b.value));e.PostNew(n,function(){if(e.IsRunEnd()){var q=e.GetRst(),r="__json"+Math.random();try{eval('window["'+r+'"]='+q)}catch(v){b.setAttribute("canopen",0);a.value=v;b.setAttribute("canopen",1);return}q=window[r];if(0!=q.length){r=q[0];var u=[];q={};for(var t in r)u.push(r[t]),q["@"+t.toUpperCase()]=r[t],r[t]&&b.setAttribute("para_"+t.toLowerCase(),r[t]);if(null==d||""==d)q=u.join("/");else{t=d.match(/@[a-zA-Z0-9\-\._:]*\b/ig);for(r=0;r<t.length;r++){u=t[r];var w=q[u.toUpperCase()];d=d.replace(u,
...
"");else{this.focus();$(this).css("background","pink");return}}q=this.id;r=this.value;"droplist"==$(this).attr("ewa_class")&&(r=$(this).next().val(),q=$(this).next().attr("id"));q={key:q,val:r};if("rgb(255, 255, 224)"==p)$Tip(n.html()),l.hide(),this.focus();else if(this.value==$(this).attr("oldv"))$(this).attr("style",""),l.hide();else{var u=this;$JP(m.POST_URL||window.location.href,q,function(t){$(u).attr("style","");l.hide();t.RST?(t="SELECT"==u.tagName?u.options[u.selectedIndex].text:u.value,"TEXTAREA"==
u.tagName?""==t?b.html("............."):(t=t.replace(/</ig,"&lt;").replace(/>/ig,"&gt;"),t="<div>"+t.split("\n").join("</div><div>")+"</div>",b.html(t)):(""==t&&(t="............."),b.text(t)),b.append('<i style="margin-left:5px" class="fa fa-edit"></i>'),$TipTL("\u4fdd\u5b58\u5b8c\u6bd5")):$Tip(t.ERR)})}})}}}
...
r+" ewa-lf-mearge ewa-lf-mearge-"+r+"' mid=\""+r+'"></span>');r!=a&&(h.find('tr[ewa_tag="HEADER"] [id="'+r+'"]').parent().hide(),h.find("td#ADD_ROW_"+r).hide());q=h.find('tr[ewa_tag="HEADER"] [id="'+r+'"]').html();n[r]=q;b.push(q)}g&&(g=b.join("<span class='ewa-lf-merge-header-split'></span>"),h.find('tr[ewa_tag="HEADER"] [id="'+a+'"]').html(g));h.find("tr[ewa_key],tr.ewa-lf-sub-tr").each(function(){if(0==$(this).find('[id="'+a+'"]').length)console.log("not find "+a);else{var u=$(this).hasClass("ewa-lf-sub-tr")?
$(this).find('[id="ADD_ROW_'+a+'"]'):$(this).find(".ewa-col-"+a);if("yes"!=u.attr("ewa-merged")){var t=$('<div style="display:none"></div>');t.html(m);for(var w in l){var v=l[w].replace("@@",""),x=$(this).find('[id="'+v+'"]');if(0!=x.length){v!=a&&x.parent().hide();var y=t.find('span[mid="'+v+'"]');d&&(y.append("<span class='ewa-lf-mearge-memo'></span>"),y.find(".ewa-lf-mearge-memo").html(n[v]));y.append(x)}}for(;0<t[0].childNodes.length;)u.append(t[0].childNodes[0]);$(t).remove();e&&e(u,this);u.attr("ewa-merged",
...
C.style.top=F+"px";C.style.height=E.H-F+"px"}));for(;0<a.childNodes.length;)m.childNodes[0].appendChild(a.childNodes[0]);d=d.find(".ewa_lf_func_caption")[0];var p=!1;e=GetInnerText(m);e=e.trim();this._ReshowButs={};g=$(g.rows[0]);for(var q in this.ItemList.Items){var r=this.ItemList.Items[q],u=this._GetSubValue("Tag","Tag",r);if(u&&("button"==u.toLowerCase()||"butflow"==u.toLowerCase())&&(u=this._GetSubValue("Name","Name",r),0!==g.find(".ewa-col-"+u).length)){p=!0;var t=this._GetSubValue("DescriptionSet",
"Info",r),w=this._GetSubValue("DescriptionSet","Memo",r),v=this._GetSubValue("EventSet","EventName",r),x="";null!=v&&"ewa_click"==v.toLowerCase().trim()&&(x=this._GetSubValue("EventSet","EventValue",r));e=e.replace(t,"");r=this._ReShowButton(t,w,null);this._ReshowButs[t]=r;r.setAttribute("f_id",this._Id);r.setAttribute("t_id",u);m.childNodes[1].appendChild(r);""==x?r.setAttribute("onclick","EWA.F.FOS['"+this._Id+"'].reShowButtonClick(this)"):r.setAttribute("onclick",x)}}p&&this._ReShowSplit(m.childNodes[1]);
u=m.getElementsByTagName("input");p={};for(g=0;g<u.length;g++){var y=u[g];y.style.display="none";if(null==y.id||""==y.id)y.id=("_tmp_"+Math.random()).replace(".","_");if("button"==y.type){if(r=this._ReShowButton(y.value,y.title,y.id),t=y.nextSibling){var z="";3==t.nodeType&&(z=t.nodeValue.trimEx());"|"==z&&this._ReShowSplit(m.childNodes[1])}}else(t=y.nextSibling)&&(z=3==t.nodeType?t.nodeValue.replace("|",""):t.innerHTML),r=this._ReShowButton(z.trimEx(),y.title,y.id),r.id=("_tmp_"+Math.random()).replace(".",
...
h,g[h.toUpperCase()]=this._SearchJson[h];this._SearchJson=g;var k="EWA.F.FOS[&quot;"+this._Id+"&quot;].ChangeSearchTextType(this)",l="EWA.F.FOS[&quot;"+this._Id+"&quot;].ChangeSearchDateType(this)",m=$J2MAP(this.RESOURCES.search_text_items,"Id");g=this._GetInitSearchMap();var n=!1,p=!1;for(h in this.ItemList.Items)if(null!=this._SearchJson[h]){var q=[],r=this._SearchJson[h],u=r.T,t=this._GetSubValue("DescriptionSet","Info",this.ItemList.Items[h]);q.push("<table class='ewa-lf-search-item ewa-ref-"+
h+" ewa-lf-search-type-"+u+" "+("fix"==u?"ewa-lf-search-fix-m"+(r.M||"0"):"")+"' cellspacing=0 cellpadding=0>");q.push("<tr><td class='ewa-lf-search-item-title' style='position:relative'><nobr><b>");q.push(t);var w=t="",v="lk";g[h.toUpperCase()]&&(w=g[h.toUpperCase()],v=w.tag,t=w.para1,w=w.para2);var x="";"text"==u?(p=m[v]||m.lk,x="enus"==EWA.LANG?p.TxtEn:p.Txt,q.push("<a class='ewa-lf-search-type' tag='"+v+"' href='javascript:void(0)' onclick='"+k+"'>"),q.push(x),q.push("</a>"),p=!0):"date"==u&&
(q.push("<a  href='javascript:void(0)' class='fa fa-calendar-check-o'  onclick='"+l+"'></a>"),n=!0);q.push("</b></nobr></td><td class='ewa-lf-search-item-ctl'>");"text"==u?"blk"==v||"nblk"==v?(r=this._SearchSingle(h,x),r=r.replace("<input","<input disabled "),q.push(r)):q.push(this._SearchSingle(h,t)):"date"==u?q.push(this._SearchDate(h,t,w)):"number"==u?q.push(this._SearchNumber(h,t,w)):"fix"==u&&(r="<div >"+this._SearchFix(h,r,"span",t)+"</div>",q.push(r));q.push("</td></tr></table>");"fix"==u&&
...
q=l+n,r=0;r<m.rows.length;r++){var u=m.rows[r].insertCell(q);u.title=p[e];if(0==r)u.className="EWA_TD_H ewa-add-column ewa-col-"+p[b],u.innerHTML='<nobr cellIdx="'+q+'" id="'+p[b]+'">'+p[d]+"</nobr>",u.title=p[e],u.width=100;else{var t=m.rows[r].getAttribute("EWA_KEY");u.className="EWA_TD_M ewa-add-column ewa-col-"+p[b];t=t+"_"+p[b];var w=this._GetAddControl(p[h],p[d],p[e]);null==w&&(w=g);var v=k.replace("[ID]",p[b]);w=0<=w.indexOf("<select")?w.replace("!!",v.replace("onblur=","onchange=")):w.replace("!!",
v);u.innerHTML=w;u.childNodes[0].id=t}}};this._GetAddControl=function(a,b,d){if(null==a)return null;a=a.toUpperCase().trim();b=b?b.replace(/</ig,"&lt;").replace(/>/ig,"&gt;").replace(/"/ig,"&quot;"):"";d=d?d.replace(/</ig,"&lt;").replace(/>/ig,"&gt;").replace(/"/ig,"&quot;"):"";return"SELECT"==a?'<select des="'+b+'"  title="'+d+'" !!></select>':"DATE"==a?'<input type=text des="'+b+'" title="'+d+'" class=EWA_INPUT onclick="EWA.UI.Calendar.Pop(this,false);" readonly !!>':"TIME"==a?'<input type=text des="'+
...
b.animate(u,"fast","swing",function(){b[0].style.cssText="";var t=1*b.attr("st");document.body.scrollTop=t;1==$("#crm_left_box").attr("show")&&$("#crm_left_box").show();b.css("display",b.attr("dsp"));0<e.length&&"DES_plugin"!=a?e[0].click():(t=$X(a.replace("DES_","")),t.style.width=t.parentNode.clientWidth-10+"px",t.style.height=u.height-30+"px",d.find('div[id="EWA_FRAME_MAIN"]').css("height",u.height-150))});else{b.attr("full","1");b.attr("st",document.body.scrollTop);b.attr("dsp",b.css("display"));
var h=document.documentElement.clientHeight;g=document.documentElement.clientWidth-10;var k=h-20,l=document.documentElement.clientWidth-30,m=h-20-20-15;h=b.offset();var n=b.position(),p=h.left-n.left,q=b.width(),r=b.height();var u={width:q,height:r,"margin-left":p,left:n.left,top:n.top};b.attr("ori",JSON.stringify(u));b.css({"margin-left":"0px",top:h.top,left:h.left,position:"absolute",display:"",overflow:"auto"});b.animate({width:g,height:k,left:10,top:0},"fast","swing",function(){d.css("width",
...
(function(a,b){"object"===typeof module&&"object"===typeof module.exports?module.exports=b():b(a)})("undefined"!==typeof window?window:this,function(a){function b(m){for(var n=[{number:0,scale:1,items:[]}],p=null,q=0;q<m.length;q++){var r=m[q],u=g[r],t=e[r];n[n.length-1].items.push([r,u,t]);if(u)0==q&&u.first?n[n.length-1].number=10:u.stop?(n[n.length-1].scale=u.value||1,n.push({number:0,scale:1,items:[]}),scale=1,p=null):(q==m.length-1&&(console.log(u),n[n.length-1].number.scale=u.value),p=u);else{if(null==
t)throw"\u975e\u6570\u5b57\u6216\u5355\u4f4d\uff1a"+r;r=q==m.length-1?null:m[q+1];var w=q==m.length-1?null:g[r];w&&w.stop&&(w=null);r=n[n.length-1].number;var v=n[n.length-1].items;u=v[v.length-2];v=v[v.length-3];null==w?null==p?(p=!1,u&&0===u[2]?v&&null!=v[1]&&(p=!0):u&&u[1]&&10===u[1].value&&(p=!0),t=p?r+t:10*r+t):t=u&&u[1]&&10===u[1].value&&(null==v||0===v[2]||null!=v[1])?r+t+10:r+t:t=r+t*w.value;n[n.length-1].number=t;p=null}}return n}function d(m){if(!m)return m;if(!isNaN(m.toString()))return 1*
m.toString();var n=m.replace(/\uff0c|,| |\u5143|\u5706/ig,"").replace(/\u70b9|\u3002/ig,".").split(".");if(2<n.length)throw"\u975e\u6570\u5b57\u8868\u8fbe\u5f0f\uff1a"+m;var p=n[0].split("");m=(2==n.length?n[1]:"").split("");n=!1;var q=p[0];h[q]?(n=!0,p=p.splice(1)):k[q]&&(n=!1,p=p.splice(1));q=p[p.length-1];l[q]?p=p.splice(0,p.length-1):1<m.length&&(q=m[m.length-1],l[q]&&(m=m.splice(0,m.length-1)));q=p;for(var r=p=0,u=q.length-1;0<=u;u-=2){var t=q[u];var w=e[q[u-1]];t=w?"\u89d2"==t?1*("0."+w):"\u5206"==
t?1*("0.0"+w):"\u5398"==t?1*("0.00"+w):"\u6beb"==t?1*("0.000"+w):-1:-1;if(-1===t)break;else p+=t,r++}q=q.splice(0,q.length-2*r);q=b(q);r=0;u=1;for(t=q.length-1;0<=t;t--){w=q[t];var v=w.number;u*=w.scale;r+=v*u}p=r+p;if(0<m.length){q=1;r=0;for(u=m.length-1;0<=u;u--){t=g[m[u]];if(!t)break;q*=t.value;r++}m=1==q?m:m.splice(0,m.length-r);r=b(m);u=0;t=1;for(w=r.length-1;0<=w;w--){v=r[w];var x=v.number;t*=v.scale;u+=x*t}m=[1*("0."+u),q,m.length]}else m=0;return 0==m?p*(n?-1:1):((p+m[0])*m[1]).toFixed(0>
...
function loadWfShow(a,b,d,e,g,h,k,l,m,n){_EWA_WF&&_EWA_WF.EN&&(_EWA_WF.EN1=$J2MAP(_EWA_WF.EN,"WF_UNIT_ID"));e=document.createElement("div");e.style.position="absolute";e.style.zIndex=123;e.innerHTML="ver: "+_EWA_WF.WF_DLV_VER;document.body.appendChild(e);var p=EWAC_WfUtil.WF,q=1000012,r=1000011;for(e=0;e<a.length;e++){var u=a[e],t=u.WF_UNIT_X,w=u.WF_UNIT_Y;null!=t&&1*t<q&&(q=1*t);null!=w&&1*w<r&&(r=1*w)}1000012==q&&(q=0);1000011==r&&(r=0);q-=10;r-=10;n={};var v;for(e=0;e<a.length;e++)u=a[e],p.Ids[e]=
u.WF_UNIT_ID,t=p.CreateUnit(),n[u.WF_UNIT_ID]=t,w=u.WF_UNIT_NAME,"enus"==EWA.LANG&&_EWA_WF.EN1&&_EWA_WF.EN1[u.WF_UNIT_ID]&&_EWA_WF.EN1[u.WF_UNIT_ID].WF_UNIT_NAME_EN&&(w=_EWA_WF.EN1[u.WF_UNIT_ID].WF_UNIT_NAME_EN),t.ChangeDes(w),t.ChangeType(u.WF_UNIT_TYPE),t.WF_UNIT_ADM=u.WF_UNIT_ADM,t.WF_UNIT_ADM_LST=u.WF_UNIT_ADM_LST,t=u.WF_UNIT_X,w=u.WF_UNIT_Y,u=$X(u.WF_UNIT_ID),null!=t&&(u.style.left=t-q+"px"),null!=w&&(u.style.top=w-r+"px");a={};for(e=0;e<b.length;e++)u=b[e],t=u.WF_UNIT_FROM,a[t]||(a[t]=[]),a[t].push(u);
for(e=0;e<b.length;e++)u=b[e],a=u.WF_UNIT_TO,t=u.WF_UNIT_FROM,t=p.Units[t],a=p.Units[a],q=new EWAC_WfCnn,q.Create(t,a),p.Cnns[q.Id]=q,u.IsHaveLogic&&!u.IsLogicOk&&(q.IsStop=!0);for(var x in p.Units)e=p.Units[x],null==v&&0==e.CnnsToLength&&(v=e);if(null==v)e="enus"==EWA.LANG?"No startup node was found, process definition error":"\u672a\u53d1\u73b0\u542f\u52a8\u8282\u70b9,\u6d41\u7a0b\u5b9a\u4e49\u9519\u8bef",alert(e);else{0==d.length&&(p.CurUnit=v);var y=[];x={};for(e=0;e<d.length;e++)b=d[e],p=b.SYS_STA_TAG,
t=n[p],null!=t&&(u=b.ADM_NAME,"enus"==EWA.LANG&&b.ADM_NAME_EN&&(u=b.ADM_NAME_EN),t.Log(u,b.SYS_STA_CDATE,b.SYS_STA_MEMO),x[p]||(x[p]=1,y.push({ID:p,DES:t.GetDes()})),p=b.SYS_STA_TAG+"G"+b.SYS_STA_VAL,EWAC_WfUtil.WF.Cnns[p]&&(EWAC_WfUtil.WF.Cnns[p].Disp(3,"blue"),t==v&&(t.ADM_ID=b.ADM_ID),e==d.length-1&&(t=n[b.SYS_STA_VAL],t.WF_UNIT_ADM_LST=b.ROLE_IDS,t.WF_UNIT_ADM=b.ROLE_TYPE,EWAC_WfUtil.WF.CurUnit=t)));null!=m&&0<m.length&&n[m]&&(EWAC_WfUtil.WF.CurUnit=n[m]);EWAC_WfUtil.WF.CurUnit.LogCur(g);m=!1;
t=EWAC_WfUtil.WF.CurUnit;d=[];if(v==t)m=!0;else if("WF_ADM_ADM"==t.WF_UNIT_ADM)if(v=","+t.WF_UNIT_ADM_LST+",",0<=v.indexOf(","+h+","))m=!0;else for(e in h=$J2MAP1(top.org,"ADM_ID"),k=t.WF_UNIT_ADM_LST.split(","),k){l=h[k[e]];for(var z in l)d.push(l[z].ADM_NAME)}else if("WF_ADM_DEPT"==t.WF_UNIT_ADM)if(v=","+t.WF_UNIT_ADM_LST+",",0<=v.indexOf(","+k+","))m=!0;else for(e in h=$J2MAP1(top.org,"DEP_ID"),k=t.WF_UNIT_ADM_LST.split(","),k)for(z in l=h[k[e]],l)d.push(l[z].ADM_NAME);else if("WF_ADM_POST"==t.WF_UNIT_ADM){v=
","+t.WF_UNIT_ADM_LST+",";h=l.split(",");for(k=0;k<h.length;k++)if(0<=v.indexOf(","+h[k]+",")){m=!0;break}if(!m)for(e in h=$J2MAP1(top.org,"DEP_POS_ID"),k=t.WF_UNIT_ADM_LST.split(","),v={},k)for(z in l=h[k[e]],l)n=l[z],v[n]||(v[n]=!0,d.push(l[z].ADM_NAME))}else if("WF_ADM_START"==t.WF_UNIT_ADM){if(v==t||h==v.ADM_ID)m=!0}else"WF_ADM_MANAGER"==t.WF_UNIT_ADM&&(z=v.ADM_ID,top||top.ORG||(e="enus"==EWA.LANG?"No organization":"\u6ca1\u6709\u7ec4\u7ec7\u673a\u6784",alert(e),m=!1),m=top.ORG.GetMaster(z).Id==
