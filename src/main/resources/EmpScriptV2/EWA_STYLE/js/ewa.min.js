...
a.find("td").text("");var b={},d;for(d in this.SubBottomsArray){var e=this.SubBottomsArray[d];b.$jscomp$loop$prop$total$38=0;var g="#EWA_LF_"+this._Id+' .ewa-lf-data-row [name="'+e+'"]';b.$jscomp$loop$prop$fm_length$39=0;$(g).each(function(h){return function(){var k=this.hasAttribute("value")?this.value:this.hasAttribute("title")?this.title:this.innerText;null!=k&&0!==k.length&&(k=k.replace(/,/ig,""),isNaN(k)||(h.$jscomp$loop$prop$total$38+=1*k,0<k.indexOf(".")&&(h.$jscomp$loop$prop$fm_length$39=
2)))}}(b));g="<span id='"+e+"' name='"+e+"' class='ewa-lf-sub'>"+b.$jscomp$loop$prop$total$38.fm(b.$jscomp$loop$prop$fm_length$39)+"</span>";a.find("td[id=ADD_ROW_"+e+"]").html(g);b={$jscomp$loop$prop$total$38:b.$jscomp$loop$prop$total$38,$jscomp$loop$prop$fm_length$39:b.$jscomp$loop$prop$fm_length$39}}};this.ChangeRowStyle=function(a,b,d){for(var e=$X("EWA_LF_"+this._Id),g=1;g<e.rows.length;g++){var h=e.rows[g];if(!(0==h.cells.length||h.cells.length<=a)){var k=h.cells[a].childNodes[0].getAttribute(b);
...
e||"0"<=e&&"9">=e?b.push(e):b.push("\\"+e)}try{return new RegExp("("+b.join("")+")")}catch(g){return console.warn("new RegExp",a,g),null}};this.SearchMark=function(){for(var a=this._SearchExp.split("@!@"),b=$X("EWA_LF_"+this._Id),d=0;d<a.length;d+=1){var e=a[d].split("~!~");if(!(3>e.length))for(var g={},h=2;h<e.length;g={$jscomp$loop$prop$regExp$41:g.$jscomp$loop$prop$regExp$41},h++){var k=e[h];if(""!=k){if("fix"==e[1]){var l=e[2];$("table#EWA_SEARCH_ITEM_"+this.Id+' [name="'+e[0]+'"]').find("option").each(function(){this.value==
l&&(k=this.text)})}g.$jscomp$loop$prop$regExp$41=this.createRegExp(k);var m=e[0].split(","),n;for(n in m){var p=m[n];this._SearchJson[p]&&(p=this._SearchJson[p].ORI_NAME);p=$(b).find(".ewa-lf-data-row .ewa-col-"+p).not("[ewa-search-marked=yes]");p.attr("ewa-search-marked","yes");try{p.find("*").each(function(q){return function(){0==this.children.length&&(this.innerHTML=this.innerHTML.replace(q.$jscomp$loop$prop$regExp$41,"<font color=red><b>$1</b></font>"))}}(g))}catch(q){console.log(q)}}}}}};this.DblClick=
