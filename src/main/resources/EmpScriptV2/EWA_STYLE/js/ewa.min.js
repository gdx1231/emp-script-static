...
$(c[0]).find("input[type=text]").attr("name",b.join(",")).attr("placeholder",a.join(", "));return c};this.switchButtonAction=function(a,b){if(b){var c=this.getUrlClass(),d={};d.ewa_switch_name=a.name;d.ewa_action=b;d.ewa_ajax="json";d.ewa_switch=a.checked?"on":"off";d.ewa_action_key=$(a).parentsUntil("tbody").last().attr("ewa_key");b=a.parentNode;var e=b.getAttributeNames(),g;for(g in e){var h=e[g],k=b.getAttribute(h);0!=h.indexOf("on")&&"name"!=h&&"id"!=h&&"class"!=h&&(d[h]=k)}var l=this;c=c.GetUrl();
$JP(c,d,function(m){l.extSwitchCallBack&&l.extSwitchCallBack(a,m)})}};this.replaceRowsData=function(a,b){var c=this.getUrlClass();c.AddParameter("EWA_AJAX","LF_RELOAD");c.AddParameter("EWA_IS_SPLIT_PAGE","no");c.AddParameter("EWA_IS_HIDDEN_CAPTION","yes");c=c.GetUrl();a&&(c+="&"+a);var d=$("#EWA_LF_"+this._Id);$J2(c,function(e){$(e).find("tr.ewa-lf-data-row").each(function(){var g=$(this).attr("ewa_key"),h=d.find('tr[ewa_key="'+g+'"]');$(this).find("td").each(function(){var k=$(this).children(0).attr("name");
...
d[0]+'"]').find("option").each(function(){this.value==h&&(g=this.text)})}for(var k=[],l=0;l<g.length;l++){var m=g[l];"a"<=m&&"z">=m||"A"<=m&&"Z">=m||"0"<=m&&"9">=m?k.push(m):k.push("\\"+m)}g=k.join("");g=eval("/("+g+")/ig");k=d[0].split(",");for(var n in k){l=k[n];this._SearchJson[l]&&(l=this._SearchJson[l].ORI_NAME);try{$(b).find(".ewa-lf-data-row .ewa-col-"+l+">*").each(function(){0==this.children.length&&(this.innerHTML=this.innerHTML.replace(g,"<font color=red><b>$1</b></font>"))})}catch(p){console.log(p)}}}}}};
this.DblClick=function(a){this._IsDblClick=a;for(var b=$X("EWA_LF_"+this._Id),c=1;c<b.rows.length;c++)b.rows[c].ondblclick=function(){var d=this.getElementsByTagName("input");d.length>a&&0==d[a].disabled&&d[a].click()}};this.Init=function(a){this.Xml=new EWA.C.Xml;this.Xml.LoadXml(a);this.ItemList.Init(this.Xml);this.Resources=new EWA_FrameResoures;this.Resources.Init(this.Xml);if((a=$X("EWA_LF_"+this._Id))&&"table"==a.tagName&&!(1>=a.rows.length)){a=a.rows[1].getElementsByTagName("input");for(var b=
0;b<a.length;b++)if("RADIO"==a[b].type.toUpperCase()){this._TrSelectMulti=!1;break}}};this.SetPageParameters=function(a,b,c,d,e){this._PageCurrent=a;this._PageSize=c;this._PageCount=b;this._RecordCount=d;this._Sort=e};this.SetPageParametersName=function(a,b,c,d,e){this._PageCurrentName=a;this._PageSizeName=c;this._PageCountName=b;this._RecordCountName=d;this._SortName=e};this.CheckedAll=function(){var a=document.getElementById("EWA_LF_"+this._Id);if(!(1>=a.rows.length)){for(var b=0;b<a.rows.length;b+=
1){var c=a.rows[b].getElementsByTagName("input");0!=c.length&&(c[0].checked=this._IsCheckedAll)}this._IsCheckedAll=this._IsCheckedAll?!1:!0;this.CheckedAllAfter&&this.CheckedAllAfter(a)}};this.SelectChecked=function(){var a=$X("EWA_LF_"+this._Id);if(!a||1>a.rows.length)return"";var b=[],c=$F(a,"input","type","radio,checkbox");if(0<c.length)for(var d=0;d<c.length;d++)c[d].checked&&b.push(c[d].value);else for(d=0;d<a.rows.length;d++)c=a.rows[d],1==c.getAttribute("__ewa_lf_mdown")&&b.push(c.getAttribute("EWA_KEY"));
return b.join(",")};this.SelectCheckedInputs=function(){var a=[],b=$X("EWA_LF_"+this._Id);if(1>b.rows.length)return a;b=b.getElementsByTagName("input");a=[];for(var c=0;c<b.length;c++)("CHECKBOX"==b[c].type.toUpperCase()||"RADIO"==b[c].type.toUpperCase())&&b[c].checked&&a.push(b[c]);return a};this.SelectCheckedRows=function(){for(var a=[],b=$X("EWA_LF_"+this._Id),c=0;c<b.rows.length;c++){var d=b.rows[c],e=d.getElementsByTagName("input");0!=e.length&&e[0].checked&&-1==e[0].parentNode.className.indexOf("ewa-switch")&&
a.push(d)}if(0==a.length)for(c=0;c<b.rows.length;c++)d=b.rows[c],1==d.getAttribute("__ewa_lf_mdown")&&a.push(d);return a};this.Reload=function(a){this.StopAjaxAfterReload?this.StopAjaxAfterReload=!1:this.Goto(this._PageCurrent,a)};this.DoAction=function(a,b,c,d,e,g){EWA.F.CID=this._Id;if(b){if(a&&$(a).parent().hasClass("scroll-delete")){var h=$(a).attr("do-action-time")||0,k=(new Date).getTime();if(1E3>k-h)return;$(a).attr("do-action-time",k)}this._Ajax=new EWA.C.Ajax;this._Ajax.LoadingType="image";
