...
function(a,b,d,e,g,h,k){var l=this;$J(e,function(m){l.createFilterCheckboxByData(a,b,d,m,g,h,k)})};this.createFilterCheckboxByData=function(a,b,d,e,g,h,k){var l={},m=m?"[]":d.split(",");d={};for(var n in m)d[m[n]]=1;m=[];var p=[],q=a.id;for(n in e){var u=e[n],t=u[g];t&&(t=t.replace(/</ig,"&lt;"));var r=u[h];r&&(r=r.replace(/</ig,"&lt;"));var w=u[k],v=d[t],y=[];y.push("<nobr class='ewa-filter-chk' style='float:left;margin-right:4px'>");y.push('<input class="ewa-filter-chkbox" type="checkbox" ');v&&
y.push(" checked ");u=q+"_"+n;y.push(' value="'+t+'" id="'+u+'" name="'+q+'"><label ');y.push(' for="'+u+'">'+r+"</label></nobr>");v?p.push(y.join("")):m.push(y.join(""));b&&(t=w?w.substring(0,1).toUpperCase():"",l[t]||(l[t]=[]),l[t].push(u))}a.innerHTML=p.join(" ")+" "+m.join(" ");if(b){e=['<a href="javascript:void(0)">ALL</a>'];for(n="A".charCodeAt();n<="Z".charCodeAt();n++)g=String.fromCharCode(n),l[g]&&e.push('<a href="javascript:void(0)">'+g+"</a>");b.innerHTML=e.join(" ");$(b).find("a").each(function(){var x=
...
"")+1*h.css("padding-right").replace("px","");e.find(".ewa-row-"+a).hide();null==d&&(d=" - ");var q=l.width(),u=g[0].tagName,t=h[0].tagName;g.parent().hasClass("ewa-select-filter-target")&&(g=e.find(".ewa-row-"+a+" .EWA_TD_M table:eq(0)"),u="SELECT_WITH_FILTER");var r=0;b=null;d&&(l.append('<span class="mearge-span">'+d+"</span>"),k&&m.find("table:eq(0) td:eq(1)").html('<span class="mearge-span">'+d+"</span>").show(),b=l.find(".mearge-span"),r=b.width()+1+1*b.css("padding-left").replace("px","")+
1*b.css("padding-right").replace("px","")+1*b.css("margin-left").replace("px","")+1*b.css("margin-right").replace("px",""));u==t?(d=(q-r)/2-n-2,g.css("width",d),h.css("width",d),k&&m.find("table:eq(0) td:eq(2), table:eq(0) td:eq(0)").css("width",d)):"SELECT"==u?(g.css("width",100),h.css("width",q-r-n-p-2-2-g.width())):"SELECT_WITH_FILTER"==u?(g.css("width",200),h.css("width",q-r-n-p-2-2-g.width())):"SELECT"==t&&(h.css("width",100),g.css("width",q-r-n-p-2-2-g.width()));h.css("float","left");b&&b.css("float",
...
[],u=this._SearchJson[h],t=u.T,r=this._GetSubValue("DescriptionSet","Info",this.ItemList.Items[h]);q.push("<table class='ewa-lf-search-item ewa-ref-"+h+" ewa-lf-search-type-"+t+" "+("fix"==t?"ewa-lf-search-fix-m"+(u.M||"0"):"")+"' cellspacing=0 cellpadding=0>");q.push("<tr><td class='ewa-lf-search-item-title' style='position:relative'><nobr><b>");q.push(r);var w=r="",v="lk";g[h.toUpperCase()]&&(w=g[h.toUpperCase()],v=w.tag,r=w.para1,w=w.para2);var y="";"text"==t?(p=m[v]||m.lk,y="enus"==EWA.LANG?p.TxtEn:
p.Txt,q.push("<a class='ewa-lf-search-type' tag='"+v+"' href='javascript:void(0)' onclick='"+k+"'>"),q.push(y),q.push("</a>"),p=!0):"date"==t&&(q.push("<a  href='javascript:void(0)' class='fa fa-calendar-check-o'  onclick='"+l+"'></a>"),n=!0);q.push("</b></nobr></td><td class='ewa-lf-search-item-ctl'>");"text"==t?"blk"==v||"nblk"==v?(u=this._SearchSingle(h,y),u=u.replace("<input","<input disabled "),q.push(u)):q.push(this._SearchSingle(h,r)):"date"==t?q.push(this._SearchDate(h,r,w)):"number"==t?q.push(this._SearchNumber(h,
r,w)):"fix"==t&&(u="<div >"+this._SearchFix(h,u,"span",r)+"</div>",q.push(u));q.push("</td></tr></table>");"fix"==t?d.push(q.join("")):e.push(q.join(""))}if(0!=e.length+d.length){b.push('<table class="ewa-lf-search-main" id="'+a+'" cellspacing="0" cellpadding="0" style="width:100%">');b.push("<tr><td class='ewa-lf-search-des'></td><td>");0<e.length&&b.push(" "+e.join("")+" ");if(0<d.length)for(e=0;e<d.length;e++)b.push(" "+d[e]+" ");b.push("</td></tr></table>");var x=document.createElement("div");
...
(function(a,b){"object"===typeof module&&"object"===typeof module.exports?module.exports=b():b(a)})("undefined"!==typeof window?window:this,function(a){function b(m){for(var n=[{number:0,scale:1,items:[]}],p=null,q=0;q<m.length;q++){var u=m[q],t=g[u],r=e[u];n[n.length-1].items.push([u,t,r]);if(t)0==q&&t.first?n[n.length-1].number=10:t.stop?(n[n.length-1].scale=t.value||1,n.push({number:0,scale:1,items:[]}),scale=1,p=null):(q==m.length-1&&(console.log(t),n[n.length-1].number.scale=t.value),p=t);else{if(null==
r)throw"\u975e\u6570\u5b57\u6216\u5355\u4f4d\uff1a"+u;u=q==m.length-1?null:m[q+1];var w=q==m.length-1?null:g[u];w&&w.stop&&(w=null);u=n[n.length-1].number;var v=n[n.length-1].items;t=v[v.length-2];v=v[v.length-3];null==w?null==p?(p=!1,t&&0===t[2]?v&&null!=v[1]&&(p=!0):t&&t[1]&&10===t[1].value&&(p=!0),r=p?u+r:10*u+r):r=t&&t[1]&&10===t[1].value&&(null==v||0===v[2]||null!=v[1])?u+r+10:u+r:r=u+r*w.value;n[n.length-1].number=r;p=null}}return n}function d(m){if(!m)return m;if(!isNaN(m.toString()))return 1*
m.toString();var n=m.replace(/\uff0c|,| |\u5143|\u5706/ig,"").replace(/\u70b9|\u3002/ig,".").split(".");if(2<n.length)throw"\u975e\u6570\u5b57\u8868\u8fbe\u5f0f\uff1a"+m;var p=n[0].split("");m=(2==n.length?n[1]:"").split("");n=!1;var q=p[0];h[q]?(n=!0,p=p.splice(1)):k[q]&&(n=!1,p=p.splice(1));q=p[p.length-1];l[q]?p=p.splice(0,p.length-1):1<m.length&&(q=m[m.length-1],l[q]&&(m=m.splice(0,m.length-1)));q=p;for(var u=p=0,t=q.length-1;0<=t;t-=2){var r=q[t];var w=e[q[t-1]];r=w?"\u89d2"==r?1*("0."+w):"\u5206"==
r?1*("0.0"+w):"\u5398"==r?1*("0.00"+w):"\u6beb"==r?1*("0.000"+w):-1:-1;if(-1===r)break;else p+=r,u++}q=q.splice(0,q.length-2*u);q=b(q);u=0;t=1;for(r=q.length-1;0<=r;r--){w=q[r];var v=w.number;t*=w.scale;u+=v*t}p=u+p;if(0<m.length){q=1;u=0;for(t=m.length-1;0<=t;t--){r=g[m[t]];if(!r)break;q*=r.value;u++}m=1==q?m:m.splice(0,m.length-u);u=b(m);t=0;r=1;for(w=u.length-1;0<=w;w--){v=u[w];var y=v.number;r*=v.scale;t+=y*r}m=[1*("0."+t),q,m.length]}else m=0;return 0==m?p*(n?-1:1):((p+m[0])*m[1]).toFixed(0>
...
for(e=0;e<b.length;e++)t=b[e],a=t.WF_UNIT_TO,r=t.WF_UNIT_FROM,r=p.Units[r],a=p.Units[a],q=new EWAC_WfCnn,q.Create(r,a),p.Cnns[q.Id]=q,t.IsHaveLogic&&!t.IsLogicOk&&(q.IsStop=!0);for(var y in p.Units)e=p.Units[y],null==v&&0==e.CnnsToLength&&(v=e);if(null==v)e="enus"==EWA.LANG?"No startup node was found, process definition error":"\u672a\u53d1\u73b0\u542f\u52a8\u8282\u70b9,\u6d41\u7a0b\u5b9a\u4e49\u9519\u8bef",alert(e);else{0==d.length&&(p.CurUnit=v);var x=[];y={};for(e=0;e<d.length;e++)b=d[e],p=b.SYS_STA_TAG,
...
"none"),showControls(z,_EWA_WF.WF_SHOW))}}function showControls(a,b){for(var d=a.rows.length-5;0<=d;d--)0!=a.rows[d].id.indexOf("_ewa_tr$EWA_WF_")&&(a.rows[d].style.display="none");var e={},g=!1;for(d=0;d<b.length;d++)for(var h=b[d].XITEMS.split(","),k=0;k<h.length;k++){var l=h[k].trim();""==l||e[l]||(e[l]=1,g=!0)}if(g)for(var m in e)if(b=a.ownerDocument.getElementById("_ewa_tr$"+m))b.style.display=""}
