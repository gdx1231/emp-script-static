...
function EWA_UrlClass(a){this._URL="";this._Paras={};this._Root="";this.SetUrl=function(b){this._URL=b=b==null?window.location.href:b.href?b.href:b.toString();this._Paras={};var c=b.indexOf("#");c>=0&&(b=b.substring(0,c));this._Root=b.split("?")[0];if(!(b.indexOf("?")<0))for(b=b.split("?")[1].split("&"),c=0;c<b.length;c++){var d=b[c].split("=");d.length==2?this._Paras[d[0].toUpperCase()]=d[1]:this._Paras[d[0].toUpperCase()]=""}};this.RemoveEwa=function(){for(var b in this._Paras){var c=b.toLowerCase();
if(c=="xmlname"||c=="itemname"||c.indexOf("ewa_")==0)this._Paras[b]=null}return this.GetUrl()};this.GetParameter=function(b){return this._Paras[b.toUpperCase().trim()]};this.GetParas=function(b){var c=[],d;for(d in this._Paras){var e=this._Paras[d];b&&(d=d.toLowerCase());d&&e!=null&&c.push(d+"="+e)}return c.join("&")};this.SetRoot=function(b){this._Root=b};this.GetUrl=function(b){b=this.GetParas(b);var c="";b.length>0&&(c=this._Root.indexOf("?")>=0?"&":"?");return this._Root+c+b};this.AddParameter=
function(b,c,d){b=b.toUpperCase().trim();this._Paras[b]=d?c:encodeURIComponent(c);return this.GetUrl()};this.RemoveParameter=function(b){b=b.toUpperCase().trim();this._Paras[b]=null;return this.GetUrl()};this.SetUrl(a||window.location.href)}
...
0&&e.trim().length>0&&(e.indexOf("fa")>=0?c.push('<i class="'+e+'"></i>'):c.push('<img src="'+b[d].Img+'">')),c.push("</td><td class='ewa_menu_m1' style='white-space:nowrap'>"+b[d].Txt+"</td></tr></table></div>"))}c.push("</div>");this.Dialog.SetHtml(c.join(""))}};this.HiddenMemu=function(){this.Dialog.Show(!1);EWA.UI.CurMenu=null};this.ShowByObject=function(b,c,d){var e=EWA.UI.Utils.GetPosition(b);this.SHOW_BY_OBJECT=b;c=c||this.Dialog;if(c._IsFixed){var g=b.ownerDocument;g=g.parentWindow?g.parentWindow:
g.defaultView;e.X-=$(g).scrollLeft();e.Y-=$(g).scrollTop()}d==null||d==0?c.Move(e.X,e.Y):c.Move(e.X+b.offsetWidth-5,e.Y-b.offsetHeight+4);c.Show(!0);$(c.GetFrame()).find(".ewa_menu_box").attr("open_time",(new Date).getTime());c.ResizeByContent()};this.ShowByMouse=function(b,c){if(this.Dialog){var d=0,e=$X(this.Dialog.FrameContent).childNodes[0];if(c==null)d=e.childNodes.length;else for(var g=0;g<e.childNodes.length;g++)e.childNodes[g].getAttribute("EWA_MG")==c||c==null||c==""?(e.childNodes[g].style.display=
"",d++):e.childNodes[g].style.display="none";d>0?(c=document.documentElement.scrollLeft||document.body.scrollLeft,d=document.documentElement.scrollTop||document.body.scrollTop,EWA.B.IE?(e=b.x+c,b=b.y+d):(e=b.pageX,b=b.pageY),this.Dialog._IsFixed&&(e-=c,b-=d),this.Dialog.Move(e,b),this.Dialog.Show(!0),this.Dialog.ResizeByContent()):this.Dialog.Show(!1)}};this.MouseOver=function(b){this._LastObj!=null&&(this.MenuShowType=="LEFT"?this._LastObj.parentNode.className=this._LastObj.parentNode.className.replace("_mv",
""):this._LastObj.className=this._LastObj.className.replace("_mv1",""));if(this.MenuShowType=="LEFT")this._LastObj=b,this._LastObj.parentNode.className+="_mv";else{this._LastObj=b;this.IsOut=!1;this._LastObj.className+="_mv1";EWA.UI.CurMenu==null&&(EWA.UI.CurMenu=this);var c=this._CalcLevel(b);b.getAttribute("EWA_MF_CHILD")=="1"&&(this.Dialogs.length<=c&&this.Dialogs.push(this._CreateDialog()),this._ShowPullDownMenu(b,this.Dialogs[c]),this.ShowByObject(b,this.Dialogs[c],c))}};this._HiddenDialogs=
function(){this.Dialog!=null&&this.Dialog.Show(!1);for(var b=0;b<this.Dialogs.length;b++){this.Dialogs[b].Show(!1);for(var c=$X(this.Dialogs[b].FrameContent).childNodes[0].rows[0].cells[0];c.childNodes.length>0;)this._ItemsDiv.appendChild(c.childNodes[0])}};this._InitEvent=function(){this.MenuShowType!="LEFT"&&(EWA.B.IE?document.body.attachEvent("onclick",function(){typeof EWA.UI.CurMenu!="undefined"&&EWA.UI.CurMenu._HiddenDialogs()}):document.body.addEventListener("click",function(){typeof EWA.UI.CurMenu!=
"undefined"&&EWA.UI.CurMenu._HiddenDialogs()}))}}
...
e=a.getAttribute("_SCROLL_Y")*1;a.style.left=c-d+$X(this.Frame).offsetLeft+"px";a.style.top=b-e+$X(this.Frame).offsetTop+"px";a.setAttribute("_SCROLL_X",c);a.setAttribute("_SCROLL_Y",b)}};this.MoveBottom=function(a){var b=$(a).offset();b.Y=b.top+$(a).height();b.X=b.left;var c=a.ownerDocument,d=c.parentWindow?c.parentWindow:c.defaultView;this._IsFixed&&(b.X-=$(d).scrollLeft(),b.Y-=$(d).scrollTop());if(d==this.CreateWindow){var e=document.body.clientWidth;c=document.body.clientHeight;var g=b.X;d=b.Y;
var h=(this.Height+"").replace("px","")*1,k=(this.Width+"").replace("px","")*1;b.Y+a.clientHeight+h>c&&(d=b.Y-a.clientHeight-h);b.X+k>e&&(g=e-k-a.clientWidth);EWA.B.PAD&&(g=0);this.Move(g,d+4)}else g=d._EWA_DialogWnd._Dialog.GetFrame(),e=g.offsetLeft,c=g.offsetTop,h=this.GetFrame(),h.setAttribute("_SCROLL_X",g.getAttribute("_SCROLL_X")),h.setAttribute("_SCROLL_Y",g.getAttribute("_SCROLL_Y")),a=$(a).height(),g=b.X+e,EWA.B.PAD&&(g=0),e=d._EWA_DialogWnd._Dialog.GetFrameTitle().clientHeight,this.Move(g,
b.Y+c+a+e)};this.Move=function(a,b){var c=this.GetFrame();c!=null&&EWA.UI.Utils.MoveTo(c,a,b<0?0:b)};this.MoveCenter=function(){var a=this.GetFrame(),b=a.clientWidth,c=a.clientHeight,d=EWA.UI.Utils.GetDocSize(this.CreateWindow);c=(d.H-c)/2-40;b=(d.W-b)/2;b<0&&(b=0);c<0&&(c=0);var e=this._IsFixed?0:d.SH;d=this._IsFixed?0:d.SW;a.setAttribute("_SCROLL_X",d);a.setAttribute("_SCROLL_Y",e);this.Move(b+d,c+e)};this.ResizeByContent=function(){var a=this.GetFrame(),b=this.GetFrameContent().childNodes[0],c=
!1;if(b&&b.tagName&&b.tagName.toUpperCase()=="IFRAME"){c=!0;var d=b.contentWindow;d&&d.document&&d.document.body&&(d=d.document.getElementById("Test1"))&&(b=d)}d=this.GetFrameContent().parentNode.parentNode.parentNode;var e=$(d).css("border-left-width").replace("px","")*1+$(d).css("border-right-width").replace("px","")*1;var g=$(d).css("border-top-width").replace("px","")*1+$(d).css("border-bottom-width").replace("px","")*1;this._calc_frame_table=e;d=$(b).width();d+=e;b=$(b).height()+g;e=a.style.left.replace("px",
"")*1;a=a.style.top.replace("px","")*1;d+e>this.CreateWindow.document.body.clientWidth&&this.Move(this.CreateWindow.document.body.clientWidth-d-20,a);this._IsHeightCapped=!1;a=EWA.UI.Utils.GetDocSize(this.CreateWindow).H-10;this.IsShowTitle&&(a-=$(this.GetFrameTitle()).parent().height());b>a&&(b=a,this._IsHeightCapped=!0);c?this.GetFrameContent().style.overflow="hidden":this.GetFrameContent().style.overflow=this._IsHeightCapped?"auto":"";this.Resize(d,b,!0)};this.Resize=function(a,b,c){if(this.IsShowTitle){var d=
this.GetFrameTitle();b+=$(d).parent().height()}var e=d=0;c||(c=this.GetFrameContent().parentNode.parentNode.parentNode,this._calc_frame_table=d=$(c).css("border-left-width").replace("px","")*1+$(c).css("border-right-width").replace("px","")*1,e=$(c).css("border-top-width").replace("px","")*1+$(c).css("border-bottom-width").replace("px","")*1);a+=d;b+=e;c=this.GetFrame();c.style.width=a+"px";c.style.height=b+"px";for(d=0;d<c.childNodes.length;d+=1)e=c.childNodes[d],e.style.width=a+"px",e.style.height=
b+"px",e.id.indexOf("divContent")>0&&(e.style.overflow="hidden");this.GetFrameContent();a=this.GetObject(this._DivCover);$(a).css("background-image","none");$(a).attr("background-image","none")};this.Show=function(a){var b=this.GetFrame();a?(b.style.display="",this.IsShowTitle&&this.AutoSize&&(_EWA_DIALOG_ON_TIMER=this,this.TimerHandle=window.setInterval(function(){var c=_EWA_DIALOG_ON_TIMER;window.clearInterval(c.TimerHandle);var d=c.GetFrame(),e=c.GetFrameContent(),g=e.childNodes[0].offsetWidth,
h=c.GetObject(c._FrameFore).offsetHeight;d.style.width=g+"px";d.style.height=h+"px";c.GetObject(c._FrameBack1)&&(c.GetObject(c._FrameBack1).style.width=g+"px",c.GetObject(c._FrameBack1).style.height=h+"px");c.GetObject(c._FrameBack2)&&(c.GetObject(c._FrameBack2).style.width=g+"px",c.GetObject(c._FrameBack2).style.height=h+3+"px");if(c=e.childNodes[0])c.style.width=g-2+"px",c.style.height=h-2+"px";c=_EWA_DIALOG_ON_TIMER=null;g=c.GetObject(this._DivCover);$(g).css("background-image","none");$(g).attr("background-image",
"none")},32))):b.style.display="none";b=oCover=null};this.Dispose=function(){this.CreateWindow&&(this.CreateWindow._EWA_DIALOGS[this._CreateWindowIndex]=null,_EWA_DIALOGS[this._WindowIndex]=null,this._DivCover!=null&&(EWA$UI$COMMON.Drop(this.GetObject(this._DivCover)),this._DivCover=null),this.GetObject(this.Frame).innerHTML="",EWA$UI$COMMON.Drop(this.GetObject(this.Frame)),this.CreateWindow=null,EWA.B.IE&&CollectGarbage())};this.SetZIndex=function(a){this.CreateWindow._EWA_UI_DIALOG_COVERINDEX+=
a;this.zIndex=this.CreateWindow._EWA_UI_DIALOG_COVERINDEX;this.GetObject(this.Frame).style.zIndex=this.zIndex};this.CreateId=function(){var a=Math.random();return(new Date).getTime()+"_"+a};this.Create=function(){this.CreateWindow==null&&(this.CreateWindow=window);this.IsCover&&this._CreateCover();var a=this.CreateId(),b=this.CreateWindow;b._EWA_UI_DIALOG_COVERINDEX==null&&(b._EWA_UI_DIALOG_COVERINDEX=0);b._EWA_UI_DIALOG_COVERINDEX+=1;this.zIndex=b._EWA_UI_DIALOG_COVERINDEX;this._IsFixed=b.EWA_UI_DIALOG_FIXED||
window.EWA_UI_DIALOG_FIXED?!0:!1;var c=EWA$UI$COMMON.CreateObject(b,"div","display:none; "+(this._IsFixed?"position:fixed":"position:absolute")+"; width:"+this.Width+"; height:"+this.Height+"; z-index:"+this.zIndex+"; left:"+this.Left+"; top:"+this.Top+";",b.document.body);c.setAttribute("EWA_NAME","DIV_FRAME");c.className="ewa-dialog";c.id=a+"divFrame";this.Frame=c.id;var d=EWA$UI$COMMON.CreateObject(b,"div","position: absolute; width: 100%; height: 100%; left: 0px; top: 0px;",c);d.setAttribute("EWA_NAME",
"DIV_BACK1");d.id=a+"divBack1";this._FrameBack1=d.id;if(this.ShadowColor!=null&&this.ShadowColor!=""){var e=EWA$UI$COMMON.CreateObject(b,"div","position: absolute; background-color:"+this.ShadowColor+"; width:100%; height:100%; left:"+this.Offset+"px; top: "+(this.Offset*1+2)+"px;",c);e.style.filter="alpha(opacity=50)";e.style.opacity=.5;e.id=a+"divBack2";this._FrameBack2=e.id}EWA.B.IE&&(d.innerHTML="<iframe src='javascript:false' style=\"position:absolute;top:0px;left:0px;width:100%; height:100%; z-index:-1; filter='progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0)';\"></iframe>",
...
null&&(this._ParentWindow._EWA_UI_WINDOW_LIST={});this._ParentWindow._EWA_UI_WINDOW_LIST[this.Id]=this};this._CreateDialog=function(c,d){this._Dialog.Id=this.Id;this._Dialog.Width=c;this._Dialog.Height=d;this._Dialog.CreateWindow=this._ParentWindow;this._Dialog.IsShowTitle=!0;this._Dialog.IsCanMove=!0;this._Dialog.IsCover=!0;this._Dialog.Create();this._Dialog.Show(!0);this.SetHtml("<iframe style='width:100%;height:100%;display:block;' name='__EMP_COMMON_IFRAME"+this._Name+"' frameborder='0'></iframe>")};
this.SetHtml=function(c){this._Dialog.SetHtml(c)};this.ResizeByContent=function(){this._Dialog.ResizeByContent()}}EWA.UI.DialogOnScroll=function(){if(typeof _EWA_DIALOGS!="undefined")for(var a=0;a<_EWA_DIALOGS.length;a+=1)if(_EWA_DIALOGS[a]!=null&&!_EWA_DIALOGS[a]._IsFixed)try{_EWA_DIALOGS[a].ScrollMoveDiv()}catch(b){_EWA_DIALOGS[a]=null}};
...
b.MoveCenter();b.Show(!0);return b},_GetTopWindow:function(){for(var a=window;a.parent!=a;)a=a.parent;return a.document.documentElement.innerHTML.toUpperCase().indexOf("<FRAMESET")>=0?window:a},_DialogMointer:function(){var a=EWA.UI.Dialog.WNDCUR;__Dialog.TimerInc++;if(!(a==null&&__Dialog.TimerInc>100)&&a!=null){var b=a.GetIframeWindow();b==null?(window.clearInterval(a._TimerHandle),a=EWA.UI.Dialog.WNDCUR=null):b.location.href!=a._Url&&(b.document.readyState&&b.document.readyState=="complete"?(window.clearInterval(a._TimerHandle),
setTimeout(function(){EWA.UI.Dialog._SetTitle(a)},111)):b.document.body!=null&&(b=b.document.documentElement.innerHTML,b=b.toLowerCase(),b.indexOf("</body>")>0||b.indexOf("</html>")>0)&&(a._IncNoIe==0?a._IncNoIe=1:EWA.UI.Dialog._SetTitle(a)))}},_SetTitle:function(a){if(a!=null){var b=a.GetIframeWindow();b._EWA_DialogWnd=a;a._Url=b.location.href;a.SetCaption(b.document.title);b.document.getElementsByTagName("INPUT");if(a._PageHideHandler)try{b.removeEventListener("pagehide",a._PageHideHandler)}catch(k){}a._PageHideHandler=
function(){try{a._Url=""}catch(k){}};b.addEventListener&&b.addEventListener("pagehide",a._PageHideHandler);var c=__Dialog._SetInputFocus(b.document);c||__Dialog._SetTextAreaFocus(b.document);c||b.document.body.focus();__Dialog._SetClose(b.document);b.document.body.style.margin="0px";b.document.body.style.overflow="hidden";c=b.contentWindow;var d=!1,e=function(){if(a!=null&&a._Dialog!=null)try{a._Dialog.ResizeByContent(),a._Dialog._IsHeightCapped&&b.document.body&&!b.document.body.querySelector(".ewa-in-dialog")&&
$(b.document.body).css("overflow","hidden auto"),a.MoveCenter()}catch(k){}},g=function(){if(!d&&a!=null&&a._Dialog!=null){d=!0;try{var k=a._Dialog.GetFrame();k&&(k.style.visibility="")}catch(l){}}};try{var h=a._Dialog.GetFrame();h&&(h.style.visibility="hidden")}catch(k){}setTimeout(function(){e();a!=null&&a._Dialog!=null&&a._Dialog._IsHeightCapped||g()},111);c&&c.document&&c.document.fonts&&c.document.fonts.ready&&c.document.fonts.ready.then(function(){setTimeout(function(){e();g()},60)});setTimeout(function(){e();
g()},600);a._CallBack!=null&&a._CallBack.Run()}},_SetClose:function(a){for(var b=a.getElementsByTagName("INPUT"),c=0;c<b.length;c+=1){var d=b[c].type.toLowerCase();d!="button"&&d!="submit"||__Dialog._SetCloseFunction(b[c])}b=a.getElementsByTagName("BUTTON");for(c=0;c<b.length;c++)__Dialog._SetCloseFunction(b[c])},_SetCloseFunction:function(a){var b=a.value.trim().toLowerCase();a.tagName.toLowerCase()=="button"&&(b=a.innerHTML);!(b.indexOf("\u5173\u95ed")>=0||b.indexOf("\u53d6\u6d88")>=0||b.indexOf("close")>=
0||b.indexOf("cancel")>=0)||a.onclick!=null&&a.onclick.toString().indexOf("EWA")>=0||(a.onclick=function(){var c=this.ownerDocument;(c.parentWindow?c.parentWindow:c.defaultView)._EWA_DialogWnd.CloseWindow()})},_SetInputFocus:function(a){a=a.getElementsByTagName("INPUT");for(var b=0;b<a.length;b+=1){var c=a[b],d=c.type.toLowerCase();if(d=="text"||d=="password")if(d=c.parentNode.innerHTML,!(d!=null&&d.toString().indexOf("EWA.F.I.Date")>=0))try{return c.focus(),a=null,!0}catch(e){}}return!1},_SetTextAreaFocus:function(a){a=
a.getElementsByTagName("TEXTAREA");for(var b=0;b<a.length;b+=1)try{return a[b].focus(),a=null,!0}catch(c){}return!1},OpenReloadClose:function(a,b,c,d,e,g,h){EWA.UI.Dialog.OpenFrame(a,b,c,d,e,"RELOAD_PARENT,CLOSE_SELF",g,h)},OpenReload:function(a,b,c,d,e,g,h){EWA.UI.Dialog.OpenFrame(a,b,c,d,e,"RELOAD_PARENT",g,h)},OpenReloadClear:function(a,b,c,d,e,g,h){EWA.UI.Dialog.OpenFrame(a,b,c,d,e,"RELOAD_PARENT,CLEAR_SELF",g,h)},OpenFrame:function(a,b,c,d,e,g,h,k){g==null&&(g="");__Dialog[a]||(__Dialog[a]={});
__Dialog[a].AfterMsg=h==null?"":h;if(k=="yes"&&EWA.F&&EWA.F.FOS[a]&&EWA.F.FOS[a].Url){k=new EWA_UrlClass(EWA.F.FOS[a].Url);k.RemoveEwa();e=new EWA_UrlClass("a1?"+e);for(var l in e._Paras)h=e.GetParameter(l),k.AddParameter(l,h);e=k.GetParas()}g="EWA_P_BEHAVIOR="+g+"&EWA_PARENT_FRAME="+a;a=EWA.UI.Dialog.GetUrl(b,c,e?e+"&"+g:g);EWA.UI.Dialog.OpenWindow(a,b+"-"+c,100,50,d)},OpenWindow:function(a,b,c,d,e,g,h,k){var l=k===void 0?!0:k;var m=e?window:EWA.UI.Dialog._GetTopWindow();e=a;e!=null&&e!="about:blank"&&
(e=a.indexOf("?")>0?e+("&_r="+Math.random()):e+("?_r="+Math.random()));e=e.replace(/\|/ig,"%7c");a="_EWA_UI_DIALOG_"+b;EWA.UI.Dialog.WND[a]=null;EWA.UI.Dialog.WND[a]=new EWA.UI.Dialog.C(m,l);l=EWA.UI.Dialog.WND[a];l.Id=a;l.DisposeOnClose=k;l.Create(c,d,b);e!=null&&l.SetUrl(e);g!=null&&l.SetCallBack(g);h!=null&&l.SetReturnBack(h);__Dialog.TimerInc=0;l._TimerHandle=window.setInterval(EWA.UI.Dialog._DialogMointer,155);l._IncNoIe=0;EWA.UI.Dialog.WNDCUR=l;return EWA.UI.Dialog.WND[a]},GetUrl:function(a,
b,c){return EWA.CP+"/EWA_STYLE/cgi-bin/?XMLNAME="+a.toURL()+"&ITEMNAME="+b.toURL()+"&"+c}};EWA.OW={Dia:null,PWin:null,Frame:null,Close:function(){EWA.OW.Dia?EWA.OW.Dia.CloseWindow():alert("Dialog not Load")},Load:function(){EWA.OW.Dia=window._EWA_DialogWnd;EWA.OW.PWin=EWA.OW.Dia?EWA.OW.Dia._OpenerWindow:null;EWA.OW.Frame=EWA.OW.PWin?EWA.OW.PWin.EWA.F.FOS[EWA.OW.PWin.EWA.F.CID]:null}};EWA.UI.Dialog=__Dialog;
...
1):(r.addClass("ewa-row-merge-target"),r.removeClass("ewa-row-must"));b[n]=this.GetItemDescription(n).Info}e=document.createElement("div");e.style.display="none";e.innerHTML=h;document.body.appendChild(e);m=k.find('[id="'+a+'"]');if(m.length==0)console.log("not find "+a);else{a=m.parent();for(var p in g)if(m=g[p].replace("@@",""),h=k.find('[id="'+m+'"]'),h.length==0)console.log("not found [id="+m+"]");else if(l=$(e).find('span[mid="'+m+'"]'),"placeholder"==c?h.attr("placeholder",b[m]):c&&(l.append("<span class='ewa-row-merge-memo'></span>"),
l.find(".ewa-row-merge-memo").text(b[m])),m=k.find(".ewa-row-"+m+" .EWA_TD_M"),m.length==0)l.append(h);else for(;m[0].childNodes.length>0;)l.append(m[0].childNodes[0]);for(;e.childNodes.length>0;)a.append(e.childNodes[0]);$(e).remove();d&&d(a)}}else console.log("mergeExp \u6ca1\u6709\u8bbe\u7f6e")};this.MergeItems=function(){for(var a in this.MeargeMap){var b=this.MeargeMap[a],c=$X("_ewa_tr$"+a);if(c){for(var d=c.cells.length,e=$(c.cells[0]);c.cells.length>1;)e.append(c.cells[1].innerHTML),c.removeChild(c.cells[1]);
...
"hidden");l.ewa_trigger_valid_name=d;l.ewa_trigger_valid=g;l.ewa_url=e.GetUrl();EWA.UI.SlidePuzzle(l,top.$("#"+k),function(n){console.log("2. puzzle cb true");let p=!1;c.attr("_onclick")&&(c.attr("onclick",$(a).attr("_onclick")),p=!0);setTimeout(function(){$(m.getMain()).animate({opacity:0},500)},500);setTimeout(function(){let r=c.attr("type");h.triggerValids[d]=n.VALID;let q=$(a).attr("ewa_trigger_valid_click_id");q?(console.log(q+".click"),b.find("#"+q).click()):p?(console.log("3. puzzle click"),
...
!1;this.CheckedAllAfter=this.ReloadAfter=null;this.REPLACE_HISTORY_STATE=this._isStickyHeaders=this.IsNotMDownAutoChecked=!1;this._IsSearchGroup=!0;this.getObj=function(a){return a?$("#EWA_LF_"+this._Id).find(a):$("#EWA_LF_"+this._Id)};this.mergeButtonsInRow=function(a){const b=[],c={};var d=this.getObj("tr:eq(0)"),e;for(e in this.ItemList.Items){var g=this.ItemList.Items[e],h=this._GetSubValue("Tag","Tag",g);if(h&&(h.toLowerCase()=="button"||h.toLowerCase()=="butflow")&&(h=this._GetSubValue("Name",
"Name",g),d.find(".ewa-col-"+h).length!==0)){var k=this._GetSubValue("DescriptionSet","Info",g),l=this._GetSubValue("DescriptionSet","Memo",g),m=this._GetSubValue("EventSet","EventName",g);m!=null&&m.toLowerCase().trim()=="ewa_click"&&(g=this._GetSubValue("EventSet","EventValue",g),g=$("<input type=button>").attr("onclick",g.replace("event","this")).addClass("IX_BUTTON").attr("value",k).attr("rid",e),l&&g.attr("title",l),b.push(g),c[h]=!0)}}if(this.getObj(".ewa-col-funcs").length==0){d=this.getObj()[0];
for(e=0;e<d.rows.length;e++)h=$(d.rows[e].insertCell(-1)),e==0?(h.addClass("EWA_TD_H ewa-col-funcs").text(a),lastCellIndex=h[0].cellIndex):h.addClass("EWA_TD_M ewa-col-funcs");this.getObj(".ewa-lf-data-row").each(function(){let n=$(this).find(".ewa-col-funcs");$(this).find("input[type=button]").each(function(){c[this.id]||n.append(this)})});this.getObj().parent().css("overflow-x","auto").css("width","100%")}a=$("#_G_"+this._Id+" .ewa-lf-search");if(!a.attr("set_new_buttons"))for(a.attr("set_new_buttons",
"yes"),d=0;d<b.length;d++)a.append(b[d])};this.changeTag=function(a){a=new EWA_UrlClass("fake?"+a);let b=new EWA_UrlClass(this.Url);for(let c in a._Paras)b.AddParameter(c,a._Paras[c]);this.Url=b.GetUrl();this.Reload()};this._InitMneu=function(){var a=window["_EWA_MENU_"+this._Id];if(a&&a.length!=0){var b=[];for(let c=0;c<a.length;c++){let d=a[c],e=(c==0?"ewa-lf-menu-marked":"")+(c==a.length-1?"ewa-lf-menu-last":""),g=d.Cmd.replace(/"/g,"&quot;");b.push(`<a class="ewa-lf-menu ${e}" onclick="${"$(this).parent().find('.ewa-lf-menu-marked').removeClass('ewa-lf-menu-marked');$(this).addClass('ewa-lf-menu-marked');"} ${g}">${d.Txt}</a>`)}a=
b.join('<b class="ewa-lf-menu-span"></b >');$("#EWA_RESHOW_"+this._Id).find(".ewa_lf_func_caption").after(a)}};this.stickyHeaders=function(){if(this._isStickyColumns)console.warn("stickyColumns is enabled, stickyHeaders will not work properly.");else{this._isStickyHeaders=!0;var a=$("#EWA_RESHOW_"+this._Id),b=EWA.UI.findScrollParent(a[0]);if(b){var c=a.find(".ewa_lf_func").parent();c.length!=1||c.hasClass("ewa-lf-sticky-func")||c.css("position","sticky").css("top",c.offset().top).addClass("ewa-lf-sticky-func");
c=a.find(".ewa-lf-search");c.length!=1||c.hasClass("ewa-lf-sticky-search")||c.css("position","sticky").css("top",c.offset().top).addClass("ewa-lf-sticky-search");a=a.find(".ewa-lf-header");a.length!=1||a.hasClass("ewa-lf-sticky-header")||a.css("position","sticky").css("top",a.offset().top).addClass("ewa-lf-sticky-header");a="scroll-ref-"+this._Id;if(!$(b).attr(a)){c=b.tagName=="BODY"?window:b;const d=b.tagName=="BODY"?document.documentElement:b;$(b).attr(a,1);$(c).on("scroll",function(){d.scrollTop===
0?$(b).removeClass("ewa-lf-sticky-scroll-vertical"):$(b).hasClass("ewa-lf-sticky-scroll-vertical")||$(b).addClass("ewa-lf-sticky-scroll-vertical")});$(c).trigger("scroll")}}}};this.stickyColumns=function(a){if(this._isStickyHeaders)console.warn("stickyHeaders is enabled, stickyColumns will not work properly.");else{this._isStickyColumns=!0;var b=$X("EWA_LF_"+this._Id);if(b==null)console.log("not found table[#EWA_LF_"+this._Id+"]");else if(b.rows.length!=0){for(let d=0;d<a;d++)this.stickyColumn(d,
d==a-1,b);var c=b.parentNode;a="scroll-ref-hor-"+this._Id;if(!$(c).attr(a)){b=c.tagName=="BODY"?window:c;const d=c.tagName=="BODY"?document.documentElement:c;$(c).attr(a,1);$(c).addClass("ewa-lf-sticky-scroll");$(b).on("scroll",function(){d.scrollLeft===0?$(c).addClass("ewa-lf-sticky-last-hide"):$(c).hasClass("ewa-lf-sticky-last-hide")&&$(c).removeClass("ewa-lf-sticky-last-hide")});$(b).trigger("scroll")}}}};this.stickyColumn=function(a,b,c){c=c||$X("EWA_LF_"+this._Id);if(c==null)console.log("not found table[#EWA_LF_"+
this._Id+"]");else if(c.rows.length!=0){var d=c.rows[0].cells[a];if(d){var e=$(d).position().left;d=$(d).width();e={left:e,width:d};for(d=0;d<c.rows.length;d++){let g=$(c.rows[d].cells[a]);g.css(e).addClass("ewa-lf-sticky");b&&g.addClass("ewa-lf-sticky-last").css("overflow","initial")}}}};this.composeSearchTexts=function(){var a=[],b=[],c=$("#EWA_RESHOW_"+this._Id+" .ewa-lf-search-type-text");if(c.length==0)return null;c.each(function(g){var h=$(this).find(".ewa-lf-search-item-title").text();h=h.replace(":",
"").replace("\uff1a","").replace("\u5305\u542b","").replace("Like","").trim();var k=$(this).find("input[type=text]").attr("name");a.push(h);b.push(k);g>0&&$(this).hide()});let d=$(c[0]);d.addClass("ewa-app-lf-search-compose");d.find("input[type=text]").attr("name",b.join(",")).attr("placeholder",a.join(", "));let e=d.find("a").clone();d.find(".ewa-lf-search-item-title b").text(EWA.LANG=="enus"?"Search":"\u7efc\u5408\u641c\u7d22").append(e);return c};this.switchButtonAction=function(a,b){if(b){var c=
this.getUrlClass();c.AddParameter("ewa_switch_name",a.name);c.AddParameter("ewa_action",b);c.AddParameter("ewa_ajax","json");c.AddParameter("ewa_switch",a.checked?"on":"off");c.AddParameter("ewa_action_key",$(a).parentsUntil("tbody").last().attr("ewa_key"));var d=a.parentNode,e=d.getAttributeNames();b={};for(let h in e){let k=e[h],l=d.getAttribute(k);k.indexOf("on")!=0&&k!="name"&&k!="id"&&k!="class"&&(b[k]=l)}var g=this;c=c.GetUrl();$JP(c,b,function(h){g.extSwitchCallBack&&g.extSwitchCallBack(a,
h)})}};this.ShowRecycle=function(){var a=$X("EWA_LF_"+this._Id);if(a!=null&&(a=$(a).parentsUntil("table").last().find(".ewa_lf_func_caption"),a.length!=0)){var b="\u56de\u6536\u7ad9";window._EWA_INFO_MSG&&window._EWA_INFO_MSG.RECYCLE&&(b=window._EWA_INFO_MSG.RECYCLE);b="<div class='ewa_lf_func_dact ewa-lf-func-recycle' onclick='EWA.F.FOS[\""+this._Id+"\"].ShowRecycle1(this)' ><b class='fa fa-recycle'></b>"+b+"</div>";a.parent().append(b)}};this.ShowRecycle1=function(a){var b=new EWA_UrlClass(this.Url);
$(a).attr("recycle")?(b=b.RemoveParameter("ewa_recycle"),$(a).attr("recycle",null).removeClass("ewa-lf-func-recycle-yes")):(b=b.AddParameter("ewa_recycle","1"),$(a).attr("recycle",1).addClass("ewa-lf-func-recycle-yes"));this.Url=b;this.Goto(1)};this.BindButton=function(a,b){$("#EWA_LF_"+this._Id+' tr[ewa_key] [id="'+a+'"]').each(function(){$(this).attr("onclick","$(this).parent().parent().find('[id=\""+b+"\"]')[0].click()");$(this).addClass("ewa-map-button")})};this.Merges=function(a){if(a==null||
a.length==0)console.log("cfg.from, cfg.to, cfg.str, cfg.func, cfg.header");else for(let b=0;b<a.length;b++){let c=a[b];this.Merge(c.from,c.to,c.str,c.func,c.header)}};this.Merge=function(a,b,c,d,e){let g=$("#EWA_LF_"+this._Id);c==null?c="":c.indexOf("<")==-1&&(c="<span class='ewa-merge-str ewa-mearge-str'>"+c+"</span>");g.find("tr[ewa_key],tr.ewa-lf-sub-tr").each(function(){let h=$(this).find('[id="'+b+'"]').parentsUntil("tr").last();if(h.attr("ewa-merged")!="yes"){h.attr("ewa-merged","yes");var k=
$(this).find('[id="'+a+'"]');k.parentsUntil("tr").last().hide().addClass("ewa-row-merge-hide");h.append(c);h.append(k);d&&d(h,this)}});e&&(e="<span class='ewa-lf-merge-header-split'></span>"+g.find('tr[ewa_tag="HEADER"] [id="'+a+'"]').html(),g.find('tr[ewa_tag="HEADER"] [id="'+b+'"]').append(e));g.find('tr[ewa_tag="HEADER"] [id="'+a+'"]').parent().hide();g.find("td#ADD_ROW_"+a).hide()};this.Mearge=function(a,b,c){console.log("\u62fc\u5199\u9519\u8bef\uff0c\u8bf7\u7528 Merge");this.Merge(a,b,c)};this.createMergeExpParameters=
function(a){const b=a.match(/@(@)?[a-zA-Z_][a-zA-Z0-9_]*/g)||[],c={};try{const d={};$(`<div>${a}</div>`).find("*").addBack().each(function(){$.each(this.attributes,function(e,g){d[g.name]=g.value})});b.forEach(e=>{const g=e.startsWith("@@")?e.slice(2):e.slice(1);let h=!1,k="";for(const [l,m]of Object.entries(d))if(m.includes(e)){h=!0;k=l;break}c[e]={id:g,isAttr:h,attrName:k}})}catch(d){b.forEach(e=>{const g=e.startsWith("@@")?e.slice(2):e.slice(1);c[e]={id:g,isAttr:!1,attrName:""}})}return c};this.MergeExp=
function(a,b,c,d,e){if(b){var g=$("#EWA_LF_"+this._Id),h=this.createMergeExpParameters(b),k=b,l={};b=[];for(const n in h){var m=h[n];const p=m.id;m.isAttr||(k=k.replace(n,"<span class='ewa-lf-merge ewa-lf-merge-"+p+" ewa-lf-mearge ewa-lf-mearge-"+p+"' mid=\""+p+'"></span>'));p!=a&&(g.find('tr[ewa_tag="HEADER"] [id="'+p+'"]').parent().hide(),g.find("td#ADD_ROW_"+p).hide());m=g.find('tr[ewa_tag="HEADER"] [id="'+p+'"]').html();l[p]=m;b.push(m)}e&&(e=b.join("<span class='ewa-lf-merge-header-split'></span>"),
g.find('tr[ewa_tag="HEADER"] [id="'+a+'"]').html(e));g.find("tr[ewa_key],tr.ewa-lf-sub-tr").each(function(){if($(this).find('[id="'+a+'"]').length==0)console.log("not find "+a);else{var n=$(this).hasClass("ewa-lf-sub-tr")?$(this).find('[id="ADD_ROW_'+a+'"]'):$(this).find(".ewa-col-"+a);if("yes"!=n.attr("ewa-merged")){var p=$('<div style="display:none"></div>');p.html(k);for(const t in h){var r=h[t];const v=r.isAttr,u=r.id;var q=$(this).find('[id="'+u+'"]');if(q.length!=0)if(u!=a&&q.parent().hide().addClass("ewa-row-merge-hide"),
v){const w=r.attrName;p.find("["+w+"='"+t+"']").each(function(){$(this).attr(w)===t&&$(this).attr(w,q.text().trim())})}else r=p.find('span[mid="'+u+'"]'),c&&(r.append("<span class='ewa-lf-mearge-memo'></span>"),r.find(".ewa-lf-mearge-memo").html(l[u])),r.append(q)}for(;p[0].childNodes.length>0;)n.append(p[0].childNodes[0]);$(p).remove();d&&d(n,this);n.attr("ewa-merged","yes")}}})}else console.log("mergeExp \u6ca1\u6709\u8bbe\u7f6e")};this.MeargeExp=function(a,b,c,d){console.log("\u62fc\u5199\u9519\u8bef\uff0c\u8bf7\u7528 MereExp");
this.MergeExp(a,b,c,d)};this.SubBottoms=function(a){this.SubBottomsArray=a.split(",");this._SubBottoms()};this.reCalcBottoms=function(){this._SubBottoms()};this._SubBottoms=function(){let a=$("#EWA_LF_"+this._Id).find(".ewa-lf-sub-tr");a.length==0?(a=$(this.AddRow([])),a.addClass("ewa-lf-sub-tr"),a.find("td").addClass("ewa-lf-sub-td")):a.find("td").text("");for(let b in this.SubBottomsArray){let c=this.SubBottomsArray[b],d=0,e=0;$("#EWA_LF_"+this._Id+' .ewa-lf-data-row [name="'+c+'"]').each(function(){let h;
h=this.hasAttribute("value")?this.value:this.hasAttribute("title")?this.title:this.innerText;h!=null&&h.length!==0&&(h=h.replace(/,/ig,""),isNaN(h)||(d+=h*1,h.indexOf(".")>0&&(e=2)))});let g="<span id='"+c+"' name='"+c+"' class='ewa-lf-sub'>"+d.fm(e)+"</span>";a.find("td[id=ADD_ROW_"+c+"]").html(g)}};this.ChangeRowStyle=function(a,b,c){for(var d=$X("EWA_LF_"+this._Id),e=1;e<d.rows.length;e++){var g=d.rows[e];if(!(g.cells.length==0||g.cells.length<=a)){var h=g.cells[a].childNodes[0].getAttribute(b);
if(h!=null&&c[h])for(var k=0;k<g.cells.length;k++)g.cells[k].style.cssText=g.cells[k].style.cssText+";"+c[h]}}if(this.ReloadAfter==null){var l=this;this.ReloadAfter=function(){l.ChangeRowStyle(a,b,c)}}};this.MDownEvent=function(a,b){};this.LoadJson=function(a,b){a!=null&&$J(this.Url+"&EWA_ACTION="+a+"&EWA_AJAX=JSON",b)};this.Workflow=function(a,b,c,d,e,g){EWA.F.CID=this._Id;this._Ajax=this.CreateAjax();this._Ajax.AddParameter("EWA_AJAX","WORKFLOW");this._Ajax.AddParameter("EWA_ID",this._Id);this._Ajax.AddParameter("EWA_WF_NAME",
b);c!=null?(a=this.GetRowKey(c),this._Ajax.AddParameter("EWA_ACTION_KEY",a),c.getAttribute("ewa_wf_control")=="true"?this._Ajax.AddParameter("EWA_WF_CTRL",1):this._Ajax.AddParameter("EWA_WF_CTRL",0)):this._Ajax.AddParameter("EWA_WF_CTRL",1);d!=null&&this._Ajax.AddParameter("EWA_ACTION_KEY",d);e&&this._Ajax.AddParameter("EWA_WF_UOK",e);g&&this._Ajax.AddParameter("EWA_WF_UMSG",g);var h=this;this._Ajax.PostNew(this.Url,function(){h._CallBackJs()})};this.GetRowKey=function(a){a=this.GetRow(a);return a!=
null?a.getAttribute("EWA_KEY"):null};this.GetRow=function(a){if(a==null||a.tagName==null||a.tagName=="")return null;var b=0;for(a=a.parentNode;a.tagName!="TR"||a.getAttribute("EWA_KEY")==null;){a=a.parentNode;if(a.tagName=="BODY"){a=null;break}b++;if(b>100){alert("\u5faa\u73af100\u6b21\uff0c\u672a\u53d1\u73b0TR");break}}return a};this.RecordModify=function(a,b,c){var d=this.SelectChecked();if(d.length==0)EWA.UI.Msg.Alter("\u8bf7\u5148\u9009\u62e9","\u4fee\u6539");else{var e="EWA_MTYPE=M";c!=null&&
c.trim().length>0&&(e+="&"+c);EWA.UI.Dialog.OpenReloadClose(this._Id,a,b,!1,e+d)}};this.RecordNew=function(a,b,c){var d="EWA_MTYPE=N";c!=null&&c.trim().length>0&&(d+="&"+c);EWA.UI.Dialog.OpenReloadClose(this._Id,a,b,!1,d)};this.SelectSingle=function(){this.IsTrSelect=!0;this._TrSelectMulti=!1};this.SelectMulti=function(){this._TrSelectMulti=this.IsTrSelect=!0};this.MOver=function(a,b){if(this.IsTrSelect&&this._CurTr!=a&&(this._CurTr!=null&&(this._CurTr.getAttribute("__ewa_lf_mdown")!="1"?this._MSetBg(this._CurTr,
""):this._MSetBg(this._CurTr,"down")),a.getAttribute("__ewa_lf_mdown")!="1"?this._MSetBg(a,"over"):this._MSetBg(a,"down"),this._CurTr=a,this.MOverEvent))try{this.MOverEvent(a,b)}catch(c){console.log(c,this.MOverEvent)}};this.MOut=function(a){if(this.IsTrSelect&&this._CurTr!=null&&this._CurTr.getAttribute("__ewa_lf_mdown")!="1"){this._MSetBg(this._CurTr,"out");if(this.MOutEvent)try{this.MOutEvent(this._CurTr,a)}catch(b){console.log(b,this.MOutEvent)}this._CurTr=null}};this.MDownEnableCheck=function(a,
b){return!0};this.checkMDownEnable=function(a,b){if(!b)return!0;b=b.srcElement?b.srcElement:b.target;return!b||b.tagName=="TD"&&b.parentNode==a?!0:b.tagName=="A"||b.parentNode.tagName=="A"||b.tagName=="INPUT"||b.tagName=="SELECT"||b.tagName=="BUTTON"||b.tagName=="TEXTAREA"||b.className.indexOf("EWA_LF_EDIT")>=0||b.className.indexOf("ewa-lf-edit")>=0||b.className.indexOf("ewa-lf-search-text-click")>=0||b.parentNode.className.indexOf("ewa-lf-search-text-click")>=0||b.parentNode.parentNode.className.indexOf("ewa-lf-search-text-click")>=
0||b.className.indexOf("ewa-mdown-stop")>=0?!1:!0};this.mDownAutoCheck=function(a,b,c){for(var d,e=0;e<b.length&&!(b[e].parentNode.className.indexOf("ewa-switch")>=0);e++){if(b[e].type.toUpperCase()=="RADIO")return d=b[e],d.click(),d;if(b[e].type.toUpperCase()=="CHECKBOX"){d=b[e];d.click();if(!this._TrSelectMulti){let g=$(b[e]).attr("id")||$(b[e]).attr("name");$(c).find(b[e]).length==0&&$(a).siblings(".ewa-lf-data-row").find("input[name='"+g+"']:checked").prop("checked",!1);d.checked=!0}return d}}return null};
this.newRowOneTd=function(a){if(a==null)return null;a=$(a)[0];var b=a.rowIndex+a.cells[0].rowSpan,c=a.parentNode.rows[b];if(c!=null&&c.getAttribute("add_pre_row")=="1")return c;c=a.parentNode.parentNode;a=$(c.rows[0]).find("td:visible").length;c=c.insertRow(b);c.setAttribute("add_pre_row",1);b=c.insertCell(-1);b.colSpan=a;b.className="ewa-lf-add-pre-cell";c.style.display="none";b.id=EWA_Utils.tempId("EWA_LF_NR_"+this._Id);$(c).addClass("ewa-lf-add-pre-row");return c};this.MDown=function(a,b){if(this.IsTrSelect){var c=
(new Date).getTime(),d=$(a).attr("mdown_time_last");if(!(d!=null&&c-d<500)&&($(a).attr("mdown_time_last",c),b=b==null?window.event:b,this.MDownEnableCheck(a)&&this.checkMDownEnable(a,b))){d=b.srcElement?b.srcElement:b.target;var e=null;c=a.getElementsByTagName("input");this.IsNotMDownAutoChecked||(e=this.mDownAutoCheck(a,c,d));if(a.getAttribute("__ewa_lf_mdown")=="1")this._TrSelectMulti&&(e&&e.type.toUpperCase()=="CHECKBOX"&&(e.checked=!1),this._MSetBg(a,""),a.removeAttribute("__ewa_lf_mdown"),$(a).removeClass("ewa-lf-mdown"));
else{e&&(e.checked=!0);this._MSetBg(a,"down");a.setAttribute("__ewa_lf_mdown",1);$(a).addClass("ewa-lf-mdown");if(!this._TrSelectMulti&&(e=a.parentNode)&&e.rows)for(d=0;d<e.rows.length;d++){var g=e.rows[d];g.getAttribute("__ewa_lf_mdown")=="1"&&g!=a&&(g.removeAttribute("__ewa_lf_mdown"),$(g).removeClass("ewa-lf-mdown"),this._MSetBg(g,""))}if(this.IsReShow)for(d=0;d<c.length;d++)c[d].type.toUpperCase()=="BUTTON"&&(e=c[d],e.getAttribute("disabled")=="true"||e.getAttribute("disabled")==1||e.parentNode.getAttribute("disabled")==
"true"||e.parentNode.getAttribute("disabled")==1?(this._ReshowButs[e.value].disabled=!0,this._ReshowButs[e.value].style.display="none"):(this._ReshowButs[e.value].disabled=!1,this._ReshowButs[e.value].style.display=""))}this._IsAddPreRow?(c=a.getAttribute("ewa_key"),this.AddPreRowCheck(a,c,b)&&(d=this.newRowOneTd(a),this.prevTr&&(this.AddPreRowCloseBeforeEvent&&this.AddPreRowCloseBeforeEvent(this._Id,a,c,this.prevTr,b),this.prevTr.style.display="none",this.prevTr.cells[0].innerHTML="",this.AddPreRowCloseEvent&&
this.AddPreRowCloseEvent(this._Id,a,c,this.prevTr,b)),this.prevTr==d?this.prevTr=null:(this.prevTr=d,this.prevTr.style.display="",this.MDownEvent(this._Id,a,c,this.prevTr,b)))):this.MDownEvent(a,b)}}};this.AddPreRow=function(a){this._IsAddPreRow||(this.MDownEvent=a);this._IsAddPreRow=!0};this.AddPreRowCheck=function(a,b,c){return!0};this.AddPreRowCloseBeforeEvent=function(a,b,c,d,e){};this.AddPreRowCloseEvent=function(a,b,c,d,e){};this._MSetBg=function(a,b){let c="";b=="down"?c="ewa_grid_down":b==
"over"&&(c="ewa_grid_mover");$(a).children("td").removeClass("ewa_grid_mover ewa_grid_down").addClass(c)};this._ReShowSearchQuick=function(){var a=this,b=EWA_ListFrameClass.prototype.RESOURCES.search_text_items,c,d,e;for(e in b){var g=b[e];g.Id=="eq"&&(c=g);g.Id=="lk"&&(d=g)}for(e in this._SearchJson){var h=this._SearchJson[e];h.isSearchQuick&&(b=$("#EWA_LF_"+this.Id+' .ewa-lf-data-row [id="'+e+'"]'),b.bind("click",function(k){k.stopPropagation();k=$(this).attr("search_tag");var l=$(this).text(),
m=l,n=$("#EWA_SEARCH_ITEM_"+a.Id+' .ewa-lf-search-item-ctl [name="'+this.id.toUpperCase()+'"]');if(k=="number"||k=="date")n=n.parentsUntil(".ewa-lf-search-item").last().find("input");var p=n.parentsUntil(".ewa-lf-search-item").last().find("a");if(k=="date")l=l.split(" ")[0],m=l+" 23:59:59";else if(k=="fix"){var r=null;n.find("option").each(function(){this.text==l&&(r=this.value)});l=r===null?"":r}n.val()==l?(k=="text"&&d&&p.length>0&&(p.attr("tag",d.Id),p.html(EWA.LANG=="enus"?d.TxtEn:d.Txt)),n.val("")):
(k=="text"&&c&&p.length>0&&(p.attr("tag",c.Id),p.html(EWA.LANG=="enus"?c.TxtEn:c.Txt)),n.val(l),k=="date"&&(n[1].value=m))}),b.each(function(){$(this).text()&&$(this).addClass("ewa-lf-search-text-click").attr("search_tag",h.T)}))}};this.ReShowWithNoButtons=function(){var a=$X("EWA_LF_"+this._Id),b=[],c;for(c in this.ItemList.Items){var d=this.ItemList.Items[c],e=this._GetSubValue("Tag","Tag",d);if(e!=null&&e.trim().toLowerCase()=="button")for(e=0;e<a.rows[0].cells.length;e++){var g=a.rows[0].cells[e];
if(g.childNodes.length>0&&g.childNodes[0].id&&g.childNodes[0].id.toUpperCase()==c){b.push(e);d.setAttribute("but_show","1");break}}}this.ShowHiddenColumns(b,!1)};this.ReShowButtonsInDailogTitle=function(){EWA.OW.Load();if(EWA.OW.Dia){var a=$(EWA.OW.Dia._Dialog.GetFrameTitle()),b=$($X("EWA_RESHOW_"+this._Id)).find("td.ewa_lf_func:eq(0)"),c=this;b.find(".ewa_lf_func_dact").each(function(){this.id=EWA_Utils.tempId("gDx_"+c._Id+"_")});a.html(b.html());a.find(".ewa_lf_func_dact").each(function(){this.onclick=
function(){b.find('div[id="'+this.id+'"]').click()}});b.parent().hide();$(EWA.OW.Dia._Dialog.GetFrame()).addClass("ewa-lf-btns-in-title")}};this.ReShow=function(a){var b=$X("EWA_LF_"+this._Id);if(b!=null){this.IsReShow=!0;this.NewDivId=a="_G_"+this._Id;var c=[];c.push("<table class='ewa-lf-reshow' id='EWA_RESHOW_"+this._Id+"'>");c.push("<tr style='display:none'><td style='padding:0'></td></tr>");c.push("<tr><td class='ewa_lf_func' style='padding:0'>");c.push("<div style='display:none'></div>");c.push("<div><div class='ewa_lf_func_caption'></div></div>");
c.push("</td></tr>");c.push("<tr><td style='width:100%;padding:0;vertical-align:top'>");c.push("<div id='"+a+"' style=''></div>");c.push("</td></tr>");c.push("</table>");var d=$(c.join("")),e=!(this.Url.toUpperCase().indexOf("EWA_AJAX=INSTALL")>0||this.Url.toUpperCase().indexOf("EWA_CALL_METHOD=INNER_CALL")>0||this.Url.toUpperCase().indexOf(".JSP")>0);e?EWA.UI.Utils.SetStyle(d[0],"width:100%;height:100%;border-spacing:0"):EWA.UI.Utils.SetStyle(d[0],"width:100%;border-spacing:0");css="";a=b.parentNode.parentNode;
c=b.parentNode;d.find("td:eq(2)>div").append(c);c=d[0];var g=d.find("tr:eq(0)")[0],h=d.find("td:eq(0)")[0],k=d.find("td:eq(1)")[0],l=d.find("td:eq(2)")[0];for(e&&window.setTimeout(function(){$X("__EWA_DEBUG")&&l.childNodes[0].appendChild($X("__EWA_DEBUG"))},100);a.childNodes.length>0;)k.childNodes[0].appendChild(a.childNodes[0]);d=d.find(".ewa_lf_func_caption")[0];var m=!1;e=GetInnerText(k);e=e.trim();this._ReshowButs={};b=$(b.rows[0]);for(var n in this.ItemList.Items){var p=this.ItemList.Items[n],
r=this._GetSubValue("Tag","Tag",p);if(r&&(r.toLowerCase()=="button"||r.toLowerCase()=="butflow")&&(r=this._GetSubValue("Name","Name",p),b.find(".ewa-col-"+r).length!==0)){m=!0;var q=this._GetSubValue("DescriptionSet","Info",p),t=this._GetSubValue("DescriptionSet","Memo",p),v=this._GetSubValue("EventSet","EventName",p),u="";v!=null&&v.toLowerCase().trim()=="ewa_click"&&(u=this._GetSubValue("EventSet","EventValue",p));e=e.replace(q,"");p=this._ReShowButton(q,t,null);this._ReshowButs[q]=p;p.setAttribute("f_id",
this._Id);p.setAttribute("t_id",r);k.childNodes[1].appendChild(p);u==""?p.setAttribute("onclick","EWA.F.FOS['"+this._Id+"'].reShowButtonClick(this)"):p.setAttribute("onclick",u)}}m&&this._ReShowSplit(k.childNodes[1]);r=k.getElementsByTagName("input");m={};for(b=0;b<r.length;b++){var w=r[b];w.style.display="none";if(w.id==null||w.id=="")w.id=("_tmp_"+Math.random()).replace(".","_");if(w.type=="button"){if(p=this._ReShowButton(w.value,w.title,w.id),q=w.nextSibling){var x="";q.nodeType==3&&(x=q.nodeValue.trimEx());
x=="|"&&this._ReShowSplit(k.childNodes[1])}}else(q=w.nextSibling)&&(x=q.nodeType==3?q.nodeValue.replace("|",""):q.innerHTML),p=this._ReShowButton(x.trimEx(),w.title,w.id),p.id=("_tmp_"+Math.random()).replace(".","_"),m[w.name]||(m[w.name]=[]),m[w.name].push(p.id);k.childNodes[1].appendChild(p)}e=e.replace(/ /ig,"").replace(/\|/ig,"").trimEx();e!=""?e.indexOf("if")>=0?g.style.display="none":h.innerHTML=e.replace(/ /,""):g.style.display="none";a.appendChild(c);for(n in m)for(x=m[n],b=0;b<x.length;b++)w=
$X(x[b]),w.setAttribute("_r_ids",x.join(",")),w.onclick=function(){var B=this.getAttribute("_ewa_event_id");$X(B).click();B=this.getAttribute("_r_ids").split(",");for(var C=0;C<B.length;C++)w.id!=B[C]&&($X(B[C]).style.fontWeight="",$X(B[C]).style.color="");this.style.fontWeight="bold";this.style.color="blue"};d.innerHTML='<b><i class="fa fa-star-o">&nbsp;</i>'+this.Description+"</b>";if(this.Url.toUpperCase().indexOf("EWA_BTNS_IN_TITLE")>0)var y=this,z=0,A=setInterval(function(){z++;z>1E3&&window.clearInterval(A);
EWA.OW.Load();EWA.OW.Dia&&(window.clearInterval(A),y.ReShowButtonsInDailogTitle())},10);this._InitMneu()}};this.reShowButtonClick=function(a){a=a.getAttribute("t_id");var b=this.SelectCheckedRows();if(b==null||b.length==0)EWA.UI.Msg.Alert(_EWA_INFO_MSG["EWA.SYS.CHOOSE_ITEM"],_EWA_INFO_MSG["EWA.SYS.CHOOSE_ITEM_TITLE"]);else{b=b[0].getElementsByTagName("input");for(var c=0;c<b.length;c++){var d=b[c];if(d.name===a){d.click();break}}}};this._ReShowSplit=function(a){EWA.UI.Utils.CreateObject(window,"div",
"",a).className="ewa_lf_func_split"};this._ReShowButton=function(a,b,c){var d=EWA.UI.Utils.CreateObject(window,"div","cursor:pointer",document.body);d.innerHTML="<nobr>"+a+"</nobr>";c!=null&&(d.setAttribute("_ewa_event_id",c),d.onclick=function(){var e=this.getAttribute("_ewa_event_id");$X(e).click()});EWA.B.IE&&(d.onselectstart=function(){return!1});d.className="ewa_lf_func_dact";d.title=b;return d};this.CheckValid=function(a){var b=a.tagName.toLowerCase();if(b!="input"&&b!="textarea"&&b!="select"||
"button"==a.type||"submit"==a.type||"image"==a.type)return!0;b=this.ItemList.GetObjectValue(a);return this.ItemList.CheckValid(a,b)};this.ShowEdit=function(a){a.parentNode.style.width=a.clientWidth+"px";a.style.display="none";__show_obj__=a.nextSibling;__show_obj__.style.display="";var b=__show_obj__.childNodes[0];b.nodeType==3&&(b=__show_obj__.childNodes[1]);b.setAttribute("old",b.value);b.tagName!="SELECT"||b.getAttribute("func")||(b.setAttribute("func",1),b.onchange=function(){EWA.F.FOS[this.getAttribute("__ewa_fid__")].EditAfter(this)});
b.getAttribute("__ewa_fid__")==null&&(b.setAttribute("__ewa_fid__",this._Id),b.onblur=function(){EWA.F.FOS[this.getAttribute("__ewa_fid__")].EditAfter(this)},b.onkeydown=function(c){c=c==null?window.event:c;var d=c.target?c.target:c.srcElement;c.keyCode==13&&d.tagName=="INPUT"&&EWA.F.FOS[this.getAttribute("__ewa_fid__")].EditAfter(this);c.keyCode==27&&EWA.F.FOS[this.getAttribute("__ewa_fid__")].EditReset(this)});setTimeout(function(){b.focus();b.select()},10)};this.EditReset=function(a){a.parentNode.style.display=
"none";var b=a.parentNode.previousSibling;b.style.display="";b.innerHTML=a.getAttribute("old");a.tagName.toLowerCase()=="select"?b.innerHTML=a.options[a.selectedIndex].text:a.value=a.getAttribute("old").replace(/</ig,"&lt;").replace(/>/ig,"&gt;")};this.EditAfter=function(a){if(this.CheckValid(a))if(a.getAttribute("old")==a.value)this.EditReset(a);else{a.parentNode.style.display="none";var b=a.parentNode.previousSibling;b.style.display="";if(a.getAttribute("ewa_class")=="droplist"){var c=$(a.parentNode.getElementsByTagName("input")[1]).attr("name");
var d=a.parentNode.getElementsByTagName("input")[1].value;b.innerHTML=a.parentNode.getElementsByTagName("input")[0].value}else a.tagName.toLowerCase()=="select"?(c=$(a).attr("name"),b.innerHTML=a.options[a.selectedIndex].text):(c=a.value.replace(/</ig,"&lt;").replace(/>/ig,"&gt;").replace(/\n/ig,"<br>"),b.innerHTML=c,c=$(a).attr("name")),d=a.value;b=a.getAttribute("action_name");if(b==""||b==null)b="OnListFrameUpdateCell";var e=a.getAttribute("action_confirm");e=e==null?"":e;var g=a.getAttribute("action_tip");
g==null&&(g="");var h=a.getAttribute("action_js"),k=this.ItemList.GetValue(c,"DataItem","DataType");c=[{Name:"EWA_FIELD_ID",Value:c},{Name:"EWA_FIELD_VAL",Value:d},{Name:"EWA_ACTION_RELOAD",Value:"0"},{Name:c,Value:d},{Name:"EWA_DATA_TYPE",Value:k},{Name:"EWA_FIELD_VAL_OLD",Value:a.getAttribute("old")}];this.EditEvent={FROM:a,OLD_VAL:a.getAttribute("old"),NEW_VAL:a.value};this.DoAction(a,b,e,g,c,h)}else a.focus()};this.RewriteInfo=function(a,b,c,d,e){var g=$("#EWA_LF_"+this._Id),h=[];if(a)this._INFO=
{},this._INFO.INFO=a,this._INFO.ID_NAME=b||"id",this._INFO.INFO_NAME=c||"info",this._INFO.MEMO_NAME=d||"memo";else if(!this._INFO||!this._INFO.INFO)return;for(a=0;a<this._INFO.INFO.length;a++)if(b=this._INFO.INFO[a],c=g.find('[id="'+b[this._INFO.ID_NAME]+'"]:eq(0)'),c.length!==0){d=c.parentsUntil("tr").last();var k=b[this._INFO.INFO_NAME],l=b[this._INFO.MEMO_NAME];if(k==null||k.trim()=="")for(k=d.parent()[0],l=0;l<k.cells.length;l++){if(k.cells[l]==d[0]){this.ShowHiddenColumn(l,!1);break}}else c.children().length==
0?c.text(k):c.children()[0].innerText=k,l&&c.attr("title",l);h.push(c[0]);e&&e(c[0],b,a)}return h};this.ShowHiddenColumn=function(a,b){var c=$X("EWA_LF_"+this._Id),d="";if(b==null||b==0||b=="none")d="none";for(b=0;b<c.rows.length;b++)c.rows[b].cells.length==c.rows[0].cells.length&&(c.rows[b].cells[a].style.display=d)};this.ShowHiddenColumns=function(a,b){var c=$X("EWA_LF_"+this._Id),d="";if(b==null||b==0||b=="none")d="none";b=c.rows[0].cells.length;for(var e=0;e<c.rows.length;e++){var g=c.rows[e];
if(g.cells.length==b)for(var h=0;h<a.length;h++)g.cells[a[h]].style.display=d}};this.GroupShowHidden=function(a){var b=$(a).attr("t"),c=(new Date).getTime();if(!(b&&c-b<333)){$(a).attr("t",c);b=a.parentNode.parentNode;c=b.getAttribute("ewa_group");c==""?(c="none",a.childNodes[0].innerHTML=" + "):(c="",a.childNodes[0].innerHTML=" - ");b.setAttribute("ewa_group",c);for(var d=b.parentNode.parentNode,e=0,g=b.rowIndex+1;g<d.rows.length;g++){b=d.rows[g];if(b.getAttribute("ewa_tag")=="group")break;b.getAttribute("add_pre_row")?
c=="none"?(b.style.display!="none"&&b.setAttribute("ewa_group_show_old",b.style.display),b.style.display="none"):b.hasAttribute("ewa_group_show_old")&&(b.style.display=b.getAttribute("ewa_group_show_old"),b.removeAttribute("ewa_group_show_old")):(b.style.display=c,e++)}a.childNodes[a.childNodes.length-1].innerHTML=" ("+e+")"}};this.Sort=function(a){var b=this._last_sort_timer||0,c=this._Sort||"",d=(new Date).getTime();c==a&&d-b<500||(this._last_sort_timer=d,this._Sort=a,this.Goto(1))};this.SearchClear=
function(a){a=a.parentNode.parentNode.parentNode.parentNode;for(var b=0;b<a.rows.length;b+=1)for(var c=a.rows[b].getElementsByTagName("input"),d=0;d<c.length;d++){var e=c[d];if(e.type=="text")e.value="";else if(e.type=="radio"||e.type=="checkbox")e.checked=!1}};this.SearchGetExpInit=function(){let a=$("#_G_"+this._Id);var b=[];$(a).find(".ewa-lf-search-item").each(function(){var c=$(this).find("input");if(c.length==0&&(c=$(this).find("select"),c.length==0))return;var d=c[0],e=$(d).attr("t");if(c.length==
1){var g=d.value;if(d.value.trim()!="")for(e=="text"&&(c=$(d.parentNode.parentNode).find("a").attr("tag"))&&(e=c),d=d.name.split(","),c=0;c<d.length;c++)b.push(d[c]+"["+e+"]"+g)}else if((e=="date"||e=="number")&&c.length==2){if(g=d.value,e=c[1].value,e!=""||g!="")b.push(d.name+"[gte]"+g),e&&b.push(d.name+"[lte]"+e)}else if(e=="fix"&&d.tagName=="INPUT"){var h=[""];$(this).find("input").each(function(){this.checked&&h.push(this.value)});h.length!=1&&b.push(d.name+"[or]"+h.join(";"))}});return b.join(",")};
this.SearchGetExp=function(a){var b=[];$(a).find(".ewa-lf-search-item").each(function(){var c=$(this).find("input");if(c.length==0&&(c=$(this).find("select"),c.length==0))return;var d=c[0],e=$(d).attr("t");if(c.length==1){var g=d.value;d.value.trim()!=""&&(e=="text"&&(c=$(d.parentNode.parentNode).find("a").attr("tag"))&&(e=c),b.push("@!@"+d.name+"~!~"+e+"~!~"+g))}else if((e=="date"||e=="number")&&c.length==2)g=d.value,c=c[1].value,c==""&&g==""||b.push("@!@"+d.name+"~!~"+e+"~!~"+g+"~!~"+c);else if(e==
"fix"&&d.tagName=="INPUT"){var h=[""];$(this).find("input").each(function(){this.checked&&h.push(this.value)});h.length!=1&&b.push("@!@"+d.name+"~!~"+e+h.join("~!~"))}});return b.join("")};this.DoSearch=function(a){var b=!1;a.id=="EWA_SEARCH_ITEM_"+this.Id?a=$X("EWA_SEARCH_ITEM_"+this.Id).childNodes[0]:(a=a.parentNode.parentNode.parentNode.parentNode,b=!0);this._SearchExp=this.SearchGetExp(a);b&&(this._SearchHtml=a.parentNode.innerHTML,this._SearchDialog.Show(!1));this.Goto(1)};this.Search=function(a){this._SearchHtml==
null?(this._SearchCreate(),this._SearchDialog=new EWA.UI.Dialog.OpenWindow("about:blank","aa",400,100,!0,null,null,!1),this._SearchDialog.SetCaption(_EWA_INFO_MSG["EWA.LF.SEARCH.BUT.TITLE"]),this._SearchDialog._Dialog.AutoSize=!0,this._SearchDialog._Dialog.SetHtml(this._SearchHtml),this._SearchDialog._Dialog.Show(!0),this._SearchDialog._Dialog.GetFrame().focus(),this._SearchDialog.MoveCenter()):(this._SearchDialog._Dialog.AutoSize=!1,this._SearchDialog.Show(!0))};this.ShowSearch=function(a,b){var c=
"EWA_SEARCH_ITEM_"+this.Id;b&&(this._IsSearchGroup=!1);$X(c)||(this._SearchCreateItm(),a&&this.composeSearchTexts(),this._ReShowSearchQuick())};this.ChangeSearchTextType=function(a){this._MENU_DATE_RANGE&&this._MENU_DATE_RANGE.HiddenMemu();this._MENU_TEXT_TYPE.ShowByObject(a,null,0);let b=$(this._MENU_TEXT_TYPE.Dialog.GetFrame());b.find(".search-text-tag").remove();b.find('div[ewa_mg="'+$(a).attr("tag")+'"] td:eq(0)').html('<b class="fa fa-check search-text-tag"></b>')};this.ChangeSearchDateType=
function(a){this._MENU_TEXT_TYPE&&this._MENU_TEXT_TYPE.HiddenMemu();this._MENU_DATE_RANGE.ShowByObject(a,null,0)};this.ChangeSearchTextTypeIt=function(a){var b=$(this._MENU_TEXT_TYPE.SHOW_BY_OBJECT).parentsUntil(".ewa-lf-search-item").last(),c=$J2MAP(this.RESOURCES.search_text_items,"Id")[a];c=EWA.LANG=="enus"?c.TxtEn:c.Txt;b.find("a").attr("tag",a).html(c);let d=b.find("input");a=="blk"||a=="nblk"?d.val(c).prop("disabled",!0):d.prop("disabled")&&b.find("input").val("").prop("disabled",!1)};this._GetInitSearchMap=
function(){var a=new EWA_UrlClass(this.Url),b={},c=a.GetParameter("ewa_search");if(!c)return b;this.Url=a.RemoveParameter("ewa_search");c=c.unURL();a=c.split(",");for(var d in a){var e=a[d],g=e.indexOf("["),h=e.indexOf("]");g<0||h<g||(c=e.substring(0,g).trim(),c.length!=0&&(g=e.substring(g+1,h).trim().toLowerCase(),g.length!=0&&(e=e.substring(h+1).trim(),e!=""&&(b[c.toUpperCase()]||(b[c.toUpperCase()]={name:c,tag:g}),c=b[c.toUpperCase()],g=="lt"||g=="lte"?c.para2=e.unURL().replace(/\+/ig," "):c.para1=
e.unURL().replace(/\+/ig," ")))))}return b};this.SearchFilterDate=function(a){var b=$(this._MENU_DATE_RANGE.SHOW_BY_OBJECT).parentsUntil(".ewa-lf-search-item").last(),c=b.find("input")[0];b=b.find("input")[1];if(a=="Clear")$(c).val(""),$(b).val("");else{var d=EWA_Utils.Date(),e="",g="";a=="Today"?g=e=d.GetDateString():a=="Week"?(a=d.GetBenWeek(),e=a[0],g=a[1]):a=="Today-Weekend"?(a=d.GetBenWeek(),e=a[d.GetDateString()],g=a[1]):a=="Month"?(a=d.GetBenMonth(),e=a[0],g=a[1]):a=="Today-EOM"?(a=d.GetBenMonth(),
e=a[d.GetDateString()],g=a[1]):a=="Quarter"?(a=d.GetBenQ(),e=a[0],g=a[1]):a=="Today-EOQ"?(a=d.GetBenQ(),e=a[d.GetDateString()],g=a[1]):a=="Year"?(a=d.GetBenYear(),e=a[0],g=a[1]):a=="Today-EOY"&&(a=d.GetBenYear(),e=a[d.GetDateString()],g=a[1]);a=EWA_Utils.Date(e).FormatDate();d=EWA_Utils.Date(g).FormatDate();d+=" 23:59:59";$(c).val(a);$(b).val(d)}};this._SearchCreateDateRange=function(){var a="_MENU_DATE_RANGE"+this._Id;$("."+a).remove();var b=EWA.LANG.toLowerCase()=="enus"?"TxtEn":"Txt",c=JSON.parse(JSON.stringify(this.RESOURCES.search_date_items)),
...
return b};this._SearchCreateItm=function(){var a="EWA_SEARCH_ITEM_"+this.Id,b=[],c=[],d=[],e=[],g={},h;for(h in this._SearchJson)this._SearchJson[h].ORI_NAME=h,g[h.toUpperCase()]=this._SearchJson[h];this._SearchJson=g;var k="EWA.F.FOS[&quot;"+this._Id+"&quot;].ChangeSearchTextType(this)",l="EWA.F.FOS[&quot;"+this._Id+"&quot;].ChangeSearchDateType(this)",m=$J2MAP(this.RESOURCES.search_text_items,"Id");g=this._GetInitSearchMap();var n=!1,p=!1;for(h in this.ItemList.Items)if(this._SearchJson[h]!=null){var r=
[],q=this._SearchJson[h],t=q.T,v=this._GetSubValue("DescriptionSet","Info",this.ItemList.Items[h]);r.push("<table class='ewa-lf-search-item ewa-ref-"+h+" ewa-lf-search-type-"+t+" "+(t=="fix"?"ewa-lf-search-fix-m"+(q.M||"0"):"")+"' cellspacing=0 cellpadding=0>");r.push("<tr><td class='ewa-lf-search-item-title' style='position:relative'><nobr><b>");r.push(v);var u=v="",w="lk";g[h.toUpperCase()]&&(u=g[h.toUpperCase()],w=u.tag,v=u.para1,u=u.para2);var x="";t=="text"?(p=m[w]||m.lk,x=EWA.LANG=="enus"?p.TxtEn:
p.Txt,r.push("<a class='ewa-lf-search-type' tag='"+w+"' href='javascript:void(0)' onclick='"+k+"'>"),r.push(x),r.push("</a>"),p=!0):t=="date"&&(r.push("<a  href='javascript:void(0)' class='fa fa-calendar-check-o'  onclick='"+l+"'></a>"),n=!0);r.push("</b></nobr></td><td class='ewa-lf-search-item-ctl'>");t=="text"?w=="blk"||w=="nblk"?(v=this._SearchSingle(h,x),v=v.replace("<input","<input disabled "),r.push(v)):r.push(this._SearchSingle(h,v)):t=="date"?r.push(this._SearchDate(h,v,u)):t=="number"?r.push(this._SearchNumber(h,
v,u)):t=="fix"&&(v="<div >"+this._SearchFix(h,q,"span",v)+"</div>",r.push(v));r.push("</td></tr></table>");"3"==q.M?e.push(r.join("")):t=="fix"&&this._IsSearchGroup?c.push(r.join("")):d.push(r.join(""))}if(d.length+c.length+e.length!=0){b.push('<table class="ewa-lf-search-main" id="'+a+'" cellspacing="0" cellpadding="0">');b.push("<tr><td class='ewa-lf-search-des'></td><td>");d.length>0&&b.push(" "+d.join("")+" ");for(d=0;d<c.length;d++)b.push(" "+c[d]+" ");for(d=0;d<e.length;d++)b.push(" "+e[d]+
" ");b.push("</td></tr></table>");var y=document.createElement("div");this._SEARCH_BOX=y;y.className="ewa-lf-search";y.innerHTML=b.join("");b=$X("EWA_LF_"+this.Id).parentNode;b.parentNode.insertBefore(y,b);a="EWA_SEARCH_ITEM_"+this.Id;var z=this;n&&this._SearchCreateDateRange();p&&this._SearchCreateTextType();z._SEARCH_ITEM_EXP=this.SearchGetExp(y);JSON.stringify(g)!="{}"&&(z._SearchExp=z._SEARCH_ITEM_EXP);$(y).find("input[type=text]").on("compositionstart",function(){z._is_search_composition=!0}).on("compositionend",
function(){z._is_search_composition=!1});this._TIMER_SEARCH=window.setInterval(function(){if(!z._is_search_composition){try{if(!$X(a)){window.clearInterval(z._TIMER_SEARCH);return}}catch(B){window.clearInterval(z._TIMER_SEARCH);return}var A=z.SearchGetExp(y);A!=z._SEARCH_ITEM_EXP&&(z._SEARCH_ITEM_EXP=A,z.DoSearch($X(a)))}},700)}};this._SearchSingle=function(a,b){var c=[];c.push('<input t="text" type="text" autocomplete="off" class="EWA_INPUT" maxlength="40" name="');c.push(a.toInputValue());c.push('"');
b!=null&&(c.push(' value="'),a=b.toInputValue(),c.push(a),c.push('" '));c.push(" />");return c.join("")};this._SearchDate=function(a,b,c){var d=[];d.push("<table border=0 cellpadding=0 cellspacing=0>");d.push('<td  ><input type="text" autocomplete="off" class="EWA_INPUT" t="date" readonly maxlength="8" onclick="EWA.UI.Calendar.Pop(this)" name="');d.push(a);d.push('"');b&&(b=b.toInputValue(),d.push(' value="'),d.push(b),d.push('"'));d.push(' /></td><td><span class="ewa-lf-search-split"> - </span></td>');
...
d.push('<input type="text" t="number" size="10" name="');d.push(a);d.push('1"');c&&(b=c*1,d.push(' value="'),d.push(b),d.push('"'));d.push("/>");return d.join("")};this._SearchFix=function(a,b,c,d){var e=[],g="radio";if(b.M=="1")g="checkbox";else{if(b.M=="2")return this._SearchFixSelect(a,b,c,d);if(b.M=="3")return this._SearchFixButton(a,b,c,d)}c="<"+c+"><nobr><input t='fix' [@CHK] id='@ID' value=\"@V\" type='"+g+"' name='"+a+"' /><label for='@ID'>@T</label></nobr></"+c+"> ";g={};if(d){d=d.split(";");
for(var h in d)g[d[h].trim().toUpperCase()]=1}for(h=0;h<b.D.length;h++){var k=b.D[h];d=k[0];var l=k[1];k[2]&&(l+="("+k[2]+")");k=this.Id+"_"+a+"_SearchFix_"+h;var m=c.replace("@ID",k);m=m.replace("@ID",k);m=m.replace("@V",d);m=g[d.toUpperCase().trim()]?m.replace("[@CHK]","checked"):m.replace("[@CHK]","");m=m.replace("@T",l);e.push(m)}return e.join("")};this.searchFixButtonClick=function(a){$(a).parent().find("button").each(function(){this==a?($(this).addClass("ewa-lf-search-fix-button-checked"),$(this).find("input").prop("checked",
!0)):($(this).removeClass("ewa-lf-search-fix-button-checked"),$(this).find("input").prop("checked",!1))})};this.reShowStatistics=function(a){this._is_search_composition=!0;for(var b in a){const e=a[b];if(e.T=="fix"){var c=$(".ewa-lf-search-item.ewa-ref-"+b);c.length==0&&(console.log("NOT fund .ewa-lf-search-item.ewa-ref-"+b),contine);c="2"===e.M?c.find("option[id]"):"3"===e.M?c.find("button"):c.find("input");for(let g=0;g<e.D.length;g++){var d=e.D[g];if(d[2]==void 0)continue;const h=c[g];h?(d=d[1]+
(d[2]?"("+d[2]+")":""),h.tagName=="INPUT"?h.value=d:h.tagName=="OPTION"?h.text=d:$(h).find("span").text(d)):console.log("not fund input/option/button index="+g)}}}this._SEARCH_ITEM_EXP=this.SearchGetExp(this._SEARCH_BOX);this._is_search_composition=!1};this._SearchFixButton=function(a,b,c,d){c="<button  class=\"@CLS\" type='button' id='@ID' onclick=\"EWA.F.FOS['"+(this._Id+"'].searchFixButtonClick(this)\"><input t='fix' style='display:none' type='radio' name=\"")+a+'" value="@V" @CKD><span>@T</span></button>';
for(var e=[],g=0,h=0;h<b.D.length;h++){var k=b.D[h],l=k[1];k[2]&&(l+="("+k[2]+")",g+=k[2]);var m="ewa-lf-search-fix-button",n=c.replace("@ID",this.Id+"_"+a+"_SearchFix_"+h);n=n.replace("@V",k[0]);n=n.replace("@T",l);d?k[0]==d?(n=n.replace("@CKD","checked"),m+=" ewa-lf-search-fix-button-checked"):n=n.replace("@CKD",""):0===h?(n=n.replace("@CKD","checked"),m+=" ewa-lf-search-fix-button-checked"):n=n.replace("@CKD","");n=n.replace("@CLS",m);e.push(n)}e[0]=e[0].replace("(-1)","("+g+")");return e.join("")};
this._SearchFixSelect=function(a,b,c,d){c=[];c.push("<select t='fix' name='"+a+"'><option></option>");for(var e=0;e<b.D.length;e++){var g=b.D[e],h=this.Id+"_"+a+"_SearchFix_"+e,k=g[1];g[2]&&(k+="("+g[2]+")");h="<option @CKD t='fix' id='@ID' value=\"@V\">@T</option> ".replace("@ID",h);h=h.replace("@V",g[0]);h=h.replace("@T",k);h=d&&g[0]==d?h.replace("@CKD","selected"):h.replace("@CKD","");c.push(h)}c.push("</select>");return c.join("")};this._SearchCreate=function(){var a=[];a.push("<div><table border=0 style='margin:0px;' class=EWA_TABLE cellspacing=1>");
for(var b in this.ItemList.Items)if(this._SearchJson[b]!=null){var c=this._SearchJson[b],d=c.T,e=this._GetSubValue("DescriptionSet","Info",this.ItemList.Items[b]);a.push("<tr><td class=EWA_TD_L width=140><nobr>"+e+"<nobr></td><td width=350 class=EWA_TD_M>");d=="text"?a.push(this._SearchSingle(b)):d=="date"?a.push(this._SearchDate(b)):d=="number"?a.push(this._SearchNumber(b)):d=="fix"&&a.push(this._SearchFix(b,c,"div"));a.push("</td></tr>")}a.push("<tr><td class=EWA_TD_B colspan=2 align=right><input onclick='EWA.F.FOS[\""+
this._Id+"\"].SearchClear(this);' type=button value='"+_EWA_INFO_MSG["EWA.LF.SEARCH.BUT.CLEAR"]+"'> <input type=submit value='"+_EWA_INFO_MSG["EWA.LF.SEARCH.BUT.SEARCH"]+"' onclick='EWA.F.FOS[\""+this._Id+"\"].DoSearch(this);'> <input type=button value='"+_EWA_INFO_MSG["EWA.LF.SEARCH.BUT.CLOSE"]+"' onclick='EWA.F.FOS[\""+this.Id+"\"]._SearchDialog.Show(false);'></td></tr>");a.push("</table></div>");this._SearchHtml=a.join("")};this.createRegExp=function(a){if(!a)return null;a=String(a).trim();if(a.length===
0)return null;for(var b=[],c=0;c<a.length;c++){var d=a[c];d>="a"&&d<="z"||d>="A"&&d<="Z"||d>="0"&&d<="9"?b.push(d):b.push("\\"+d)}try{return new RegExp("("+b.join("")+")")}catch(e){return console.warn("new RegExp",a,e),null}};this.SearchMark=function(){for(var a=this._SearchExp.split("@!@"),b=$X("EWA_LF_"+this._Id),c=0;c<a.length;c+=1){var d=a[c].split("~!~");if(!(d.length<3))for(var e=2;e<d.length;e++){var g=d[e];if(g=="")continue;if(d[1]=="fix"){var h=d[2];$("table#EWA_SEARCH_ITEM_"+this.Id+' [name="'+
d[0]+'"]').find("option").each(function(){this.value==h&&(g=this.text)})}let l=this.createRegExp(g),m=d[0].split(",");for(let n in m){var k=m[n];this._SearchJson[k]&&(k=this._SearchJson[k].ORI_NAME);k=$(b).find(".ewa-lf-data-row .ewa-col-"+k).not("[ewa-search-marked=yes]");k.attr("ewa-search-marked","yes");try{k.find("*").each(function(){this.children.length==0&&(this.innerHTML=this.innerHTML.replace(l,"<font color=red><b>$1</b></font>"))})}catch(p){console.log(p)}}}}};this.DblClick=function(a){this._IsDblClick=
a;for(var b=$X("EWA_LF_"+this._Id),c=1;c<b.rows.length;c++)b.rows[c].ondblclick=function(){var d=this.getElementsByTagName("input");d.length>a&&d[a].disabled==0&&d[a].click()}};this.Init=function(a){this.Xml=new EWA.C.Xml;this.Xml.LoadXml(a);this.ItemList.Init(this.Xml);this.ItemList.F=this;this.Resources=new EWA_FrameResoures;this.Resources.Init(this.Xml);if((a=$("#EWA_LF_"+this._Id)[0])&&a.tagName=="table"&&!(a.rows.length<=1)){a=a.rows[1].getElementsByTagName("input");for(var b=0;b<a.length;b++)if(a[b].type.toUpperCase()==
"RADIO"){this._TrSelectMulti=!1;break}}};this.SetPageParameters=function(a,b,c,d,e){this._PageCurrent=a;this._PageSize=c;this._PageCount=b;this._RecordCount=d;this._Sort=e};this.SetPageParametersName=function(a,b,c,d,e){this._PageCurrentName=a;this._PageSizeName=c;this._PageCountName=b;this._RecordCountName=d;this._SortName=e};this.CheckedAll=function(){var a=document.getElementById("EWA_LF_"+this._Id);if(!(a.rows.length<=1)){for(var b=0;b<a.rows.length;b+=1){var c=a.rows[b].getElementsByTagName("input");
c.length!=0&&(c[0].checked=this._IsCheckedAll)}this._IsCheckedAll=this._IsCheckedAll?!1:!0;this.CheckedAllAfter&&this.CheckedAllAfter(a)}};this.SelectChecked=function(){var a=$X("EWA_LF_"+this._Id);if(!a||a.rows.length<1)return"";var b=[];let c=[];$("#EWA_LF_"+this._Id+">tbody>tr").find("input[type='radio'],input[type='checkbox']").each(function(){this.parentNode.className.indexOf("ewa-switch")==-1&&c.push(this)});if(c.length>0)for(var d=0;d<c.length;d++)c[d].checked&&b.push(c[d].value);else for(d=
0;d<a.rows.length;d++){var e=a.rows[d];e.getAttribute("__ewa_lf_mdown")==1&&b.push(e.getAttribute("EWA_KEY"))}return b.join(",")};this.SelectCheckedInputs=function(){var a=[],b=$X("EWA_LF_"+this._Id);if(b.rows.length<1)return a;b=b.getElementsByTagName("input");a=[];for(var c=0;c<b.length;c++)(b[c].type.toUpperCase()=="CHECKBOX"||b[c].type.toUpperCase()=="RADIO")&&b[c].checked&&a.push(b[c]);return a};this.SelectCheckedRows=function(){for(var a=[],b=$X("EWA_LF_"+this._Id),c=0;c<b.rows.length;c++){var d=
b.rows[c],e=d.getElementsByTagName("input");e.length!=0&&e[0].checked&&e[0].parentNode.className.indexOf("ewa-switch")==-1&&a.push(d)}if(a.length==0)for(c=0;c<b.rows.length;c++)d=b.rows[c],d.getAttribute("__ewa_lf_mdown")==1&&a.push(d);return a};this.Reload=function(a){this.StopAjaxAfterReload?this.StopAjaxAfterReload=!1:this.Goto(this._PageCurrent,a)};this.refreshPage=function(a,b,c){this.replaceRowsData(null,null,a,b,c)};this.replaceRowsData=function(a,b,c,d,e){var g=this.getUrlClass();g.AddParameter("EWA_AJAX",
"LF_RELOAD");g.AddParameter("EWA_IS_SPLIT_PAGE","no");g.AddParameter("EWA_IS_HIDDEN_CAPTION","yes");g=g.GetUrl();a&&(g+="&"+a);let h=a?new EWA_AjaxClass:this.CreateAjax(),k=this;h.PostNew(g,function(){if(h._Http.readyState==4){h.HiddenWaitting();var l=h._Http.responseText;h._Http.status!=200?(console.error(l),alert("ERROR:\r\n"+h._Http.statusText)):EWA.F.CheckCallBack(l)?k.replaceRowsWithDataHtml(l,b,c,d,e):console.error(l)}})};this.replaceRowsWithDataHtml=function(a,b,c,d,e){let g=$("<div></div>");
g.html(a);let h=[],k=$("#EWA_LF_"+this._Id);g.find(".ewa-lf-data-row").each(function(){var l='tr[ewa_key="'+$(this).attr("ewa_key")+'"]';let m=k.find(l);if(m.length==0)console.warn(l+" not found");else{l=$(this).attr("ewa_row_sign");var n=m.attr("ewa_row_sign"),p=!0;(l||n)&&n===l&&(p=!1);if(p){m.attr("ewa_row_sign",l);l=m.clone()[0];for(n=0;n<this.cells.length;n++){p=m[0].cells[n];let r=this.cells[n];p.removeAttribute("ewa-merged");p.removeAttribute("ewa-search-marked");b?b(r,p):p.innerHTML=r.innerHTML}h.push({before:l,
current:m[0]})}}});g.remove();h.length!=0&&(this.SearchMark(),this.IsReShow&&this.ReShowWithNoButtons(),this._ReShowSearchQuick(),this._IsDblClick!=null&&this.DblClick(this._IsDblClick),this.SubBottomsArray&&this._SubBottoms(),!e&&this.ReloadAfter&&this.ReloadAfter(c),!e&&this.ReloadAfterApp&&this.ReloadAfterApp(c));d&&d(h)};this.DoAction=function(a,b,c,d,e,g){EWA.F.CID=this._Id;if(b){if(a&&$(a).parent().hasClass("scroll-delete")){var h=$(a).attr("do-action-time")||0,k=(new Date).getTime();if(k-h<
1E3)return;$(a).attr("do-action-time",k)}this.lastAction=b;this._Ajax=new EWA_AjaxClass;this._Ajax.LoadingType="image";this._Ajax.AddParameter("EWA_AJAX","1");this._Ajax.AddParameter("EWA_ACTION",b);this._Ajax.AddParameter("EWA_ID",this._Id);this._Ajax.AddParameter("EWA_NO_CONTENT","1");this._Ajax.AddParameter("EWA_ACTION_TIP",d);g!=null&&g.trim().length>0?(this._Ajax.AddParameter("EWA_AFTER_EVENT",g),this.StopAjaxAfterReload=!0,this._Ajax.AddParameter("EWA_ACTION_RELOAD","0")):this.StopAjaxAfterReload=
!1;if(a&&a.getAttribute&&a.getAttribute("box_menu_item")=="yes"){if(this._Ajax.AddParameter("___CALL__FROM__","box"),a=a.parentNode)a=a.getAttribute("json_ref"),a=JSON.parse(a),a.EWA_KEY&&this._Ajax.AddParameter("EWA_ACTION_KEY",a.EWA_KEY)}else(a=this.GetRowKey(a))&&this._Ajax.AddParameter("EWA_ACTION_KEY",a);a=this.SelectChecked();this._Ajax.AddParameter("IDS_SPLIT",a);if(e!=null&&e.length>0)for(a=0;a<e.length;a+=1)this._Ajax.AddParameter(e[a].Name,e[a].Value);e=new EWA.C.Url;e.SetUrl(this.Url);
var l=e.RemoveParameter("EWA_ACTION"),m=this;c?((e=_EWA_INFO_MSG[c])||(e=c),$Confirm(e,"Confirm",function(){m._Ajax.PostNew(l,function(){m._CallBackJs()})})):this._Ajax.PostNew(l,function(){m._CallBackJs()})}};this.NewPageSize=function(a){this._PageSize!=a&&(this._PageSize=a,this.Goto(1))};this.Goto=function(a,b){EWA.F.CID=this._Id;this._PageCurrent=a;this._Ajax=this.CreateAjax();a=new EWA_UrlClass;a.SetUrl(this.Url==null?document.location.href:this.Url);a.RemoveParameter("EWA_AJAX");var c=this;this._Ajax.PostNew(a.GetUrl(),
function(){c._CallBack(b)});this.REPLACE_HISTORY_STATE&&this.replaceHistoryState()};this.createReplaceHistoryStateUrl=function(){var a=new EWA_UrlClass;a.SetUrl(this.Url==null?document.location.href:this.Url);a.AddParameter(this._PageCurrentName,this._PageCurrent);this._PageSize&&a.AddParameter(this._PageSizeName,this._PageSize);this._Sort!=null&&a.AddParameter(this._SortName,this._Sort);var b=this.SearchGetExpInit();b&&a.AddParameter("EWA_SEARCH",b);if(this.GotoParas!=null&&this.GotoParas.length>
0)for(b=0;b<this.GotoParas.length;b+=1)a.AddParameter(this.GotoParas[b].Name,this.GotoParas[b].Value);a.RemoveParameter("EWA_AJAX");return a};this.replaceHistoryState=function(){let a=this.createReplaceHistoryStateUrl();window.history.replaceState("",null,a.GetUrl())};this.CreateAjax=function(){var a=new EWA.C.Ajax;a.LoadingType="image";a.AddParameter(this._PageCurrentName,this._PageCurrent);this._PageSize&&a.AddParameter(this._PageSizeName,this._PageSize);this._Sort!=null&&a.AddParameter(this._SortName,
this._Sort);this._SearchExp!=null&&this._SearchExp!=""&&a.AddParameter("EWA_LF_SEARCH",this._SearchExp);a.AddParameter("EWA_AJAX","LF_RELOAD");if(this.GotoParas!=null&&this.GotoParas.length>0)for(var b=0;b<this.GotoParas.length;b+=1)a.AddParameter(this.GotoParas[b].Name,this.GotoParas[b].Value);return a};this.DownlodData=function(a,b){EWA.F.CID=this._Id;this._Ajax=new EWA.C.Ajax;this._Ajax.LoadingType="image";this._Sort!=null&&this._Ajax.AddParameter(this._SortName,this._Sort);this._SearchExp!=null&&
this._SearchExp!=""&&this._Ajax.AddParameter("EWA_LF_SEARCH",this._SearchExp);this._Ajax.AddParameter("EWA_AJAX","DOWN_DATA");this._Ajax.AddParameter("EWA_AJAX_DOWN_TYPE",a);b&&this._Ajax.AddParameter("EWA_ACTION",b);if(this.GotoParas!=null&&this.GotoParas.length>0)for(a=0;a<this.GotoParas.length;a+=1)this._Ajax.AddParameter(this.GotoParas[a].Name,this.GotoParas[a].Value);this._AjaxType="DOWN_DATA";a=new EWA_UrlClass;a.SetUrl(this.Url==null?document.location.href:this.Url);var c=this;this._Ajax.PostNew(a.GetUrl(),
function(){c._CallBack()})};this.Get=function(a){EWA.F.CID=this._Id;this._Ajax=new EWA.C.Ajax;this._Ajax.LoadingType="image";var b=this;this._Ajax.Get(a,function(){b._CallBack()})};this.Post=function(a,b){EWA.F.CID=this._Id;this._Ajax=new EWA.C.Ajax;this._Ajax.LoadingType="image";var c=this;this._Ajax.Post(a,b,function(){c._CallBackJs()})};this._CallBack=function(a){var b=this._Ajax;if(b._Http.readyState==4){b.HiddenWaitting();if(top.EWA&&top.EWA.MQE){var c=new EWA_MqeClass;c.Data=b.GetRst();c.FromId=
EWA.F.CID;c.Type="POST";top.EWA.MQE.AddMqe(c)}b._Http.status==200?(b=b._Http.responseText,EWA.F.CheckCallBack(b)&&(this._AjaxType=="DOWN_DATA"?(this._AjaxType=null,b.trim()=="DENY"?alert(_EWA_INFO_MSG["EWA.LF.DOWN_DATA.DENY"]):window.location.href=b.trim()):(c=$X("EWA_LF_"+this._Id).parentNode,$(c).replaceWith(b),this.SearchMark(),this.IsReShow&&this.ReShowWithNoButtons(),this._ReShowSearchQuick(),this._IsDblClick!=null&&this.DblClick(this._IsDblClick),this._IsAddPreRow&&this.AddPreRow(),this.SubBottomsArray&&
this._SubBottoms(),this._isStickyHeaders&&this.stickyHeaders(),this.ReloadAfter&&this.ReloadAfter(a),this.ReloadAfterApp&&this.ReloadAfterApp(a),this._IsCheckedAll=!0))):alert("ERROR:\r\n"+b._Http.statusText);this.RewriteInfo()}};this._CallBackJs=function(){var a=this._Ajax;if(a._Http.readyState==4){if(top.EWA&&top.EWA.MQE){var b=new EWA_MqeClass;b.Data=a.GetRst();b.FromId=EWA.F.CID;b.Type="POST";top.EWA.MQE.AddMqe(b)}a.HiddenWaitting();a._Http.status==200?(a=a._Http.responseText,EWA.F.CheckCallBack(a)&&
eval(a)):alert("ERROR:\r\n"+a._Http.statusText);this.EditAfterEvent&&this.EditAfterEvent()}};this.EditAfterEvent=function(){this.getUrlClass().GetParameter("EWA_ROW_SIGN")&&this.refreshPage()};this._GetItem=function(a){for(var b=this.ItemList,c=0;c<b.length;c+=1){var d=b[c],e=this._GetSubItem("Name",d);if(this.Xml.GetElementAttribute(e,"Name")==a)return d}};this._GetSubItem=function(a,b){b=this.Xml.GetElements(a+"/Set",b);if(b==null||b.length==0)return null;if(a=="DescriptionSet")for(a=0;a<b.length;a+=
1){var c=b[a].getAttribute("Lang");if(c!=null&&c.trim().toUpperCase()==EWA.LANG.trim().toUpperCase())return b[a]}return b[0]};this._GetSubValue=function(a,b,c){a=this._GetSubItem(a,c);return a==null?null:this.Xml.GetElementAttribute(a,b)};this.AddColumns=function(a,b,c,d,e,g,h,k){var l=$X("EWA_LF_"+this._Id);k==null&&(k=l.rows[0].cells.length);for(var m=0;m<a.length;m++)for(var n=a[m],p=k+m,r=0;r<l.rows.length;r++){var q=l.rows[r].insertCell(p);q.title=n[d];if(r==0)q.className="EWA_TD_H ewa-add-column ewa-col-"+
n[b],q.innerHTML='<nobr cellIdx="'+p+'" id="'+n[b]+'">'+n[c]+"</nobr>",q.title=n[d],q.width=100;else{var t=l.rows[r].getAttribute("EWA_KEY");q.className="EWA_TD_M ewa-add-column ewa-col-"+n[b];t=t+"_"+n[b];var v=this._GetAddControl(n[g],n[c],n[d]);v==null&&(v=e);var u=h.replace("[ID]",n[b]);v=v.indexOf("<select")>=0?v.replace("!!",u.replace("onblur=","onchange=")):v.replace("!!",u);q.innerHTML=v;q.childNodes[0].id=t}}};this._GetAddControl=function(a,b,c){if(a==null)return null;a=a.toUpperCase().trim();
b=b?b.replace(/</ig,"&lt;").replace(/>/ig,"&gt;").replace(/"/ig,"&quot;"):"";c=c?c.replace(/</ig,"&lt;").replace(/>/ig,"&gt;").replace(/"/ig,"&quot;"):"";return a=="SELECT"?'<select des="'+b+'"  title="'+c+'" !!></select>':a=="DATE"?'<input type=text des="'+b+'" title="'+c+'" class=EWA_INPUT onclick="EWA.UI.Calendar.Pop(this,false);" readonly !!>':a=="TIME"?'<input type=text des="'+b+'" title="'+c+'"  class=EWA_INPUT onclick="EWA.UI.Calendar.Pop(this,true);" readonly !!>':a=="STRING"||a=="NUMBER"?
'<input type=text des="'+b+'"  title="'+c+'" class=EWA_INPUT !!>':null};this.AddedValues=function(a,b,c,d,e){for(var g=0;g<a.length;g++){var h=a[g],k=$X(h[b]+"_"+h[c]);if(!k)break;var l=k.tagName.toUpperCase();l=="INPUT"||l=="SELECT"||l=="TEXTAREA"?k.value=h[d]:k.innerHTML=h[d];e&&(this.GetRow(k).getElementsByTagName("input")[0].checked=!0)}};this.AddRow=function(a){for(var b=$X("EWA_LF_"+this._Id),c=b.insertRow(-1),d=0;d<b.rows[0].cells.length;d++){var e=c.insertCell(-1),g=b.rows[0].cells[d];e.className=
"EWA_TD_M";a[d]!=null&&(e.innerHTML=a[d]);e.style.display=g.style.display;e.id="ADD_ROW_"+g.childNodes[0].id}return c};this.Calc=function(a,b,c,d){for(var e=$X("EWA_LF_"+this._Id),g={},h=0;h<a.length;h++)g[a[h]]=0;for(h=b;h<c;h++)for(a=e.rows[h],b=0;b<a.cells.length;b++)if(g[b]!=null){var k=a.cells[b],l=k.getElementsByTagName("input");l.length>0?g[b]+=l[0].value*1:(k=GetInnerText(k),k==""&&(k=0),g[b]+=k*1)}c=e.rows[d];for(h=0;h<c.cells.length;h++)g[h]!=null&&(k=Math.round(g[h]*100)/100,d=GetInnerText(c.cells[h]),
k!=d*1&&(this.IsCalcChanged=!0),c.cells[h].innerHTML=k.fm())};this.mergeHeaders=function(a,b,c){var d=$($X("EWA_LF_"+this._Id)),e=d.find('tr[ewa_tag="HEADER"]');if(e.length!=0&&(a=e.find('nobr[id="'+a+'"]').parent(),a.length!=0)){var g=d.find("tr[ewa_mearge]");g.length==0&&(g=$(d[0].insertRow(1)),g.attr("ewa_mearge","gdx"));d=-1;for(var h=0;h<e[0].cells.length;h++){var k=e[0].cells[h];k==a[0]&&(d=h,k.colSpan=c);if(d==-1)$(k).attr("ewa_tdmearged")!="1"&&(k.rowSpan=2);else if(d+c==h)d=-1,k.rowSpan=
