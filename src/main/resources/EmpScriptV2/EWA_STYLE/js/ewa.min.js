...
return b};this._SearchCreateItm=function(){var a="EWA_SEARCH_ITEM_"+this.Id,b=[],c=[],d=[],e={},g;for(g in this._SearchJson)this._SearchJson[g].ORI_NAME=g,e[g.toUpperCase()]=this._SearchJson[g];this._SearchJson=e;var h="EWA.F.FOS[&quot;"+this._Id+"&quot;].ChangeSearchTextType(this)",k="EWA.F.FOS[&quot;"+this._Id+"&quot;].ChangeSearchDateType(this)",l=$J2MAP(this.RESOURCES.search_text_items,"Id");e=this._GetInitSearchMap();var m=!1,n=!1;for(g in this.ItemList.Items)if(this._SearchJson[g]!=null){var p=
[],r=this._SearchJson[g],q=r.T,t=this._GetSubValue("DescriptionSet","Info",this.ItemList.Items[g]);p.push("<table class='ewa-lf-search-item ewa-ref-"+g+" ewa-lf-search-type-"+q+" "+(q=="fix"?"ewa-lf-search-fix-m"+(r.M||"0"):"")+"' cellspacing=0 cellpadding=0>");p.push("<tr><td class='ewa-lf-search-item-title' style='position:relative'><nobr><b>");p.push(t);var v=t="",u="lk";e[g.toUpperCase()]&&(v=e[g.toUpperCase()],u=v.tag,t=v.para1,v=v.para2);var w="";q=="text"?(n=l[u]||l.lk,w=EWA.LANG=="enus"?n.TxtEn:
n.Txt,p.push("<a class='ewa-lf-search-type' tag='"+u+"' href='javascript:void(0)' onclick='"+h+"'>"),p.push(w),p.push("</a>"),n=!0):q=="date"&&(p.push("<a  href='javascript:void(0)' class='fa fa-calendar-check-o'  onclick='"+k+"'></a>"),m=!0);p.push("</b></nobr></td><td class='ewa-lf-search-item-ctl'>");q=="text"?u=="blk"||u=="nblk"?(r=this._SearchSingle(g,w),r=r.replace("<input","<input disabled "),p.push(r)):p.push(this._SearchSingle(g,t)):q=="date"?p.push(this._SearchDate(g,t,v)):q=="number"?p.push(this._SearchNumber(g,
t,v)):q=="fix"&&(r="<div >"+this._SearchFix(g,r,"span",t)+"</div>",p.push(r));p.push("</td></tr></table>");q=="fix"&&this._IsSearchGroup?c.push(p.join("")):d.push(p.join(""))}if(d.length+c.length!=0){b.push('<table class="ewa-lf-search-main" id="'+a+'" cellspacing="0" cellpadding="0">');b.push("<tr><td class='ewa-lf-search-des'></td><td>");d.length>0&&b.push(" "+d.join("")+" ");for(d=0;d<c.length;d++)b.push(" "+c[d]+" ");b.push("</td></tr></table>");var x=document.createElement("div");this._SEARCH_BOX=
x;x.className="ewa-lf-search";x.innerHTML=b.join("");b=$X("EWA_LF_"+this.Id).parentNode;b.parentNode.insertBefore(x,b);a="EWA_SEARCH_ITEM_"+this.Id;var y=this;m&&this._SearchCreateDateRange();n&&this._SearchCreateTextType();y._SEARCH_ITEM_EXP=this.SearchGetExp(x);JSON.stringify(e)!="{}"&&(y._SearchExp=y._SEARCH_ITEM_EXP);$(x).find("input[type=text]").on("compositionstart",function(){y._is_search_composition=!0}).on("compositionend",function(){y._is_search_composition=!1});this._TIMER_SEARCH=window.setInterval(function(){if(!y._is_search_composition){try{if(!$X(a)){window.clearInterval(y._TIMER_SEARCH);
return}}catch(A){window.clearInterval(y._TIMER_SEARCH);return}var z=y.SearchGetExp(x);z!=y._SEARCH_ITEM_EXP&&(y._SEARCH_ITEM_EXP=z,y.DoSearch($X(a)))}},700)}};this._SearchSingle=function(a,b){var c=[];c.push('<input t="text" type="text" autocomplete="off" class="EWA_INPUT" maxlength="40" name="');c.push(a.toInputValue());c.push('"');b!=null&&(c.push(' value="'),a=b.toInputValue(),c.push(a),c.push('" '));c.push(" />");return c.join("")};this._SearchDate=function(a,b,c){var d=[];d.push("<table border=0 cellpadding=0 cellspacing=0>");
...
d.push(b),d.push('"'));d.push("/></td></tr></table>");return d.join("")};this._SearchNumber=function(a,b,c){var d=[];d.push('<input type="text" t="number" size="10" name="');d.push(a);d.push('"');b&&(b*=1,d.push(' value="'),d.push(b),d.push('"'));d.push(' /><span class="ewa-lf-search-split"> - </span>');d.push('<input type="text" t="number" size="10" name="');d.push(a);d.push('1"');c&&(b=c*1,d.push(' value="'),d.push(b),d.push('"'));d.push("/>");return d.join("")};this._SearchFix=function(a,b,c,d){var e=
[],g="radio";if(b.M=="1")g="checkbox";else if(b.M=="2")return this._SearchFix1(a,b,c,d);c="<"+c+"><nobr><input t='fix' [@CHK] id='@ID' value=\"@V\" type='"+g+"' name='"+a+"' /><label for='@ID'>@T</label></nobr></"+c+"> ";g={};if(d){d=d.split(";");for(var h in d)g[d[h].trim().toUpperCase()]=1}for(h=0;h<b.D.length;h++){var k=b.D[h];d=k[0];k=k[1];var l=this.Id+"_"+a+"_SearchFix_"+h,m=c.replace("@ID",l);m=m.replace("@ID",l);m=m.replace("@V",d);m=g[d.toUpperCase().trim()]?m.replace("[@CHK]","checked"):
...
[];a.push("<div><table border=0 style='margin:0px;' class=EWA_TABLE cellspacing=1>");for(var b in this.ItemList.Items)if(this._SearchJson[b]!=null){var c=this._SearchJson[b],d=c.T,e=this._GetSubValue("DescriptionSet","Info",this.ItemList.Items[b]);a.push("<tr><td class=EWA_TD_L width=140><nobr>"+e+"<nobr></td><td width=350 class=EWA_TD_M>");d=="text"?a.push(this._SearchSingle(b)):d=="date"?a.push(this._SearchDate(b)):d=="number"?a.push(this._SearchNumber(b)):d=="fix"&&a.push(this._SearchFix(b,c,"div"));
...
this._SearchHtml=a.join("")};this.createRegExp=function(a){if(!a)return null;a=String(a).trim();if(a.length===0)return null;for(var b=[],c=0;c<a.length;c++){var d=a[c];d>="a"&&d<="z"||d>="A"&&d<="Z"||d>="0"&&d<="9"?b.push(d):b.push("\\"+d)}try{return new RegExp("("+b.join("")+")")}catch(e){return console.warn("new RegExp",a,e),null}};this.SearchMark=function(){for(var a=this._SearchExp.split("@!@"),b=$X("EWA_LF_"+this._Id),c=0;c<a.length;c+=1){var d=a[c].split("~!~");if(!(d.length<3))for(var e=2;e<
d.length;e++){var g=d[e];if(g=="")continue;if(d[1]=="fix"){var h=d[2];$("table#EWA_SEARCH_ITEM_"+this.Id+' [name="'+d[0]+'"]').find("option").each(function(){this.value==h&&(g=this.text)})}let l=this.createRegExp(g),m=d[0].split(",");for(let n in m){var k=m[n];this._SearchJson[k]&&(k=this._SearchJson[k].ORI_NAME);k=$(b).find(".ewa-lf-data-row .ewa-col-"+k).not("[ewa-search-marked=yes]");k.attr("ewa-search-marked","yes");try{k.find("*").each(function(){this.children.length==0&&(this.innerHTML=this.innerHTML.replace(l,
"<font color=red><b>$1</b></font>"))})}catch(p){console.log(p)}}}}};this.DblClick=function(a){this._IsDblClick=a;for(var b=$X("EWA_LF_"+this._Id),c=1;c<b.rows.length;c++)b.rows[c].ondblclick=function(){var d=this.getElementsByTagName("input");d.length>a&&d[a].disabled==0&&d[a].click()}};this.Init=function(a){this.Xml=new EWA.C.Xml;this.Xml.LoadXml(a);this.ItemList.Init(this.Xml);this.ItemList.F=this;this.Resources=new EWA_FrameResoures;this.Resources.Init(this.Xml);if((a=$("#EWA_LF_"+this._Id)[0])&&
a.tagName=="table"&&!(a.rows.length<=1)){a=a.rows[1].getElementsByTagName("input");for(var b=0;b<a.length;b++)if(a[b].type.toUpperCase()=="RADIO"){this._TrSelectMulti=!1;break}}};this.SetPageParameters=function(a,b,c,d,e){this._PageCurrent=a;this._PageSize=c;this._PageCount=b;this._RecordCount=d;this._Sort=e};this.SetPageParametersName=function(a,b,c,d,e){this._PageCurrentName=a;this._PageSizeName=c;this._PageCountName=b;this._RecordCountName=d;this._SortName=e};this.CheckedAll=function(){var a=document.getElementById("EWA_LF_"+
this._Id);if(!(a.rows.length<=1)){for(var b=0;b<a.rows.length;b+=1){var c=a.rows[b].getElementsByTagName("input");c.length!=0&&(c[0].checked=this._IsCheckedAll)}this._IsCheckedAll=this._IsCheckedAll?!1:!0;this.CheckedAllAfter&&this.CheckedAllAfter(a)}};this.SelectChecked=function(){var a=$X("EWA_LF_"+this._Id);if(!a||a.rows.length<1)return"";var b=[];let c=[];$("#EWA_LF_"+this._Id+">tbody>tr").find("input[type='radio'],input[type='checkbox']").each(function(){this.parentNode.className.indexOf("ewa-switch")==
-1&&c.push(this)});if(c.length>0)for(var d=0;d<c.length;d++)c[d].checked&&b.push(c[d].value);else for(d=0;d<a.rows.length;d++){var e=a.rows[d];e.getAttribute("__ewa_lf_mdown")==1&&b.push(e.getAttribute("EWA_KEY"))}return b.join(",")};this.SelectCheckedInputs=function(){var a=[],b=$X("EWA_LF_"+this._Id);if(b.rows.length<1)return a;b=b.getElementsByTagName("input");a=[];for(var c=0;c<b.length;c++)(b[c].type.toUpperCase()=="CHECKBOX"||b[c].type.toUpperCase()=="RADIO")&&b[c].checked&&a.push(b[c]);return a};
this.SelectCheckedRows=function(){for(var a=[],b=$X("EWA_LF_"+this._Id),c=0;c<b.rows.length;c++){var d=b.rows[c],e=d.getElementsByTagName("input");e.length!=0&&e[0].checked&&e[0].parentNode.className.indexOf("ewa-switch")==-1&&a.push(d)}if(a.length==0)for(c=0;c<b.rows.length;c++)d=b.rows[c],d.getAttribute("__ewa_lf_mdown")==1&&a.push(d);return a};this.Reload=function(a){this.StopAjaxAfterReload?this.StopAjaxAfterReload=!1:this.Goto(this._PageCurrent,a)};this.refreshPage=function(a,b,c){this.replaceRowsData(null,
null,a,b,c)};this.replaceRowsData=function(a,b,c,d,e){var g=this.getUrlClass();g.AddParameter("EWA_AJAX","LF_RELOAD");g.AddParameter("EWA_IS_SPLIT_PAGE","no");g.AddParameter("EWA_IS_HIDDEN_CAPTION","yes");g=g.GetUrl();a&&(g+="&"+a);let h=a?new EWA_AjaxClass:this.CreateAjax(),k=this;h.PostNew(g,function(){if(h._Http.readyState==4){h.HiddenWaitting();var l=h._Http.responseText;h._Http.status!=200?(console.error(l),alert("ERROR:\r\n"+h._Http.statusText)):EWA.F.CheckCallBack(l)?k.replaceRowsWithDataHtml(l,
b,c,d,e):console.error(l)}})};this.replaceRowsWithDataHtml=function(a,b,c,d,e){let g=$("<div></div>");g.html(a);let h=[],k=$("#EWA_LF_"+this._Id);g.find(".ewa-lf-data-row").each(function(){var l='tr[ewa_key="'+$(this).attr("ewa_key")+'"]';let m=k.find(l);if(m.length==0)console.warn(l+" not found");else{l=$(this).attr("ewa_row_sign");var n=m.attr("ewa_row_sign"),p=!0;(l||n)&&n===l&&(p=!1);if(p){m.attr("ewa_row_sign",l);l=m.clone()[0];for(n=0;n<this.cells.length;n++){p=m[0].cells[n];let r=this.cells[n];
p.removeAttribute("ewa-merged");p.removeAttribute("ewa-search-marked");b?b(r,p):p.innerHTML=r.innerHTML}h.push({before:l,current:m[0]})}}});g.remove();h.length!=0&&(this.SearchMark(),this.IsReShow&&this.ReShowWithNoButtons(),this._ReShowSearchQuick(),this._IsDblClick!=null&&this.DblClick(this._IsDblClick),this.SubBottomsArray&&this._SubBottoms(),!e&&this.ReloadAfter&&this.ReloadAfter(c),!e&&this.ReloadAfterApp&&this.ReloadAfterApp(c));d&&d(h)};this.DoAction=function(a,b,c,d,e,g){EWA.F.CID=this._Id;
if(b){if(a&&$(a).parent().hasClass("scroll-delete")){var h=$(a).attr("do-action-time")||0,k=(new Date).getTime();if(k-h<1E3)return;$(a).attr("do-action-time",k)}this.lastAction=b;this._Ajax=new EWA_AjaxClass;this._Ajax.LoadingType="image";this._Ajax.AddParameter("EWA_AJAX","1");this._Ajax.AddParameter("EWA_ACTION",b);this._Ajax.AddParameter("EWA_ID",this._Id);this._Ajax.AddParameter("EWA_NO_CONTENT","1");this._Ajax.AddParameter("EWA_ACTION_TIP",d);g!=null&&g.trim().length>0?(this._Ajax.AddParameter("EWA_AFTER_EVENT",
g),this.StopAjaxAfterReload=!0,this._Ajax.AddParameter("EWA_ACTION_RELOAD","0")):this.StopAjaxAfterReload=!1;if(a&&a.getAttribute&&a.getAttribute("box_menu_item")=="yes"){if(this._Ajax.AddParameter("___CALL__FROM__","box"),a=a.parentNode)a=a.getAttribute("json_ref"),a=JSON.parse(a),a.EWA_KEY&&this._Ajax.AddParameter("EWA_ACTION_KEY",a.EWA_KEY)}else(a=this.GetRowKey(a))&&this._Ajax.AddParameter("EWA_ACTION_KEY",a);a=this.SelectChecked();this._Ajax.AddParameter("IDS_SPLIT",a);if(e!=null&&e.length>0)for(a=
0;a<e.length;a+=1)this._Ajax.AddParameter(e[a].Name,e[a].Value);e=new EWA.C.Url;e.SetUrl(this.Url);var l=e.RemoveParameter("EWA_ACTION"),m=this;c?((e=_EWA_INFO_MSG[c])||(e=c),$Confirm(e,"Confirm",function(){m._Ajax.PostNew(l,function(){m._CallBackJs()})})):this._Ajax.PostNew(l,function(){m._CallBackJs()})}};this.NewPageSize=function(a){this._PageSize!=a&&(this._PageSize=a,this.Goto(1))};this.Goto=function(a,b){EWA.F.CID=this._Id;this._PageCurrent=a;this._Ajax=this.CreateAjax();a=new EWA_UrlClass;
a.SetUrl(this.Url==null?document.location.href:this.Url);a.RemoveParameter("EWA_AJAX");var c=this;this._Ajax.PostNew(a.GetUrl(),function(){c._CallBack(b)});this.REPLACE_HISTORY_STATE&&this.replaceHistoryState()};this.createReplaceHistoryStateUrl=function(){var a=new EWA_UrlClass;a.SetUrl(this.Url==null?document.location.href:this.Url);a.AddParameter(this._PageCurrentName,this._PageCurrent);this._PageSize&&a.AddParameter(this._PageSizeName,this._PageSize);this._Sort!=null&&a.AddParameter(this._SortName,
this._Sort);var b=this.SearchGetExpInit();b&&a.AddParameter("EWA_SEARCH",b);if(this.GotoParas!=null&&this.GotoParas.length>0)for(b=0;b<this.GotoParas.length;b+=1)a.AddParameter(this.GotoParas[b].Name,this.GotoParas[b].Value);a.RemoveParameter("EWA_AJAX");return a};this.replaceHistoryState=function(){let a=this.createReplaceHistoryStateUrl();window.history.replaceState("",null,a.GetUrl())};this.CreateAjax=function(){var a=new EWA.C.Ajax;a.LoadingType="image";a.AddParameter(this._PageCurrentName,this._PageCurrent);
this._PageSize&&a.AddParameter(this._PageSizeName,this._PageSize);this._Sort!=null&&a.AddParameter(this._SortName,this._Sort);this._SearchExp!=null&&this._SearchExp!=""&&a.AddParameter("EWA_LF_SEARCH",this._SearchExp);a.AddParameter("EWA_AJAX","LF_RELOAD");if(this.GotoParas!=null&&this.GotoParas.length>0)for(var b=0;b<this.GotoParas.length;b+=1)a.AddParameter(this.GotoParas[b].Name,this.GotoParas[b].Value);return a};this.DownlodData=function(a,b){EWA.F.CID=this._Id;this._Ajax=new EWA.C.Ajax;this._Ajax.LoadingType=
"image";this._Sort!=null&&this._Ajax.AddParameter(this._SortName,this._Sort);this._SearchExp!=null&&this._SearchExp!=""&&this._Ajax.AddParameter("EWA_LF_SEARCH",this._SearchExp);this._Ajax.AddParameter("EWA_AJAX","DOWN_DATA");this._Ajax.AddParameter("EWA_AJAX_DOWN_TYPE",a);b&&this._Ajax.AddParameter("EWA_ACTION",b);if(this.GotoParas!=null&&this.GotoParas.length>0)for(a=0;a<this.GotoParas.length;a+=1)this._Ajax.AddParameter(this.GotoParas[a].Name,this.GotoParas[a].Value);this._AjaxType="DOWN_DATA";
a=new EWA_UrlClass;a.SetUrl(this.Url==null?document.location.href:this.Url);var c=this;this._Ajax.PostNew(a.GetUrl(),function(){c._CallBack()})};this.Get=function(a){EWA.F.CID=this._Id;this._Ajax=new EWA.C.Ajax;this._Ajax.LoadingType="image";var b=this;this._Ajax.Get(a,function(){b._CallBack()})};this.Post=function(a,b){EWA.F.CID=this._Id;this._Ajax=new EWA.C.Ajax;this._Ajax.LoadingType="image";var c=this;this._Ajax.Post(a,b,function(){c._CallBackJs()})};this._CallBack=function(a){var b=this._Ajax;
if(b._Http.readyState==4){b.HiddenWaitting();if(top.EWA&&top.EWA.MQE){var c=new EWA_MqeClass;c.Data=b.GetRst();c.FromId=EWA.F.CID;c.Type="POST";top.EWA.MQE.AddMqe(c)}b._Http.status==200?(b=b._Http.responseText,EWA.F.CheckCallBack(b)&&(this._AjaxType=="DOWN_DATA"?(this._AjaxType=null,b.trim()=="DENY"?alert(_EWA_INFO_MSG["EWA.LF.DOWN_DATA.DENY"]):window.location.href=b.trim()):(c=$X("EWA_LF_"+this._Id).parentNode,$(c).replaceWith(b),this.SearchMark(),this.IsReShow&&this.ReShowWithNoButtons(),this._ReShowSearchQuick(),
this._IsDblClick!=null&&this.DblClick(this._IsDblClick),this._IsAddPreRow&&this.AddPreRow(),this.SubBottomsArray&&this._SubBottoms(),this._isStickyHeaders&&this.stickyHeaders(),this.ReloadAfter&&this.ReloadAfter(a),this.ReloadAfterApp&&this.ReloadAfterApp(a),this._IsCheckedAll=!0))):alert("ERROR:\r\n"+b._Http.statusText);this.RewriteInfo()}};this._CallBackJs=function(){var a=this._Ajax;if(a._Http.readyState==4){if(top.EWA&&top.EWA.MQE){var b=new EWA_MqeClass;b.Data=a.GetRst();b.FromId=EWA.F.CID;b.Type=
"POST";top.EWA.MQE.AddMqe(b)}a.HiddenWaitting();a._Http.status==200?(a=a._Http.responseText,EWA.F.CheckCallBack(a)&&eval(a)):alert("ERROR:\r\n"+a._Http.statusText);this.EditAfterEvent&&this.EditAfterEvent()}};this.EditAfterEvent=function(){this.getUrlClass().GetParameter("EWA_ROW_SIGN")&&this.refreshPage()};this._GetItem=function(a){for(var b=this.ItemList,c=0;c<b.length;c+=1){var d=b[c],e=this._GetSubItem("Name",d);if(this.Xml.GetElementAttribute(e,"Name")==a)return d}};this._GetSubItem=function(a,
b){b=this.Xml.GetElements(a+"/Set",b);if(b==null||b.length==0)return null;if(a=="DescriptionSet")for(a=0;a<b.length;a+=1){var c=b[a].getAttribute("Lang");if(c!=null&&c.trim().toUpperCase()==EWA.LANG.trim().toUpperCase())return b[a]}return b[0]};this._GetSubValue=function(a,b,c){a=this._GetSubItem(a,c);return a==null?null:this.Xml.GetElementAttribute(a,b)};this.AddColumns=function(a,b,c,d,e,g,h,k){var l=$X("EWA_LF_"+this._Id);k==null&&(k=l.rows[0].cells.length);for(var m=0;m<a.length;m++)for(var n=
a[m],p=k+m,r=0;r<l.rows.length;r++){var q=l.rows[r].insertCell(p);q.title=n[d];if(r==0)q.className="EWA_TD_H ewa-add-column ewa-col-"+n[b],q.innerHTML='<nobr cellIdx="'+p+'" id="'+n[b]+'">'+n[c]+"</nobr>",q.title=n[d],q.width=100;else{var t=l.rows[r].getAttribute("EWA_KEY");q.className="EWA_TD_M ewa-add-column ewa-col-"+n[b];t=t+"_"+n[b];var v=this._GetAddControl(n[g],n[c],n[d]);v==null&&(v=e);var u=h.replace("[ID]",n[b]);v=v.indexOf("<select")>=0?v.replace("!!",u.replace("onblur=","onchange=")):
v.replace("!!",u);q.innerHTML=v;q.childNodes[0].id=t}}};this._GetAddControl=function(a,b,c){if(a==null)return null;a=a.toUpperCase().trim();b=b?b.replace(/</ig,"&lt;").replace(/>/ig,"&gt;").replace(/"/ig,"&quot;"):"";c=c?c.replace(/</ig,"&lt;").replace(/>/ig,"&gt;").replace(/"/ig,"&quot;"):"";return a=="SELECT"?'<select des="'+b+'"  title="'+c+'" !!></select>':a=="DATE"?'<input type=text des="'+b+'" title="'+c+'" class=EWA_INPUT onclick="EWA.UI.Calendar.Pop(this,false);" readonly !!>':a=="TIME"?'<input type=text des="'+
b+'" title="'+c+'"  class=EWA_INPUT onclick="EWA.UI.Calendar.Pop(this,true);" readonly !!>':a=="STRING"||a=="NUMBER"?'<input type=text des="'+b+'"  title="'+c+'" class=EWA_INPUT !!>':null};this.AddedValues=function(a,b,c,d,e){for(var g=0;g<a.length;g++){var h=a[g],k=$X(h[b]+"_"+h[c]);if(!k)break;var l=k.tagName.toUpperCase();l=="INPUT"||l=="SELECT"||l=="TEXTAREA"?k.value=h[d]:k.innerHTML=h[d];e&&(this.GetRow(k).getElementsByTagName("input")[0].checked=!0)}};this.AddRow=function(a){for(var b=$X("EWA_LF_"+
this._Id),c=b.insertRow(-1),d=0;d<b.rows[0].cells.length;d++){var e=c.insertCell(-1),g=b.rows[0].cells[d];e.className="EWA_TD_M";a[d]!=null&&(e.innerHTML=a[d]);e.style.display=g.style.display;e.id="ADD_ROW_"+g.childNodes[0].id}return c};this.Calc=function(a,b,c,d){for(var e=$X("EWA_LF_"+this._Id),g={},h=0;h<a.length;h++)g[a[h]]=0;for(h=b;h<c;h++)for(a=e.rows[h],b=0;b<a.cells.length;b++)if(g[b]!=null){var k=a.cells[b],l=k.getElementsByTagName("input");l.length>0?g[b]+=l[0].value*1:(k=GetInnerText(k),
k==""&&(k=0),g[b]+=k*1)}c=e.rows[d];for(h=0;h<c.cells.length;h++)g[h]!=null&&(k=Math.round(g[h]*100)/100,d=GetInnerText(c.cells[h]),k!=d*1&&(this.IsCalcChanged=!0),c.cells[h].innerHTML=k.fm())};this.mergeHeaders=function(a,b,c){var d=$($X("EWA_LF_"+this._Id)),e=d.find('tr[ewa_tag="HEADER"]');if(e.length!=0&&(a=e.find('nobr[id="'+a+'"]').parent(),a.length!=0)){var g=d.find("tr[ewa_mearge]");g.length==0&&(g=$(d[0].insertRow(1)),g.attr("ewa_mearge","gdx"));d=-1;for(var h=0;h<e[0].cells.length;h++){var k=
e[0].cells[h];k==a[0]&&(d=h,k.colSpan=c);if(d==-1)$(k).attr("ewa_tdmearged")!="1"&&(k.rowSpan=2);else if(d+c==h)d=-1,k.rowSpan=2;else{k.rowSpan=1;var l=g[0].insertCell(-1);l.innerHTML=k.innerHTML;l.className=k.className;l.style.cssText=k.style.cssText;k.style.display="none";$(k).attr("ewa_tdmearged",1)}k==a[0]&&(k.innerHTML=b,k.style.display="",k.style.width="")}}};this.MeargeHeader=function(a,b,c){console.log("\u62fc\u5199\u9519\u8bef\uff0c\u8bf7\u7528 mergeHeaders");this.mergeHeaders(a,b,c)};this.getUrlClass=
