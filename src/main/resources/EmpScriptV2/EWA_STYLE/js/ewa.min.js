...
a+'"]').html(),h.find('tr[ewa_tag="HEADER"] [id="'+b+'"]').append(g));h.find('tr[ewa_tag="HEADER"] [id="'+a+'"]').parent().hide();h.find("td#ADD_ROW_"+a).hide()};this.Mearge=function(a,b,d){console.log("\u62fc\u5199\u9519\u8bef\uff0c\u8bf7\u7528 Merge");this.Merge(a,b,d)};this.MergeExp=function(a,b,d,e,g){if(b){var h=$("#EWA_LF_"+this._Id),k=b.match(/@@[a-zA-Z0-9\-\._:]*\b/ig),l=[],m=b,n={};b=[];for(var p=0;p<k.length;p++){var q=k[p];l.push(q);var u=q.replace("@@","");m=m.replace(q,"<span class='ewa-lf-merge ewa-lf-merge-"+
u+" ewa-lf-mearge ewa-lf-mearge-"+u+"' mid=\""+u+'"></span>');u!=a&&(h.find('tr[ewa_tag="HEADER"] [id="'+u+'"]').parent().hide(),h.find("td#ADD_ROW_"+u).hide());q=h.find('tr[ewa_tag="HEADER"] [id="'+u+'"]').html();n[u]=q;b.push(q)}g&&(g=b.join("<span class='ewa-lf-merge-header-split'></span>"),h.find('tr[ewa_tag="HEADER"] [id="'+a+'"]').html(g));h.find("tr[ewa_key],tr.ewa-lf-sub-tr").each(function(){if(0==$(this).find('[id="'+a+'"]').length)console.log("not find "+a);else{var r=$(this).find('[id="'+
a+'"]').parent();if("yes"!=r.attr("ewa-merged")){r.attr("ewa-merged","yes");var t=$('<div style="display:none"></div>');t.html(m);for(var w in l){var v=l[w].replace("@@",""),y=$(this).find('[id="'+v+'"]');if(0!=y.length){v!=a&&y.parent().hide();var x=t.find('span[mid="'+v+'"]');d&&(x.append("<span class='ewa-lf-mearge-memo'></span>"),x.find(".ewa-lf-mearge-memo").html(n[v]));x.append(y)}}for(;0<t[0].childNodes.length;)r.append(t[0].childNodes[0]);$(t).remove();e&&e(r,this)}}})}else console.log("mergeExp \u6ca1\u6709\u8bbe\u7f6e")};
...
this.SubBottomsArray[d],g=0;$("#EWA_LF_"+this._Id+' .ewa-lf-data-row [name="'+e+'"]').each(function(){var k=null!=this.value?this.value.replace(/,/ig,""):this.title;isNaN(k)||(g+=1*k,0<k.indexOf(".")&&(b=2))});var h="<span id='"+e+"' name='"+e+"' class='ewa-lf-sub'>"+g.fm(b)+"</span>";a.find("td[id=ADD_ROW_"+e+"]").html(h)}};this.ChangeRowStyle=function(a,b,d){for(var e=$X("EWA_LF_"+this._Id),g=1;g<e.rows.length;g++){var h=e.rows[g];if(!(0==h.cells.length||h.cells.length<=a)){var k=h.cells[a].childNodes[0].getAttribute(b);
...
b));this.prevTr==g?this.prevTr=null:(this.prevTr=g,this.prevTr.style.display="",this.MDownEvent(this._Id,a,d,this.prevTr,b))}}else this.MDownEvent(a,b)}}};this.AddPreRow=function(a){this._IsAddPreRow||(this.MDownEvent=a);this._IsAddPreRow=!0};this.AddPreRowCheck=function(a,b,d){return!0};this.AddPreRowCloseBeforeEvent=function(a,b,d,e,g){};this.AddPreRowCloseEvent=function(a,b,d,e,g){};this._MSetBg=function(a,b){var d="EWA_TD_M";"down"==b?d="ewa_grid_down":"over"==b&&(d="ewa_grid_mover");for(b=0;b<
a.cells.length;b++)$(a.cells[b]).removeClass("ewa_grid_mover ewa_grid_down EWA_TD_M").addClass(d)};this._ReShowSearchQuick=function(){var a=this,b=EWA_ListFrameClass.prototype.RESOURCES.search_text_items,d,e,g;for(g in b){var h=b[g];"eq"==h.Id&&(d=h);"lk"==h.Id&&(e=h)}for(g in this._SearchJson){var k=this._SearchJson[g];k.isSearchQuick&&(b=$("#EWA_LF_"+this.Id+' .ewa-lf-data-row [id="'+g+'"]'),b.bind("click",function(l){l.stopPropagation();l=$(this).attr("search_tag");var m=$(this).text(),n=m,p=$("#EWA_SEARCH_ITEM_"+
a.Id+' .ewa-lf-search-item-ctl [name="'+this.id.toUpperCase()+'"]');if("number"==l||"date"==l)p=p.parentsUntil(".ewa-lf-search-item").last().find("input");var q=p.parentsUntil(".ewa-lf-search-item").last().find("a");if("date"==l)m=m.split(" ")[0],n=m+" 23:59:59";else if("fix"==l){var u=null;p.find("option").each(function(){this.text==m&&(u=this.value)});m=null===u?"":u}p.val()==m?("text"==l&&e&&0<q.length&&(q.attr("tag",e.Id),q.html("enus"==EWA.LANG?e.TxtEn:e.Txt)),p.val("")):("text"==l&&d&&0<q.length&&
(q.attr("tag",d.Id),q.html("enus"==EWA.LANG?d.TxtEn:d.Txt)),p.val(m),"date"==l&&(p[1].value=n))}),b.each(function(){$(this).text()&&$(this).addClass("ewa-lf-search-text-click").attr("search_tag",k.T)}))}};this.ReShowWithNoButtons=function(){var a=$X("EWA_LF_"+this._Id),b=[],d;for(d in this.ItemList.Items){var e=this.ItemList.Items[d],g=this._GetSubValue("Tag","Tag",e);if(null!=g&&"button"==g.trim().toLowerCase())for(g=0;g<a.rows[0].cells.length;g++)if(a.rows[0].cells[g].childNodes[0].id.toUpperCase()==
...
this.ReShow=function(a){this.IsReShow=!0;var b="_G_"+this._Id;this.NewDivId=b;a=[];a.push("<table class='ewa-lf-reshow' id='EWA_RESHOW_"+this._Id+"'>");a.push("<tr style='display:none'><td style='padding:0'></td></tr>");a.push("<tr><td class='ewa_lf_func' style='padding:0'>");a.push("<div style='display:none'></div>");a.push("<div><div style='cursor:pointer' class='ewa_lf_func_caption'></div></div>");a.push("</td></tr>");a.push("<tr><td style='width:100%;padding:0;vertical-align:top'>");a.push("<div id='"+
...
d.find("tr:eq(0)")[0],l=d.find("td:eq(0)")[0],m=d.find("td:eq(1)")[0],n=d.find("td:eq(2)")[0];e&&(window.setTimeout(function(){$X("__EWA_DEBUG")&&n.childNodes[0].appendChild($X("__EWA_DEBUG"));var B=$X(b);B.getElementsByTagName("table");var D=EWA.UI.Utils.GetDocSize(window),E=B.parentNode.parentNode.previousSibling.offsetHeight;B.style.top=E+"px";B.style.height=D.H-E+"px"},100),addEvent(window,"resize",function(){var B=$X(b),D=EWA.UI.Utils.GetDocSize(window),E=B.parentNode.parentNode.previousSibling.offsetHeight;
B.style.top=E+"px";B.style.height=D.H-E+"px"}));for(;0<a.childNodes.length;)m.childNodes[0].appendChild(a.childNodes[0]);d=d.find(".ewa_lf_func_caption")[0];var p=!1;e=GetInnerText(m);e=e.trim();this._ReshowButs={};g=$(g.rows[0]);for(var q in this.ItemList.Items){var u=this.ItemList.Items[q],r=this._GetSubValue("Tag","Tag",u);if(r&&("button"==r.toLowerCase()||"butflow"==r.toLowerCase())&&(r=this._GetSubValue("Name","Name",u),0!==g.find(".ewa-col-"+r).length)){p=!0;var t=this._GetSubValue("DescriptionSet",
"Info",u),w=this._GetSubValue("DescriptionSet","Memo",u),v=this._GetSubValue("EventSet","EventName",u),y="";null!=v&&"ewa_click"==v.toLowerCase().trim()&&(y=this._GetSubValue("EventSet","EventValue",u));e=e.replace(t,"");u=this._ReShowButton(t,w,null);this._ReshowButs[t]=u;u.setAttribute("f_id",this._Id);u.setAttribute("t_id",r);m.childNodes[1].appendChild(u);""==y?u.setAttribute("onclick","EWA.F.FOS['"+this._Id+"'].reShowButtonClick(this)"):u.setAttribute("onclick",y)}}p&&this._ReShowSplit(m.childNodes[1]);
r=m.getElementsByTagName("input");p={};for(g=0;g<r.length;g++){var x=r[g];x.style.display="none";if(null==x.id||""==x.id)x.id=("_tmp_"+Math.random()).replace(".","_");if("button"==x.type){if(u=this._ReShowButton(x.value,x.title,x.id),t=x.nextSibling){var z="";3==t.nodeType&&(z=t.nodeValue.trimEx());"|"==z&&this._ReShowSplit(m.childNodes[1])}}else(t=x.nextSibling)&&(z=3==t.nodeType?t.nodeValue.replace("|",""):t.innerHTML),u=this._ReShowButton(z.trimEx(),x.title,x.id),u.id=("_tmp_"+Math.random()).replace(".",
"_"),p[x.name]||(p[x.name]=[]),p[x.name].push(u.id);m.childNodes[1].appendChild(u)}e=e.replace(/ /ig,"").replace(/\|/ig,"").trimEx();""!=e?0<=e.indexOf("if")?k.style.display="none":l.innerHTML=e.replace(/ /,""):k.style.display="none";a.appendChild(h);for(q in p)for(z=p[q],g=0;g<z.length;g++)x=$X(z[g]),x.setAttribute("_r_ids",z.join(",")),x.onclick=function(){var B=this.getAttribute("_ewa_event_id");$X(B).click();B=this.getAttribute("_r_ids").split(",");for(var D=0;D<B.length;D++)x.id!=B[D]&&($X(B[D]).style.fontWeight=
"",$X(B[D]).style.color="");this.style.fontWeight="bold";this.style.color="blue"};d.innerHTML='<b><i class="fa fa-star-o">&nbsp;</i>'+this.Description+"</b>";if(0<this.Url.toUpperCase().indexOf("EWA_BTNS_IN_TITLE"))var A=this,C=0,F=setInterval(function(){C++;1E3<C&&window.clearInterval(F);EWA.OW.Load();EWA.OW.Dia&&(window.clearInterval(F),A.ReShowButtonsInDailogTitle())},10)};this.reShowButtonClick=function(a){a=a.getAttribute("t_id");var b=this.SelectCheckedRows();if(null==b||0==b.length)EWA.UI.Msg.Alert(_EWA_INFO_MSG["EWA.SYS.CHOOSE_ITEM"],
...
h,g[h.toUpperCase()]=this._SearchJson[h];this._SearchJson=g;var k="EWA.F.FOS[&quot;"+this._Id+"&quot;].ChangeSearchTextType(this)",l="EWA.F.FOS[&quot;"+this._Id+"&quot;].ChangeSearchDateType(this)",m=$J2MAP(this.RESOURCES.search_text_items,"Id");g=this._GetInitSearchMap();var n=!1,p=!1;for(h in this.ItemList.Items)if(null!=this._SearchJson[h]){var q=[],u=this._SearchJson[h],r=u.T,t=this._GetSubValue("DescriptionSet","Info",this.ItemList.Items[h]);q.push("<table class='ewa-lf-search-item ewa-ref-"+
h+" ewa-lf-search-type-"+r+" "+("fix"==r?"ewa-lf-search-fix-m"+(u.M||"0"):"")+"' cellspacing=0 cellpadding=0>");q.push("<tr><td class='ewa-lf-search-item-title' style='position:relative'><nobr><b>");q.push(t);var w=t="",v="lk";g[h.toUpperCase()]&&(w=g[h.toUpperCase()],v=w.tag,t=w.para1,w=w.para2);var y="";"text"==r?(p=m[v]||m.lk,y="enus"==EWA.LANG?p.TxtEn:p.Txt,q.push("<a class='ewa-lf-search-type' tag='"+v+"' href='javascript:void(0)' onclick='"+k+"'>"),q.push(y),q.push("</a>"),p=!0):"date"==r&&
(q.push("<a  href='javascript:void(0)' class='fa fa-calendar-check-o'  onclick='"+l+"'></a>"),n=!0);q.push("</b></nobr></td><td class='ewa-lf-search-item-ctl'>");"text"==r?"blk"==v||"nblk"==v?(u=this._SearchSingle(h,y),u=u.replace("<input","<input disabled "),q.push(u)):q.push(this._SearchSingle(h,t)):"date"==r?q.push(this._SearchDate(h,t,w)):"number"==r?q.push(this._SearchNumber(h,t,w)):"fix"==r&&(u="<div >"+this._SearchFix(h,u,"span",t)+"</div>",q.push(u));q.push("</td></tr></table>");"fix"==r?
d.push(q.join("")):e.push(q.join(""))}if(0!=e.length+d.length){b.push('<table class="ewa-lf-search-main" id="'+a+'" cellspacing="0" cellpadding="0" style="width:100%">');b.push("<tr><td class='ewa-lf-search-des'></td><td>");0<e.length&&b.push(" "+e.join("")+" ");if(0<d.length)for(e=0;e<d.length;e++)b.push(" "+d[e]+" ");b.push("</td></tr></table>");var x=document.createElement("div");this._SEARCH_BOX=x;x.className="ewa-lf-search";x.innerHTML=b.join("");b=$X("EWA_LF_"+this.Id).parentNode;b.parentNode.insertBefore(x,
b);a="EWA_SEARCH_ITEM_"+this.Id;var z=this;n&&this._SearchCreateDateRange();p&&this._SearchCreateTextType();z._SEARCH_ITEM_EXP=this.SearchGetExp(x);"{}"!=JSON.stringify(g)&&(z._SearchExp=z._SEARCH_ITEM_EXP);$(x).find("input[type=text]").on("compositionstart",function(){z._is_search_composition=!0}).on("compositionend",function(){z._is_search_composition=!1});this._TIMER_SEARCH=window.setInterval(function(){if(!z._is_search_composition){try{if(!$X(a)){window.clearInterval(z._TIMER_SEARCH);return}}catch(C){window.clearInterval(z._TIMER_SEARCH);
return}var A=z.SearchGetExp(x);A!=z._SEARCH_ITEM_EXP&&(z._SEARCH_ITEM_EXP=A,z.DoSearch($X(a)))}},700)}};this._SearchSingle=function(a,b){var d=[];d.push('<input t="text" type="text" autocomplete="off" class="EWA_INPUT" maxlength="40" name="');d.push(a.toInputValue());d.push('"');null!=b&&(d.push(' value="'),a=b.toInputValue(),d.push(a),d.push('" '));d.push(" />");return d.join("")};this._SearchDate=function(a,b,d){var e=[];e.push("<table border=0 cellpadding=0 cellspacing=0>");e.push('<td  ><input type="text" autocomplete="off" class="EWA_INPUT" t="date" readonly maxlength="8" onclick="EWA.UI.Calendar.Pop(this)" name="');
