...
function EWA_FrameClass(){this._Ajax=this.Xml=null;this.ItemList=new EWA_FrameCommonItems;this.Resources={};this.ReloadAfter=this._Id=this._ValidExObj=this._ValidExFail=this._ValidExOk=this.Url=null;this.isShowPostWaitting=!0;this.textareaAutoSize=function(){window.autosize?autosize(this.getObj("textarea").addClass("ewa-textarea-auto-size")):console.warn("autosize.js \u6ca1\u6709\u5f15\u5165\uff0c/third-party/autosize-master/dist/autosize.min.js")};this.getObj=function(a){var b=$("#EWA_FRAME_"+this._Id);
...
this.convertFilterCheckbox=function(a,b,d,e,g,h){function k(v){var y=$("<nobr></nobr>");q.append(y);q.append(" ");var x=$(v).parent();v=$(v).attr("id")+"_"+u;x.attr("id",v);y.append(x.children());y.attr("ref_id",v)}function l(v){v=$(v).parent();var y=v.attr("ref_id");y&&($("#"+y).append(v.children()),v.remove())}var m={},n=this.getObj(".ewa-row-"+a),p=this.getObj(".ewa-row-"+b),q=$("<div class='ewa-filter-checked'></div>");n.find("#"+a).prepend(q).addClass("ewa-filter-chks");var u=this._Id;d||(d=
"PY");n.find("input[type=checkbox]").each(function(){var v=JSON.parse($(this).attr("json"))[d];v&&(v=v.toUpperCase().trim().substring(0,1),m[v]||(m[v]=[]),m[v].push(this.id),this.checked&&k(this),$(this).on("change",function(){this.checked?k(this):l(this)}))});a="ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("");b=["<a class='ewa-filter-char ewa-filter-char-all' code=''>ALL</a>"];for(var r in a){var t=a[r],w=m[t];w&&b.push("<a class='ewa-filter-char' code='"+t+"'>"+t+"<span class='ewa-filter-char-num'>("+w.length+
...
function(a,b,d,e,g,h,k){var l=this;$J(e,function(m){l.createFilterCheckboxByData(a,b,d,m,g,h,k)})};this.createFilterCheckboxByData=function(a,b,d,e,g,h,k){var l={},m=m?"[]":d.split(",");d={};for(var n in m)d[m[n]]=1;m=[];var p=[],q=a.id;for(n in e){var u=e[n],r=u[g];r&&(r=r.replace(/</ig,"&lt;"));var t=u[h];t&&(t=t.replace(/</ig,"&lt;"));var w=u[k],v=d[r],y=[];y.push("<nobr class='ewa-filter-chk' style='float:left;margin-right:4px'>");y.push('<input class="ewa-filter-chkbox" type="checkbox" ');v&&
y.push(" checked ");u=q+"_"+n;y.push(' value="'+r+'" id="'+u+'" name="'+q+'"><label ');y.push(' for="'+u+'">'+t+"</label></nobr>");v?p.push(y.join("")):m.push(y.join(""));b&&(r=w?w.substring(0,1).toUpperCase():"",l[r]||(l[r]=[]),l[r].push(u))}a.innerHTML=p.join(" ")+" "+m.join(" ");if(b){e=['<a href="javascript:void(0)">ALL</a>'];for(n="A".charCodeAt();n<="Z".charCodeAt();n++)g=String.fromCharCode(n),l[g]&&e.push('<a href="javascript:void(0)">'+g+"</a>");b.innerHTML=e.join(" ");$(b).find("a").each(function(){var x=
$(this).text();"ALL"!=x&&$(this).attr("ids",l[x].join(","));$(this).on("click",function(){if("ALL"==$(this).text())$(a).find("nobr.ewa-filter-chk").show();else{var z=$(this).attr("ids").split(","),A={},C;for(C in z)A[z[C]]=1;$(a).find("input.ewa-filter-chkbox").each(function(){this.checked||A[this.id]?$(this).parent().show():$(this).parent().hide()})}})})}};this.hiddenNoContentRow=function(){var a=$("#EWA_FRAME_"+this._Id);a.find(".EWA_TD_M").each(function(){if(""==$(this).text()&&0==$(this).find("img,a,input[type=text],input[type=button],textarea").length){$(this).parent().hide();
var b=$(this).find("*:eq(0)").attr("id");b&&a.find(".ewa-row-"+b).hide().attr("hiddennocontentrow",1)}})};this.itemReload=function(a,b,d){if(a){var e=new EWA_UrlClass(this.Url);e.AddParameter("EWA_AJAX","SELECT_RELOAD");e.AddParameter("EWA_RELOAD_ID",a);e=e.GetUrl();var g=this.getObj("#"+a)[0],h={};for(a=0;a<g.options.length;a++)h[g.options[a].value]=1;$J(e,function(k){if(k.RST){var l=k.ITEM.LST;g.options.length=0;var m=[],n;for(n in l){var p=l[n];if(!p.V||!h[p.V]){var q=new Option(p.T,p.V);p.json&&
$(q).attr("json",p.json);g.options[g.options.length]=q;m.push(q);p._used=!0}}for(n in l)p=l[n],p._used||(q=new Option(p.T,p.V),p.json&&$(q).attr("json",p.json),g.options[g.options.length]=q);if(b)g.value=b;else for(var u in m)if(m[u].value){g.value=m[u].value;break}if(d)try{d(k,g)}catch(r){console.log(r)}}else $Tip(k.ERR)})}};this._checkSelectOptionsChange=function(){if(this._SelectFilters){var a=this;this.isSelectFilter&&setTimeout(function(){a._checkSelectOptionsChange()},311);for(var b in this._SelectFilters){var d=
this._SelectFilters[b];this.getObj("#"+d.id).html()!=d.html&&a._selectOptionsChangeed(d)}setTimeout(function(){a._checkSelectOptionsChange()},311)}};this._selectOptionsChangeed=function(a){console.log("changed");var b=this.getObj("select#"+a.id),d="initSelectFilter"+this._Id+"-"+a.id+"-filter",e="initSelectFilter"+this._Id+"-"+a.id+"-filter2",g=this._initSelectFilterCreateCode(a.codeJsonFrom||a.filterFrom,b);this.getObj("#"+d).html(g);this.getObj("#"+e).html("");a.html=b.html()};this._initSelectFilterCreateCode=
function(a,b){var d=["<option value=''>-ALL-</option>"];if(a){var e={},g=[];b.find("option").each(function(){if(""!=this.value){var l;this.hasAttribute("para_"+a)?l=this.getAttribute("para_"+a):this.getAttribute("json")&&(l=JSON.parse(this.getAttribute("json"))[a]);l&&(e[l]?e[l]+=1:(e[l]=1,g.push(l)))}});var h=g.sort(),k;for(k in h)b=h[k],b=b.replace(/>/ig,"&lt;").replace(/"/,"&quot;"),d.push('<option value="'+b+'">'+b+" ["+e[b]+"]</option>")}else for(k="A".charCodeAt();k<="Z".charCodeAt();k++)b=
String.fromCharCode(k),d.push('<option value="'+b+'">'+b+"</option>");return d.join("")};this.initSelectFilter=function(a,b,d,e){if(!this["INIT_SELECT_FILTER"+a]){this["INIT_SELECT_FILTER"+a]=!0;var g=$("#EWA_FRAME_"+this._Id+" select#"+a),h=g.parent(),k="initSelectFilter"+this._Id+"-"+a+"-filter",l="initSelectFilter"+this._Id+"-"+a+"-filter2",m="initSelectFilter"+this._Id+"-"+a+"-filter-table";$X(m)&&(h.append(g),$($X(m)).remove());h.addClass("ewa-select-filter");k=["<table id='"+m+"' cellpadding=0 cellspacing=0 width=100%><tr><td class='ewa-select-filter-char'><select class='EWA_SELECT' style='margin-right:4px' id='"+
k+"' onchange='EWA.F.FOS[\""+this._Id+'"]._chCode(this,"'+b+'","'+d+"\")'>"];m=this._initSelectFilterCreateCode(e||d,g);k.push(m);k.push("</select><select id='"+l+"' style='display:none'></select></td><td class='ewa-select-filter-target'></td></tr></table>");h.prepend(k.join(""));h.find(".ewa-select-filter-target").append(g);this._SelectFilters||(this._SelectFilters={});h={};h.html=g.html();h.id=a;h.filterType=b;h.filterFrom=d;h.codeJsonFrom=e;this._SelectFilters[h.id]=h}};this._chCode=function(a,
b,d){this.isSelectFilter=!0;var e=$(a).next(),g=$(a).parent().parent().find(".ewa-select-filter-target select");0==e[0].options.length&&e.html(g.html());if(""==a.value)g.html(e.html());else{for(var h=["<option></option>"],k=1;k<e[0].options.length;k++){var l=e[0].options[k];this._chCodeFilter(l,b,d,a.value)&&h.push(GetOuterHTML(l))}g.html(h.join(""));this._SelectFilters[g[0].id].html=g.html();this.isSelectFilter=!1}};this._chCodeFilter=function(a,b,d,e){"json"==b?(a=JSON.parse(a.getAttribute("json"))[d],
"ADM_LID"==d&&(d=a.split("."),a=d[d.length-1])):a="text"==b?a.text:a.value;return a?a.substring(0,e.length).toUpperCase()==e.toUpperCase()?!0:!1:!1};this.setUnMust=function(a){(a=this.ItemList.Items[a])&&a.getElementsByTagName("IsMustInput")[0].getElementsByTagName("Set")[0].setAttribute("IsMustInput",0)};this.setMust=function(a){(a=this.ItemList.Items[a])&&a.getElementsByTagName("IsMustInput")[0].getElementsByTagName("Set")[0].setAttribute("IsMustInput",1)};this.RedrawCreateSpans=function(){for(var a=
$("#EWA_FRAME_"+this._Id+" tr"),b=0;b<a.length;b++){var d=a[b];if("none"!=d.style.display)for(var e=0,g=!1,h=d.cells.length-1;0<=h;h--){var k=d.cells[h];"none"==k.style.display?g?e++:(e=1,g=!0):g&&(k.colSpan=e+1,g=!1)}}EWA_FrameShowAlert=EWA_FrameShowAlert1;EWA_FrameRemoveAlert=EWA_FrameRemoveAlert1;$("#EWA_FRAME_"+this._Id+" .ewa_d1").each(function(){this.title=this.innerHTML});for(var l in this.ItemList.Items)a=this.GetItemDescription(l).Memo,null!=a&&0<a.trim().length&&($("#"+l).attr("placeHolder",
a),$("#"+l).attr("title",a));this.RedrawChangWidth()};this.GetItemDescription=function(a){return this.ItemList.GetDescription(a)};this.ShowPlaceHolder=function(a){$("#EWA_FRAME_"+this._Id).addClass("ewa-frame-cols-"+a);for(var b in this.ItemList.Items){var d=this.ItemList.GetValue(b,"Name","Name");if(d=this.GetObject(d)){d=$(d);var e=this.GetItemDescription(b),g=e.Memo,h=e.Info;e=null;1==a?(e=h,null!=g&&0<g.trim().length&&g!=e&&("smsValid"==d.attr("ewa_tag")?e=g:(g="<div class='ewa-item-memo'>"+g+
"</div>",d.parentsUntil("tr").last().append(g))),d.hasClass("ewa-switch")&&d.prepend("<div class='ewa-switch-info'>"+e+"</div>")):2==a&&null!=g&&0<g.trim().length&&(e=g);e&&("hidden"==d[0].type?d.prev().attr("placeholder",e):"SELECT"==d[0].tagName?0<d[0].options.length&&""==d[0].options[0].value&&(d[0].options[0].text="-- "+e+" --",d[0].options[0].value=""):(d.attr("placeHolder",e),d.attr("title",e)))}}};this.RedrawChangWidth=function(){var a=$("#EWA_FRAME_"+this._Id),b=a.width(),d=a.find(".ewa_msg_box")[0].colSpan/
2;$("#EWA_FRAME_"+this._Id+">tbody>tr[class*='ewa-row-']:not('.ewa-row-msg-box')").each(function(){var g=$(this).find("td.ewa_redraw_info:visible").length,h=(b-90*g)/d;$(this).find("td.ewa_redraw_ctl:visible").each(function(){var k=this.colSpan||1;$(this).css("width",h*k)})});var e=this;addEvent(window,"size",function(){e.RedrawChangWidth()})};this.Mearge=function(a,b,d){console.log("\u62fc\u5199\u9519\u8bef\uff1a\u8bf7\u7528 Merge");this.Merge(a,b,d)};this.Merge=function(a,b,d){if(a||b){var e=$("#EWA_FRAME_"+
this._Id),g=e.find("#"+a),h=e.find("#"+b);if(0==g.length)console.log("Mearge:"+a+"\u4e0d\u5b58\u5728");else if(0==h.length)console.log("Mearge:"+b+"\u4e0d\u5b58\u5728");else{var k=!1,l=e.find(".ewa-row-"+b+" .EWA_TD_M"),m=null;0==l.length&&(k=!0,l=e.find(".ewa-row-"+b+" .EWA_TD_1"),m=e.find(".ewa-row-"+b+" .EWA_TD"),1==m.length&&(k=!0),b=m.html(),m.html("<table cellpadding=0 cellspacing=0 width=100%><tr><td style='width:50%'></td><td style='display:none'></td><td style='width:50%'></td></tr></table>"),
...
if("nofile"==a.UPLOAD_STATUS)return this.uploadProcess=!1,a.removeWaitBox(),!1;$X(a.WAIT_ID)||a.createWaitBox();return this.uploadProcess=!0};this.DoPost=function(a,b,d){if(this.posting)return!1;if(!d){d=this.DoPostBefore();if(d instanceof Array){var e=this;this._checkStatusInc=0;this._checkStatusFristResult=d;this._checkStatusTimer=window.setInterval(function(){var n=0==e._checkStatusInc?e._checkStatusFristResult:e.DoPostBefore();e._checkStatusInc++;null!=n[0]&&(window.clearInterval(e._checkStatusTimer),
e._checkStatusTimer=null,e._checkStatusInc=null,e._checkStatusFristResult=null,0==n[0]?n[2]?$Confirm(n[1],n[1],function(){n[0]=n[1]=null;e.DoPost(a,b,!0)},function(){}):$Tip(n[1]):(n[0]=n[1]=null,e.DoPost(a,b,!0)))},232);return}if(!d)return}var g=$(a).find("input[ewa_tag=ht5upload]");if(!(0<g.length)||this.uploadProcess||this.CheckValidAll(a)){var h=0;e=this;for(d=0;d<g.length>0;d++){var k=g[d];this._uploadingFileCheck(k)&&h++}if(0<h)setTimeout(function(){e.DoPost(a,b)},700);else{g=$(a).find("input[ewa_tag=ht5takephoto]");
if(0<g.length&&!this.uploadProcess&&!this.CheckValidAll(a))return!1;for(d=0;d<g.length>0;d++){k=g[d];h=!1;k=window["h5takephoto_"+this._Id+"$"+k.id];if("ok"!=k.UPLOAD_STATUS){if(!$X(k.H5_UPLOAD.WAIT_ID)){try{k.submitUpload()}catch(n){console.log("ht5takephotos - ht5.submitUpload: "+n);$Tip("\u60a8\u6ca1\u6709\u83b7\u53d6\u7167\u7247");return}k.H5_UPLOAD.createWaitBox();this.uploadProcess=!0}h||(e=this,setTimeout(function(){e.DoPost(a,b)},700),h=!0);return}k.H5_UPLOAD.removeWaitBox()}if(this._stopPostError)return EWA.UI.Msg.ShowError(this._stopPostError,
""),!1;d=this.CreateAjax(a);if(null==d)return!1;null==b&&(b=this.Url);if(""==b||null==b)b=window.location.href;this._Ajax=d;this._Ajax.AddParameter("EWA_AJAX","1");this._Ajax.AddParameter("EWA_POST","1");this._Ajax.AddParameter("EWA_NO_CONTENT","1");this._Ajax.LoadingType="image";this.DoPostStart&&this.DoPostStart(this._Ajax);d=new EWA_UrlClass(b);for(var l in this._Ajax._Parameters)d.RemoveParameter(l);b=d.GetUrl();this.posting=!0;var m=this;this._Ajax.PostNew(b,function(){m._CallBack()});this.isShowPostWaitting||
this._Ajax.HiddenWaitting();$("#EWA_FRAME_"+this._Id+" input[type=submit]").attr("disabled","disabled");return!0}}};this._CallBack=function(){var a=this._Ajax;if(a.IsRunEnd())if($("#EWA_FRAME_"+this._Id+" input[type=submit]").attr("disabled",null),this.posting=!1,this.DoPostEnd&&this.DoPostEnd(a,a._Http.status,a._Http.responseText,a._Http.statusText),200==a._Http.status){if(a=a._Http.responseText,EWA.F.CheckCallBack(a)){this.ValidCodeError(!0);try{this.ReloadAfter&&this.ReloadAfter(a),eval(a)}catch(b){alert(a)}}}else alert("ERROR:\r\n"+
...
h?!0:!1);g&&window.parent&&window.parent.$X(g)&&(z=window.parent.$X(g).parentNode.parentNode.parentNode,m?0<x.length&&(e=z.rows[z.rows.length-1].cells[0],h=window.parent.document.createElement("input"),h.type="button",h.value="enus"==EWA.LANG?"Back":"\u6253\u56de",h.onclick=function(){var A=window.parent.$X(g);A.options.length=1;for(var C=0;C<x.length;C++){var F=x[C];A.options[A.length]=new Option(F.DES,F.ID);this.style.display="none"}A.parentNode.parentNode.cells[0].innerHTML="enus"==EWA.LANG?'<div style="color:red">Back steps</div>':
