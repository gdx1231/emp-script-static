...
c=new EWA_UrlClass(b);for(var k in this._Ajax._Parameters)c.RemoveParameter(k);b=c.GetUrl();this._Ajax.PostNew(b,EWA.F.F.CUR._CallBack);this.isShowPostWaitting||this._Ajax.HiddenWaitting();$("#EWA_FRAME_"+this._Id+" input[type=submit]").attr("disabled","disabled")}}}};this._CallBack=function(){var a=EWA.F.F.CUR._Ajax;if(a.IsRunEnd())if($("#EWA_FRAME_"+EWA.F.F.CUR._Id+" input[type=submit]").attr("disabled",null),EWA.F.F.CUR.DoPostEnd&&EWA.F.F.CUR.DoPostEnd(a,a._Http.status,a._Http.responseText,a._Http.statusText),
...
b;for(b in this.ItemList.Items){var c=this.ItemList.Items[b],d=this.ItemList.GetItemValue(c,"Name","Name");d=a.find('[id="'+d+'"]');if(0!=d.length){var e=this.ItemList.GetItemValue(c,"Tag","Tag"),g=this.ItemList.GetItemValue(c,"DataItem","Valid");this.ItemList.GetItemValue(c,"DataItem","Format");c=this.ItemList.GetItemValue(c,"IsMustInput","IsMustInput");"Email"==g&&d.attr("type","email");"valid"==e&&d.attr("type","tel");1==c&&d.attr("required","required")}}};this.GetObject=function(a,b){b||(b=$("FORM#f_"+
