...
(function(a,b){"object"===typeof module&&"object"===typeof module.exports?module.exports=b():b(a)})("undefined"!==typeof window?window:this,function(a){function b(m){for(var n=[{number:0,scale:1,items:[]}],p=null,q=0;q<m.length;q++){var u=m[q],r=g[u],t=e[u];n[n.length-1].items.push([u,r,t]);if(r)0==q&&r.first?n[n.length-1].number=10:r.stop?(n[n.length-1].scale=r.value||1,n.push({number:0,scale:1,items:[]}),scale=1,p=null):(q==m.length-1&&(console.log(r),n[n.length-1].number.scale=r.value),p=r);else{if(null==
t)throw"\u975e\u6570\u5b57\u6216\u5355\u4f4d\uff1a"+u;u=q==m.length-1?null:m[q+1];var w=q==m.length-1?null:g[u];w&&w.stop&&(w=null);u=n[n.length-1].number;var v=n[n.length-1].items;r=v[v.length-2];v=v[v.length-3];null==w?null==p?(p=!1,r&&0===r[2]?v&&null!=v[1]&&(p=!0):r&&r[1]&&10===r[1].value&&(p=!0),t=p?u+t:10*u+t):t=r&&r[1]&&10===r[1].value&&(null==v||0===v[2]||null!=v[1])?u+t+10:u+t:t=u+t*w.value;n[n.length-1].number=t;p=null}}return n}function d(m){if(!m)return m;if(!isNaN(m.toString()))return 1*
m.toString();var n=m.replace(/\uff0c|,| |\u5143|\u5706/ig,"").replace(/\u70b9|\u3002/ig,".").split(".");if(2<n.length)throw"\u975e\u6570\u5b57\u8868\u8fbe\u5f0f\uff1a"+m;var p=n[0].split("");m=(2==n.length?n[1]:"").split("");n=!1;var q=p[0];h[q]?(n=!0,p=p.splice(1)):k[q]&&(n=!1,p=p.splice(1));q=p[p.length-1];l[q]?p=p.splice(0,p.length-1):1<m.length&&(q=m[m.length-1],l[q]&&(m=m.splice(0,m.length-1)));q=p;for(var u=p=0,r=q.length-1;0<=r;r-=2){var t=q[r];var w=e[q[r-1]];t=w?"\u89d2"==t?1*("0."+w):"\u5206"==
t?1*("0.0"+w):"\u5398"==t?1*("0.00"+w):"\u6beb"==t?1*("0.000"+w):-1:-1;if(-1===t)break;else p+=t,u++}q=q.splice(0,q.length-2*u);q=b(q);u=0;r=1;for(t=q.length-1;0<=t;t--){w=q[t];var v=w.number;r*=w.scale;u+=v*r}p=u+p;if(0<m.length){q=1;u=0;for(r=m.length-1;0<=r;r--){t=g[m[r]];if(!t)break;q*=t.value;u++}m=1==q?m:m.splice(0,m.length-u);u=b(m);r=0;t=1;for(w=u.length-1;0<=w;w--){v=u[w];var y=v.number;t*=v.scale;r+=y*t}m=[1*("0."+r),q,m.length]}else m=0;return 0==m?p*(n?-1:1):((p+m[0])*m[1]).toFixed(0>
...
d;else if(a)a.chineseToNumber=d;else return d});
