...
function EWA_Html5UploadClass(){this.uploadIframes=[];this.files=[];this.UPLOAD_STATUS=this.POST_REF=this.POST_NAME=this.POST_ITEMNAME=this.POST_XMLNAME="";this.WAIT_ID=EWA_Utils.tempId("ht5upload_wait_");this.haveFile=function(){for(var a=0;a<this.files.length;a++)if(""!=this.files[a].value)return!0;return!1};this.getValue=function(){if(0==this.uploadIframes.length&&0==this.files.length)return"";for(var a=[],b=0;b<this.uploadIframes.length;b++){var c=this.uploadIframes[b];if("string"==typeof c){c=
JSON.parse(c);for(var d in c)a.push(c[d])}}return JSON.stringify(a)};this.fileIco=function(a,b){a=EWA.UI.Ext.FileExt(a.name);$X(b).onerror=function(){};$X(b).src=EWA.UI.Ext.FileIco(a)};this.showPreview=function(a,b){b=a.parentNode;var c=b.outerHTML;b.style.display="none";a.setAttribute("ok",1);gFileReaders={};for(var d=0;d<a.files.length;d++){var e=a.files[d];this.showPreview1(b,e);this.files.push(e)}"yes"==this.UpMulti&&$(b).before(c)};this.addOther=function(a){for(var b=$X(this.POST_NAME).parentNode.getElementsByTagName("input")[1].parentNode,
c=0;c<a.length;c++)this.showPreview1(b,a[c]);this.uploadIframes.push(JSON.stringify(a))};this.createPreviewBox=function(a){var b=a||$("#EWA_FRAME_"+this.SYS_FRAME_UNID+" input#"+this.POST_NAME).parent().find(".ewa_app_upload_form")[0],c="_upload_"+Math.random();c=c.replace(".","G");a=document.createElement("img");a.id=c;c=document.createElement("table");c.cellPadding=0;c.cellSpacing=0;c.className="ewa_app_upload_form1";b.parentNode.appendChild(c);b=c.insertRow(-1).insertCell(-1);b.align="center";
b.appendChild(a);b.className="img";b.vAlign="middle";c.insertRow(-1).insertCell(-1).className="name";var d=this;a.onerror=function(e){console.log(e);d.fileIco({name:this.src},this.id)};return c};this.showPreviewExists=function(){var a=$("#EWA_FRAME_"+this.SYS_FRAME_UNID+" input#"+this.POST_NAME).val();if(""!=a){try{EWA.F.FOS[this.SYS_FRAME_UNID].setUnMust(this.POST_NAME)}catch(b){}this.showPreviewUrl(a)}};this.showPreviewUrl=function(a){var b=this.createPreviewBox(),c=$(b).find("td")[1],d=a.split("/");
d=d[d.length-1];c.innerHTML="<div title='"+d+"'>"+d+"</div>";$(b).find("img")[0].src=a;return b};this.showPreview1=function(a,b){var c=this.createPreviewBox(a),d=$(c).find("td")[1],e=$(c).find("img")[0];a=e.id;b.ref=a;d.innerHTML="<div>"+b.name+"</div><div id='"+a+"_process1' class='ewa-app-upload-process1'></div><div id='"+a+"_process' class='ewa-app-upload-process'>0%</div>";d.className="name";$(c).attr("file_name",b.name);$(c).attr("file_type",b.type);$(c).attr("file_size",b.size);if(b.type&&0==
...
g=$('div[id="'+b.ref+'_process"]');g.show().html("0%");var h=$('div[id="'+b.ref+'_process1"]');h.css("width",0).show();b=new XMLHttpRequest;b.upload.addEventListener("progress",function(k){k.lengthComputable?(k=Math.round(100*k.loaded/k.total-10),0>k&&(k=0),g.html(k.toString()+"%"),h.css("width",k.toString()+"%")):g.html("unable to compute")},!1);b.addEventListener("load",function(k){g.html("100%");h.css("width","100%");for(var l in e.uploadIframes)if(e.uploadIframes[l].xhr==k.target){e.uploadIframes[l]=
k.target.responseText;break}e.chkAllXhrComplete()},!1);b.addEventListener("error",function(k){console.log(k);g.html(k);e.UPLOAD_STATUS="error"},!1);b.addEventListener("abort",function(k){g.html("The upload has been canceled by the user or the browser dropped the connection.");console.log(k);e.UPLOAD_STATUS="abort"},!1);b.open("POST",this.action,!0);b.send(c);a.xhr=b;delete a.file};this.createNewSizes=function(a,b){if("client"===this.NewSizesIn){if(!this.siezs){var c=this.UpNewSizes.split(",");this.siezs=
[];for(var d in c){var e=c[d],g=e.split("x");2==g.length&&(e=g[0],g=g[1],isNaN(e)||isNaN(g)||this.siezs.push({w:1*e,h:1*g}))}}if(0!=this.siezs.length)for(d in this.siezs)e=this.siezs[d],c=EWA_ImageCalss.resizeAsBlob(a,e.w,e.h),b.append(this.POST_NAME,c,"resized$"+e.w+"x"+e.h+".jpg")}};this.chkAllXhrComplete=function(){for(var a=0,b=0;b<this.uploadIframes.length;b++)"string"==typeof this.uploadIframes[b]&&a++;a==this.uploadIframes.length&&(this.UPLOAD_STATUS="ok")};this.createWaitBox=function(){if(!$X(this.WAIT_ID)){var a=
document.createElement("div");a.id=this.WAIT_ID;var b=$("body");$(a).css({width:300,height:100,"line-height":"100px","font-size":"16px",position:"fixed",top:.5*b.height()-35,left:.5*b.width()-150,"z-index":12838,"background-color":"f0f0f0",color:"#08c","border-radius":"10px",opacity:.9}).addClass("ewa-upload-wait-box");a.innerHTML="<table height=100% align=center><tr><td><img src='"+((EWA.RV_STATIC_PATH||"/EmpScriptV2")+"/EWA_STYLE/images/loading2.gif'></td><td>")+("enus"==EWA.LANG?"Please waitting ...":
"\u8bf7\u7b49\u5f85\u6587\u4ef6\u4e0a\u4f20\u5b8c\u6bd5...")+"</td></table>";b.append(a);this.noWaitBox&&$(a).hide()}};this.removeWaitBox=function(){$("#"+this.WAIT_ID).remove()};this.submitUploads_old=function(a){this.uploadIframes=[];var b=this;this.UPLOAD_STATUS="start";try{$("input[type=file][ok=1]").each(function(c){var d=this.id.split("$")[1];b.POST_NAME==d&&b.submitUpload_old(c,this)})}catch(c){alert(c)}0<this.uploadIframes.length?window.setTimeout(function(){b.UPLOAD_STATUS="checking";b.chkframeSubmt()},
400):this.UPLOAD_STATUS="nofile"};this.submitUpload_old=function(a,b){a="GDX"+a;var c=document.createElement("iframe");c.id=a;c.name=a;document.body.appendChild(c);c=$(c);c.hide();var d=document.createElement("form");document.body.appendChild(d);d.method="post";d.enctype="multipart/form-data";d.innerHTML="<input type=submit>";var e=$(d);e.hide();d=EWA.CP+"/EWA_STYLE/cgi-bin/_up_/?xmlname="+this.POST_XMLNAME+"&ITEMNAME="+this.POST_ITEMNAME+"&NAME="+this.POST_NAME+"&EWA_UP_TYPE=SWFUPLOAD&EWA_UP_REF="+
...
this.createP(a.parentNode),h=this.createSpan(a.parentNode);g.appendChild(h);g.appendChild(d);e.appendChild(g);this.docks.push(g)}else e.appendChild(d);this.lastWR=null}else if("IMG"==b)e=this.getDock(),d=this.createPic(a),"w:p"!=e.tagName?(g=this.createP(a.parentNode),h=this.createSpan(a.parentNode),g.appendChild(h),g.appendChild(d),e.appendChild(g),this.docks.push(g)):e.appendChild(d),this.lastWR=null;else if("TABLE"==b)h=this.getDockTable(),d=a.previousElementSibling,null!=d&&"TABLE"==d.tagName&&
(e=this.createPVanish(),h.appendChild(e[0])),d=this.createTable(a),null!=d&&(c=!0,h.appendChild(d),this.docks.push(d));else if("TBODY"==b)d=this.createTbody(a),this.getDock().appendChild(d);else if("TR"==b)d=this.createTr(a),this.getDock().appendChild(d),this.docks.push(d),c=!0;else if("TD"==b)d=this.createTd(a),this.getDock().appendChild(d),this.docks.push(d),c=!0;else if("HR"==b)e=this.getDockTable(),d=this.createP(a),h=this.createSpan(a),d.appendChild(h),g=this.createHr(),h.appendChild(g),this.lastWR=
null,e.appendChild(d);else if("LI"==a.parentNode.tagName||"H1"!=b&&"H2"!=b&&"H3"!=b&&"P"!=b&&"CENTER"!=b&&"DIV"!=b){if("SCRIPT"==b)return;"BODY"!=b&&"OL"!=b&&"UL"!=b&&(e=this.getDock(),d=this.createSpan(a),"w:p"!=e.tagName?(g=this.createP(a.parentNode),g.appendChild(d),e.appendChild(g),this.docks.push(g)):e.appendChild(d))}else e=this.getDockTable(),d=this.createP(a),e.appendChild(d),this.docks.push(d),c=!0;for(e=0;e<a.childNodes.length;e++)this.walker(a.childNodes[e]);c&&(this.docksPop(d),this.lastWR=
null);"TD"==b&&1==d.childNodes.length?(g=this.createEle("w:p"),d.appendChild(g)):"TABLE"==b&&null==d.nextSibling&&"w:tc"==d.parentNode.tagName&&(g=this.createEle("w:p"),d.parentNode.appendChild(g))}};this.walkerTxt=function(a){if(""!=a.nodeValue.trimEx()){var b=this.createText(a),c=this.getDock();if("w:p"!=c.tagName){var d=this.createP(a.parentNode);a=this.createSpan(a.parentNode);d.appendChild(a);a.appendChild(b);c.appendChild(d);this.docks.push(d)}else this.lastWR||(a=this.createSpan(a.parentNode),
