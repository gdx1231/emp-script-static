...
function _$J_SELECT(a,b,c,d,e,g,h,k){function l(v,u){if(!u)return"not defined textName";if(u instanceof Function)return u[v];if(u.indexOf("@@")==-1)return v[u];let x=u.match(/@@[a-zA-Z0-9\-\._:]*\b/ig);for(let w=0;w<x.length;w++){let y=x[w],z=y.replace("@@","");u=u.replace(y,v[z]||"")}return u}function m(v,u){var x=l(v,c),w=b.options[b.options.length]=new Option(x,v[d]);if(u){for(var y=[],z=0;z<u;z++)y.push("");y.push(x);y.join("\u3000")}for(var B in v)v[B]&&w.setAttribute("para_"+B.toLowerCase(),
v[B]);w.setAttribute("json",JSON.stringify(v));return w}b.options.length=0;e&&(b.options[0]=new Option("",""),e instanceof Array&&(b.options[0].text=e[0],b.options[0].value=e[1]));if(k){var n={___ROOT___:[]};e={___ROOT___:{}};for(var p,r,q,t=0;t<a.length;t++){q=a[t];p=q[k];r=q[d];if(p===null||p===""||p==="0")p="___ROOT___";n[p]||(n[p]=[]);n[p].push(q);e[r]=q;e[p]&&(e[p]._children||(e[p]._children=[]),e[p]._children.push(q))}function v(u,x){if((u=n[u])&&u._children&&u._children.length!=0)for(var w,
y=0;y<u._children.length;y++)w=u._children[y],m(w,x),v(w[d],x+1)}v("___ROOT___",0)}else for(t=0;t<a.length;t++)q=a[t],m(q);g&&(b.value=g);$(b).trigger("change");h&&h(b)}$Install=function(a,b,c,d){a.GetUrl?a=a.GetUrl():a.getUrl?a=a.getUrl():a.href&&(a=a.href);var e=new EWA_UrlClass;e.SetUrl(a);e.GetParameter("EWA_AJAX")||e.AddParameter("EWA_AJAX","INSTALL");(new EWA_AjaxClass).Install(e.GetUrl(),"",b,c,d)};
...
Function)b(a);else{typeof b=="string"&&(b=document.getElementById(b));var c=b.tagName;c==null&&(c="");c=c.toUpperCase();if(c=="SELECT"||c=="INPUT"||c=="TEXTAREA"){b.value=a;b.setAttribute("ewa_trans_end","ok");if(b.onblur!=null)b.onblur();c=="TEXTAREA"&&window.autosize&&autosize.update&&autosize.update(b)}else c=="DIV"||c=="SPAN"?c=="DIV"&&$(b).attr("EWA_DHTML")?($(b).find("iframe")[0].contentWindow.frames[0].document.body.innerHTML=a,$(b).find("input[type=hidden]")[0].value=a):(b.setAttribute("ewa_trans_end",
...
u=k.$(".ewa-slider-puzzle-swiper");let y=k.$(".ewa-slider-puzzle-verify");x=x.clientX-v-$(w).offset().left;w=$(w).width()+29;x<0?x=0:x>w&&(x=w);u.css("left",x);y.css("left",x+10)});l.bind(q,function(u){l.unbind(r);l.unbind(q);u=new EWA_UrlClass(g.ewa_url);let x={};x.left=n.position().left;x.top=n.position().top;x.left1=n.offset().left;x.top1=n.offset().top;x.ewa_trigger_valid_mode="check";x.ewa_trigger_valid_name=g.ewa_trigger_valid_name;x.ewa_ajax=g.ewa_trigger_valid;console.log("0. Puzzle post check");
$JP(u.GetUrl(),x,function(w){w.RST?(console.log("1. puzzle valid ok"),l.addClass("ewa-slider-puzzle-successful"),d&&d(w)):(l.addClass("ewa-slider-puzzle-fail"),setTimeout(()=>{l.removeClass("ewa-slider-puzzle-fail");m.css("left","0px");n.css("left","10px");p.css("opacity","1");e&&e(w);w.newConfig&&(w.newConfig.ewa_trigger_valid_name=g.ewa_trigger_valid_name,w.newConfig.ewa_trigger_valid=g.ewa_trigger_valid,w.newConfig.ewa_url=g.ewa_url,g=w.newConfig,b(g))},500))})})})}function b(l){var m=k.$(".ewa-slider-puzzle-imgbox"),
...
!1;this.CheckedAllAfter=this.ReloadAfter=null;this.REPLACE_HISTORY_STATE=this.IsNotMDownAutoChecked=!1;this._IsSearchGroup=!0;this.changeTag=function(a){a=new EWA_UrlClass("fake?"+a);let b=new EWA_UrlClass(this.Url);for(let c in a._Paras)b.AddParameter(c,a._Paras[c]);this.Url=b.GetUrl();this.Reload()};this._InitMneu=function(){var a=window["_EWA_MENU_"+this._Id];if(a&&a.length!=0){var b=[];for(let c=0;c<a.length;c++){let d=a[c],e=(c==0?"ewa-lf-menu-marked":"")+(c==a.length-1?"ewa-lf-menu-last":""),
g=d.Cmd.replace(/"/g,"&quot;");b.push(`<a class="ewa-lf-menu ${e}" onclick="${"$(this).parent().find('.ewa-lf-menu-marked').removeClass('ewa-lf-menu-marked');$(this).addClass('ewa-lf-menu-marked');"} ${g}">${d.Txt}</a>`)}a=b.join('<b class="ewa-lf-menu-span"></b >');$("#EWA_RESHOW_"+this._Id).find(".ewa_lf_func_caption").after(a)}};this.stickyColumns=function(a){var b=$X("EWA_LF_"+this._Id);if(b==null)console.log("not found table[#EWA_LF_"+this._Id+"]");else if(b.rows.length!=0){var c=$(b).parent();
c.addClass("ewa-lf-sticky-scroll");c.on("scroll",function(){this.scrollLeft==0?c.addClass("ewa-lf-stick-last-hide"):c.hasClass("ewa-lf-stick-last-hide")&&c.removeClass("ewa-lf-stick-last-hide")});for(let d=0;d<a;d++)this.stickyColumn(d,d==a-1,b);c.trigger("scroll")}};this.stickyColumn=function(a,b,c){c=c||$X("EWA_LF_"+this._Id);if(c==null)console.log("not found table[#EWA_LF_"+this._Id+"]");else if(c.rows.length!=0){var d=c.rows[0].cells[a];if(d){var e=$(d).position().left;d=$(d).width();e={left:e,
width:d};for(d=0;d<c.rows.length;d++){let g=$(c.rows[d].cells[a]);g.css(e).addClass("ewa-lf-sticky");b&&g.addClass("ewa-lf-sticky-last").css("overflow","initial")}}}};this.composeSearchTexts=function(){var a=[],b=[],c=$("#EWA_RESHOW_"+this._Id+" .ewa-lf-search-type-text");if(c.length==0)return null;c.each(function(g){var h=$(this).find(".ewa-lf-search-item-title").text();h=h.replace(":","").replace("\uff1a","").replace("\u5305\u542b","").replace("Like","").trim();var k=$(this).find("input[type=text]").attr("name");
a.push(h);b.push(k);g>0&&$(this).hide()});let d=$(c[0]);d.addClass("ewa-app-lf-search-compose");d.find("input[type=text]").attr("name",b.join(",")).attr("placeholder",a.join(", "));let e=d.find("a").clone();d.find(".ewa-lf-search-item-title b").text(EWA.LANG=="enus"?"Search":"\u7efc\u5408\u641c\u7d22").append(e);return c};this.switchButtonAction=function(a,b){if(b){var c=this.getUrlClass();c.AddParameter("ewa_switch_name",a.name);c.AddParameter("ewa_action",b);c.AddParameter("ewa_ajax","json");c.AddParameter("ewa_switch",
a.checked?"on":"off");c.AddParameter("ewa_action_key",$(a).parentsUntil("tbody").last().attr("ewa_key"));var d=a.parentNode,e=d.getAttributeNames();b={};for(let h in e){let k=e[h],l=d.getAttribute(k);k.indexOf("on")!=0&&k!="name"&&k!="id"&&k!="class"&&(b[k]=l)}var g=this;c=c.GetUrl();$JP(c,b,function(h){g.extSwitchCallBack&&g.extSwitchCallBack(a,h)})}};this.ShowRecycle=function(){var a=$X("EWA_LF_"+this._Id);if(a!=null&&(a=$(a).parentsUntil("table").last().find(".ewa_lf_func_caption"),a.length!=0)){var b=
...
"");l=l.replace(p,"<span class='ewa-lf-merge ewa-lf-merge-"+r+" ewa-lf-mearge ewa-lf-mearge-"+r+"' mid=\""+r+'"></span>');r!=a&&(g.find('tr[ewa_tag="HEADER"] [id="'+r+'"]').parent().hide(),g.find("td#ADD_ROW_"+r).hide());p=g.find('tr[ewa_tag="HEADER"] [id="'+r+'"]').html();m[r]=p;b.push(p)}e&&(e=b.join("<span class='ewa-lf-merge-header-split'></span>"),g.find('tr[ewa_tag="HEADER"] [id="'+a+'"]').html(e));g.find("tr[ewa_key],tr.ewa-lf-sub-tr").each(function(){if($(this).find('[id="'+a+'"]').length==
0)console.log("not find "+a);else{var q=$(this).hasClass("ewa-lf-sub-tr")?$(this).find('[id="ADD_ROW_'+a+'"]'):$(this).find(".ewa-col-"+a);if("yes"!=q.attr("ewa-merged")){var t=$('<div style="display:none"></div>');t.html(l);for(var v in k){var u=k[v].replace("@@",""),x=$(this).find('[id="'+u+'"]');if(x.length!=0){u!=a&&x.parent().hide().addClass("ewa-row-merge-hide");var w=t.find('span[mid="'+u+'"]');c&&(w.append("<span class='ewa-lf-mearge-memo'></span>"),w.find(".ewa-lf-mearge-memo").html(m[u]));
w.append(x)}}for(;t[0].childNodes.length>0;)q.append(t[0].childNodes[0]);$(t).remove();d&&d(q,this);q.attr("ewa-merged","yes")}}})}else console.log("mergeExp \u6ca1\u6709\u8bbe\u7f6e")};this.MeargeExp=function(a,b,c,d){console.log("\u62fc\u5199\u9519\u8bef\uff0c\u8bf7\u7528 MereExp");this.MergeExp(a,b,c,d)};this.SubBottoms=function(a){this.SubBottomsArray=a.split(",");this._SubBottoms()};this.reCalcBottoms=function(){this._SubBottoms()};this._SubBottoms=function(){let a=$("#EWA_LF_"+this._Id).find(".ewa-lf-sub-tr");
a.length==0?(a=$(this.AddRow([])),a.addClass("ewa-lf-sub-tr"),a.find("td").addClass("ewa-lf-sub-td")):a.find("td").text("");for(let b in this.SubBottomsArray){let c=this.SubBottomsArray[b],d=0,e=0;$("#EWA_LF_"+this._Id+' .ewa-lf-data-row [name="'+c+'"]').each(function(){let h;h=this.hasAttribute("value")?this.value:this.hasAttribute("title")?this.title:this.innerText;h!=null&&h.length!==0&&(h=h.replace(/,/ig,""),isNaN(h)||(d+=h*1,h.indexOf(".")>0&&(e=2)))});let g="<span id='"+c+"' name='"+c+"' class='ewa-lf-sub'>"+
d.fm(e)+"</span>";a.find("td[id=ADD_ROW_"+c+"]").html(g)}};this.ChangeRowStyle=function(a,b,c){for(var d=$X("EWA_LF_"+this._Id),e=1;e<d.rows.length;e++){var g=d.rows[e];if(!(g.cells.length==0||g.cells.length<=a)){var h=g.cells[a].childNodes[0].getAttribute(b);if(h!=null&&c[h])for(var k=0;k<g.cells.length;k++)g.cells[k].style.cssText=g.cells[k].style.cssText+";"+c[h]}}if(this.ReloadAfter==null){var l=this;this.ReloadAfter=function(){l.ChangeRowStyle(a,b,c)}}};this.MDownEvent=function(a,b){};this.LoadJson=
function(a,b){a!=null&&$J(this.Url+"&EWA_ACTION="+a+"&EWA_AJAX=JSON",b)};this.Workflow=function(a,b,c,d,e,g){EWA.F.CID=this._Id;this._Ajax=this.CreateAjax();this._Ajax.AddParameter("EWA_AJAX","WORKFLOW");this._Ajax.AddParameter("EWA_ID",this._Id);this._Ajax.AddParameter("EWA_WF_NAME",b);c!=null?(a=this.GetRowKey(c),this._Ajax.AddParameter("EWA_ACTION_KEY",a),c.getAttribute("ewa_wf_control")=="true"?this._Ajax.AddParameter("EWA_WF_CTRL",1):this._Ajax.AddParameter("EWA_WF_CTRL",0)):this._Ajax.AddParameter("EWA_WF_CTRL",
1);d!=null&&this._Ajax.AddParameter("EWA_ACTION_KEY",d);e&&this._Ajax.AddParameter("EWA_WF_UOK",e);g&&this._Ajax.AddParameter("EWA_WF_UMSG",g);var h=this;this._Ajax.PostNew(this.Url,function(){h._CallBackJs()})};this.GetRowKey=function(a){a=this.GetRow(a);return a!=null?a.getAttribute("EWA_KEY"):null};this.GetRow=function(a){if(a==null||a.tagName==null||a.tagName=="")return null;var b=0;for(a=a.parentNode;a.tagName!="TR"||a.getAttribute("EWA_KEY")==null;){a=a.parentNode;if(a.tagName=="BODY"){a=null;
break}b++;if(b>100){alert("\u5faa\u73af100\u6b21\uff0c\u672a\u53d1\u73b0TR");break}}return a};this.RecordModify=function(a,b,c){var d=this.SelectChecked();if(d.length==0)EWA.UI.Msg.Alter("\u8bf7\u5148\u9009\u62e9","\u4fee\u6539");else{var e="EWA_MTYPE=M";c!=null&&c.trim().length>0&&(e+="&"+c);EWA.UI.Dialog.OpenReloadClose(this._Id,a,b,!1,e+d)}};this.RecordNew=function(a,b,c){var d="EWA_MTYPE=N";c!=null&&c.trim().length>0&&(d+="&"+c);EWA.UI.Dialog.OpenReloadClose(this._Id,a,b,!1,d)};this.SelectSingle=
function(){this.IsTrSelect=!0;this._TrSelectMulti=!1};this.SelectMulti=function(){this._TrSelectMulti=this.IsTrSelect=!0};this.MOver=function(a,b){if(this.IsTrSelect&&this._CurTr!=a&&(this._CurTr!=null&&(this._CurTr.getAttribute("__ewa_lf_mdown")!="1"?this._MSetBg(this._CurTr,""):this._MSetBg(this._CurTr,"down")),a.getAttribute("__ewa_lf_mdown")!="1"?this._MSetBg(a,"over"):this._MSetBg(a,"down"),this._CurTr=a,this.MOverEvent))try{this.MOverEvent(a,b)}catch(c){console.log(c,this.MOverEvent)}};this.MOut=
function(a){if(this.IsTrSelect&&this._CurTr!=null&&this._CurTr.getAttribute("__ewa_lf_mdown")!="1"){this._MSetBg(this._CurTr,"out");if(this.MOutEvent)try{this.MOutEvent(this._CurTr,a)}catch(b){console.log(b,this.MOutEvent)}this._CurTr=null}};this.MDownEnableCheck=function(a,b){return!0};this.checkMDownEnable=function(a,b){if(!b)return!0;b=b.srcElement?b.srcElement:b.target;return!b||b.tagName=="TD"&&b.parentNode==a?!0:b.tagName=="A"||b.parentNode.tagName=="A"||b.tagName=="INPUT"||b.tagName=="SELECT"||
b.tagName=="BUTTON"||b.tagName=="TEXTAREA"||b.className.indexOf("EWA_LF_EDIT")>=0||b.className.indexOf("ewa-lf-edit")>=0||b.className.indexOf("ewa-lf-search-text-click")>=0||b.parentNode.className.indexOf("ewa-lf-search-text-click")>=0||b.parentNode.parentNode.className.indexOf("ewa-lf-search-text-click")>=0||b.className.indexOf("ewa-mdown-stop")>=0?!1:!0};this.mDownAutoCheck=function(a,b,c){for(var d,e=0;e<b.length&&!(b[e].parentNode.className.indexOf("ewa-switch")>=0);e++){if(b[e].type.toUpperCase()==
"RADIO")return d=b[e],d.click(),d;if(b[e].type.toUpperCase()=="CHECKBOX"){d=b[e];d.click();if(!this._TrSelectMulti){let g=$(b[e]).attr("id")||$(b[e]).attr("name");$(c).find(b[e]).length==0&&$(a).siblings(".ewa-lf-data-row").find("input[name='"+g+"']:checked").prop("checked",!1);d.checked=!0}return d}}return null};this.newRowOneTd=function(a){if(a==null)return null;a=$(a)[0];var b=a.parentNode.rows[a.rowIndex+1];if(b!=null&&b.getAttribute("add_pre_row")=="1")return b;b=a.parentNode.parentNode;let c=
b.rows[0].cells.length;b=b.insertRow(a.rowIndex+1);b.setAttribute("add_pre_row",1);a=b.insertCell(-1);a.colSpan=c;a.className="ewa-lf-add-pre-cell";b.style.display="none";a.id=EWA_Utils.tempId("EWA_LF_NR_"+this._Id);$(b).addClass("ewa-lf-add-pre-row");return b};this.MDown=function(a,b){if(this.IsTrSelect){var c=(new Date).getTime(),d=$(a).attr("mdown_time_last");if(!(d!=null&&c-d<500)&&($(a).attr("mdown_time_last",c),b=b==null?window.event:b,this.MDownEnableCheck(a)&&this.checkMDownEnable(a,b))){d=
b.srcElement?b.srcElement:b.target;var e=null;c=a.getElementsByTagName("input");this.IsNotMDownAutoChecked||(e=this.mDownAutoCheck(a,c,d));if(a.getAttribute("__ewa_lf_mdown")=="1")this._TrSelectMulti&&(e&&e.type.toUpperCase()=="CHECKBOX"&&(e.checked=!1),this._MSetBg(a,""),a.removeAttribute("__ewa_lf_mdown"),$(a).removeClass("ewa-lf-mdown"));else{e&&(e.checked=!0);this._MSetBg(a,"down");a.setAttribute("__ewa_lf_mdown",1);$(a).addClass("ewa-lf-mdown");if(!this._TrSelectMulti&&(e=a.parentNode)&&e.rows)for(d=
0;d<e.rows.length;d++){var g=e.rows[d];g.getAttribute("__ewa_lf_mdown")=="1"&&g!=a&&(g.removeAttribute("__ewa_lf_mdown"),$(g).removeClass("ewa-lf-mdown"),this._MSetBg(g,""))}if(this.IsReShow)for(d=0;d<c.length;d++)c[d].type.toUpperCase()=="BUTTON"&&(e=c[d],e.getAttribute("disabled")=="true"||e.getAttribute("disabled")==1||e.parentNode.getAttribute("disabled")=="true"||e.parentNode.getAttribute("disabled")==1?(this._ReshowButs[e.value].disabled=!0,this._ReshowButs[e.value].style.display="none"):(this._ReshowButs[e.value].disabled=
!1,this._ReshowButs[e.value].style.display=""))}this._IsAddPreRow?(c=a.getAttribute("ewa_key"),this.AddPreRowCheck(a,c,b)&&(d=this.newRowOneTd(a),this.prevTr&&(this.AddPreRowCloseBeforeEvent&&this.AddPreRowCloseBeforeEvent(this._Id,a,c,this.prevTr,b),this.prevTr.style.display="none",this.prevTr.cells[0].innerHTML="",this.AddPreRowCloseEvent&&this.AddPreRowCloseEvent(this._Id,a,c,this.prevTr,b)),this.prevTr==d?this.prevTr=null:(this.prevTr=d,this.prevTr.style.display="",this.MDownEvent(this._Id,a,
c,this.prevTr,b)))):this.MDownEvent(a,b)}}};this.AddPreRow=function(a){this._IsAddPreRow||(this.MDownEvent=a);this._IsAddPreRow=!0};this.AddPreRowCheck=function(a,b,c){return!0};this.AddPreRowCloseBeforeEvent=function(a,b,c,d,e){};this.AddPreRowCloseEvent=function(a,b,c,d,e){};this._MSetBg=function(a,b){let c="";b=="down"?c="ewa_grid_down":b=="over"&&(c="ewa_grid_mover");$(a).children("td").removeClass("ewa_grid_mover ewa_grid_down").addClass(c)};this._ReShowSearchQuick=function(){var a=this,b=EWA_ListFrameClass.prototype.RESOURCES.search_text_items,
c,d,e;for(e in b){var g=b[e];g.Id=="eq"&&(c=g);g.Id=="lk"&&(d=g)}for(e in this._SearchJson){var h=this._SearchJson[e];h.isSearchQuick&&(b=$("#EWA_LF_"+this.Id+' .ewa-lf-data-row [id="'+e+'"]'),b.bind("click",function(k){k.stopPropagation();k=$(this).attr("search_tag");var l=$(this).text(),m=l,n=$("#EWA_SEARCH_ITEM_"+a.Id+' .ewa-lf-search-item-ctl [name="'+this.id.toUpperCase()+'"]');if(k=="number"||k=="date")n=n.parentsUntil(".ewa-lf-search-item").last().find("input");var p=n.parentsUntil(".ewa-lf-search-item").last().find("a");
if(k=="date")l=l.split(" ")[0],m=l+" 23:59:59";else if(k=="fix"){var r=null;n.find("option").each(function(){this.text==l&&(r=this.value)});l=r===null?"":r}n.val()==l?(k=="text"&&d&&p.length>0&&(p.attr("tag",d.Id),p.html(EWA.LANG=="enus"?d.TxtEn:d.Txt)),n.val("")):(k=="text"&&c&&p.length>0&&(p.attr("tag",c.Id),p.html(EWA.LANG=="enus"?c.TxtEn:c.Txt)),n.val(l),k=="date"&&(n[1].value=m))}),b.each(function(){$(this).text()&&$(this).addClass("ewa-lf-search-text-click").attr("search_tag",h.T)}))}};this.ReShowWithNoButtons=
...
a.push("<tr><td class='ewa_lf_func' style='padding:0'>");a.push("<div style='display:none'></div>");a.push("<div><div class='ewa_lf_func_caption'></div></div>");a.push("</td></tr>");a.push("<tr><td style='width:100%;padding:0;vertical-align:top'>");a.push("<div id='"+b+"' style=''></div>");a.push("</td></tr>");a.push("</table>");var c=$(a.join("")),d=!(this.Url.toUpperCase().indexOf("EWA_AJAX=INSTALL")>0||this.Url.toUpperCase().indexOf("EWA_CALL_METHOD=INNER_CALL")>0||this.Url.toUpperCase().indexOf(".JSP")>
0);d?EWA.UI.Utils.SetStyle(c[0],"width:100%;height:100%;border-spacing:0"):EWA.UI.Utils.SetStyle(c[0],"width:100%;border-spacing:0");css="";var e=$X("EWA_LF_"+this._Id);a=e.parentNode.parentNode;var g=e.parentNode;c.find("td:eq(2)>div").append(g);g=c[0];let h=c.find("tr:eq(0)")[0],k=c.find("td:eq(0)")[0],l=c.find("td:eq(1)")[0],m=c.find("td:eq(2)")[0];d&&(window.setTimeout(function(){$X("__EWA_DEBUG")&&m.childNodes[0].appendChild($X("__EWA_DEBUG"));var A=$X(b);A.getElementsByTagName("table");var C=
EWA.UI.Utils.GetDocSize(window),D=A.parentNode.parentNode.previousSibling.offsetHeight;A.style.top=D+"px";A.style.height=C.H-D+"px"},100),addEvent(window,"resize",function(){var A=$X(b),C=EWA.UI.Utils.GetDocSize(window),D=A.parentNode.parentNode.previousSibling.offsetHeight;A.style.top=D+"px";A.style.height=C.H-D+"px"}));for(;a.childNodes.length>0;)l.childNodes[0].appendChild(a.childNodes[0]);c=c.find(".ewa_lf_func_caption")[0];var n=!1;d=GetInnerText(l);d=d.trim();this._ReshowButs={};e=$(e.rows[0]);
for(var p in this.ItemList.Items){var r=this.ItemList.Items[p],q=this._GetSubValue("Tag","Tag",r);if(q&&(q.toLowerCase()=="button"||q.toLowerCase()=="butflow")&&(q=this._GetSubValue("Name","Name",r),e.find(".ewa-col-"+q).length!==0)){n=!0;var t=this._GetSubValue("DescriptionSet","Info",r),v=this._GetSubValue("DescriptionSet","Memo",r),u=this._GetSubValue("EventSet","EventName",r),x="";u!=null&&u.toLowerCase().trim()=="ewa_click"&&(x=this._GetSubValue("EventSet","EventValue",r));d=d.replace(t,"");
r=this._ReShowButton(t,v,null);this._ReshowButs[t]=r;r.setAttribute("f_id",this._Id);r.setAttribute("t_id",q);l.childNodes[1].appendChild(r);x==""?r.setAttribute("onclick","EWA.F.FOS['"+this._Id+"'].reShowButtonClick(this)"):r.setAttribute("onclick",x)}}n&&this._ReShowSplit(l.childNodes[1]);q=l.getElementsByTagName("input");n={};for(e=0;e<q.length;e++){var w=q[e];w.style.display="none";if(w.id==null||w.id=="")w.id=("_tmp_"+Math.random()).replace(".","_");if(w.type=="button"){if(r=this._ReShowButton(w.value,
w.title,w.id),t=w.nextSibling){var y="";t.nodeType==3&&(y=t.nodeValue.trimEx());y=="|"&&this._ReShowSplit(l.childNodes[1])}}else(t=w.nextSibling)&&(y=t.nodeType==3?t.nodeValue.replace("|",""):t.innerHTML),r=this._ReShowButton(y.trimEx(),w.title,w.id),r.id=("_tmp_"+Math.random()).replace(".","_"),n[w.name]||(n[w.name]=[]),n[w.name].push(r.id);l.childNodes[1].appendChild(r)}d=d.replace(/ /ig,"").replace(/\|/ig,"").trimEx();d!=""?d.indexOf("if")>=0?h.style.display="none":k.innerHTML=d.replace(/ /,""):
h.style.display="none";a.appendChild(g);for(p in n)for(y=n[p],e=0;e<y.length;e++)w=$X(y[e]),w.setAttribute("_r_ids",y.join(",")),w.onclick=function(){var A=this.getAttribute("_ewa_event_id");$X(A).click();A=this.getAttribute("_r_ids").split(",");for(var C=0;C<A.length;C++)w.id!=A[C]&&($X(A[C]).style.fontWeight="",$X(A[C]).style.color="");this.style.fontWeight="bold";this.style.color="blue"};c.innerHTML='<b><i class="fa fa-star-o">&nbsp;</i>'+this.Description+"</b>";if(this.Url.toUpperCase().indexOf("EWA_BTNS_IN_TITLE")>
0)var z=this,B=0,E=setInterval(function(){B++;B>1E3&&window.clearInterval(E);EWA.OW.Load();EWA.OW.Dia&&(window.clearInterval(E),z.ReShowButtonsInDailogTitle())},10);this._InitMneu()};this.reShowButtonClick=function(a){a=a.getAttribute("t_id");var b=this.SelectCheckedRows();if(b==null||b.length==0)EWA.UI.Msg.Alert(_EWA_INFO_MSG["EWA.SYS.CHOOSE_ITEM"],_EWA_INFO_MSG["EWA.SYS.CHOOSE_ITEM_TITLE"]);else{b=b[0].getElementsByTagName("input");for(var c=0;c<b.length;c++){var d=b[c];if(d.name===a){d.click();
break}}}};this._ReShowSplit=function(a){EWA.UI.Utils.CreateObject(window,"div","",a).className="ewa_lf_func_split"};this._ReShowButton=function(a,b,c){var d=EWA.UI.Utils.CreateObject(window,"div","cursor:pointer",document.body);d.innerHTML="<nobr>"+a+"</nobr>";c!=null&&(d.setAttribute("_ewa_event_id",c),d.onclick=function(){var e=this.getAttribute("_ewa_event_id");$X(e).click()});EWA.B.IE&&(d.onselectstart=function(){return!1});d.className="ewa_lf_func_dact";d.title=b;return d};this.CheckValid=function(a){var b=
a.tagName.toLowerCase();if(b!="input"&&b!="textarea"&&b!="select"||"button"==a.type||"submit"==a.type||"image"==a.type)return!0;b=this.ItemList.GetObjectValue(a);return this.ItemList.CheckValid(a,b)};this.ShowEdit=function(a){a.parentNode.style.width=a.clientWidth+"px";a.style.display="none";__show_obj__=a.nextSibling;__show_obj__.style.display="";var b=__show_obj__.childNodes[0];b.nodeType==3&&(b=__show_obj__.childNodes[1]);b.setAttribute("old",b.value);b.tagName!="SELECT"||b.getAttribute("func")||
(b.setAttribute("func",1),b.onchange=function(){EWA.F.FOS[this.getAttribute("__ewa_fid__")].EditAfter(this)});b.getAttribute("__ewa_fid__")==null&&(b.setAttribute("__ewa_fid__",this._Id),b.onblur=function(){EWA.F.FOS[this.getAttribute("__ewa_fid__")].EditAfter(this)},b.onkeydown=function(c){c=c==null?window.event:c;var d=c.target?c.target:c.srcElement;c.keyCode==13&&d.tagName=="INPUT"&&EWA.F.FOS[this.getAttribute("__ewa_fid__")].EditAfter(this);c.keyCode==27&&EWA.F.FOS[this.getAttribute("__ewa_fid__")].EditReset(this)});
setTimeout(function(){b.focus();b.select()},10)};this.EditReset=function(a){a.parentNode.style.display="none";var b=a.parentNode.previousSibling;b.style.display="";b.innerHTML=a.getAttribute("old");a.tagName.toLowerCase()=="select"?b.innerHTML=a.options[a.selectedIndex].text:a.value=a.getAttribute("old").replace(/</ig,"&lt;").replace(/>/ig,"&gt;")};this.EditAfter=function(a){if(this.CheckValid(a))if(a.getAttribute("old")==a.value)this.EditReset(a);else{a.parentNode.style.display="none";var b=a.parentNode.previousSibling;
b.style.display="";if(a.getAttribute("ewa_class")=="droplist"){var c=$(a.parentNode.getElementsByTagName("input")[1]).attr("name");var d=a.parentNode.getElementsByTagName("input")[1].value;b.innerHTML=a.parentNode.getElementsByTagName("input")[0].value}else a.tagName.toLowerCase()=="select"?(c=$(a).attr("name"),b.innerHTML=a.options[a.selectedIndex].text):(c=a.value.replace(/</ig,"&lt;").replace(/>/ig,"&gt;").replace(/\n/ig,"<br>"),b.innerHTML=c,c=$(a).attr("name")),d=a.value;b=a.getAttribute("action_name");
if(b==""||b==null)b="OnListFrameUpdateCell";var e=a.getAttribute("action_confirm");e=e==null?"":e;var g=a.getAttribute("action_tip");g==null&&(g="");var h=a.getAttribute("action_js"),k=this.ItemList.GetValue(c,"DataItem","DataType");c=[{Name:"EWA_FIELD_ID",Value:c},{Name:"EWA_FIELD_VAL",Value:d},{Name:"EWA_ACTION_RELOAD",Value:"0"},{Name:c,Value:d},{Name:"EWA_DATA_TYPE",Value:k},{Name:"EWA_FIELD_VAL_OLD",Value:a.getAttribute("old")}];this.EditEvent={FROM:a,OLD_VAL:a.getAttribute("old"),NEW_VAL:a.value};
this.DoAction(a,b,e,g,c,h)}else a.focus()};this.RewriteInfo=function(a,b,c,d,e){var g=$("#EWA_LF_"+this._Id),h=[];if(a)this._INFO={},this._INFO.INFO=a,this._INFO.ID_NAME=b||"id",this._INFO.INFO_NAME=c||"info",this._INFO.MEMO_NAME=d||"memo";else if(!this._INFO||!this._INFO.INFO)return;for(a=0;a<this._INFO.INFO.length;a++)if(b=this._INFO.INFO[a],c=g.find('[id="'+b[this._INFO.ID_NAME]+'"]:eq(0)'),c.length!==0){d=c.parentsUntil("tr").last();var k=b[this._INFO.INFO_NAME],l=b[this._INFO.MEMO_NAME];if(k==
null||k.trim()=="")for(k=d.parent()[0],l=0;l<k.cells.length;l++){if(k.cells[l]==d[0]){this.ShowHiddenColumn(l,!1);break}}else c.children().length==0?c.text(k):c.children()[0].innerText=k,l&&c.attr("title",l);h.push(c[0]);e&&e(c[0],b,a)}return h};this.ShowHiddenColumn=function(a,b){var c=$X("EWA_LF_"+this._Id),d="";if(b==null||b==0||b=="none")d="none";for(b=0;b<c.rows.length;b++)c.rows[b].cells.length==c.rows[0].cells.length&&(c.rows[b].cells[a].style.display=d)};this.ShowHiddenColumns=function(a,
b){var c=$X("EWA_LF_"+this._Id),d="";if(b==null||b==0||b=="none")d="none";b=c.rows[0].cells.length;for(var e=0;e<c.rows.length;e++){var g=c.rows[e];if(g.cells.length==b)for(var h=0;h<a.length;h++)g.cells[a[h]].style.display=d}};this.GroupShowHidden=function(a){var b=$(a).attr("t"),c=(new Date).getTime();if(!(b&&c-b<333)){$(a).attr("t",c);b=a.parentNode.parentNode;c=b.getAttribute("ewa_group");c==""?(c="none",a.childNodes[0].innerHTML=" + "):(c="",a.childNodes[0].innerHTML=" - ");b.setAttribute("ewa_group",
c);for(var d=b.parentNode.parentNode,e=0,g=b.rowIndex+1;g<d.rows.length;g++){b=d.rows[g];if(b.getAttribute("ewa_tag")=="group")break;b.getAttribute("add_pre_row")?c=="none"?(b.style.display!="none"&&b.setAttribute("ewa_group_show_old",b.style.display),b.style.display="none"):b.hasAttribute("ewa_group_show_old")&&(b.style.display=b.getAttribute("ewa_group_show_old"),b.removeAttribute("ewa_group_show_old")):(b.style.display=c,e++)}a.childNodes[a.childNodes.length-1].innerHTML=" ("+e+")"}};this.Sort=
function(a){var b=this._last_sort_timer||0,c=this._Sort||"",d=(new Date).getTime();c==a&&d-b<500||(this._last_sort_timer=d,this._Sort=a,this.Goto(1))};this.SearchClear=function(a){a=a.parentNode.parentNode.parentNode.parentNode;for(var b=0;b<a.rows.length;b+=1)for(var c=a.rows[b].getElementsByTagName("input"),d=0;d<c.length;d++){var e=c[d];if(e.type=="text")e.value="";else if(e.type=="radio"||e.type=="checkbox")e.checked=!1}};this.SearchGetExpInit=function(){let a=$("#_G_"+this._Id);var b=[];$(a).find(".ewa-lf-search-item").each(function(){var c=
$(this).find("input");if(c.length==0&&(c=$(this).find("select"),c.length==0))return;var d=c[0],e=$(d).attr("t");if(c.length==1){var g=d.value;if(d.value.trim()!="")for(e=="text"&&(c=$(d.parentNode.parentNode).find("a").attr("tag"))&&(e=c),d=d.name.split(","),c=0;c<d.length;c++)b.push(d[c]+"["+e+"]"+g)}else if((e=="date"||e=="number")&&c.length==2){if(g=d.value,e=c[1].value,e!=""||g!="")b.push(d.name+"[gte]"+g),e&&b.push(d.name+"[lte]"+e)}else if(e=="fix"&&d.tagName=="INPUT"){var h=[""];$(this).find("input").each(function(){this.checked&&
h.push(this.value)});h.length!=1&&b.push(d.name+"[or]"+h.join(";"))}});return b.join(",")};this.SearchGetExp=function(a){var b=[];$(a).find(".ewa-lf-search-item").each(function(){var c=$(this).find("input");if(c.length==0&&(c=$(this).find("select"),c.length==0))return;var d=c[0],e=$(d).attr("t");if(c.length==1){var g=d.value;d.value.trim()!=""&&(e=="text"&&(c=$(d.parentNode.parentNode).find("a").attr("tag"))&&(e=c),b.push("@!@"+d.name+"~!~"+e+"~!~"+g))}else if((e=="date"||e=="number")&&c.length==
2)g=d.value,c=c[1].value,c==""&&g==""||b.push("@!@"+d.name+"~!~"+e+"~!~"+g+"~!~"+c);else if(e=="fix"&&d.tagName=="INPUT"){var h=[""];$(this).find("input").each(function(){this.checked&&h.push(this.value)});h.length!=1&&b.push("@!@"+d.name+"~!~"+e+h.join("~!~"))}});return b.join("")};this.DoSearch=function(a){var b=!1;a.id=="EWA_SEARCH_ITEM_"+this.Id?a=$X("EWA_SEARCH_ITEM_"+this.Id).childNodes[0]:(a=a.parentNode.parentNode.parentNode.parentNode,b=!0);this._SearchExp=this.SearchGetExp(a);b&&(this._SearchHtml=
a.parentNode.innerHTML,this._SearchDialog.Show(!1));this.Goto(1)};this.Search=function(a){this._SearchHtml==null?(this._SearchCreate(),this._SearchDialog=new EWA.UI.Dialog.OpenWindow("about:blank","aa",400,100,!0,null,null,!1),this._SearchDialog.SetCaption(_EWA_INFO_MSG["EWA.LF.SEARCH.BUT.TITLE"]),this._SearchDialog._Dialog.AutoSize=!0,this._SearchDialog._Dialog.SetHtml(this._SearchHtml),this._SearchDialog._Dialog.Show(!0),this._SearchDialog._Dialog.GetFrame().focus(),this._SearchDialog.MoveCenter()):
...
"&quot;].ChangeSearchDateType(this)",l=$J2MAP(this.RESOURCES.search_text_items,"Id");e=this._GetInitSearchMap();var m=!1,n=!1;for(g in this.ItemList.Items)if(this._SearchJson[g]!=null){var p=[],r=this._SearchJson[g],q=r.T,t=this._GetSubValue("DescriptionSet","Info",this.ItemList.Items[g]);p.push("<table class='ewa-lf-search-item ewa-ref-"+g+" ewa-lf-search-type-"+q+" "+(q=="fix"?"ewa-lf-search-fix-m"+(r.M||"0"):"")+"' cellspacing=0 cellpadding=0>");p.push("<tr><td class='ewa-lf-search-item-title' style='position:relative'><nobr><b>");
p.push(t);var v=t="",u="lk";e[g.toUpperCase()]&&(v=e[g.toUpperCase()],u=v.tag,t=v.para1,v=v.para2);var x="";q=="text"?(n=l[u]||l.lk,x=EWA.LANG=="enus"?n.TxtEn:n.Txt,p.push("<a class='ewa-lf-search-type' tag='"+u+"' href='javascript:void(0)' onclick='"+h+"'>"),p.push(x),p.push("</a>"),n=!0):q=="date"&&(p.push("<a  href='javascript:void(0)' class='fa fa-calendar-check-o'  onclick='"+k+"'></a>"),m=!0);p.push("</b></nobr></td><td class='ewa-lf-search-item-ctl'>");q=="text"?u=="blk"||u=="nblk"?(r=this._SearchSingle(g,
x),r=r.replace("<input","<input disabled "),p.push(r)):p.push(this._SearchSingle(g,t)):q=="date"?p.push(this._SearchDate(g,t,v)):q=="number"?p.push(this._SearchNumber(g,t,v)):q=="fix"&&(r="<div >"+this._SearchFix(g,r,"span",t)+"</div>",p.push(r));p.push("</td></tr></table>");q=="fix"&&this._IsSearchGroup?c.push(p.join("")):d.push(p.join(""))}if(d.length+c.length!=0){b.push('<table class="ewa-lf-search-main" id="'+a+'" cellspacing="0" cellpadding="0">');b.push("<tr><td class='ewa-lf-search-des'></td><td>");
d.length>0&&b.push(" "+d.join("")+" ");for(d=0;d<c.length;d++)b.push(" "+c[d]+" ");b.push("</td></tr></table>");var w=document.createElement("div");this._SEARCH_BOX=w;w.className="ewa-lf-search";w.innerHTML=b.join("");b=$X("EWA_LF_"+this.Id).parentNode;b.parentNode.insertBefore(w,b);a="EWA_SEARCH_ITEM_"+this.Id;var y=this;m&&this._SearchCreateDateRange();n&&this._SearchCreateTextType();y._SEARCH_ITEM_EXP=this.SearchGetExp(w);JSON.stringify(e)!="{}"&&(y._SearchExp=y._SEARCH_ITEM_EXP);$(w).find("input[type=text]").on("compositionstart",
function(){y._is_search_composition=!0}).on("compositionend",function(){y._is_search_composition=!1});this._TIMER_SEARCH=window.setInterval(function(){if(!y._is_search_composition){try{if(!$X(a)){window.clearInterval(y._TIMER_SEARCH);return}}catch(B){window.clearInterval(y._TIMER_SEARCH);return}var z=y.SearchGetExp(w);z!=y._SEARCH_ITEM_EXP&&(y._SEARCH_ITEM_EXP=z,y.DoSearch($X(a)))}},700)}};this._SearchSingle=function(a,b){var c=[];c.push('<input t="text" type="text" autocomplete="off" class="EWA_INPUT" maxlength="40" name="');
c.push(a.toInputValue());c.push('"');b!=null&&(c.push(' value="'),a=b.toInputValue(),c.push(a),c.push('" '));c.push(" />");return c.join("")};this._SearchDate=function(a,b,c){var d=[];d.push("<table border=0 cellpadding=0 cellspacing=0>");d.push('<td  ><input type="text" autocomplete="off" class="EWA_INPUT" t="date" readonly maxlength="8" onclick="EWA.UI.Calendar.Pop(this)" name="');d.push(a);d.push('"');b&&(b=b.toInputValue(),d.push(' value="'),d.push(b),d.push('"'));d.push(' /></td><td><span class="ewa-lf-search-split"> - </span></td>');
...
a+"_SearchFix_"+e);h=h.replace("@V",g[0]);h=h.replace("@T",g[1]);h=d&&g[0]==d?h.replace("@CKD","selected"):h.replace("@CKD","");c.push(h)}c.push("</select>");return c.join("")};this._SearchCreate=function(){var a=[];a.push("<div><table border=0 style='margin:0px;' class=EWA_TABLE cellspacing=1>");for(var b in this.ItemList.Items)if(this._SearchJson[b]!=null){var c=this._SearchJson[b],d=c.T,e=this._GetSubValue("DescriptionSet","Info",this.ItemList.Items[b]);a.push("<tr><td class=EWA_TD_L width=140><nobr>"+
e+"<nobr></td><td width=350 class=EWA_TD_M>");d=="text"?a.push(this._SearchSingle(b)):d=="date"?a.push(this._SearchDate(b)):d=="number"?a.push(this._SearchNumber(b)):d=="fix"&&a.push(this._SearchFix(b,c,"div"));a.push("</td></tr>")}a.push("<tr><td class=EWA_TD_B colspan=2 align=right><input onclick='EWA.F.FOS[\""+this._Id+"\"].SearchClear(this);' type=button value='"+_EWA_INFO_MSG["EWA.LF.SEARCH.BUT.CLEAR"]+"'> <input type=submit value='"+_EWA_INFO_MSG["EWA.LF.SEARCH.BUT.SEARCH"]+"' onclick='EWA.F.FOS[\""+
this._Id+"\"].DoSearch(this);'> <input type=button value='"+_EWA_INFO_MSG["EWA.LF.SEARCH.BUT.CLOSE"]+"' onclick='EWA.F.FOS[\""+this.Id+"\"]._SearchDialog.Show(false);'></td></tr>");a.push("</table></div>");this._SearchHtml=a.join("")};this.createRegExp=function(a){if(!a)return null;a=String(a).trim();if(a.length===0)return null;for(var b=[],c=0;c<a.length;c++){var d=a[c];d>="a"&&d<="z"||d>="A"&&d<="Z"||d>="0"&&d<="9"?b.push(d):b.push("\\"+d)}try{return new RegExp("("+b.join("")+")")}catch(e){return console.warn("new RegExp",
a,e),null}};this.SearchMark=function(){for(var a=this._SearchExp.split("@!@"),b=$X("EWA_LF_"+this._Id),c=0;c<a.length;c+=1){var d=a[c].split("~!~");if(!(d.length<3))for(var e=2;e<d.length;e++){var g=d[e];if(g=="")continue;if(d[1]=="fix"){var h=d[2];$("table#EWA_SEARCH_ITEM_"+this.Id+' [name="'+d[0]+'"]').find("option").each(function(){this.value==h&&(g=this.text)})}let l=this.createRegExp(g),m=d[0].split(",");for(let n in m){var k=m[n];this._SearchJson[k]&&(k=this._SearchJson[k].ORI_NAME);k=$(b).find(".ewa-lf-data-row .ewa-col-"+
k).not("[ewa-search-marked=yes]");k.attr("ewa-search-marked","yes");try{k.find("*").each(function(){this.children.length==0&&(this.innerHTML=this.innerHTML.replace(l,"<font color=red><b>$1</b></font>"))})}catch(p){console.log(p)}}}}};this.DblClick=function(a){this._IsDblClick=a;for(var b=$X("EWA_LF_"+this._Id),c=1;c<b.rows.length;c++)b.rows[c].ondblclick=function(){var d=this.getElementsByTagName("input");d.length>a&&d[a].disabled==0&&d[a].click()}};this.Init=function(a){this.Xml=new EWA.C.Xml;this.Xml.LoadXml(a);
this.ItemList.Init(this.Xml);this.Resources=new EWA_FrameResoures;this.Resources.Init(this.Xml);if((a=$X("EWA_LF_"+this._Id))&&a.tagName=="table"&&!(a.rows.length<=1)){a=a.rows[1].getElementsByTagName("input");for(var b=0;b<a.length;b++)if(a[b].type.toUpperCase()=="RADIO"){this._TrSelectMulti=!1;break}}};this.SetPageParameters=function(a,b,c,d,e){this._PageCurrent=a;this._PageSize=c;this._PageCount=b;this._RecordCount=d;this._Sort=e};this.SetPageParametersName=function(a,b,c,d,e){this._PageCurrentName=
a;this._PageSizeName=c;this._PageCountName=b;this._RecordCountName=d;this._SortName=e};this.CheckedAll=function(){var a=document.getElementById("EWA_LF_"+this._Id);if(!(a.rows.length<=1)){for(var b=0;b<a.rows.length;b+=1){var c=a.rows[b].getElementsByTagName("input");c.length!=0&&(c[0].checked=this._IsCheckedAll)}this._IsCheckedAll=this._IsCheckedAll?!1:!0;this.CheckedAllAfter&&this.CheckedAllAfter(a)}};this.SelectChecked=function(){var a=$X("EWA_LF_"+this._Id);if(!a||a.rows.length<1)return"";var b=
[];let c=[];$("#EWA_LF_"+this._Id+">tbody>tr").find("input[type='radio'],input[type='checkbox']").each(function(){this.parentNode.className.indexOf("ewa-switch")==-1&&c.push(this)});if(c.length>0)for(var d=0;d<c.length;d++)c[d].checked&&b.push(c[d].value);else for(d=0;d<a.rows.length;d++){var e=a.rows[d];e.getAttribute("__ewa_lf_mdown")==1&&b.push(e.getAttribute("EWA_KEY"))}return b.join(",")};this.SelectCheckedInputs=function(){var a=[],b=$X("EWA_LF_"+this._Id);if(b.rows.length<1)return a;b=b.getElementsByTagName("input");
a=[];for(var c=0;c<b.length;c++)(b[c].type.toUpperCase()=="CHECKBOX"||b[c].type.toUpperCase()=="RADIO")&&b[c].checked&&a.push(b[c]);return a};this.SelectCheckedRows=function(){for(var a=[],b=$X("EWA_LF_"+this._Id),c=0;c<b.rows.length;c++){var d=b.rows[c],e=d.getElementsByTagName("input");e.length!=0&&e[0].checked&&e[0].parentNode.className.indexOf("ewa-switch")==-1&&a.push(d)}if(a.length==0)for(c=0;c<b.rows.length;c++)d=b.rows[c],d.getAttribute("__ewa_lf_mdown")==1&&a.push(d);return a};this.Reload=
function(a){this.StopAjaxAfterReload?this.StopAjaxAfterReload=!1:this.Goto(this._PageCurrent,a)};this.refreshPage=function(a,b){this.replaceRowsData(null,null,a,b)};this.replaceRowsData=function(a,b,c,d){var e=this.getUrlClass();e.AddParameter("EWA_AJAX","LF_RELOAD");e.AddParameter("EWA_IS_SPLIT_PAGE","no");e.AddParameter("EWA_IS_HIDDEN_CAPTION","yes");e=e.GetUrl();a&&(e+="&"+a);let g=a?new EWA_AjaxClass:this.CreateAjax(),h=this;g.PostNew(e,function(){if(g._Http.readyState==4){g.HiddenWaitting();
var k=g._Http.responseText;g._Http.status!=200?(console.error(k),alert("ERROR:\r\n"+g._Http.statusText)):EWA.F.CheckCallBack(k)?h.replaceRowsWithDataHtml(k,b,c,d):console.error(k)}})};this.replaceRowsWithDataHtml=function(a,b,c,d){let e=$("<div></div>");e.html(a);let g=[],h=$("#EWA_LF_"+this._Id);e.find(".ewa-lf-data-row").each(function(){var k='tr[ewa_key="'+$(this).attr("ewa_key")+'"]';let l=h.find(k);if(l.length==0)console.warn(k+" not found");else{k=$(this).attr("ewa_row_sign");var m=l.attr("ewa_row_sign"),
n=!0;(k||m)&&m===k&&(n=!1);if(n){l.attr("ewa_row_sign",k);k=l.clone()[0];for(m=0;m<this.cells.length;m++){n=l[0].cells[m];let p=this.cells[m];n.removeAttribute("ewa-merged");n.removeAttribute("ewa-search-marked");b?b(p,n):n.innerHTML=p.innerHTML}g.push({before:k,current:l[0]})}}});e.remove();this.SearchMark();this.IsReShow&&this.ReShowWithNoButtons();this._ReShowSearchQuick();this._IsDblClick!=null&&this.DblClick(this._IsDblClick);this.SubBottomsArray&&this._SubBottoms();this.ReloadAfter&&this.ReloadAfter(c);
...
"1");this._Ajax.AddParameter("EWA_ACTION_TIP",d);g!=null&&g.trim().length>0?(this._Ajax.AddParameter("EWA_AFTER_EVENT",g),this.StopAjaxAfterReload=!0,this._Ajax.AddParameter("EWA_ACTION_RELOAD","0")):this.StopAjaxAfterReload=!1;if(a&&a.getAttribute&&a.getAttribute("box_menu_item")=="yes"){if(this._Ajax.AddParameter("___CALL__FROM__","box"),a=a.parentNode)a=a.getAttribute("json_ref"),a=JSON.parse(a),a.EWA_KEY&&this._Ajax.AddParameter("EWA_ACTION_KEY",a.EWA_KEY)}else(a=this.GetRowKey(a))&&this._Ajax.AddParameter("EWA_ACTION_KEY",
...
b){EWA.F.CID=this._Id;this._PageCurrent=a;this._Ajax=this.CreateAjax();a=new EWA_UrlClass;a.SetUrl(this.Url==null?document.location.href:this.Url);a.RemoveParameter("EWA_AJAX");var c=this;this._Ajax.PostNew(a.GetUrl(),function(){c._CallBack(b)});this.REPLACE_HISTORY_STATE&&this.replaceHistoryState()};this.createReplaceHistoryStateUrl=function(){var a=new EWA_UrlClass;a.SetUrl(this.Url==null?document.location.href:this.Url);a.AddParameter(this._PageCurrentName,this._PageCurrent);this._PageSize&&a.AddParameter(this._PageSizeName,
this._PageSize);this._Sort!=null&&a.AddParameter(this._SortName,this._Sort);var b=this.SearchGetExpInit();b&&a.AddParameter("EWA_SEARCH",b);if(this.GotoParas!=null&&this.GotoParas.length>0)for(b=0;b<this.GotoParas.length;b+=1)a.AddParameter(this.GotoParas[b].Name,this.GotoParas[b].Value);a.RemoveParameter("EWA_AJAX");return a};this.replaceHistoryState=function(){let a=this.createReplaceHistoryStateUrl();window.history.replaceState("",null,a.GetUrl())};this.CreateAjax=function(){var a=new EWA.C.Ajax;
a.LoadingType="image";a.AddParameter(this._PageCurrentName,this._PageCurrent);this._PageSize&&a.AddParameter(this._PageSizeName,this._PageSize);this._Sort!=null&&a.AddParameter(this._SortName,this._Sort);this._SearchExp!=null&&this._SearchExp!=""&&a.AddParameter("EWA_LF_SEARCH",this._SearchExp);a.AddParameter("EWA_AJAX","LF_RELOAD");if(this.GotoParas!=null&&this.GotoParas.length>0)for(var b=0;b<this.GotoParas.length;b+=1)a.AddParameter(this.GotoParas[b].Name,this.GotoParas[b].Value);return a};this.DownlodData=
function(a,b){EWA.F.CID=this._Id;this._Ajax=new EWA.C.Ajax;this._Ajax.LoadingType="image";this._Sort!=null&&this._Ajax.AddParameter(this._SortName,this._Sort);this._SearchExp!=null&&this._SearchExp!=""&&this._Ajax.AddParameter("EWA_LF_SEARCH",this._SearchExp);this._Ajax.AddParameter("EWA_AJAX","DOWN_DATA");this._Ajax.AddParameter("EWA_AJAX_DOWN_TYPE",a);b&&this._Ajax.AddParameter("EWA_ACTION",b);if(this.GotoParas!=null&&this.GotoParas.length>0)for(a=0;a<this.GotoParas.length;a+=1)this._Ajax.AddParameter(this.GotoParas[a].Name,
this.GotoParas[a].Value);this._AjaxType="DOWN_DATA";a=new EWA_UrlClass;a.SetUrl(this.Url==null?document.location.href:this.Url);var c=this;this._Ajax.PostNew(a.GetUrl(),function(){c._CallBack()})};this.Get=function(a){EWA.F.CID=this._Id;this._Ajax=new EWA.C.Ajax;this._Ajax.LoadingType="image";var b=this;this._Ajax.Get(a,function(){b._CallBack()})};this.Post=function(a,b){EWA.F.CID=this._Id;this._Ajax=new EWA.C.Ajax;this._Ajax.LoadingType="image";var c=this;this._Ajax.Post(a,b,function(){c._CallBackJs()})};
...
this.ReShowWithNoButtons(),this._ReShowSearchQuick(),this._IsDblClick!=null&&this.DblClick(this._IsDblClick),this._IsAddPreRow&&this.AddPreRow(),this.SubBottomsArray&&this._SubBottoms(),this.ReloadAfter&&this.ReloadAfter(a),this.ReloadAfterApp&&this.ReloadAfterApp(a),this._IsCheckedAll=!0))):alert("ERROR:\r\n"+b._Http.statusText);this.RewriteInfo()}};this._CallBackJs=function(){var a=this._Ajax;if(a._Http.readyState==4){if(top.EWA&&top.EWA.MQE){var b=new EWA_MqeClass;b.Data=a.GetRst();b.FromId=EWA.F.CID;
b.Type="POST";top.EWA.MQE.AddMqe(b)}a.HiddenWaitting();a._Http.status==200?(a=a._Http.responseText,EWA.F.CheckCallBack(a)&&eval(a)):alert("ERROR:\r\n"+a._Http.statusText);this.EditAfterEvent&&this.EditAfterEvent()}};this.EditAfterEvent=function(){this.getUrlClass().GetParameter("EWA_ROW_SIGN")&&this.refreshPage()};this._GetItem=function(a){for(var b=this.ItemList,c=0;c<b.length;c+=1){var d=b[c],e=this._GetSubItem("Name",d);if(this.Xml.GetElementAttribute(e,"Name")==a)return d}};this._GetSubItem=function(a,
...
for(d=0;d<b.length;d++)q=b[d],a=q.WF_UNIT_TO,t=q.WF_UNIT_FROM,t=n.Units[t],a=n.Units[a],p=new EWAC_WfCnn,p.Create(t,a),n.Cnns[p.Id]=p,q.IsHaveLogic&&!q.IsLogicOk&&(p.IsStop=!0);for(var x in n.Units)d=n.Units[x],u==null&&d.CnnsToLength==0&&(u=d);if(u==null)d=EWA.LANG=="enus"?"No startup node was found, process definition error":"\u672a\u53d1\u73b0\u542f\u52a8\u8282\u70b9,\u6d41\u7a0b\u5b9a\u4e49\u9519\u8bef",alert(d);else{c.length==0&&(n.CurUnit=u);var w=[];x={};for(d=0;d<c.length;d++)b=c[d],n=b.SYS_STA_TAG,
t=m[n],t!=null&&(q=b.ADM_NAME,EWA.LANG=="enus"&&b.ADM_NAME_EN&&(q=b.ADM_NAME_EN),t.Log(q,b.SYS_STA_CDATE,b.SYS_STA_MEMO),x[n]||(x[n]=1,w.push({ID:n,DES:t.GetDes()})),n=b.SYS_STA_TAG+"G"+b.SYS_STA_VAL,EWAC_WfUtil.WF.Cnns[n]&&(EWAC_WfUtil.WF.Cnns[n].Disp(3,"blue"),t==u&&(t.ADM_ID=b.ADM_ID),d==c.length-1&&(t=m[b.SYS_STA_VAL],t.WF_UNIT_ADM_LST=b.ROLE_IDS,t.WF_UNIT_ADM=b.ROLE_TYPE,EWAC_WfUtil.WF.CurUnit=t)));l!=null&&l.length>0&&m[l]&&(EWAC_WfUtil.WF.CurUnit=m[l]);EWAC_WfUtil.WF.CurUnit.LogCur(e);l=!1;
...
"\u6ca1\u6709\u7ec4\u7ec7\u673a\u6784",alert(d),l=!1),l=top.ORG.GetMaster(y).Id==g?!0:!1);e&&window.parent&&window.parent.$X(e)&&(y=window.parent.$X(e).parentNode.parentNode.parentNode,l?w.length>0&&(d=y.rows[y.rows.length-1].cells[0],g=window.parent.document.createElement("input"),g.type="button",g.value=EWA.LANG=="enus"?"Back":"\u6253\u56de",g.onclick=function(){var z=window.parent.$X(e);z.options.length=1;for(var B=0;B<w.length;B++){var E=w[B];z.options[z.length]=new Option(E.DES,E.ID);this.style.display=
