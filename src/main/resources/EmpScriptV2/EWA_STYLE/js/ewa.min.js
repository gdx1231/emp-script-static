...
this.SubBottomsArray[d];b.$jscomp$loop$prop$total$28=0;var g="#EWA_LF_"+this._Id+' .ewa-lf-data-row [name="'+e+'"]';b.$jscomp$loop$prop$fm_length$29=0;$(g).each(function(h){return function(){var k=this.hasAttribute("value")?this.value:this.hasAttribute("title")?this.title:this.innerText;null!=k&&0!==k.length&&(k=k.replace(/,/ig,""),isNaN(k)||(h.$jscomp$loop$prop$total$28+=1*k,0<k.indexOf(".")&&(h.$jscomp$loop$prop$fm_length$29=2)))}}(b));g="<span id='"+e+"' name='"+e+"' class='ewa-lf-sub'>"+b.$jscomp$loop$prop$total$28.fm(b.$jscomp$loop$prop$fm_length$29)+
"</span>";a.find("td[id=ADD_ROW_"+e+"]").html(g);b={$jscomp$loop$prop$total$28:b.$jscomp$loop$prop$total$28,$jscomp$loop$prop$fm_length$29:b.$jscomp$loop$prop$fm_length$29}}};this.ChangeRowStyle=function(a,b,d){for(var e=$X("EWA_LF_"+this._Id),g=1;g<e.rows.length;g++){var h=e.rows[g];if(!(0==h.cells.length||h.cells.length<=a)){var k=h.cells[a].childNodes[0].getAttribute(b);if(null!=k&&d[k])for(var l=0;l<h.cells.length;l++)h.cells[l].style.cssText=h.cells[l].style.cssText+";"+d[k]}}if(null==this.ReloadAfter){var m=
...
"&quot;].ChangeSearchDateType(this)",m=$J2MAP(this.RESOURCES.search_text_items,"Id");g=this._GetInitSearchMap();var n=!1,p=!1;for(h in this.ItemList.Items)if(null!=this._SearchJson[h]){var q=[],u=this._SearchJson[h],r=u.T,t=this._GetSubValue("DescriptionSet","Info",this.ItemList.Items[h]);q.push("<table class='ewa-lf-search-item ewa-ref-"+h+" ewa-lf-search-type-"+r+" "+("fix"==r?"ewa-lf-search-fix-m"+(u.M||"0"):"")+"' cellspacing=0 cellpadding=0>");q.push("<tr><td class='ewa-lf-search-item-title' style='position:relative'><nobr><b>");
q.push(t);var w=t="",v="lk";g[h.toUpperCase()]&&(w=g[h.toUpperCase()],v=w.tag,t=w.para1,w=w.para2);var y="";"text"==r?(p=m[v]||m.lk,y="enus"==EWA.LANG?p.TxtEn:p.Txt,q.push("<a class='ewa-lf-search-type' tag='"+v+"' href='javascript:void(0)' onclick='"+k+"'>"),q.push(y),q.push("</a>"),p=!0):"date"==r&&(q.push("<a  href='javascript:void(0)' class='fa fa-calendar-check-o'  onclick='"+l+"'></a>"),n=!0);q.push("</b></nobr></td><td class='ewa-lf-search-item-ctl'>");"text"==r?"blk"==v||"nblk"==v?(u=this._SearchSingle(h,
y),u=u.replace("<input","<input disabled "),q.push(u)):q.push(this._SearchSingle(h,t)):"date"==r?q.push(this._SearchDate(h,t,w)):"number"==r?q.push(this._SearchNumber(h,t,w)):"fix"==r&&(u="<div >"+this._SearchFix(h,u,"span",t)+"</div>",q.push(u));q.push("</td></tr></table>");"fix"==r?d.push(q.join("")):e.push(q.join(""))}if(0!=e.length+d.length){b.push('<table class="ewa-lf-search-main" id="'+a+'" cellspacing="0" cellpadding="0" style="width:100%">');b.push("<tr><td class='ewa-lf-search-des'></td><td>");
...
$(x).find("input[type=text]").on("compositionstart",function(){z._is_search_composition=!0}).on("compositionend",function(){z._is_search_composition=!1});this._TIMER_SEARCH=window.setInterval(function(){if(!z._is_search_composition){try{if(!$X(a)){window.clearInterval(z._TIMER_SEARCH);return}}catch(C){window.clearInterval(z._TIMER_SEARCH);return}var A=z.SearchGetExp(x);A!=z._SEARCH_ITEM_EXP&&(z._SEARCH_ITEM_EXP=A,z.DoSearch($X(a)))}},700)}};this._SearchSingle=function(a,b){var d=[];d.push('<input t="text" type="text" autocomplete="off" class="EWA_INPUT" maxlength="40" name="');
