...
null);try{a.ReloadAfter&&a.ReloadAfter(b)}catch(d){alert(b)}}}else alert("ERROR:\r\n"+b._Http.statusText)};this.RewriteInfo=function(a,b,d,e){for(var g=$("#EWA_FRAME_"+this._Id),h=[],k=0;k<a.length;k++){var l=a[k],m=g.find("#"+l[b]);0!=m.length&&(m=m.parentsUntil("tr[show_msg]").last().parent(),null==l[d]||""==l[d].trim()?m.hide():3==m[0].cells.length?(h.push(m[0]),m[0].cells[2].innerHTML=l[e],m[0].cells[0].innerHTML=l[d]):2==m[0].cells.length?(h.push(m[0]),m[0].cells[0].innerHTML=l[d]):1==m[0].cells.length&&
(m=m.prev(),1<m[0].cells.length&&(m[0].cells[1].innerHTML=l[e]),h.push(m[0]),m[0].cells[0].innerHTML=l[d]))}return h};this._HiddenMemo=function(a,b){var d=a.parentNode.parentNode;b=!1;"none"==d.style.display&&(b=!0);d.style.display="none";d.id="tr_"+a.id;d=d.previousSibling;3==d.nodeType&&(d=d.previousSibling);d.style.display="none";a="<span id=tab_"+a.id+' class="ewa_merge_tab" onclick="EWA.F.FOS[\''+this._Id+"'].ShowMemo(this.id);\">"+d.cells[0].innerHTML+"</span>";return b?"":a};this.ShowMemo=
function(a){for(var b=$X(this._Id+"__ewa_meger").getElementsByTagName("SPAN"),d=0;d<b.length;d++){var e=b[d].id.replace("tab_","tr_");"ewa_merge_tab1"==b[d].className&&(b[d].className="ewa_merge_tab",$X(e).style.display="none");b[d].id==a&&(b[d].className="ewa_merge_tab1",$X(e).style.display="")}};this.MergeMemo=function(){var a=$X("EWA_FRAME_"+this._Id).getElementsByTagName("textarea");if(0!=a.length&&"1"!=a[0].getAttribute("ewa_merge")){for(var b=[],d=0;d<a.length;d++){var e=this._HiddenMemo(a[d]);
e="<nobr>"+e.replace("\u2022 ","").trim()+"</nobr>";b.push(e)}a=a[0];d=a.parentNode.parentNode;e=d.previousSibling;3==e.nodeType&&(e=e.previousSibling);e.id=this._Id+"__ewa_meger";e.cells[0].colSpan=3;e.cells[0].innerHTML=b.join("");e.cells[0].style.width="100%";e.cells[0].style.height="25px";e.style.display="";d.style.display="";$X("tab_"+a.id).className="ewa_merge_tab1";null!=window._EWA_DialogWnd&&EWA.UI.Dialog._SetTitle(window._EWA_DialogWnd)}};this.DoValidEx=function(a,b,d,e,g,h){""!=a.value&&
(this._ValidExOk=this.Resources[g],this._ValidExFail=this.Resources[h],this._ValidExObj=a,"action"==b.toLowerCase().trim()?this._DoValidExAction(a,e,this.Resources[g].GetInfo(),this.Resources[h].GetInfo()):eval(d)?(EWA_FrameShowAlert(a,this._ValidExOk.GetInfo()),a.setAttribute("EWA_VALID_EX","")):(EWA_FrameShowAlert(a,this._ValidExFail.GetInfo()),a.setAttribute("EWA_VALID_EX",this._ValidExFail.GetInfo())))};this._DoValidExAction=function(a,b,d,e){if(null!=a&&!$(a).is(":hidden"))if(null==b||""==b.trim())alert("action not value");
else{var g=a.id+","+a.value;if(this.DoValidExAction_RETS&&null!=this.DoValidExAction_RETS[g])0==this.DoValidExAction_RETS[g]?(EWA_FrameRemoveAlert(a,d),a.setAttribute("EWA_VALID_EX","")):(EWA_FrameShowAlert(a,e),a.setAttribute("EWA_VALID_EX",e));else{var h=new EWA.C.Ajax;h.LoadingType="image";h.AddParameter("EWA_AJAX","HAVE_DATA");h.AddParameter("EWA_ACTION",b);h.AddParameter("EWA_ID",this._Id);h.AddParameter("EWA_NO_CONTENT","1");h.AddParameter(a.id,a.value);this.DoValidExAction_RETS||(this.DoValidExAction_RETS=
{});var k=this;h.PostNew(this.Url,function(){if(4==h._Http.readyState){h.HiddenWaitting();if(200==h._Http.status){var l=h._Http.responseText;if(EWA.F.CheckCallBack(l))try{var m=parseInt(l.trim());k.DoValidExAction_RETS[g]=m;0==m?(EWA_FrameRemoveAlert(a,d),a.setAttribute("EWA_VALID_EX","")):(EWA_FrameShowAlert(a,e),a.setAttribute("EWA_VALID_EX",e))}catch(n){alert(n+","+l)}}else alert("ERROR:\r\n"+h._Http.statusText);h=null}})}}};this.ValidCodeError=function(a){$(".ewa-valid-code-img").each(function(){this.click()});
a||($Tip(_EWA_EVENT_MSG.ValidCodeError),$(".ewa-valid-code-img").parent().parent().find("input").focus().select())};this.Init=function(a){this.Xml=new EWA.C.Xml;this.Xml.LoadXml(a);this.ItemList.F=this;this.ItemList.Init(this.Xml);this.Resources=new EWA_FrameResoures;this.Resources.Init(this.Xml);"enus"==EWA.LANG&&$(".ewa-ipt-droplist").attr("placeholder","Input character selection");this._InitTranslations();this._GetDropListValue();this._InitMustInputs();this._InitEncyptions()};this._InitMustInputs=
...
Array){var e=this;this._checkStatusInc=0;this._checkStatusFristResult=d;this._checkStatusTimer=window.setInterval(function(){var m=0==e._checkStatusInc?e._checkStatusFristResult:e.DoPostBefore();e._checkStatusInc++;null!=m[0]&&(window.clearInterval(e._checkStatusTimer),e._checkStatusTimer=null,e._checkStatusInc=null,e._checkStatusFristResult=null,0==m[0]?m[2]?$Confirm(m[1],m[1],function(){m[0]=m[1]=null;e.DoPost(a,b,!0)},function(){}):$Tip(m[1]):(m[0]=m[1]=null,e.DoPost(a,b,!0)))},232);return}if(!d)return}var g=
$(a).find("input[ewa_tag=ht5upload]");if(!(0<g.length)||this.uploadProcess||this.CheckValidAll(a)){var h=0;e=this;for(d=0;d<g.length>0;d++){var k=g[d];this._uploadingFileCheck(k)&&h++}if(0<h)setTimeout(function(){e.DoPost(a,b)},700);else if(g=$(a).find("input[ewa_tag=ht5takephoto]"),!(0<g.length)||this.uploadProcess||this.CheckValidAll(a)){for(d=0;d<g.length>0;d++){k=g[d];h=!1;k=window["h5takephoto_"+this._Id+"$"+k.id];if("ok"!=k.UPLOAD_STATUS){if(!$X(k.H5_UPLOAD.WAIT_ID)){try{k.submitUpload()}catch(m){console.log("ht5takephotos - ht5.submitUpload: "+
m);$Tip("\u60a8\u6ca1\u6709\u83b7\u53d6\u7167\u7247");return}k.H5_UPLOAD.createWaitBox();this.uploadProcess=!0}h||(e=this,setTimeout(function(){e.DoPost(a,b)},700),h=!0);return}k.H5_UPLOAD.removeWaitBox()}if(this._stopPostError)EWA.UI.Msg.ShowError(this._stopPostError,"");else if(d=this.CreateAjax(a),null!=d){null==b&&(b=this.Url);if(""==b||null==b)b=window.location.href;this._Ajax=d;this._Ajax.AddParameter("EWA_AJAX","1");this._Ajax.AddParameter("EWA_POST","1");this._Ajax.AddParameter("EWA_NO_CONTENT",
"1");this._Ajax.LoadingType="image";this.DoPostStart&&this.DoPostStart(this._Ajax);d=new EWA_UrlClass(b);for(var l in this._Ajax._Parameters)d.RemoveParameter(l);b=d.GetUrl();this._Ajax.PostNew(b,EWA.F.F.CUR._CallBack);this.isShowPostWaitting||this._Ajax.HiddenWaitting();$("#EWA_FRAME_"+this._Id+" input[type=submit]").attr("disabled","disabled")}}}};this._CallBack=function(){var a=EWA.F.F.CUR._Ajax;if(a.IsRunEnd())if($("#EWA_FRAME_"+EWA.F.F.CUR._Id+" input[type=submit]").attr("disabled",null),EWA.F.F.CUR.DoPostEnd&&
