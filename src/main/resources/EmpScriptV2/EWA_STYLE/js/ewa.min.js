...
"\u6587\u4ef6\u6570\u91cf\u9650\u5236"),a.value="";else{var c=0;if(this.UpLimit)if(b=(this.UpLimit+"").toLocaleLowerCase().replace(/,/ig,"").replace(/ /ig,""),b.endsWith("m")){b=b.substring(0,b.length-1);if(isNaN(b)){alert("Invalid sizeLimit parameter:"+this.sizeLimit);return}c=1048576*b}else if(b.endsWith("k")){b=b.substring(0,b.length-1);if(isNaN(b)){alert("Invalid sizeLimit parameter:"+this.sizeLimit);return}c=1024*b}else{if(isNaN(b)){alert("Invalid sizeLimit parameter:"+this.sizeLimit);return}c=
1*b}var d=[];if(0<c)for(b=0;b<a.files.length;b++){var e=a.files[b];if(e.size>c){var g=EWA.UI.Ext.getFileLen(e.size,2);d.push(e.name.replace(/</ig,"%lt;").replace(/>/ig,"%gt;")+" ["+g+"]")}}if(0<d.length)a.value="","enus"==EWA.LANG?(d.push("Exceed "+this.UpLimit+" limit"),EWA.UI.Msg.ShowError(d.join("<br>"),"Exceed "+this.UpLimit+" limit")):(d.push("\u8d85\u8fc7"+this.UpLimit+"\u5927\u5c0f\u9650\u5236"),EWA.UI.Msg.ShowError(d.join("<br>"),"\u8d85\u8fc7"+this.UpLimit+"\u5927\u5c0f\u9650\u5236"));else{c=
a.parentNode;d=c.outerHTML;c.style.display="none";a.setAttribute("ok",1);gFileReaders={};for(b=0;b<a.files.length;b++)e=a.files[b],this.showPreview1(c,e),this.files.push(e);"yes"==this.UpMulti&&$(c).before(d)}}};this.addOther=function(a){for(var b=$X(this.POST_NAME).parentNode.getElementsByTagName("input")[1].parentNode,c=0;c<a.length;c++)this.showPreview1(b,a[c]);this.uploadIframes.push(JSON.stringify(a))};this.createPreviewBox=function(a){var b=a||$("#EWA_FRAME_"+this.SYS_FRAME_UNID+" input#"+this.POST_NAME).parent().find(".ewa_app_upload_form")[0],
c="_upload_"+Math.random();c=c.replace(".","G");a=document.createElement("img");a.id=c;c=document.createElement("table");c.cellPadding=0;c.cellSpacing=0;c.className="ewa_app_upload_form1";b.parentNode.appendChild(c);b=c.insertRow(-1).insertCell(-1);b.align="center";b.appendChild(a);b.className="img";b.vAlign="middle";c.insertRow(-1).insertCell(-1).className="name";var d=this;a.onerror=function(e){console.log(e);d.fileIco({name:this.src},this.id)};return c};this.showPreviewExists=function(){var a=
$("#EWA_FRAME_"+this.SYS_FRAME_UNID+" input#"+this.POST_NAME).val();if(""!=a){try{EWA.F.FOS[this.SYS_FRAME_UNID].setUnMust(this.POST_NAME)}catch(b){}this.showPreviewUrl(a)}};this.showPreviewUrl=function(a){var b=this.createPreviewBox(),c=$(b).find("td")[1],d=a.split("/");d=d[d.length-1];c.innerHTML="<div title='"+d+"'>"+d+"</div>";$(b).find("img")[0].src=a;return b};this.showPreview1=function(a,b){var c=this.createPreviewBox(a),d=$(c).find("td")[1],e=$(c).find("img")[0];a=e.id;b.ref=a;d.innerHTML=
"<div>"+b.name+"</div><div id='"+a+"_process1' class='ewa-app-upload-process1'></div><div id='"+a+"_process' class='ewa-app-upload-process'>0%</div>";d.className="name";$(c).attr("file_name",b.name);$(c).attr("file_type",b.type);$(c).attr("file_size",b.size);if(b.type&&0==b.type.indexOf("image/")){e.src=(EWA.RV_STATIC_PATH||"/EmpScriptV2")+"/EWA_STYLE/images/loading2.gif";c=new FileReader;gFileReaders[a]=c;var g=this;c.onloadend=function(h){for(var k in gFileReaders)gFileReaders[k]==h.target&&($X(k).src=
