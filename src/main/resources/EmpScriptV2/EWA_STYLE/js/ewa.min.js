...
new EWA_MqeClass;b.Data=a.GetRst();b.FromId=EWA.F.CID;b.Type="POST";top.EWA.MQE.AddMqe(b)}a.HiddenWaitting();200==a._Http.status?(a=a._Http.responseText,EWA.F.CheckCallBack(a)&&eval(a)):alert("ERROR:\r\n"+a._Http.statusText);this.EditAfterEvent&&this.EditAfterEvent()}};this.EditAfterEvent=function(){};this._GetItem=function(a){for(var b=this.ItemList,c=0;c<b.length;c+=1){var d=b[c],e=this._GetSubItem("Name",d);if(this.Xml.GetElementAttribute(e,"Name")==a)return d}};this._GetSubItem=function(a,b){b=
this.Xml.GetElements(a+"/Set",b);if(null==b||0==b.length)return null;if("DescriptionSet"==a)for(a=0;a<b.length;a+=1){var c=b[a].getAttribute("Lang");if(null!=c&&c.trim().toUpperCase()==EWA.LANG.trim().toUpperCase())return b[a]}return b[0]};this._GetSubValue=function(a,b,c){a=this._GetSubItem(a,c);return null==a?null:this.Xml.GetElementAttribute(a,b)};this.AddColumns=function(a,b,c,d,e,g,h,k){var l=$X("EWA_LF_"+this._Id);null==k&&(k=l.rows[0].cells.length);for(var m=0;m<a.length;m++)for(var n=a[m],
p=k+m,t=0;t<l.rows.length;t++){var r=l.rows[t].insertCell(p);if(0==t)r.className="EWA_TD_H",r.innerHTML='<nobr cellIdx="'+p+'" id="'+n[b]+'">'+n[c]+"</nobr>",r.title=n[d],r.width=100;else{var q=l.rows[t].getAttribute("EWA_KEY");r.className="EWA_TD_M";q=q+"_"+n[b];var v=this._GetAddControl(n[g]);null==v&&(v=e);var u=h.replace("[ID]",n[b]);v=0<=v.indexOf("<select")?v.replace("!!",u.replace("onblur=","onchange=")):v.replace("!!",u);r.innerHTML=v;r.childNodes[0].id=q}}};this._GetAddControl=function(a){if(null==
a)return null;a=a.toUpperCase().trim();return"SELECT"==a?"<select !!></select>":"DATE"==a?'<input type=text class=EWA_INPUT onclick="EWA.UI.Calendar.Pop(this,false);" readonly !!>':"TIME"==a?'<input type=text class=EWA_INPUT onclick="EWA.UI.Calendar.Pop(this,true);" readonly !!>':"STRING"==a||"NUMBER"==a?"<input type=text class=EWA_INPUT !!>":null};this.AddedValues=function(a,b,c,d,e){for(var g=0;g<a.length;g++){var h=a[g],k=$X(h[b]+"_"+h[c]);if(!k)break;var l=k.tagName.toUpperCase();"INPUT"==l||
"SELECT"==l||"TEXTAREA"==l?k.value=h[d]:k.innerHTML=h[d];e&&(this.GetRow(k).getElementsByTagName("input")[0].checked=!0)}};this.AddRow=function(a){for(var b=$X("EWA_LF_"+this._Id),c=b.insertRow(-1),d=0;d<b.rows[0].cells.length;d++){var e=c.insertCell(-1),g=b.rows[0].cells[d];e.className="EWA_TD_M";null!=a[d]&&(e.innerHTML=a[d]);e.style.display=g.style.display;e.id="ADD_ROW_"+g.childNodes[0].id}return c};this.Calc=function(a,b,c,d){for(var e=$X("EWA_LF_"+this._Id),g={},h=0;h<a.length;h++)g[a[h]]=0;
for(h=b;h<c;h++)for(a=e.rows[h],b=0;b<a.cells.length;b++)if(null!=g[b]){var k=a.cells[b],l=k.getElementsByTagName("input");0<l.length?g[b]+=1*l[0].value:(k=GetInnerText(k),""==k&&(k=0),g[b]+=1*k)}c=e.rows[d];for(h=0;h<c.cells.length;h++)null!=g[h]&&(k=Math.round(100*g[h])/100,d=GetInnerText(c.cells[h]),k!=1*d&&(this.IsCalcChanged=!0),c.cells[h].innerHTML=k.fm())};this.mergeHeaders=function(a,b,c){var d=$($X("EWA_LF_"+this._Id)),e=d.find('tr[ewa_tag="HEADER"]');if(0!=e.length&&(a=e.find('nobr[id="'+
