...
function EWA_FrameClass() {
	...
	this._selectOptionsChangeed = function(o) {
		...
		// 选择code
		var options = this._initSelectFilterCreateCode(o.codeJsonFrom, target);
	...
	this.initSelectFilter = function(obj_id, filterType, filterFrom, codeJsonFrom) {
		...
		// 选择code
		var options = this._initSelectFilterCreateCode(codeJsonFrom, target);
		ss.push(options);
	...
	// this._DoValidExActionCallBack = function() {
	...
	// var a = parseInt(ret.trim());
	// if (a == 0) {
	// EWA_FrameShowAlert(EWA.F.F.CUR._ValidExObj,
	// EWA.F.F.CUR._ValidExOk.GetInfo());
	// EWA.F.F.CUR._ValidExObj.setAttribute('EWA_VALID_EX', "");
	// } else {
	// EWA_FrameShowAlert(EWA.F.F.CUR._ValidExObj,
	// EWA.F.F.CUR._ValidExFail.GetInfo());
	// EWA.F.F.CUR._ValidExObj.setAttribute('EWA_VALID_EX',
	// EWA.F.F.CUR._ValidExFail.GetInfo());
	// }
	// } catch (e) {
...
function EWA_ListFrameClass() {
	...
	this.EditAfter = function (obj) {
		...
		if (obj.getAttribute("ewa_class") == "droplist") {
			id = obj.parentNode.getElementsByTagName('input')[1].id;
		...
		var dt = this.ItemList.GetValue(obj.id, "DataItem", "DataType");
		paras[4] = {
			Name : "EWA_DATA_TYPE",
			Value : dt
		};
...
function loadWfShow(units, cnns, sts, logics, pid, admId, deptId, postId, curUnitId, curLogicVal) {
	...
	if (startUnit == unit) {
		...
		if (s.indexOf(',' + postId + ',') >= 0) {
			...
			var map2 = $J2MAP1(top.org, 'POS_ID');
			var ids = unit.WF_UNIT_ADM_LST.split(',');
