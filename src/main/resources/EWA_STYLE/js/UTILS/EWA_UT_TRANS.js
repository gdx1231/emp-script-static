 ...
 * bing trans
 */
var bingAppId = '50F7C8D4BC00A6E047C046D012F334DEC61FA003';
var bingTrans = '//api.microsofttranslator.com/V2/Ajax.svc/Translate?appid=' + bingAppId;

var trans_to_obj;

var youdaoTrans = "http://fanyi.youdao.com/openapi.do?keyfrom=wwwoneworldcc&key=1597375709&type=data&doctype=jsonp&version=1.1";
/*
...
function EWA_UT_TRANS() {
	...
	this.TransToCn = function(english, func) {
		...
		this.IsRun = true;
		if (this._Trans == 'bing') {
			this._trans_cn_bing(english, func);
		} else if (this._Trans == 'youdao') {
	...
	this._trans_youdao = function(fromStr, func) {
		...
		var u = youdaoTrans + "&callback=" + funcName + "&q=" + fromStr.toURL();

		s.src = u;
		document.getElementsByTagName("head")[0].appendChild(s);
	};
	this._trans_cn_bing = function(fromStr, func) {
		var funcName = this._get_func_name();
		var funcName1 = funcName + "AAAAA";
		var funcStr = this.trans_en_back_bing_tmp.toString().replace("trans_en_back_bing_tmp", '');
		var js = this._get_user_func_str(funcName, funcName1);
		funcStr = funcStr.replace("/* FF */", js);
		...
		var s = document.createElement("script");
		fromStr = fromStr.replace(/\n/ig, '[GDX]').replace(/\r/ig, '');
		var u = bingTrans + "&oncomplete=" + funcName + "&from=en&to=zh-CHS&text=" + fromStr.toURL();

		s.src = u;
		document.getElementsByTagName("head")[0].appendChild(s);
	};
	this._trans_en_bing = function(fromStr, func) {
		var funcName = this._get_func_name();
		var funcName1 = funcName + "AAAAA";
	 ...
	 * bing 回调模板脚本
	 */
	this.trans_en_back_bing_tmp = function(rst) {
		// 替换 下面 FF 脚本内容
		/* FF */
		try {
...
function trans_en_baidu(fromObj, toObj) {
	// trans_en_google(fromObj,toObj);

	var val = fromObj.value;
	if (!check_changed(fromObj)) {
		return;
	}
	if (val == "" || val.trim() == "") {
		toObj.value = "";
	...
	// baidu
	var u = EWA.CP + "/utils/trans_en.jsp?q=" + val.toURL();
	$J(u, trans_en_back1, toObj);
}
function trans_en_back1(data, cmds) {
	if (data.IsError) {
		alert("错误");
		return;
	}
	...
	cmds[0].value = txt.toUpperCase().trim();
	if (window.saveTrans) {
		saveTrans(cmds[0]);
	}
}

function trans_en(fromObj, toObj) {
	var val = fromObj.value;
	if (!check_changed(fromObj)) {
		return;
	}
	trans_to_obj = toObj;
	var s = document.createElement("script");
	var u = bingTrans + "&oncomplete=trans_en_back&from=zh-CHS&to=en&text=" + val.toURL();
	s.src = u;
	document.getElementsByTagName("head")[0].appendChild(s);
}
function trans_en_back(s) {
	trans_to_obj.value = s.toUpperCase();
	if (window.saveTrans) {
		saveTrans(trans_to_obj);
	}
}
