...
this._TIMER_SEARCH=window.setInterval(function(){try{if(!$X(a)){window.clearInterval(w._TIMER_SEARCH);return}}catch(A){window.clearInterval(w._TIMER_SEARCH);return}var y=w.SearchGetExp(x);y!=w._SEARCH_ITEM_EXP&&(w._SEARCH_ITEM_EXP=y,w.DoSearch($X(a)))},700)}};this._SearchSingle=function(a,b){var c=[];c.push('<input t="text" type="text" autocomplete="off" class="EWA_INPUT" maxlength="40" name="');c.push(a.toInputValue());c.push('"');null!=b&&(c.push(' value="'),a=b.toInputValue(),c.push(a),c.push('" '));
...
a.push("</table></div>");this._SearchHtml=a.join("")};this.SearchMark=function(){for(var a=this._SearchExp.split("@!@"),b=$X("EWA_LF_"+this._Id),c=0;c<a.length;c+=1){var d=a[c].split("~!~");if(!(3>d.length))for(var e=2;e<d.length;e++){var g=d[e];if(""!=g){if("fix"==d[1]){var h=d[0],k=d[2];$("table#EWA_SEARCH_ITEM_"+this.Id+' [name="'+h+'"]').find("option").each(function(){this.value==k&&(g=this.text)})}h=[];for(var l=0;l<g.length;l++){var m=g[l];"a"<=m&&"z">=m||"A"<=m&&"Z">=m||"0"<=m&&"9">=m?h.push(m):
h.push("\\"+m)}g=h.join("");h=d[0];this._SearchJson[h]&&(h=this._SearchJson[h].ORI_NAME);try{g=eval("/("+g+")/ig"),$(b).find('.EWA_TD_M [id="'+h+'"]').each(function(){0==this.children.length&&(this.innerHTML=this.innerHTML.replace(g,"<font color=red><b>$1</b></font>"))})}catch(n){console.log(n)}}}}};this.DblClick=function(a){this._IsDblClick=a;for(var b=$X("EWA_LF_"+this._Id),c=1;c<b.rows.length;c++)b.rows[c].ondblclick=function(){var d=this.getElementsByTagName("input");d.length>a&&0==d[a].disabled&&
...
this.DoAction=function(a,b,c,d,e,g){EWA.F.CID=this._Id;if(b){if(a&&$(a).parent().hasClass("scroll-delete")){var h=$(a).attr("do-action-time")||0,k=(new Date).getTime();if(1E3>k-h)return;$(a).attr("do-action-time",k)}this._Ajax=new EWA.C.Ajax;this._Ajax.LoadingType="image";this._Ajax.AddParameter("EWA_AJAX","1");this._Ajax.AddParameter("EWA_ACTION",b);this._Ajax.AddParameter("EWA_ID",this._Id);this._Ajax.AddParameter("EWA_NO_CONTENT","1");this._Ajax.AddParameter("EWA_ACTION_TIP",d);null!=g&&0<g.trim().length?
(this._Ajax.AddParameter("EWA_AFTER_EVENT",g),this.StopAjaxAfterReload=!0,this._Ajax.AddParameter("EWA_ACTION_RELOAD","0")):this.StopAjaxAfterReload=!1;if(a&&a.getAttribute&&"yes"==a.getAttribute("box_menu_item")){if(this._Ajax.AddParameter("___CALL__FROM__","box"),a=a.parentNode)a=a.getAttribute("json_ref"),a=JSON.parse(a),a.EWA_KEY&&this._Ajax.AddParameter("EWA_ACTION_KEY",a.EWA_KEY)}else(a=this.GetRowKey(a))&&this._Ajax.AddParameter("EWA_ACTION_KEY",a);a=this.SelectChecked();this._Ajax.AddParameter("IDS_SPLIT",
a);if(null!=e&&0<e.length)for(a=0;a<e.length;a+=1)this._Ajax.AddParameter(e[a].Name,e[a].Value);e=new EWA.C.Url;e.SetUrl(this.Url);var l=e.RemoveParameter("EWA_ACTION"),m=this;c?((e=_EWA_INFO_MSG[c])||(e=c),$Confirm(e,"Confirm",function(){m._Ajax.PostNew(l,function(){m._CallBackJs()})})):this._Ajax.PostNew(l,function(){m._CallBackJs()})}};this.NewPageSize=function(a){this._PageSize!=a&&(this._PageSize=a,this.Goto(1))};this.Goto=function(a,b){EWA.F.CID=this._Id;this._PageCurrent=a;this._Ajax=this.CreateAjax();
...
b)};this.AddColumns=function(a,b,c,d,e,g,h,k){var l=$X("EWA_LF_"+this._Id);null==k&&(k=l.rows[0].cells.length);for(var m=0;m<a.length;m++)for(var n=a[m],p=k+m,t=0;t<l.rows.length;t++){var r=l.rows[t].insertCell(p);if(0==t)r.className="EWA_TD_H",r.innerHTML='<nobr cellIdx="'+p+'" id="'+n[b]+'">'+n[c]+"</nobr>",r.title=n[d],r.width=100;else{var q=l.rows[t].getAttribute("EWA_KEY");r.className="EWA_TD_M";q=q+"_"+n[b];var v=this._GetAddControl(n[g]);null==v&&(v=e);var u=h.replace("[ID]",n[b]);v=0<=v.indexOf("<select")?
v.replace("!!",u.replace("onblur=","onchange=")):v.replace("!!",u);r.innerHTML=v;r.childNodes[0].id=q}}};this._GetAddControl=function(a){if(null==a)return null;a=a.toUpperCase().trim();return"SELECT"==a?"<select !!></select>":"DATE"==a?'<input type=text class=EWA_INPUT onclick="EWA.UI.Calendar.Pop(this,false);" readonly !!>':"TIME"==a?'<input type=text class=EWA_INPUT onclick="EWA.UI.Calendar.Pop(this,true);" readonly !!>':"STRING"==a||"NUMBER"==a?"<input type=text class=EWA_INPUT !!>":null};this.AddedValues=
function(a,b,c,d,e){for(var g=0;g<a.length;g++){var h=a[g],k=$X(h[b]+"_"+h[c]);if(!k)break;var l=k.tagName.toUpperCase();"INPUT"==l||"SELECT"==l||"TEXTAREA"==l?k.value=h[d]:k.innerHTML=h[d];e&&(this.GetRow(k).getElementsByTagName("input")[0].checked=!0)}};this.AddRow=function(a){for(var b=$X("EWA_LF_"+this._Id),c=b.insertRow(-1),d=0;d<b.rows[0].cells.length;d++){var e=c.insertCell(-1),g=b.rows[0].cells[d];e.className="EWA_TD_M";null!=a[d]&&(e.innerHTML=a[d]);e.style.display=g.style.display;e.id="ADD_ROW_"+
g.childNodes[0].id}return c};this.Calc=function(a,b,c,d){for(var e=$X("EWA_LF_"+this._Id),g={},h=0;h<a.length;h++)g[a[h]]=0;for(h=b;h<c;h++)for(a=e.rows[h],b=0;b<a.cells.length;b++)if(null!=g[b]){var k=a.cells[b],l=k.getElementsByTagName("input");0<l.length?g[b]+=1*l[0].value:(k=GetInnerText(k),""==k&&(k=0),g[b]+=1*k)}c=e.rows[d];for(h=0;h<c.cells.length;h++)null!=g[h]&&(k=Math.round(100*g[h])/100,d=GetInnerText(c.cells[h]),k!=1*d&&(this.IsCalcChanged=!0),c.cells[h].innerHTML=k.fm())};this.mergeHeaders=
function(a,b,c){var d=$($X("EWA_LF_"+this._Id)),e=d.find('tr[ewa_tag="HEADER"]');if(0!=e.length&&(a=e.find('nobr[id="'+a+'"]').parent(),0!=a.length)){var g=d.find("tr[ewa_mearge]");0==g.length&&(g=$(d[0].insertRow(1)),g.attr("ewa_mearge","gdx"));d=-1;for(var h=0;h<e[0].cells.length;h++){var k=e[0].cells[h];k==a[0]&&(d=h,k.colSpan=c);if(-1==d)"1"!=$(k).attr("ewa_tdmearged")&&(k.rowSpan=2);else if(d+c==h)d=-1,k.rowSpan=2;else{k.rowSpan=1;var l=g[0].insertCell(-1);l.innerHTML=k.innerHTML;l.className=
k.className;l.style.cssText=k.style.cssText;k.style.display="none";$(k).attr("ewa_tdmearged",1)}k==a[0]&&(k.innerHTML=b,k.style.display="",k.style.width="")}}};this.MeargeHeader=function(a,b,c){console.log("\u62fc\u5199\u9519\u8bef\uff0c\u8bf7\u7528 mergeHeaders");this.mergeHeaders(a,b,c)};this.getUrlClass=function(){var a=new EWA_UrlClass;a.SetUrl(this.Url);return a}}EWA.F.L={CUR:null,C:EWA_ListFrameClass};
