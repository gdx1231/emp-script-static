...
$(x).find("input[type=text]").on("compositionstart",function(){w._is_search_composition=!0});$(x).find("input[type=text]").on("compositionend",function(){w._is_search_composition=!1});this._TIMER_SEARCH=window.setInterval(function(){if(!w._is_search_composition){try{if(!$X(a)){window.clearInterval(w._TIMER_SEARCH);return}}catch(A){window.clearInterval(w._TIMER_SEARCH);return}var y=w.SearchGetExp(x);y!=w._SEARCH_ITEM_EXP&&(w._SEARCH_ITEM_EXP=y,w.DoSearch($X(a)))}},700)}};this._SearchSingle=function(a,
b){var c=[];c.push('<input t="text" type="text" autocomplete="off" class="EWA_INPUT" maxlength="40" name="');c.push(a.toInputValue());c.push('"');null!=b&&(c.push(' value="'),a=b.toInputValue(),c.push(a),c.push('" '));c.push(" />");return c.join("")};this._SearchDate=function(a,b,c){var d=[];d.push("<table border=0 cellpadding=0 cellspacing=0>");d.push('<td  ><input type="text" autocomplete="off" class="EWA_INPUT" t="date" readonly maxlength="8" onclick="EWA.UI.Calendar.Pop(this)" name="');d.push(a);
d.push('"');b&&(b=b.toInputValue(),d.push(' value="'),d.push(b),d.push('"'));d.push(' /></td><td><span class="ewa-lf-search-split"> - </span></td>');d.push('<td  ><input type="text" autocomplete="off" class="EWA_INPUT" t="date" readonly maxlength="8" onclick="EWA.UI.Calendar.Pop(this)" name="');d.push(a);d.push('1"');c&&(b=c.toInputValue(),d.push(' value="'),d.push(b),d.push('"'));d.push("/></td></tr></table>");return d.join("")};this._SearchNumber=function(a,b,c){var d=[];d.push('<input type="text" t="number" size="10" name="');
...
d.length;e++){var g=d[e];if(""!=g){if("fix"==d[1]){var h=d[0],k=d[2];$("table#EWA_SEARCH_ITEM_"+this.Id+' [name="'+h+'"]').find("option").each(function(){this.value==k&&(g=this.text)})}h=[];for(var l=0;l<g.length;l++){var m=g[l];"a"<=m&&"z">=m||"A"<=m&&"Z">=m||"0"<=m&&"9">=m?h.push(m):h.push("\\"+m)}g=h.join("");h=d[0];this._SearchJson[h]&&(h=this._SearchJson[h].ORI_NAME);try{g=eval("/("+g+")/ig"),$(b).find('.EWA_TD_M [id="'+h+'"]').each(function(){0==this.children.length&&(this.innerHTML=this.innerHTML.replace(g,
"<font color=red><b>$1</b></font>"))})}catch(n){console.log(n)}}}}};this.DblClick=function(a){this._IsDblClick=a;for(var b=$X("EWA_LF_"+this._Id),c=1;c<b.rows.length;c++)b.rows[c].ondblclick=function(){var d=this.getElementsByTagName("input");d.length>a&&0==d[a].disabled&&d[a].click()}};this.Init=function(a){this.Xml=new EWA.C.Xml;this.Xml.LoadXml(a);this.ItemList.Init(this.Xml);this.Resources=new EWA_FrameResoures;this.Resources.Init(this.Xml);if((a=$X("EWA_LF_"+this._Id))&&"table"==a.tagName&&!(1>=
a.rows.length)){a=a.rows[1].getElementsByTagName("input");for(var b=0;b<a.length;b++)if("RADIO"==a[b].type.toUpperCase()){this._TrSelectMulti=!1;break}}};this.SetPageParameters=function(a,b,c,d,e){this._PageCurrent=a;this._PageSize=c;this._PageCount=b;this._RecordCount=d;this._Sort=e};this.SetPageParametersName=function(a,b,c,d,e){this._PageCurrentName=a;this._PageSizeName=c;this._PageCountName=b;this._RecordCountName=d;this._SortName=e};this.CheckedAll=function(){var a=document.getElementById("EWA_LF_"+
this._Id);if(!(1>=a.rows.length)){for(var b=0;b<a.rows.length;b+=1){var c=a.rows[b].getElementsByTagName("input");0!=c.length&&(c[0].checked=this._IsCheckedAll)}this._IsCheckedAll=this._IsCheckedAll?!1:!0;this.CheckedAllAfter&&this.CheckedAllAfter(a)}};this.SelectChecked=function(){var a=$X("EWA_LF_"+this._Id);if(!a||1>a.rows.length)return"";var b=[],c=$F(a,"input","type","radio,checkbox");if(0<c.length)for(var d=0;d<c.length;d++)c[d].checked&&b.push(c[d].value);else for(d=0;d<a.rows.length;d++)c=
a.rows[d],1==c.getAttribute("__ewa_lf_mdown")&&b.push(c.getAttribute("EWA_KEY"));return b.join(",")};this.SelectCheckedInputs=function(){var a=[],b=$X("EWA_LF_"+this._Id);if(1>b.rows.length)return a;b=b.getElementsByTagName("input");a=[];for(var c=0;c<b.length;c++)("CHECKBOX"==b[c].type.toUpperCase()||"RADIO"==b[c].type.toUpperCase())&&b[c].checked&&a.push(b[c]);return a};this.SelectCheckedRows=function(){for(var a=[],b=$X("EWA_LF_"+this._Id),c=0;c<b.rows.length;c++){var d=b.rows[c],e=d.getElementsByTagName("input");
0!=e.length&&e[0].checked&&a.push(d)}if(0==a.length)for(c=0;c<b.rows.length;c++)d=b.rows[c],1==d.getAttribute("__ewa_lf_mdown")&&a.push(d);return a};this.Reload=function(a){this.StopAjaxAfterReload?this.StopAjaxAfterReload=!1:this.Goto(this._PageCurrent,a)};this.DoAction=function(a,b,c,d,e,g){EWA.F.CID=this._Id;if(b){if(a&&$(a).parent().hasClass("scroll-delete")){var h=$(a).attr("do-action-time")||0,k=(new Date).getTime();if(1E3>k-h)return;$(a).attr("do-action-time",k)}this._Ajax=new EWA.C.Ajax;this._Ajax.LoadingType=
"image";this._Ajax.AddParameter("EWA_AJAX","1");this._Ajax.AddParameter("EWA_ACTION",b);this._Ajax.AddParameter("EWA_ID",this._Id);this._Ajax.AddParameter("EWA_NO_CONTENT","1");this._Ajax.AddParameter("EWA_ACTION_TIP",d);null!=g&&0<g.trim().length?(this._Ajax.AddParameter("EWA_AFTER_EVENT",g),this.StopAjaxAfterReload=!0,this._Ajax.AddParameter("EWA_ACTION_RELOAD","0")):this.StopAjaxAfterReload=!1;if(a&&a.getAttribute&&"yes"==a.getAttribute("box_menu_item")){if(this._Ajax.AddParameter("___CALL__FROM__",
"box"),a=a.parentNode)a=a.getAttribute("json_ref"),a=JSON.parse(a),a.EWA_KEY&&this._Ajax.AddParameter("EWA_ACTION_KEY",a.EWA_KEY)}else(a=this.GetRowKey(a))&&this._Ajax.AddParameter("EWA_ACTION_KEY",a);a=this.SelectChecked();this._Ajax.AddParameter("IDS_SPLIT",a);if(null!=e&&0<e.length)for(a=0;a<e.length;a+=1)this._Ajax.AddParameter(e[a].Name,e[a].Value);e=new EWA.C.Url;e.SetUrl(this.Url);var l=e.RemoveParameter("EWA_ACTION"),m=this;c?((e=_EWA_INFO_MSG[c])||(e=c),$Confirm(e,"Confirm",function(){m._Ajax.PostNew(l,
...
this.mergeHeaders(a,b,c)};this.getUrlClass=function(){var a=new EWA_UrlClass;a.SetUrl(this.Url);return a}}EWA.F.L={CUR:null,C:EWA_ListFrameClass};
